...
const AUDIO_PROPERTY_HINT =
  /(?:\bvolume\b|\bmute(?:d)?\b|\bsilent\b|\baudio\s*(?:volume|level|gain|enable|enabled|toggle)\b|\bmusic\s*(?:volume|level|gain|size|enable|enabled|toggle)\b|\bsound\s*(?:volume|level|gain|enable|enabled|toggle)\b|音量|静音|无声|音乐(?:大小|音量|开关)|声音(?:大小|音量|开关)|音效(?:大小|音量|开关))/i;
const AUDIO_PROPERTY_KEY =
  /^(?:volume|dbvolume|musicvolume|music_volume|audiovolume|audio_volume|soundvolume|sound_volume|bgmvolume|bgm_volume|muteaudio|audiomute|mutemusic|musicmute|music|audio|sound|bgm)$/i;
const AUDIO_STANDALONE_LABEL =
  /^(?:(?:background\s*(?:audio|music|sound)|背景(?:音频|音乐|声音))|(?:audio|music|sound|bgm|音频|音乐|声音)(?:[\s,，、/_-]*(?:audio|music|sound|bgm|音频|音乐|声音|\d+))*)$/i;
const AUDIO_VISUAL_HINT =
  /(?:visuali[sz](?:er|ation)|\bbars?\b|\bring\b|\bpulse\b|\bthreshold\b|\bsensitiv(?:e|ity)\b|\bintensity\b|\bcolou?r\b|\bopacity\b|\btransparen(?:cy|t)\b|\bbounce\b|\bflicker\b|\balbum\b|\binformation\b|\bresponse\b|\breactive\b|\bfrequency\b|\bspectrum\b|\bwave\b|\bnote\b|可视|频谱|跳动|闪烁|响应|颜色|透明|专辑|封面|信息)/i;
const AUDIO_DECIBEL_HINT = /(?:\bdb\b|decibel|分贝)/i;
const AUDIO_OFF_OPTION_HINT =
  /(?:\bnone\b|\boff\b|\bmute(?:d)?\b|\bsilent\b|\bdisable(?:d)?\b|关闭|静音|无声|不要音乐|无音乐)/i;
const MUTE_PROPERTY_HINT = /(?:\bmute(?:d)?\b|\bsilent\b|静音|无声)/i;
const AUDIO_MUTE_PROPERTY_KEYS = new Set([
  "muteaudio",
  "audiomute",
  "mutemusic",
  "musicmute",
...
function analyzeSceneProperties(project) {
  ...
  for (const [rawKey, rawProperty] of Object.entries(properties)) {
    ...
    const explicitAudioControl =
      AUDIO_PROPERTY_HINT.test(hint) ||
    ...
    if (audioProperty) {
      ...
      if (type === "bool" || typeof property.value === "boolean") {
        ...
        const offOption =
          options.find((option) => AUDIO_OFF_OPTION_HINT.test(option.label)) ||
          (exactAudioKey
...
function deriveWorkshopId(project, projectRoot, sourceKind = "") {
  ...
  for (const candidate of urlCandidates) {
    const match = /(?:[?&]id=|\/filedetails\/?)(\d{5,32})/i.exec(
      String(candidate || ""),
    );
    if (match) return match[1];
  }
  const directoryId = path.basename(String(projectRoot || ""));
  return sourceKind === "workshop" && /^\d{5,32}$/.test(directoryId)
    ? directoryId
    : "";
}
...
async function validateScenePackage(file) {
  if (!file || !SCENE_PACKAGE_EXTENSIONS.has(path.extname(file).toLowerCase()))
    return "";
  try {
    ...
    try {
      ...
      return result.bytesRead === header.length &&
        (/^PKGV\d{4}$/.test(signatureAtStart) ||
          /^PKGV\d{4}$/.test(signatureAfterLength))
        ? file
        : "";
    } finally {
      await handle.close();
...
async function windowsSteamRegistryRoots() {
  ...
  for (const [key, value] of queries) {
    ...
    const match = output.match(
      new RegExp(`${value}\\s+REG_\\w+\\s+(.+)$`, "mi"),
    );
    if (!match) continue;
  ...
  for (const candidate of candidates) {
    ...
    for (const library of await readSteamLibraryFolders(candidate)) {
      if (await isDirectory(library))
        libraries.add(normalizeAbsolutePath(library));
    }
  }
  return [...libraries];
}
...
async function directProjectDirectories(container) {
  ...
  for (const entry of entries) {
    ...
    if (await statSafe(path.join(projectRoot, "project.json")))
      output.push(projectRoot);
  }
  return output;
}
...
async function manualProjectDirectories(root) {
  ...
  for (const container of knownWallpaperContainers(root)) {
    if (await isDirectory(container))
      known.push(...(await directProjectDirectories(container)));
  }
...
async function readProjectManifest(projectRoot) {
  ...
  if (
    ...
    !stat.isFile() ||
    stat.size <= 0 ||
    stat.size > MAX_PROJECT_JSON_BYTES
  )
    return null;
  try {
...
async function indexProject(projectRoot, source, scenePackageOverride = "") {
  ...
  const inferredMedia = VIDEO_MIME.has(directExt)
    ? "video"
    : IMAGE_MIME.has(directExt)
      ? "image"
      : "";
  const allowDirectMedia =
    projectType === "video" ||
    projectType === "image" ||
    (!projectType && !!inferredMedia);
  ...
  const mediaType = VIDEO_MIME.has(mediaExt)
    ? "video"
  ...
  const propertyAnalysis =
    projectType === "scene"
      ? analyzeSceneProperties(project)
      : {
          propertyCount: 0,
          audioPropertyCount: 0,
          mutedAudioPropertyCount: 0,
...
function mimeForPath(file) {
  return (
    SAFE_MIME.get(path.extname(file).toLowerCase()) ||
...
class WallpaperEngineLibrary {
  constructor(options = {}) {
    this.userDataPath = normalizeAbsolutePath(
      options.userDataPath || process.cwd(),
    );
    this.configPath = normalizeAbsolutePath(
      options.configPath || path.join(this.userDataPath, CONFIG_FILE),
    );
  ...
  async addManualRoot(root) {
    ...
    if (!root || !(await isDirectory(root)))
      throw new Error("请选择存在的 Wallpaper Engine 项目目录");
    const projectDirs = await manualProjectDirectories(root);
    if (!projectDirs.length)
      throw new Error("所选目录中没有识别到 project.json");
  ...
  async addManualProjectFile(file) {
    ...
    if (!stat || !stat.isFile())
      throw new Error("请选择存在的 Wallpaper Engine 项目文件");
    ...
    if (!SCENE_PACKAGE_EXTENSIONS.has(path.extname(file).toLowerCase())) {
      throw new Error(
        "请选择 project.json 或 Wallpaper Engine 场景包（.pkg/.pak）",
      );
    }
    const scenePackage = await validateScenePackage(file);
    if (!scenePackage) {
      throw new Error(
        "所选文件不是有效的 Wallpaper Engine PKGV 场景包；普通游戏或 Chromium .pak 无法运行",
      );
    }
    ...
    if (
      !this.manualRoots.some((value) => pathKey(value) === pathKey(projectRoot))
    ) {
      this.manualRoots.push(projectRoot);
      this.manualRoots = this.manualRoots.slice(-32);
    }
    if (
      !this.manualProjectFiles.some(
        (value) => pathKey(value) === pathKey(scenePackage),
      )
    ) {
  ...
  async removeManualRoot(id) {
    const removedRoots = this.manualRoots.filter(
      (root) => opaqueId(root) === String(id || ""),
    );
    const beforeRoots = this.manualRoots.length;
    const beforeFiles = this.manualProjectFiles.length;
    this.manualRoots = this.manualRoots.filter(
      (root) => !removedRoots.includes(root),
    );
    this.manualProjectFiles = this.manualProjectFiles.filter(
      (file) =>
        !removedRoots.some((root) =>
          isInside(path.resolve(root), path.resolve(file)),
        ),
    );
    if (
      this.manualRoots.length !== beforeRoots ||
      this.manualProjectFiles.length !== beforeFiles
    )
      await this.saveConfig();
    return this.list({ force: true });
  }
    ...
    if (this.autoDiscover) {
      for (const library of await discoverSteamLibraries()) {
        for (const container of knownWallpaperContainers(library)) {
          ...
          output.push({
            root: container,
            kind: /workshop[\\/]content/i.test(container)
              ? "workshop"
              : "local",
            label: /workshop[\\/]content/i.test(container)
              ? "Steam 创意工坊"
              : "Wallpaper Engine 本地项目",
            direct: true,
          });
        }
      }
  ...
  async performScan() {
    ...
    for (const source of sources) {
      ...
      for (const projectRoot of projects) {
        ...
        if (!projectSources.has(key))
          projectSources.set(key, { projectRoot, source });
      }
    }
    ...
    const snapshot = {
      ...
      dynamicCount: projects.filter(
        (item) => item.playable && item.mediaType === "video",
      ).length,
      enginePlayableCount: projects.filter((item) => item.enginePlayable)
        .length,
  ...
  async list(options = {}) {
    ...
    if (
      !force &&
      this.snapshot &&
      Date.now() - this.snapshot.scannedAt < CACHE_TTL_MS
    )
  ...
  async validatedRecordFile(record, kind) {
    ...
    try {
      ...
      if (
        ...
        !SAFE_MIME.has(path.extname(realTarget).toLowerCase())
      )
        return "";
      const stat = await fs.promises.stat(realTarget);
      return stat.isFile() ? realTarget : "";
  ...
  async getNativeSceneTarget(id) {
    ...
    if (!/^[a-f0-9]{24}$/.test(id))
      throw new Error("WALLPAPER_SCENE_ID_INVALID");
    if (!this.snapshot && !this.scanPromise) await this.list({ force: false });
    const record = this.index.get(id);
    if (!record || !record.scenePackage)
      throw new Error("WALLPAPER_SCENE_NOT_FOUND");
    const target = await resolveProjectFile(
      record.projectRoot,
      path.relative(record.projectRoot, record.scenePackage),
      SCENE_PACKAGE_EXTENSIONS,
    );
  ...
  async getProjectDetails(id) {
    id = String(id || "").toLowerCase();
    if (!/^[a-f0-9]{24}$/.test(id))
      throw new Error("WALLPAPER_PROJECT_ID_INVALID");
    if (!this.snapshot && !this.scanPromise) await this.list({ force: false });
    const record = this.index.get(id);
    ...
    const workshopId =
      record.workshopId || deriveWorkshopId(project, record.projectRoot);
    return {
      ok: true,
      id,
      title: sanitizeText(project.title, path.basename(record.projectRoot)),
      projectType:
        String(project.type || "unknown")
          .trim()
          .toLowerCase()
          .replace(/[^a-z0-9_-]/g, "")
          .slice(0, 32) || "unknown",
