...
const APP_NAME =
  process.env.MINERADIO_RUNTIME_NAME ||
  APP_METADATA.runtimeName ||
  APP_PACKAGE_INFO.productName ||
  "Mineradio";
const APP_USER_MODEL_ID =
  process.env.MINERADIO_APP_USER_MODEL_ID ||
  APP_METADATA.appUserModelId ||
...
const QISHUI_WEB_LOGIN_URL =
  process.env.QISHUI_WEB_LOGIN_URL || "https://qishui.douyin.com/";
const QISHUI_WEB_LOGIN_FALLBACK_URL =
  process.env.QISHUI_WEB_LOGIN_FALLBACK_URL ||
  "https://bff-pc.qishui.com/ucenter_web/app/sdk-next";
const QISHUI_OFFICIAL_CLIENT_DATA_DIRS = (
  process.env.QISHUI_OFFICIAL_CLIENT_DATA_DIRS || ""
)
  .split(/[;,]/)
...
const fullDesktopModeRuntime = new FullDesktopModeRuntime({
  ...
  nativeTempPath: NATIVE_HELPER_TEMP_PATH,
  beforePassive: ({ win, reason }) =>
    prepareWallpaperEngineProjectPreviewBeforeDesktopEmbedding(win, reason),
...
const MAIN_WINDOW_BACKGROUND_THROTTLING =
  process.env.MINERADIO_KEEP_BACKGROUND_RENDERING === "1" ? false : true;

function wallpaperEngineTargetFps(display, requestedFps) {
  const displayFrequency = Math.max(
    24,
    Math.min(
...
function wallpaperEnginePhysicalContentBounds(win, fallback = {}) {
  ...
  if (
    ...
    try {
      ...
      if (
        physicalRect &&
        Number(physicalRect.width) > 0 &&
        Number(physicalRect.height) > 0
      ) {
        return {
          bounds,
          display,
  ...
  const dipEnd = {
    x:
      dipOrigin.x +
      Math.max(1, Number(bounds.width) || Number(fallback.width) || 1280),
    y:
      dipOrigin.y +
      Math.max(1, Number(bounds.height) || Number(fallback.height) || 720),
  };
  const physicalOrigin =
    typeof screen.dipToScreenPoint === "function"
      ? screen.dipToScreenPoint(dipOrigin)
      : {
          x: Math.round(dipOrigin.x * scaleFactor),
          y: Math.round(dipOrigin.y * scaleFactor),
        };
  const physicalEnd =
    typeof screen.dipToScreenPoint === "function"
      ? screen.dipToScreenPoint(dipEnd)
      : {
          x: Math.round(dipEnd.x * scaleFactor),
          y: Math.round(dipEnd.y * scaleFactor),
        };
  return {
    bounds,
    display,
    ...
    width: Math.max(
      1,
      Math.abs(Math.round(Number(physicalEnd.x) - Number(physicalOrigin.x))) ||
        Math.round((Number(bounds.width) || 1280) * scaleFactor),
    ),
    height: Math.max(
      1,
      Math.abs(Math.round(Number(physicalEnd.y) - Number(physicalOrigin.y))) ||
...
function chromiumSessionDataPath(settings) {
  const chromiumRoot =
    settings && settings.chromiumPath
...
function ensureCacheDirectories(settings) {
  ...
  try {
    ...
    const fallback = normalizeCacheSettings({
      rootPath: path.join(STABLE_USER_DATA_PATH, "cache-fallback"),
    });
    console.warn(
      "[CacheSettings] cache root unavailable, using startup fallback:",
      error.message,
    );
...
async function cacheSettingsSnapshot() {
  ...
  const activeWallpaperEnginePath = path.join(
    activeNativePath,
    "wallpaper-engine-muted-package-cache",
  );
  const [
...
async function pruneLyricCache() {
  let entries = [];
  try {
    entries = await fs.promises.readdir(cacheSettings.lyricsPath, {
      withFileTypes: true,
    });
  } catch (_) {
    return;
...
const CHROMIUM_OPT_IN_PERFORMANCE_SWITCHES = [
  ...
  [
    ...
    null,
    "MINERADIO_KEEP_BACKGROUND_RENDERING",
  ],
  [
    ...
    const timer = setTimeout(
      () => {
        ...
        const error = new Error(
          `${label || "startup operation"} timed out after ${timeoutMs}ms`,
        );
  ...
  return new Promise((resolve, reject) => {
    ...
    const probe = () => {
      ...
      if (Date.now() >= deadline) {
        const error = new Error(
          `local HTTP server did not become ready within ${timeoutMs}ms`,
        );
      ...
      activeRequest.once(
        ...
        () =>
          activeRequest &&
          activeRequest.destroy(new Error("HTTP probe timeout")),
      );
...
function readCurrentFxAutosaveFile() {
  try {
    ...
    if (!stat || stat.size <= 0 || stat.size > CURRENT_FX_AUTOSAVE_MAX_BYTES)
      return null;
...
function flushMainWindowFxAutosave(reason) {
  ...
  const safeReason =
    String(reason || "main-close")
      .replace(/[^a-z0-9:_-]/gi, "")
      .slice(0, 48) || "main-close";
  const script = `
    (function () {
      try {
  ...
  return Promise.race([
    ...
    new Promise((resolve) =>
      setTimeout(() => resolve({ ok: false, timeout: true }), 800),
    ),
  ]).catch((e) => ({ ok: false, error: e.message || String(e) }));
}

const LOCAL_APP_PERMISSION_ALLOWLIST = new Set([
  "speaker-selection",
  "pointerLock",
  "pointer-lock",
]);
...
function isTrustedMainWindowIpc(event) {
  try {
    if (!event || !event.sender || !mainWindow || mainWindow.isDestroyed())
      return false;
...
function clearWallpaperEngineCaptureGrant(sessionId = "") {
  ...
  if (
    expectedSessionId &&
    wallpaperEngineCaptureGrant.sessionId !== expectedSessionId
  )
    return false;
  if (!wallpaperEngineCaptureGrant) return false;
  if (
    wallpaperEngineCaptureGrant &&
    wallpaperEngineCapturePreparationOperation ===
      wallpaperEngineCaptureGrant.operation
  ) {
...
function getWallpaperEngineCaptureGrant() {
  ...
  if (
    ...
    !active ||
    !active.active ||
    active.sessionId !== grant.sessionId
  ) {
...
function isTransientWallpaperEngineCaptureError(value) {
  return /NotReadableError|WALLPAPER_ENGINE_REFRESH_SUPERSEDED|WALLPAPER_CAPTURE_FAILED|WALLPAPER_CAPTURE_PREPARED_STREAM_MISSING/i.test(
    String(value || ""),
  );
}
...
function resetWallpaperEngineCaptureGrantForRetry(grant) {
  ...
  const active = wallpaperEngineRuntime.getStatus();
  if (!active || !active.active || active.sessionId !== grant.sessionId)
    return false;
...
function isTrustedWallpaperEngineDisplayCapturePermission(
  webContents,
  origin,
  details,
) {
...
function isTrustedWallpaperEnginePreparationMediaPermission(
  webContents,
  origin,
  details,
) {
  const grant = getWallpaperEngineCaptureGrant();
  if (!grant || wallpaperEngineCapturePreparationOperation !== grant.operation)
    return false;
  ...
  if (
    ...
    mediaTypes.some((value) => value.includes("audio"))
  )
    return false;
  if (mediaType && !mediaType.includes("video")) return false;
  if (
    mediaTypes.length &&
    !mediaTypes.every((value) => value.includes("video"))
  )
    return false;
  return isTrustedWallpaperEngineDisplayCapturePermission(
    webContents,
    origin,
    details,
  );
...
async function prepareWallpaperEngineRendererCapture(sessionId, fps) {
  if (
    !mainWindow ||
    mainWindow.isDestroyed() ||
    !/^[a-f0-9]{24}$/i.test(String(sessionId || ""))
  ) {
  ...
  const safeFps = Math.max(
    24,
    Math.min(WALLPAPER_ENGINE_MAX_CAPTURE_FPS, Number(fps) || 60),
  );
  const grant = getWallpaperEngineCaptureGrant();
  if (!grant || grant.sessionId !== safeSessionId)
    return { ok: false, error: "WALLPAPER_CAPTURE_GRANT_MISSING" };
  const safeSourceId = /^window:\d+:\d+$/.test(String(grant.sourceId || ""))
    ? String(grant.sourceId)
    : "";
  if (!safeSourceId)
    return { ok: false, error: "WALLPAPER_CAPTURE_SOURCE_INVALID" };
  const script = `(() => {
  ...
  try {
    ...
    const result = await Promise.race([
      ...
      new Promise((resolve) => {
        timeout = setTimeout(
          () =>
            resolve({ ok: false, error: "WALLPAPER_CAPTURE_PREPARE_TIMEOUT" }),
          WALLPAPER_ENGINE_CAPTURE_PREPARE_TIMEOUT_MS,
        );
      }),
    ]);
    ...
    return {
      ...
      error: String(
        (error && (error.message || error.name)) ||
          error ||
          "WALLPAPER_CAPTURE_PREPARE_FAILED",
      ).slice(0, 500),
    };
  } finally {
    if (wallpaperEngineCapturePreparationOperation === grant.operation)
      wallpaperEngineCapturePreparationOperation = 0;
    if (timeout) clearTimeout(timeout);
  }
}

async function prepareWallpaperEngineRendererGlassCapture(
  sessionId,
  fps,
  sourceId,
) {
  ...
  const safeSourceId = /^window:\d+:\d+$/.test(String(sourceId || ""))
    ? String(sourceId)
    : "";
  const grant = getWallpaperEngineCaptureGrant();
  if (
    !grant ||
    grant.kind !== "dwm-glass" ||
    grant.sessionId !== safeSessionId ||
  ...
  try {
      ...
      new Promise((resolve) => {
        timeout = setTimeout(
          () =>
            resolve({
              ok: false,
              error: "WALLPAPER_GLASS_CAPTURE_PREPARE_TIMEOUT",
            }),
          WALLPAPER_ENGINE_CAPTURE_PREPARE_TIMEOUT_MS,
        );
      }),
    ...
    return {
      ...
      error: String(
        (error && (error.message || error.name)) ||
          error ||
          "WALLPAPER_GLASS_CAPTURE_PREPARE_FAILED",
      ).slice(0, 500),
    };
  } finally {
    if (wallpaperEngineCapturePreparationOperation === grant.operation)
...
async function prepareWallpaperEngineRendererHostBoundsFrame(
  ...
  if (
    ...
    return {
      ok: false,
      frozen: false,
      error: "WALLPAPER_BOUNDS_FREEZE_RENDERER_UNAVAILABLE",
    };
  }
  const safeSessionId = String(sessionId);
  ...
  try {
    ...
    return result && typeof result === "object"
      ...
      : {
          ok: false,
          frozen: false,
          error: "WALLPAPER_BOUNDS_FREEZE_RESULT_INVALID",
        };
  } catch (error) {
    return {
      ok: false,
      ...
      error: String(
        (error && (error.message || error.name)) ||
          error ||
          "WALLPAPER_BOUNDS_FREEZE_FAILED",
      ).slice(0, 500),
    };
  }
}

async function prepareWallpaperEngineRendererDesktopPreview(
  sessionId,
  reason = "full-desktop-passive",
) {
  const safeSessionId = String(sessionId || "");
  ...
  if (
    ...
    return {
      ok: false,
      preview: false,
      error: "WALLPAPER_DESKTOP_PREVIEW_RENDERER_UNAVAILABLE",
    };
  }
  const script = `(() => {
  ...
  try {
    ...
    return result && typeof result === "object"
      ...
      : {
          ok: false,
          preview: false,
          error: "WALLPAPER_DESKTOP_PREVIEW_RESULT_INVALID",
        };
  } catch (error) {
    return {
      ok: false,
      ...
      error: String(
        (error && (error.message || error.name)) ||
          error ||
          "WALLPAPER_DESKTOP_PREVIEW_FAILED",
      ).slice(0, 500),
    };
  }
}
...
function waitForWallpaperEngineHelperExit(child, timeoutMs = 2200) {
  ...
  return new Promise((resolve) => {
    ...
    timer = setTimeout(
      () => finish(false),
      Math.max(600, Number(timeoutMs) || 2200),
    );
  });
}

async function prepareWallpaperEngineProjectPreviewBeforeDesktopEmbedding(
  win,
  reason = "full-desktop-passive",
) {
  if (!win || win.isDestroyed() || appQuitting) {
    return {
      ok: false,
      error: "FULL_DESKTOP_WALLPAPER_ENGINE_HOST_UNAVAILABLE",
    };
  }
  ...
  const prepared = await prepareWallpaperEngineRendererDesktopPreview(
    sessionId,
    reason,
  );
  if (!prepared || prepared.ok !== true) {
    return {
      ok: false,
      error: String(
        (prepared && prepared.error) || "WALLPAPER_DESKTOP_PREVIEW_UNAVAILABLE",
      ),
    };
  }

  if (
    wallpaperEngineRuntime.pending ||
  ...
  try {
    ...
    return {
      ...
      error: String(
        (error && (error.message || error.name)) ||
          error ||
          "FULL_DESKTOP_WALLPAPER_ENGINE_SUSPEND_FAILED",
      ),
    };
  }
  ...
  if (helperProcess && helperExited !== true) {
    return {
      ok: false,
      error: "FULL_DESKTOP_WALLPAPER_ENGINE_HELPER_EXIT_TIMEOUT",
    };
  }
  return {
    ok: true,
...
function stopWallpaperEngineRuntimeForRenderer(reason = "") {
  ...
  return wallpaperEngineRuntime.stop().catch((error) => {
    console.warn(
      "[Wallpaper Engine] renderer cleanup failed:",
...
function finishWallpaperEngineVisibleHostResume(win) {
  ...
  const desktopMode = fullDesktopModeRuntime.getStatus(
    "wallpaper-engine-resume-finished",
  );
  setMainWindowBackgroundThrottling(
    win,
    desktopMode.enabled === true ? false : MAIN_WINDOW_BACKGROUND_THROTTLING,
  );
}
...
function suspendWallpaperEngineForHiddenHost(win, reason = "hidden") {
  ...
  if (wallpaperEngineHostVisibilitySuspended) {
    return (
      wallpaperEngineHostVisibilityStopPromise ||
      Promise.resolve({ ok: true, stopped: true })
    );
  }
  wallpaperEngineHostVisibilitySuspended = true;
  wallpaperEngineHostVisibilityOperation += 1;
...
function resumeWallpaperEngineForVisibleHost(win, reason = "visible") {
  ...
  const notifyRestart = () => {
    if (
      wallpaperEngineHostVisibilityOperation !== visibilityOperation ||
  ...
  Promise.resolve(stopped)
    .catch(() => null)
    .finally(() => {
      if (wallpaperEngineHostVisibilityStopPromise === stopped)
...
function requestFullDesktopEscapeExit(reason = "escape-key") {
  ...
  if (
    fullDesktopEscapeExitPending ||
    (status.enabled !== true && fullDesktopEnablePending !== true)
  )
    return false;
  ...
  const exitOperation =
    status.enabled === true
      ...
      : syncWallpaperEngineDesktopIconLayering(
          `${reason}-cancelled-enable`,
          false,
        ).then(() => ({
          ok: true,
          enabled: false,
  ...
  Promise.resolve(exitOperation)
    .catch((error) => {
      console.warn(
        "[FullDesktopMode] Escape exit failed:",
        (error && error.message) || error,
      );
    })
    .finally(() => {
...
function registerFullDesktopEscapeShortcut() {
  ...
  try {
    registered = globalShortcut.register("Escape", () =>
  ...
  if (!fullDesktopEscapeRegistered && fullDesktopEscapeSuspendedBinding) {
    ...
    try {
      if (
        globalShortcut.register(suspended.accelerator, () =>
          sendGlobalHotkeyAction(suspended.action),
        )
      ) {
...
function syncFullDesktopEscapeShortcut(reason = "desktop-state") {
  ...
  if (status.enabled === true || fullDesktopEnablePending === true)
    registerFullDesktopEscapeShortcut();
  else unregisterFullDesktopEscapeShortcut();
}

function syncWallpaperEngineDesktopIconLayering(
  reason = "desktop-state",
  desiredOverride,
) {
  const operation = async () => {
    const desired =
      typeof desiredOverride === "boolean"
        ? desiredOverride
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      if (
        ...
        active.active !== true ||
        !active.sessionId ||
        active.captureMode !== "dwm-thumbnail"
      )
        return true;
      try {
        const updated =
          await wallpaperEngineRuntime.updateDwmDesktopIconLayering(
            active.sessionId,
            desired,
          );
        ...
        console.warn(
          "[FullDesktopMode] DWM desktop-icon layering sync failed:",
          reason,
          (error && error.message) || error,
        );
      }
    ...
    console.warn(
      "[FullDesktopMode] DWM desktop-icon layering was not acknowledged:",
      reason,
      desired,
    );
    return false;
  };
  wallpaperEngineDesktopIconLayeringQueue =
    wallpaperEngineDesktopIconLayeringQueue.then(operation, operation);
...
function syncWallpaperEngineWithFullDesktopMode(win, reason = "desktop-state") {
  ...
  if (
    !appQuitting &&
    (desktopMode.enabled !== true || desktopMode.interactive === true)
  ) {
...
async function enableFullDesktopMode(win, options = {}) {
  ...
  try {
    if (!options || options.interactive !== false) {
      ...
      await syncWallpaperEngineDesktopIconLayering(
        "enable-coexist-preflight",
        true,
      );
    }
    if (
      enableOperation !== fullDesktopEnableOperation ||
      fullDesktopEnablePending !== true
    ) {
      return {
        ok: false,
        enabled: false,
        cancelled: true,
        error: "FULL_DESKTOP_ENABLE_CANCELLED",
      };
    }
    return await fullDesktopModeRuntime.enable(win, options);
  } finally {
    if (enableOperation === fullDesktopEnableOperation)
      fullDesktopEnablePending = false;
    await syncWallpaperEngineDesktopIconLayering("enable-settled").catch(
      () => false,
    );
    fullDesktopModeHostVisibilityTransitionDepth = Math.max(
      0,
      fullDesktopModeHostVisibilityTransitionDepth - 1,
    );
    syncWallpaperEngineWithFullDesktopMode(win, "enable-settled");
    if (
      fullDesktopModeRuntime.getStatus("enable-settled-cleanup").enabled !==
      true
    ) {
...
async function setFullDesktopModeInteractive(
  value,
  reason = "interaction-changed",
) {
  fullDesktopModeHostVisibilityTransitionDepth += 1;
  try {
    if (value === true)
      await syncWallpaperEngineDesktopIconLayering(
        `${reason}-coexist-preflight`,
        true,
      );
    return await fullDesktopModeRuntime.setInteractive(value, reason);
  } finally {
    await syncWallpaperEngineDesktopIconLayering(`${reason}-settled`).catch(
      () => false,
    );
    fullDesktopModeHostVisibilityTransitionDepth = Math.max(
      0,
    ...
    if (
      fullDesktopModeRuntime.getStatus(`${reason}-cleanup`).enabled !== true
    ) {
      releaseFullDesktopModeRecoveryTray();
    }
...
async function toggleFullDesktopModeInteraction(
  reason = "interaction-toggled",
) {
  fullDesktopModeHostVisibilityTransitionDepth += 1;
  try {
    const before = fullDesktopModeRuntime.getStatus(`${reason}-before`);
    if (before.interactive !== true)
      await syncWallpaperEngineDesktopIconLayering(
        `${reason}-coexist-preflight`,
        true,
      );
...
function scheduleWallpaperEngineHostBoundsRestart(
  win,
  reason = "bounds-changed",
) {
  if (!win || win.isDestroyed()) return;
  ...
  if (
    ...
    return;
  if (
    !wallpaperEngineHostBoundsRestartPending &&
  ...
  if (job && job.started === true) {
    ...
    wallpaperEngineHostBoundsFollowupReason = String(
      reason || "bounds-changed",
    ).slice(0, 80);
    return;
  }
  ...
  wallpaperEngineHostBoundsRestartTimer = setTimeout(() => {
    wallpaperEngineHostBoundsRestartTimer = null;
    if (wallpaperEngineHostBoundsStopPromise !== job || job.started === true)
      return;
    const currentBeforePrepare = wallpaperEngineRuntime.getStatus();
    if (
      !currentBeforePrepare ||
      currentBeforePrepare.active !== true ||
    ...
    clearWallpaperEngineCaptureGrant();
    job.promise = prepareWallpaperEngineRendererHostBoundsFrame(
      job.sessionId,
      job.reason,
    ).then(async (prepared) => {
        ...
        return {
          ...
          error: String(
            (prepared && prepared.error) ||
              "WALLPAPER_BOUNDS_FREEZE_UNAVAILABLE",
          ),
        };
      }
      try {
        ...
        return {
          ...
          error: String(
            (error && (error.message || error.name)) ||
              error ||
              "WALLPAPER_BOUNDS_RUNTIME_STOP_FAILED",
          ),
        };
      }
    });
    Promise.resolve(job.promise)
      .then((result) => {
        ...
        const recoveryOnly =
          !ownsCurrentJob || !operationCurrent || result.stale === true;
        setTimeout(() => {
          if (
            wallpaperEngineHostVisibilitySuspended ||
            win.isDestroyed() ||
          ...
          try {
            win.webContents.send(
              "mineradio-wallpaper-engine-host-bounds-changed",
              {
                phase: "restart",
...
function configureLocalAppPermissions() {
  ...
  ses.setPermissionCheckHandler(
    (webContents, permission, requestingOrigin, details) => {
      ...
      if (permission === "media")
        return isTrustedWallpaperEnginePreparationMediaPermission(
          webContents,
          origin,
          details,
        );
      return (
        LOCAL_APP_PERMISSION_ALLOWLIST.has(permission) && isLocalAppUrl(origin)
      );
    },
  );
  ses.setPermissionRequestHandler(
    (webContents, permission, callback, details) => {
      ...
      if (permission === "display-capture") {
        callback(
          isTrustedWallpaperEngineDisplayCapturePermission(
            webContents,
            origin,
            details,
          ),
      ...
      if (permission === "media") {
        callback(
          isTrustedWallpaperEnginePreparationMediaPermission(
            webContents,
            origin,
            details,
          ),
      ...
      Promise.resolve()
        .then(async () => {
          ...
          if (grant.kind === "dwm-glass") {
            ...
            const sourceMatch = /^window:(\d+):\d+$/.exec(
              String((source && source.id) || ""),
            );
            if (
              wallpaperEngineCaptureGrant !== grant ||
              !current ||
              current.active !== true ||
              ...
              Number(sourceMatch[1]) !==
                Number(current.dwmGlassSurfaceWindowId) ||
              String((source && source.name) || "") !==
                "Mineradio WE DWM Surface"
            ) {
              reply({});
              return;
            }
          ...
          let refreshed =
            typeof wallpaperEngineRuntime.refreshActiveSource === "function"
              ? await wallpaperEngineRuntime.refreshActiveSource(
                  grant.sessionId,
                  {
                    timeoutMs: 1600,
                    pollIntervalMs: 80,
          ...
          if (
            ...
            String(refreshed.sourceId) !== String(grant.sourceId || "")
          ) {
            await wallpaperEngineRuntime.embedActiveWindow(grant.sessionId, {
              ...
              desktopIconLayering: fullDesktopIconLayeringDesired(
                "wallpaper-engine-source-refresh",
              ),
            });
            refreshed = await wallpaperEngineRuntime.refreshActiveSource(
              grant.sessionId,
              {
                timeoutMs: 1600,
                pollIntervalMs: 80,
...
function getDisplayState(win) {
  ...
  const hasDisplayOnLeft = displays.some((candidate) => {
    if (!candidate || candidate.id === displayId || !candidate.bounds)
      return false;
    return (
      rectsOverlapOnY(bounds, candidate.bounds) &&
      Math.abs(candidate.bounds.x + candidate.bounds.width - bounds.x) <=
        edgeTolerance
    );
  });
...
function getWindowState(win) {
  ...
  return {
    ...
    isFullScreen:
      win.isFullScreen() || htmlFullscreenActive || windowFullscreenActive,
    isMinimized: win.isMinimized(),
    isVisible: win.isVisible(),
    isFocused: win.isFocused(),
    isDesktopEmbedded: desktopMode.enabled === true,
    isDesktopInteractive: desktopMode.interactive === true,
    isDesktopIconCoexisting:
      desktopMode.coexisting === true && desktopMode.iconShapeActive === true,
    ...getDisplayState(win),
  };
}
...
function setMainWindowFullscreenResizeGuard(win, fullscreen) {
  ...
  try {
    if (
      typeof win.isResizable === "function" &&
      win.isResizable() === shouldResize
    )
      return;
    ...
    console.warn(
      "[WindowResizeGuard]",
      fullscreen ? "fullscreen-lock" : "windowed-restore",
      e.message || e,
    );
  }
}
...
function collectAppTrimPids() {
  ...
  try {
    app.getAppMetrics().forEach((row) => {
      if (row && Number.isFinite(Number(row.pid)))
        pids.add(Math.round(Number(row.pid)));
    });
  } catch (e) {}
...
async function trimAppMemoryNow(reason) {
  ...
  try {
    ...
    return {
      ok: false,
      reason: lastAppMemoryTrimReason,
      error: e.message || "APP_MEMORY_TRIM_FAILED",
...
function scheduleAppMemoryTrim(reason, delay = 9000) {
  ...
  if (
    memoryAutoState.appTrimEnabled === false ||
    memoryAutoState.backgroundTrimEnabled === false
  )
    return;
...
function normalizeMemoryAutoState(payload = {}) {
  const systemEnabled =
    systemMemory.SYSTEM_PURGE_AVAILABLE === true &&
    systemMemory.SYSTEM_PURGE_ENABLED === true;
  return {
    ...
    mask: systemMemory.normalizeMask(
      payload.mask != null ? payload.mask : memoryAutoState.mask,
    ),
    intervalMin: Math.max(
      ...
      Math.min(
        ...
        Math.round(
          Number(
            payload.intervalMin != null
              ? payload.intervalMin
              : memoryAutoState.intervalMin,
          ) || 30,
        ),
      ),
    ),
    thresholdPercent: Math.max(
      ...
      Math.min(
        ...
        Math.round(
          Number(
            payload.thresholdPercent != null
              ? payload.thresholdPercent
              : memoryAutoState.thresholdPercent,
          ) || 0,
        ),
      ),
    ),
...
async function runMemoryAutoTick(reason = "auto") {
  ...
  try {
    const result = await systemMemory.purgeSystemMemorySmart(
      memoryAutoState.mask,
      {
    ...
    memoryAutoState.lastResult = {
      ok: false,
      error: memoryAutoState.lastError,
    };
    return {
      ok: false,
      error: memoryAutoState.lastError,
...
function createOrUpdateTray() {
  ...
  const menu = Menu.buildFromTemplate([
    { label: `显示 ${APP_NAME}`, click: () => focusMainWindow() },
    {
      ...
      click: () =>
        disableFullDesktopMode("tray-exit-desktop-mode").catch((error) => {
          console.warn(
            "[FullDesktopMode] tray exit failed:",
            (error && error.message) || error,
          );
        }),
    },
...
function releaseFullDesktopModeRecoveryTray() {
  if (
    fullDesktopModeRuntime.getStatus("release-recovery-tray").enabled === true
  )
    return false;
...
function resolveStartupErrorCode(context, error) {
  ...
  if (/EADDRINUSE|address already in use|listen EADDRINUSE|端口/i.test(text))
    return "MR-BOOT-SERVER-PORT";
  if (/waitForServer|server|ECONNREFUSED|ERR_CONNECTION_REFUSED/i.test(text))
    return "MR-BOOT-SERVER-START";
  if (/loadURL|ERR_FAILED|ERR_ABORTED|navigation|did-fail-load/i.test(text))
    return "MR-BOOT-WINDOW-LOAD";
  if (/ReferenceError|TypeError|is not defined|Cannot read/i.test(text))
    return "MR-BOOT-MAIN-RUNTIME";
  if (/EPERM|EACCES|access is denied|permission/i.test(text))
    return "MR-BOOT-PERMISSION";
  ...
  try {
    ...
    startupState = {
      ...
      events: (startupState.events || [])
        .concat({ phase: String(phase || "unknown"), at: now, ...detail })
        .slice(-32),
    };
...
function buildStartupErrorMessage(context, code, logInfo, error) {
  const detail = startupErrorText(error);
  const reason = String((error && error.message) || error || "未知错误")
    .split(/\r?\n/)[0]
    .slice(0, 360);
  return [
    `错误代码：${code}`,
...
function reportWindowCreationFailure(context, error) {
  ...
  writeStartupState("failed", {
    ...
    code,
    error: startupErrorText(error).slice(0, 1200),
  });
...
function shouldEnsureDesktopShortcut() {
  ...
  return (
    app.isPackaged || process.env.MINERADIO_CREATE_DESKTOP_SHORTCUT === "1"
  );
}
...
function isQQCookieDomain(domain) {
  ...
  return (
    ...
    normalized.endsWith("qqmusic.qq.com")
  );
}
...
function readSavedQishuiCookieHeader() {
  ...
  try {
    add(path.join(app.getPath("sessionData"), ".qishui-cookie"));
  } catch (e) {}
  for (const filePath of candidates) {
    try {
      ...
      if (qishuiCookieHasLogin(cookie))
        return { cookie, source: filePath, method: "persisted-cookie" };
    } catch (e) {
...
function qishuiOfficialClientDataDirCandidates() {
  ...
  try {
    appDataPath = app.getPath("appData") || appDataPath;
  } catch (_) {}
  try {
    homePath = app.getPath("home") || homePath;
  } catch (_) {}
...
function sqliteDecodeSerialValue(buffer, offset, type) {
  ...
  if (type >= 12 && type % 2 === 0)
    return { value: buffer.slice(offset, offset + size), size };
  if (type >= 13 && type % 2 === 1)
...
function sqliteParseRecord(buffer, offset, payloadSize) {
  ...
  if (!header || header.value <= 0 || offset + header.value > payloadEnd)
    return [];
...
function sqliteLeafRecords(buffer) {
  if (
    !buffer ||
    buffer.length < 100 ||
    buffer.toString("ascii", 0, 16) !== "SQLite format 3\0"
  )
    return [];
  ...
  for (let pageNo = 1; pageNo <= pageCount; pageNo++) {
    ...
    if (headerStart + 8 > buffer.length || buffer[headerStart] !== 0x0d)
      continue;
    ...
    for (let i = 0; i < cellCount; i++) {
      ...
      const payloadSize = readSqliteVarint(
        buffer,
        cellOffset,
        Math.min(buffer.length, cellOffset + 10),
      );
      if (!payloadSize) continue;
      const rowId = readSqliteVarint(
        buffer,
        payloadSize.next,
        Math.min(buffer.length, payloadSize.next + 10),
      );
      if (!rowId) continue;
...
function sqliteCookieColumns(records) {
  const master = records.find((record) =>
    record.some(
      (value) =>
        typeof value === "string" && /CREATE\s+TABLE\s+cookies/i.test(value),
    ),
  );
  const sql =
    master &&
...
function readQishuiOfficialClientCookieDatabase(target) {
  const store =
    target && typeof target === "object" && target.cookieDbPath
      ? target
      : {
          cookieDbPath:
            path.basename(String(target || "")).toLowerCase() === "cookies"
  ...
  if (!fs.existsSync(cookieDb))
    return { cookie: "", source: "", missing: true, dbPath: cookieDb };
  try {
    const cookie = extractQishuiCookieHeaderFromCookieDatabase(cookieDb);
    if (!qishuiCookieHasLogin(cookie))
      return { cookie: "", source: "", noSession: true, dbPath: cookieDb };
    ...
    const locked =
      ...
      /used by another process|EBUSY|locked|busy|access.*denied|无法访问|另一个程序正在使用|进程无法访问/i.test(
        message,
      );
    return {
      cookie: "",
...
async function readQishuiOfficialClientCookieHeader() {
  ...
  for (const store of stores) {
    ...
    if (direct && direct.cookie) {
      return finish(Object.assign({ method: "cookie-db" }, direct), "login", {
        method: "cookie-db",
  ...
  if (!stores.length)
    last = { cookie: "", source: "", missing: true, dbPath: "" };
  if (!session || typeof session.fromPath !== "function") {
    return finish(
      Object.assign(
        { cookie: "", source: "", skipped: "session.fromPath unavailable" },
        lastLocked || last || {},
      ),
  ...
  for (const store of stores) {
    ...
    try {
      const clientSession = session.fromPath(store.sessionPath, {
        cache: false,
      });
      const cookie = await readQishuiLoginCookieHeader(clientSession);
      if (attempt)
        attempt.electronRead = qishuiCookieHasLogin(cookie)
          ? "login"
          : "no-login";
      if (qishuiCookieHasLogin(cookie)) {
        return finish(
          ...
          "login",
          {
            ...
            rootHint: (attempt && attempt.rootHint) || "Detected/client-data",
            storeLayout:
              (attempt && attempt.storeLayout) || "nested-cookie-store",
            relativeStore:
              (attempt && attempt.relativeStore) || "Network/Cookies",
          },
        );
      }
    } catch (e) {
      ...
      console.warn(
        "[QishuiLocalSession] electron import skipped",
        JSON.stringify({
          rootHint: (attempt && attempt.rootHint) || "Detected/client-data",
          storeLayout:
            (attempt && attempt.storeLayout) || "nested-cookie-store",
    ...
    Object.assign(
      {
        cookie: "",
        source: "",
        skipped: "no logged-in SodaMusic client session",
      },
      lastLocked || last || {},
    ),
    lastLocked ? "locked" : "not-found",
...
function qqLoginCookieCandidateScore(cookie) {
  ...
  else if (domain === "qqmusic.qq.com" || domain.endsWith(".qqmusic.qq.com"))
    score += 360;
...
function buildCookieHeaderFor(
  ...
  (cookies || []).forEach((cookie) => {
    if (
      ...
      cookieIsExpired(cookie, nowSeconds)
    )
      return;
    const score =
      typeof candidateScore === "function"
        ? Number(candidateScore(cookie)) || 0
        : 0;
    ...
    if (
      ...
      (score === previous.score &&
        ...
        tieKey > previous.tieKey)
    ) {
      picked.set(cookie.name, {
        value: cookie.value || "",
        score,
        expirationDate,
        tieKey,
...
function buildCookieHeader(cookies) {
  return buildCookieHeaderFor(
    cookies,
    isQQCookieDomain,
    QQ_LOGIN_COOKIE_PRIORITY,
    qqLoginCookieCandidateScore,
  );
}
          ...
          setTimeout(() => {
            if (!settled && loginWindow && !loginWindow.isDestroyed()) {
              loginWindow
                ...
                .catch((e) =>
                  console.warn("QQ login warmup navigation failed:", e.message),
                );
            }
          }, 900);
        }
...
async function openKugouMusicLoginWindow(owner) {
  ...
  if (kugouCookieHasPlayback(initialCookie))
    return { ok: true, cookie: initialCookie, reused: true };
      ...
      try {
        ...
        if (kugouCookieHasPlayback(cookie)) {
          ...
          setTimeout(() => {
            if (!settled && loginWindow && !loginWindow.isDestroyed()) {
              loginWindow
                .loadURL(KUGOU_LOGIN_WARMUP_URL)
                .catch((e) =>
                  console.warn(
                    "Kugou login warmup navigation failed:",
                    e.message,
                  ),
                );
            }
    ...
    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      if (/^https?:\/\//i.test(url)) {
        loginWindow
          .loadURL(url)
          .catch((e) =>
            console.warn("Kugou login popup navigation failed:", e.message),
          );
      } else {
        shell.openExternal(url).catch(() => {});
      }
          ...
          kugouCookieHasPlayback(cookie)
            ...
            : kugouCookieHasLogin(cookie)
              ? {
                  ...
                  partial: true,
                  message:
                    "酷狗账号已登录，但播放 token 不完整，请稍后在播放器内重试登录",
                }
  ...
  return new Promise((resolve) => {
    ...
    const buildQrHtml = (statusText) => {
      ...
      return [
        ...
        qrPayload && qrPayload.qrcodeIndexUrl
          ? "<p>这个二维码来自汽水 PC 登录接口；抖音 App 扫描可能打开 404 页面，请用汽水音乐 App。</p>"
          : "",
        "</main>",
      ].join("");
    };
    ...
    const armQrExpireTimer = () => {
      ...
      const ttlMs =
        qrPayload && qrPayload.expireTime
          ? Math.max(30000, qrPayload.expireTime * 1000 - Date.now())
          : 180000;
      ...
      loginWindow
        .loadURL(
          "data:text/html;charset=utf-8," +
            encodeURIComponent(buildQrHtml(statusText)),
        )
        .catch((e) => {
          console.warn(
            "Qishui QR fallback page failed:",
            (e && e.message) || e,
          );
        });
    };
    ...
    const qrNextPollDelay = (status) => {
      ...
      if (status.retryAfterMs)
        return Math.max(
          60000,
          Math.min(90000, Number(status.retryAfterMs) || 60000),
        );
      ...
      if (
        status.cookie ||
        status.confirmed ||
        /scan|confirm|success|login/.test(key)
      )
        return 2400;
    ...
    const pollQrStatus = async () => {
      ...
      if (qrPollBackoffUntil && Date.now() < qrPollBackoffUntil) {
        setQrStatusText(
          lastStatus || "汽水确认接口临时限流，已自动降频继续确认…",
        );
      ...
      try {
        const status = await checkQishuiPcQrLogin(
          qrPayload.token,
          qrPayload.cookie || "",
          qrPayload,
        );
        ...
        if (status && status.retryAfterMs) {
          qrPollBackoffUntil =
            Date.now() +
            Math.max(5000, Math.min(90000, Number(status.retryAfterMs) || 0));
          setQrStatusText(
            lastStatus || "汽水确认接口临时限流，已自动降频继续确认…",
          );
          return;
        ...
        if (status && status.needsSms) {
          ...
          setQrStatusText(
            lastStatus ||
              "汽水要求短信或二次验证，请先在汽水 App 内完成账号安全验证",
          );
          return;
        }
...
async function openQishuiOfficialWebLoginWindowLegacy(owner, config) {
  ...
  return new Promise((resolve) => {
    ...
    const officialLoginUrls = [
      QISHUI_WEB_LOGIN_URL,
      QISHUI_WEB_LOGIN_FALLBACK_URL,
    ]
      .map((value) => String(value || "").trim())
    ...
    const readResult = async (extra) => {
      try {
        ...
        return publicResult(
          "",
          Object.assign({ warning: e.message }, extra || {}),
        );
      }
    };
    ...
    const showLoadFallbackPage = (message) => {
      ...
      const html = [
        ...
        String(lastLoadError).replace(
          /[<>&]/g,
          (ch) => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;" })[ch],
        ),
        "</p><p>窗口不会自动关闭；可以稍后重试，或在浏览器里打开汽水官方扫码页。</p></main>",
      ].join("");
      safeLoadLoginWindowUrl(
    ...
    const loadOfficialLoginUrl = () => {
      ...
      safeLoadLoginWindowUrl(targetUrl).then((loadResult) => {
        ...
        lastLoadError =
          (loadResult && loadResult.error) || "汽水官方网页打开失败";
      ...
      if (/^https?:\/\//i.test(url)) {
        safeLoadLoginWindowUrl(url).then((result) => {
          if (result && !result.ok && !result.skipped)
            console.warn(
              "Qishui official window navigation failed:",
              result.error,
            );
        });
      } else {
    ...
    loginWindow.webContents.on(
      ...
      (event, errorCode, errorDescription, validatedURL, isMainFrame) => {
        ...
        console.warn(
          "Qishui official window did-fail-load:",
          lastLoadError,
          validatedURL || "",
        );
        scheduleOfficialLoginLoad();
    ...
    loginWindow.on("closed", async () => {
      ...
      clearLoadRetryTimer();
      resolve(
        await readResult({ cancelled: false, loadError: lastLoadError || "" }),
      );
    });
...
function qishuiOAuthRedirectMatches(targetUrl, redirectUri) {
  try {
    ...
    return (
      target.protocol === redirect.protocol &&
...
async function openQishuiMusicLoginWindow(owner) {
  ...
  if (imported && imported.cookie) {
    ...
    try {
      ...
      console.warn(
        "Imported Qishui cookie validation skipped:",
        (error && error.message) || error,
      );
    }
    return {
      ...
      message:
        "已读取本机汽水 PC 登录态，正在导入 Mineradio 并同步我的喜欢和歌单",
    };
  }
  ...
  if (saved && saved.cookie) {
    try {
      ...
      console.warn(
        "Saved Qishui cookie validation skipped:",
        (error && error.message) || error,
      );
    }
  }
  if (
    ...
    saved.cookie &&
    savedStatus &&
    savedStatus.loggedIn &&
    savedStatus.webSession
  ) {
    return {
      ...savedStatus,
      ok: true,
  ...
  return {
    ...
    localSessionDiagnostics: (imported && imported.diagnostics) || null,
    error: locked
      ? "QISHUI_LOCAL_COOKIE_DB_LOCKED"
      : "QISHUI_LOCAL_COOKIE_NOT_FOUND",
    message: locked
...
function spotifyOAuthResultHtml(ok, message) {
  ...
  return [
    ...
    "</style>",
    '<main><div class="brand">SPOTIFY</div><h1>' +
      (ok ? "授权完成" : "授权失败") +
      "</h1><p>" +
      escaped +
      "</p></main>",
...
function startSpotifyOAuthCallbackServer(redirectUri, onCallback) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      reject(
        Object.assign(new Error("SPOTIFY_REDIRECT_URI_INVALID"), {
    ...
    if (redirect.protocol !== "http:") {
      reject(
        Object.assign(
          new Error("SPOTIFY_REDIRECT_URI_MUST_BE_HTTP_LOCALHOST"),
          { code: "SPOTIFY_REDIRECT_URI_MUST_BE_HTTP_LOCALHOST" },
        ),
      );
      return;
    }
    ...
    const callbackServer = http.createServer(async (req, res) => {
      ...
      try {
        ...
        res.end(
          spotifyOAuthResultHtml(
            ok,
            (result && (result.message || result.error)) ||
              (ok ? "可以回到 Mineradio。" : "请回到 Mineradio 重新尝试。"),
          ),
        );
      } catch (e) {
        ...
        res.end(
          spotifyOAuthResultHtml(
            false,
            (e && e.message) || "SPOTIFY_OAUTH_CALLBACK_FAILED",
          ),
        );
      }
    });
    callbackServer.once("error", (err) => {
      const code =
        err && err.code === "EADDRINUSE"
          ...
          : (err && err.code) || "SPOTIFY_CALLBACK_SERVER_FAILED";
...
async function openSpotifyMusicLoginWindow(owner) {
  ...
  if (!config.configured) {
    return {
      ...
      message:
        "Spotify 登录需要先配置 SPOTIFY_CLIENT_ID，并在 Spotify Developer Dashboard 登记本地回调地址 " +
        config.redirectUri,
    };
  }
  ...
  return new Promise(async (resolve) => {
    ...
    const finish = (result) => {
      ...
      settled = true;
      if (callbackServer && typeof callbackServer.close === "function")
        callbackServer.close();
      if (loginWindow && !loginWindow.isDestroyed()) loginWindow.close();
      resolve(result);
      return result;
    ...
    const exchangeFromRedirect = async (targetUrl, event) => {
      ...
      try {
        ...
        return finish(
          Object.assign(
            { ok: true, provider: "spotify", opened: true },
            info || {},
            {
              redirectUri: config.redirectUri,
              message:
                "Spotify 登录成功，会员状态、歌单和 Liked Songs 已可同步。",
            },
          ),
        );
      } catch (e) {
    ...
    try {
      callbackServer = await startSpotifyOAuthCallbackServer(
        config.redirectUri,
        exchangeFromRedirect,
      );
    } catch (e) {
      resolve({
        ok: false,
        ...
        redirectUri: config.redirectUri,
        message:
          (e.code || e.message) === "SPOTIFY_CALLBACK_PORT_BUSY"
    ...
    const handleMaybeRedirect = (targetUrl, event) => {
      if (!spotifyOAuthRedirectMatches(targetUrl, config.redirectUri))
        return false;
      exchangeFromRedirect(targetUrl, event).catch((e) => {
        finish({
          ok: false,
          provider: "spotify",
          error: e.message || "SPOTIFY_OAUTH_EXCHANGE_FAILED",
        });
      });
      return true;
    };
    ...
    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      ...
      if (/^https?:\/\//i.test(url)) {
        loginWindow
          .loadURL(url)
          .catch((e) =>
            console.warn("Spotify login popup navigation failed:", e.message),
          );
      } else {
        shell.openExternal(url).catch(() => {});
      }
...
async function initializeLoginEasterEggGate() {
  const status = await loginEasterEggGate.initialize(() =>
    clearAllProviderLoginState("startup-gate"),
  );
...
async function clearAllProviderLoginState(reason) {
  if (
    localServer &&
    typeof localServer.clearAllLoginCredentials === "function"
  ) {
    const result = localServer.clearAllLoginCredentials(
      reason || "login-reset",
    );
    if (!result || result.ok !== true) {
      throw new Error(
        (result && result.error) || "LOCAL_SERVER_LOGIN_STATE_CLEAR_FAILED",
      );
    }
  }
...
function getDisplayArea(display) {
  return (
    (display && (display.workArea || display.bounds)) ||
...
function getAdaptiveWindowMinimumSize(display) {
  ...
  const margin = Math.min(
    WINDOWED_MARGIN,
    Math.max(8, Math.round(Math.min(area.width, area.height) * 0.04)),
  );
  ...
  return {
    width: Math.round(
      Math.max(
        360,
        Math.min(portrait ? 540 : MIN_WINDOWED_WIDTH, availableWidth),
      ),
    ),
    height: Math.round(
      Math.max(
        360,
        Math.min(portrait ? 720 : MIN_WINDOWED_HEIGHT, availableHeight),
      ),
    ),
  };
}
...
function clampBoundsToDisplayArea(bounds, display) {
  ...
  let width = Math.round(
    Math.min(
      Math.max(Number(bounds && bounds.width) || minimum.width, minimum.width),
      area.width,
    ),
  );
  let height = Math.round(
    Math.min(
      Math.max(
        Number(bounds && bounds.height) || minimum.height,
        minimum.height,
      ),
      area.height,
    ),
  ...
  const x = Math.round(
    Math.max(area.x, Math.min(Number.isFinite(rawX) ? rawX : area.x, maxX)),
  );
  const y = Math.round(
    Math.max(area.y, Math.min(Number.isFinite(rawY) ? rawY : area.y, maxY)),
  );
...
function getWindowedBounds(win) {
  ...
  const margin = Math.min(
    WINDOWED_MARGIN,
    Math.max(12, Math.round(Math.min(area.width, area.height) * 0.04)),
  );
  const maxWidth = Math.max(360, area.width - margin);
  const maxHeight = Math.max(360, area.height - margin);
  const minimum = getAdaptiveWindowMinimumSize(display);
  const aspect = portrait
    ? Math.max(0.52, Math.min(0.82, area.width / Math.max(1, area.height)))
    : WINDOWED_ASPECT;
...
function desktopLyricsDefaultBounds(payload = desktopLyricsState) {
  ...
  const width = Math.round(
    Math.min(Math.max(880, bounds.width * 0.72), bounds.width - 96),
  );
  const height = Math.round(
    Math.min(Math.max(340, bounds.height * 0.38), 560, bounds.height - 96),
  );
  return {
...
function broadcastDesktopLyricsEnabledState(enabled) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("mineradio-desktop-lyrics-enabled-state", {
      enabled: !!enabled,
    });
  }
}

function positionDesktopLyricsWindow(
  payload = desktopLyricsState,
  options = {},
) {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) return;
  const shouldUseManualBounds = desktopLyricsUserBounds && !options.force;
  setDesktopLyricsBounds(
    shouldUseManualBounds
      ? desktopLyricsUserBounds
      : desktopLyricsDefaultBounds(payload),
  );
...
function sendDesktopLyricsState() {
  ...
  desktopLyricsWindow.webContents.send(
    "mineradio-desktop-lyrics-state",
    desktopLyricsState,
  );
}
...
function createDesktopLyricsWindow(payload = {}) {
  ...
  const yChanged =
    hasY &&
    Number.isFinite(Number(previousY)) &&
    Math.abs(nextY - clampNumber(previousY, 0.08, 0.92, 0.76)) > 0.001;
  const opacityChanged =
    Object.prototype.hasOwnProperty.call(payload || {}, "opacity") &&
    Math.abs(
      clampNumber(desktopLyricsState.opacity, 0.28, 1, 0.92) -
        clampNumber(previousOpacity, 0.28, 1, 0.92),
    ) > 0.001;
  ...
  if (desktopLyricsWindow && !desktopLyricsWindow.isDestroyed()) {
    if (yChanged) {
      ...
      opacityChanged &&
      typeof desktopLyricsWindow.setOpacity === "function"
    ) {
      desktopLyricsWindow.setOpacity(
        clampNumber(desktopLyricsState.opacity, 0.28, 1, 0.92),
      );
    }
    applyDesktopLyricsMouseBehavior();
  ...
  positionDesktopLyricsWindow(desktopLyricsState, {
    force: yChanged || !desktopLyricsUserBounds,
  });
  ...
  desktopLyricsWindow.webContents.once(
    "did-finish-load",
    sendDesktopLyricsState,
  );
...
function hookExplorerRestartForFullDesktop(win) {
  if (
    ...
    !win ||
    win.isDestroyed() ||
    typeof win.hookWindowMessage !== "function"
  )
    return;
  if (
    win.__mineradioTaskbarCreatedHookPending ||
    win.__mineradioTaskbarCreatedMessageId
  )
    return;
...
`;
  execFile(
    ...
    ["-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", script],
    {
      ...
      env: {
        ...process.env,
        TEMP: NATIVE_HELPER_TEMP_PATH,
        TMP: NATIVE_HELPER_TEMP_PATH,
      },
    },
    (error, stdout) => {
      ...
      try {
        win.hookWindowMessage(messageId, () => {
          setTimeout(() => {
            reconcileFullDesktopMode("explorer-restarted").catch(
              (reconcileError) => {
                console.warn(
                  "[FullDesktopMode] Explorer restart reconcile failed:",
        ...
        console.warn(
          "[FullDesktopMode] Explorer restart hook failed:",
          (hookError && hookError.message) || hookError,
        );
      }
    },
  );
...
function positionWallpaperWindow(reason = "display-change") {
  reconcileFullDesktopMode(reason).catch((error) => {
    console.warn(
      "[FullDesktopMode] display reconcile failed:",
      (error && error.message) || error,
    );
  });
}
...
async function createWallpaperWindow(payload = {}) {
  ...
  if (result && result.ok === true && result.enabled === true) {
    const backdrop = {
      ...
      enabled: true,
      active: true,
      kind: wallpaperEngineProvidesDesktopBackdrop()
        ? "wallpaper-engine-dwm"
        : "system-desktop",
    };
...
ipcMain.handle("desktop-window-minimize", async (event) => {
  ...
  if (
    win === mainWindow &&
    fullDesktopModeRuntime.getStatus("window-minimize").enabled === true
  ) {
...
ipcMain.handle("desktop-window-restore", async (event) => {
  ...
  if (
    win === mainWindow &&
    fullDesktopModeRuntime.getStatus("window-restore").enabled === true
  ) {
...
ipcMain.handle("desktop-window-toggle-maximize", (event) => {
  ...
  if (
    win === mainWindow &&
    fullDesktopModeRuntime.getStatus("window-toggle-maximize").enabled === true
  ) {
    return getWindowState(win);
  }
...
ipcMain.handle("desktop-window-toggle-fullscreen", (event) => {
  ...
  if (
    win === mainWindow &&
    fullDesktopModeRuntime.getStatus("window-toggle-fullscreen").enabled ===
      true
  ) {
    return getWindowState(win);
  }
...
ipcMain.handle("desktop-window-exit-fullscreen-windowed", (event) => {
  ...
  if (
    win === mainWindow &&
    fullDesktopModeRuntime.getStatus("window-exit-fullscreen").enabled === true
  ) {
    return getWindowState(win);
  }
  exitFullscreenToWindow(win);
...
ipcMain.on("mineradio-full-desktop-icon-shields", (event, payload = {}) => {
  ...
  const rects =
    payload && payload.enabled === true && payload.interactive === true
      ? payload.rects
      : [];
  fullDesktopModeRuntime.updateIconShields(
    Array.isArray(rects) ? rects : [],
    payload && payload.viewport && typeof payload.viewport === "object"
      ? payload.viewport
      : {},
  );
});
...
ipcMain.handle(
  ...
  async (event, visible) => {
    if (!isTrustedMainWindowIpc(event))
      return { ok: false, error: "DESKTOP_MODE_UNTRUSTED_SENDER" };
    return fullDesktopModeRuntime.setDesktopIconsVisible(
      visible !== false,
      "renderer-icons-visible",
    );
  },
);
...
ipcMain.handle(
  ...
  async (event, locked) => {
    if (!isTrustedMainWindowIpc(event))
      return { ok: false, error: "DESKTOP_MODE_UNTRUSTED_SENDER" };
    return fullDesktopModeRuntime.setSoftwareInteractionLocked(
      locked === true,
      "renderer-software-lock",
    );
  },
);
...
ipcMain.on("mineradio-full-desktop-request-keyboard-focus", (event, reason) => {
  ...
  const focusResult = fullDesktopModeRuntime.requestKeyboardFocus(
    `renderer-${String(reason || "pointerdown")
      .replace(/[^a-z0-9_-]+/gi, "-")
      .slice(0, 64)}`,
  );
  if (focusResult && focusResult.ok) return;
  const desktopStatus = fullDesktopModeRuntime.getStatus(
    "renderer-keyboard-focus-fallback",
  );
...
ipcMain.handle("mineradio-memory-get-snapshot", async () => {
  try {
    ...
    return {
      ok: false,
      error: e.message || "MEMORY_SNAPSHOT_FAILED",
      snapshot: systemMemory.getMemorySnapshot(),
      auto: memoryAutoState,
    };
  }
...
ipcMain.handle(
  ...
  async (_event, payload = {}) => {
    ...
    try {
      if (isMainWindowForegroundVisible()) {
        return {
          ...
          result: {
            ...
            reason: "foreground-visible",
            message:
              "System memory purge is skipped while Mineradio is visible.",
          },
          snapshot: systemMemory.getMemorySnapshot(),
          elevated: false,
          systemPurgeAvailable: systemMemory.SYSTEM_PURGE_AVAILABLE === true,
      ...
      const result = await systemMemory.purgeSystemMemorySmart(mask, {
        autoElevate,
        manual: true,
      });
      return {
...
ipcMain.handle("mineradio-cache-choose-directory", async () => {
  ...
  return {
    ok: true,
    canceled: false,
    rootPath: normalizeCacheRootPath(result.filePaths[0]),
  };
});
...
ipcMain.handle("mineradio-cache-set-settings", async (_event, payload = {}) => {
  try {
    ...
    cacheSettings = ensureCacheDirectories(
      writeCacheSettings({ rootPath: nextRoot }),
    );
    ...
    try {
      if (!isTrustedWallpaperEngineIpc(event))
        return {
          ...
          projects: [],
          count: 0,
          error: "WALLPAPER_ENGINE_UNTRUSTED_CALLER",
        };
      const snapshot = await wallpaperEngineLibrary.list({
        force: payload && payload.force === true,
      });
      const runtime = await wallpaperEngineRuntime.probe(
      ...
      return {
        ...
        projects: [],
        count: 0,
        error: error.message || "WALLPAPER_ENGINE_SCAN_FAILED",
      };
    }
  },
);
...
ipcMain.handle(
  ...
  async (event, id) => {
    try {
      if (!isTrustedWallpaperEngineIpc(event))
        return { ok: false, error: "WALLPAPER_ENGINE_UNTRUSTED_CALLER" };
      return await wallpaperEngineLibrary.getProjectDetails(String(id || ""));
    } catch (error) {
      return {
        ok: false,
        error: error.message || "WALLPAPER_ENGINE_PROJECT_DETAILS_FAILED",
      };
    }
  },
);
...
ipcMain.handle(
  ...
  async (event, payload = {}) => {
    try {
      if (!isTrustedWallpaperEngineIpc(event))
        return { ok: false, error: "WALLPAPER_ENGINE_UNTRUSTED_CALLER" };
      const details = await wallpaperEngineLibrary.getProjectDetails(
      ...
      const target =
        payload && payload.target === "workshop" ? "workshop" : "we";
      let revealError = "";
      if (target === "we") {
        try {
          ...
          revealError =
            ...
            "WALLPAPER_ENGINE_REVEAL_FAILED";
        }
      }
      ...
      return {
        ok: false,
        error: error.message || "WALLPAPER_ENGINE_OPEN_PROJECT_DETAILS_FAILED",
      };
    }
  },
);
...
ipcMain.handle("mineradio-wallpaper-engine-choose-directory", async (event) => {
  try {
    ...
    if (result.canceled || !result.filePaths || !result.filePaths[0])
      return { ok: true, canceled: true };
    const snapshot = await wallpaperEngineLibrary.addManualRoot(
      result.filePaths[0],
    );
    ...
    return {
      ...
      projects: [],
      count: 0,
      error: error.message || "WALLPAPER_ENGINE_IMPORT_FAILED",
    };
  }
});
...
ipcMain.handle(
  ...
  async (event) => {
    try {
      ...
      const snapshot =
        await wallpaperEngineLibrary.addManualProjectFile(selected);
      const runtime = await wallpaperEngineRuntime.probe(false);
      return { ...snapshot, runtime, canceled: false };
    } catch (error) {
      return {
        ...
        projects: [],
        count: 0,
        error: error.message || "WALLPAPER_ENGINE_IMPORT_PROJECT_FAILED",
      };
    }
  },
);
  ...
  async (event, rootId) => {
    try {
      ...
      return {
        ...
        projects: [],
        count: 0,
        error: error.message || "WALLPAPER_ENGINE_REMOVE_ROOT_FAILED",
      };
    }
  },
);
...
ipcMain.handle(
  ...
  async (event, payload = {}) => {
    try {
      ...
      const probe = await wallpaperEngineRuntime.probe(
        payload && payload.force === true,
      );
      return {
        ...probe,
        ...wallpaperEngineRuntime.getStatus(),
        pending: wallpaperEngineRuntime.pending != null,
      };
    } catch (error) {
      return {
        ok: false,
        available: false,
        error: error.message || "WALLPAPER_ENGINE_RUNTIME_PROBE_FAILED",
      };
    }
  },
);
...
ipcMain.handle(
  ...
  async (event, payload = {}) => {
    ...
    try {
      ...
      try {
        hostElevated = await systemMemory.probeProcessElevation();
      } catch (_) {}
      if (operation !== wallpaperEngineCaptureOperation)
        return { ok: false, error: "WALLPAPER_ENGINE_START_SUPERSEDED" };
      if (hostElevated)
        return { ok: false, error: "WALLPAPER_ENGINE_HOST_ELEVATED" };
      const desktopMode = fullDesktopModeRuntime.getStatus(
        "wallpaper-engine-start-scene",
      );
      if (
        wallpaperEngineHostVisibilitySuspended ||
        (desktopMode.enabled === true &&
          (desktopMode.interactive !== true ||
            desktopMode.phase !== "interactive"))
      ) {
        return { ok: false, error: "WALLPAPER_ENGINE_HOST_SUSPENDED" };
      }
      const physicalBounds = wallpaperEnginePhysicalContentBounds(
        mainWindow,
        payload,
      );
      ...
      const result = await wallpaperEngineRuntime.start(
        String(payload.id || ""),
        {
      ...
      if (operation !== wallpaperEngineCaptureOperation) {
        ...
        return {
          ok: false,
          error: "WALLPAPER_ENGINE_START_SUPERSEDED",
          sessionId: startedSessionId,
        };
      }
      let embedded;
      try {
        embedded = await wallpaperEngineRuntime.embedActiveWindow(
          startedSessionId,
          {
            hostWindowId: nativeWindowHandleDecimal(mainWindow),
            hostExecutable: process.execPath,
        ...
        return {
          ...
          error:
            (embeddingError &&
              (embeddingError.code || embeddingError.message)) ||
      ...
      const grant = createWallpaperEngineCaptureGrant(
        { ...result, ...embedded },
        operation,
      );
      if (!grant) {
        ...
        return {
          ok: false,
          error: "WALLPAPER_ENGINE_CAPTURE_UNAVAILABLE",
          sessionId: startedSessionId,
        };
      }
      const embeddedDesktop = fullDesktopModeRuntime.getStatus(
        "wallpaper-engine-embed-finished",
      );
      if (
        mainWindow &&
        !mainWindow.isDestroyed() &&
        embeddedDesktop.enabled !== true
      ) {
        try {
          mainWindow.moveTop();
        } catch (_) {}
        ...
        embeddedDesktop.enabled === true &&
        embeddedDesktop.interactive === true
      ) {
        fullDesktopModeRuntime.ensureIconLayerOrder().catch((error) => {
          console.warn(
            "[FullDesktopMode] WE coexistence z-order refresh failed:",
            (error && error.message) || error,
          );
        });
      }
      ...
      if (startedSessionId) {
        ...
        wallpaperEngineCaptureGrant &&
        wallpaperEngineCaptureGrant.operation === operation
      ) {
        clearWallpaperEngineCaptureGrant();
      }
      return {
        ok: false,
        error:
          error.code || error.message || "WALLPAPER_ENGINE_SCENE_START_FAILED",
        sessionId: startedSessionId,
      };
    }
  },
...
ipcMain.handle(
  ...
  async (event, payload = {}) => {
    ...
    if (!/^[a-f0-9]{24}$/i.test(sessionId))
      return { ok: false, error: "WALLPAPER_ENGINE_SESSION_INVALID" };
    ...
    if (
      ...
      payload &&
      payload.ok === true &&
      typeof wallpaperEngineRuntime.confirmCaptureReady === "function"
    ) {
      confirmed = await wallpaperEngineRuntime
        .confirmCaptureReady(sessionId)
        .catch(() => false);
    }
    ...
    if (matched && confirmed) {
      syncWallpaperEngineDesktopIconLayering(
        "wallpaper-engine-capture-ready",
      ).catch(() => {});
    }
    return {
  ...
  async (event, payload = {}) => {
    ...
    try {
      ...
      if (
        ...
        status.captureMode !== "dwm-thumbnail" ||
        status.dwmGlassSurfaceReady !== true ||
        status.dwmGlassSurfaceActive !== true
      ) {
        return {
          ok: false,
      ...
      const source = await wallpaperEngineRuntime.getDwmGlassCaptureSource(
        sessionId,
        {
          timeoutMs: 1800,
          pollIntervalMs: 60,
      ...
      if (
        captureOperation !== wallpaperEngineCaptureOperation ||
      ...
      if (!grant)
        return { ok: false, error: "WALLPAPER_GLASS_CAPTURE_SOURCE_INVALID" };
      const prepared = await prepareWallpaperEngineRendererGlassCapture(
        sessionId,
        payload && payload.fps,
        source.id,
      );
      ...
      return {
        ok: false,
        error: String(
          (error && (error.code || error.message || error.name)) ||
            error ||
            "WALLPAPER_GLASS_CAPTURE_PREPARE_FAILED",
        ).slice(0, 500),
      };
  ...
  async (event, payload = {}) => {
    ...
    try {
      ...
      return {
        ...
        captureMode: "dwm-thumbnail",
        error:
          result && result.dwmSurfaceActive === true
            ? ""
...
ipcMain.on(
  ...
  (event, payload = {}) => {
    if (
      !isTrustedWallpaperEngineIpc(event) ||
      typeof wallpaperEngineRuntime.updateGlassSurface !== "function"
    )
      return;
    const sessionId = String((payload && payload.sessionId) || "");
    ...
    if (
      ...
      return;
    try {
      wallpaperEngineRuntime.updateGlassSurface(sessionId, payload);
    } catch (_) {}
  },
);
...
ipcMain.on(
  ...
  (event, payload = {}) => {
    ...
    if (
      ...
      typeof wallpaperEngineRuntime.noteHostPointerActivity !== "function"
    )
      return;
    try {
  ...
  async (event, payload = {}) => {
    try {
      ...
      if (
        ...
        (!current.active ||
          (wallpaperEngineCaptureGrant &&
            wallpaperEngineCaptureGrant.sessionId === sessionId))
      ) {
        clearWallpaperEngineCaptureGrant(sessionId);
      }
      return result;
      ...
      return {
        ok: false,
        error:
          error.code || error.message || "WALLPAPER_ENGINE_SCENE_STOP_FAILED",
      };
    }
  },
);
...
ipcMain.handle("mineradio-cache-read-lyric", async (_event, key) => {
  try {
    ...
    const stat = await fs.promises.stat(file);
    if (!stat || stat.size <= 0 || stat.size > LYRIC_CACHE_ENTRY_MAX_BYTES)
      return { ok: true, hit: false };
    const record = JSON.parse(await fs.promises.readFile(file, "utf8"));
    if (
      !record ||
      record.version !== LYRIC_CACHE_VERSION ||
      !record.payload ||
    ...
    return {
      ok: false,
      hit: false,
      error: error.message || "LYRIC_CACHE_READ_FAILED",
    };
  }
});
...
ipcMain.handle("mineradio-cache-write-lyric", async (_event, key, payload) => {
  try {
    ...
    if (Buffer.byteLength(text, "utf8") > LYRIC_CACHE_ENTRY_MAX_BYTES)
...
ipcMain.handle("desktop-window-set-close-behavior", (_event, behavior) => {
  ...
  if (closeBehavior === "tray") createOrUpdateTray();
    ...
    fullDesktopModeRuntime.getStatus("close-behavior-changed").enabled !== true
  ) {
    releaseFullDesktopModeRecoveryTray();
  }
...
function loginCookieExportMeta(provider) {
  ...
  const entries = {
    netease: {
      label: "网易云音乐",
      files: [process.env.COOKIE_FILE, path.join(userData, ".cookie")],
    },
    qq: {
      label: "QQ音乐",
      files: [process.env.QQ_COOKIE_FILE, path.join(userData, ".qq-cookie")],
    },
    kugou: {
      label: "酷狗音乐",
      files: [
        process.env.KUGOU_COOKIE_FILE,
        path.join(userData, ".kugou-cookie"),
      ],
    },
    qishui: {
      label: "汽水音乐",
      files: [
        process.env.QISHUI_COOKIE_FILE,
        path.join(userData, ".qishui-cookie"),
        process.env.QISHUI_TOKEN_FILE,
        path.join(userData, ".qishui-token"),
      ],
    },
    spotify: {
      label: "Spotify",
      files: [
        ...
        path.join(userData, ".spotify-token.json"),
      ],
    },
  };
  ...
  try {
    ...
    if (!source)
      return {
        ...
        message: `${meta.label} 当前没有可导出的登录 cookie`,
      };
    const text = fs.readFileSync(source, "utf8");
    const safeName = String(`${meta.label}_登录cookie.txt`).replace(
      /[\\/:*?"<>|]+/g,
      "-",
    );
...
ipcMain.handle("mineradio-export-json-file", async (event, payload = {}) => {
  try {
    ...
    const defaultName = String(
      payload.defaultName || "mineradio-export.json",
    ).replace(/[\\/:*?"<>|]+/g, "-");
    const result = await dialog.showSaveDialog(owner, {
      title: "导出 Mineradio 存档",
      defaultPath: defaultName.toLowerCase().endsWith(".json")
        ? defaultName
        : `${defaultName}.json`,
...
ipcMain.handle("mineradio-login-easter-egg-status", async (event) => {
  if (!isTrustedMainWindowIpc(event))
    return { ok: false, error: "UNTRUSTED_SENDER", unlocked: false };
...
ipcMain.handle("mineradio-login-easter-egg-reset", async (event) => {
  if (!isTrustedMainWindowIpc(event))
    return { ok: false, error: "UNTRUSTED_SENDER", unlocked: false };
  return loginEasterEggGate.resetForReplay(() =>
    clearAllProviderLoginState("renderer-replay-reset"),
  );
});
...
ipcMain.handle(
  ...
  async (_event, bounds) => {
    try {
      ...
      return {
        ok: false,
        error: e.message || "DESKTOP_LYRICS_HOT_BOUNDS_FAILED",
      };
    }
  },
);
...
ipcMain.handle(
  ...
  async (event, enabled, payload) => {
    try {
      if (!isTrustedMainWindowIpc(event))
        return {
          ok: false,
          enabled: false,
          error: "WALLPAPER_UNTRUSTED_SENDER",
        };
      ...
      return {
        ok: false,
        enabled: false,
        error: e.message || "WALLPAPER_FAILED",
        status: fullDesktopModeRuntime.getStatus("ipc-failed"),
      };
    }
  },
);
...
ipcMain.handle("mineradio-wallpaper-update", async (event) => {
  if (!isTrustedMainWindowIpc(event))
    return { ok: false, enabled: false, error: "WALLPAPER_UNTRUSTED_SENDER" };
  const status = {
...
function configureLocalServerEnvironment(port) {
  ...
  process.env.KUGOU_COOKIE_FILE = path.join(
    STABLE_USER_DATA_PATH,
    ".kugou-cookie",
  );
  process.env.QISHUI_COOKIE_FILE = path.join(
    STABLE_USER_DATA_PATH,
    ".qishui-cookie",
  );
  process.env.QISHUI_TOKEN_FILE = path.join(
    STABLE_USER_DATA_PATH,
    ".qishui-token",
  );
  process.env.MINERADIO_LISTEN_SYNC_FILE = path.join(
    STABLE_USER_DATA_PATH,
    "listen-sync-journal.json",
  );
  process.env.MINERADIO_LOGIN_EASTER_EGG_GATE_FILE = path.join(
    STABLE_USER_DATA_PATH,
    LOGIN_EASTER_EGG_STATE_FILE,
  );
  ...
  if (!process.env.QISHUI_OAUTH_CONFIG_FILE) {
    process.env.QISHUI_OAUTH_CONFIG_FILE = path.join(
      STABLE_USER_DATA_PATH,
      ".qishui-oauth.json",
    );
  }
  process.env.SPOTIFY_TOKEN_FILE = path.join(
    STABLE_USER_DATA_PATH,
    ".spotify-token.json",
  );
  if (
    ...
    process.env.SPOTIFY_CONFIG_FILE = path.join(
      STABLE_USER_DATA_PATH,
...
function migrateMisplacedAppOwnedFiles() {
  ...
  const addSource = (value) => {
    ...
    if (
      resolved === path.resolve(STABLE_USER_DATA_PATH) ||
      sources.includes(resolved)
    )
      return;
    sources.push(resolved);
  };
  try {
    addSource(app.getPath("sessionData"));
  } catch (_) {}
...
function removeDeprecatedKugouVipEvidenceFiles() {
  ...
  const candidates = [
    {
      label: "stable-user-data",
      file: path.join(STABLE_USER_DATA_PATH, fileName),
    },
    {
      label: "legacy-resource-dir",
  ...
  for (const candidate of candidates) {
    try {
      ...
      console.warn(
        "[UserDataMigration] deprecated Kugou VIP evidence cleanup skipped",
        candidate.label,
        error.message,
      );
    }
  ...
  if (removed.length) {
    console.log(
      "[UserDataMigration] removed deprecated Kugou VIP evidence",
      removed.join(","),
    );
  }
}
...
function migrateLegacyAuthStorage() {
  ...
  try {
    ...
    for (const legacyQishuiOAuth of legacyQishuiOAuthFiles) {
      if (
        qishuiOAuthTarget &&
        fs.existsSync(legacyQishuiOAuth) &&
        !fs.existsSync(qishuiOAuthTarget)
      ) {
  ...
  try {
    const legacySpotifyToken = path.join(
      __dirname,
      "..",
      ".spotify-token.json",
    );
  ...
  try {
    ...
    for (const legacySpotifyConfig of legacySpotifyConfigFiles) {
      if (
        spotifyConfigTarget &&
        fs.existsSync(legacySpotifyConfig) &&
        !fs.existsSync(spotifyConfigTarget)
      ) {
...
async function ensureLocalServerStarted() {
  ...
  localServerStartPromise = (async () => {
    const injectedDelay = Math.max(
      0,
      Math.min(
        15000,
        Number(process.env.MINERADIO_STARTUP_TEST_SERVER_DELAY_MS) || 0,
      ),
    );
...
function showMainWindowSafely(win, reason) {
  ...
  if (!startupState.windowVisibleAt) {
    writeStartupState("window-visible", {
      windowVisibleAt: Date.now(),
      visibleReason: String(reason || ""),
    });
  }
...
async function loadMainWindowWithRetry(win) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    if (!win || win.isDestroyed())
      throw new Error("Main BrowserWindow was destroyed before navigation");
    const targetUrl = `${baseUrl}/?startupAttempt=${attempt}&startupAt=${Date.now()}`;
    try {
      writeStartupState("navigation-attempt", {
      ...
      if (
        attempt === 1 &&
        process.env.MINERADIO_STARTUP_TEST_FAIL_FIRST_NAV === "1"
      ) {
        const injected = new Error(
          "Injected first navigation failure for startup QA",
        );
      ...
      console.warn(
        `[StartupWindow] navigation attempt ${attempt} failed:`,
        error.message || error,
      );
      try {
        win.webContents.stop();
  ...
  const error = new Error(
    `loadURL failed after retry: ${startupErrorText(lastError)}`,
  );
...
async function createWindowOnce() {
  ...
  const initialMinimum = getAdaptiveWindowMinimumSize(
    screen.getPrimaryDisplay(),
  );
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    stopWallpaperEngineRuntimeForRenderer(
      `render-process-gone:${(details && details.reason) || "unknown"}`,
    );
    closeWallpaperWindow(
      `main-renderer-gone:${(details && details.reason) || "unknown"}`,
    ).catch(() => {});
    const error = new Error(
  ...
  win.on("minimize", () => {
    ...
    if (fullDesktopModeHostVisibilityTransitionDepth <= 0)
      suspendWallpaperEngineForHiddenHost(win, "minimize");
  ...
  win.on("close", (event) => {
    ...
    if (desktopMode.enabled === true) {
      ...
      disableFullDesktopMode("main-window-close")
        .then((result) => {
          if (result && result.ok === true) {
          ...
          console.warn(
            ...
            (result &&
              (result.error || (result.status && result.status.lastError))) ||
              "unknown",
          );
          if (!win.isDestroyed()) {
            if (!win.isVisible()) win.show();
        ...
        .catch((error) => {
          win.__mineradioDesktopModeCloseArmed = false;
          console.warn(
            "[FullDesktopMode] close detach failed; keeping main window open:",
            (error && error.message) || error,
          );
          if (!win.isDestroyed()) {
            if (!win.isVisible()) win.show();
  ...
  win.on("enter-full-screen", () => {
    ...
    setTimeout(
      () => scheduleWallpaperEngineHostBoundsRestart(win, "enter-full-screen"),
      40,
    );
  });
  ...
  win.on("enter-html-full-screen", () => {
    ...
    setTimeout(
      () =>
        scheduleWallpaperEngineHostBoundsRestart(win, "enter-html-full-screen"),
      40,
    );
  });
  ...
  if (fs.existsSync(startupShell)) {
    win.loadFile(startupShell).catch((error) => {
      if (
        !/ERR_ABORTED|ERR_FAILED/i.test(
          String((error && error.message) || error),
        )
      ) {
        console.warn(
          "[StartupWindow] startup shell skipped:",
          error.message || error,
        );
      }
    });
  ...
  const qaExitMs = Math.max(
    0,
    Math.min(10000, Number(process.env.MINERADIO_STARTUP_QA_EXIT_MS) || 0),
  );
  if (qaExitMs) {
    setTimeout(() => {
      appQuitting = true;
...
function createWindow() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    showMainWindowSafely(
      mainWindow,
      startupCompleted ? "reuse" : "startup-in-progress",
    );
...
if (!gotSingleInstanceLock) {
  ...
  app
    ...
    .then(async () => {
      try {
        ...
        console.warn(
          "[Wallpaper Engine] local media protocol unavailable:",
          (error && error.message) || error,
        );
      }
      const handleDisplayLayoutChanged = (_event, _display, changedMetrics) => {
        ...
        if (
          fullDesktopModeRuntime.getStatus("display-layout-clamp").enabled !==
          true
        ) {
  ...
  app.on("before-quit", (event) => {
    ...
    const forceDestroyQuitMainWindow = (reason, detail) => {
      console.error(
        `[FullDesktopMode] ${reason}; destroying the exact main window as the HWND cleanup fallback.`,
        detail || "",
      );
      if (!quitMainWindow || quitMainWindow.isDestroyed()) {
        console.warn(
          "[FullDesktopMode] main window HWND fallback was already unavailable.",
        );
        return;
      }
      try {
        ...
        console.warn(
          "[FullDesktopMode] main window destroyed after incomplete desktop-mode cleanup.",
        );
      } catch (destroyError) {
        console.error(
          "[FullDesktopMode] main window HWND fallback destroy failed:",
          (destroyError && destroyError.message) || destroyError,
        );
      }
    };
    const disposeFullDesktopModeWithGuard = async () => {
      ...
      if (!result || result.ok !== true) {
        const detail =
          ...
          "unknown";
        forceDestroyQuitMainWindow(
          timedOut ? "dispose timed out after 7000ms" : "dispose incomplete",
          detail,
        );
      }
    };
    ...
    const fullDesktopAndWallpaperEngineCleanup = (async () => {
      ...
      await wallpaperEngineRuntime
        .dispose()
        .then((result) => {
          if (result && result.ok === false) {
            console.warn(
              "[Wallpaper Engine] dispose incomplete:",
              result.reason || "WALLPAPER_ENGINE_WINDOW_CLOSE_FAILED",
            );
          }
        })
        .catch((error) => {
          console.warn(
            "[Wallpaper Engine] dispose failed:",
            (error && error.message) || error,
          );
        });
    })();
    ...
    const timeoutCleanup = new Promise((resolve) => {
      cleanupTimeout = setTimeout(() => {
        console.warn(
          "[Shutdown] runtime cleanup exceeded 15000ms; continuing bounded application exit.",
        );
        resolve();
      }, 15000);
    });
    appQuitCleanupPromise = Promise.race([
      runtimeCleanup,
      timeoutCleanup,
    ]).finally(() => {
