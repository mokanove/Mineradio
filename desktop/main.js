...
function getWindowedBounds(win) {
  ...
  if (
    ...
    maxWidth >= MIN_WINDOWED_WIDTH &&
    maxHeight >= MIN_WINDOWED_HEIGHT
  ) {
    width = MIN_WINDOWED_WIDTH;
...
`;
  execFile(
    ...
    (error) => {
      if (error)
        console.warn("Wallpaper WorkerW attach failed:", error.message);
    },
  );
}
...
function createWallpaperWindow(payload = {}) {
  ...
  wallpaperWindow
    .loadURL(overlayUrl("wallpaper.html"))
  ...
  app.on("second-instance", () => {
    if (!focusMainWindow()) {
      app
        ...
        .then(() => createWindow())
        .catch((e) =>
          console.error("Second instance window restore failed:", e),
        );
    }
  });
