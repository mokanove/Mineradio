...
contextBridge.exposeInMainWorld("desktopOverlay", {
  ...
  setLyricsDrag: (dragging) =>
    ipcRenderer.invoke("mineradio-desktop-lyrics-set-dragging", !!dragging),
  ...
  setLyricsHotBounds: (bounds) =>
    ipcRenderer.invoke("mineradio-desktop-lyrics-set-hot-bounds", bounds || {}),
  setLyricsLockState: (locked) =>
    ipcRenderer.invoke("mineradio-desktop-lyrics-set-lock-state", !!locked),
  moveLyricsBy: (dx, dy) =>
    ipcRenderer.invoke(
  ...
  closeLyrics: () =>
    ipcRenderer.invoke("mineradio-desktop-lyrics-set-enabled", false, {}),
});
