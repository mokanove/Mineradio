...
function scoreEntryPromise(entry) {
  ...
  if (
    entry.resolvesTo &&
    (entry.resolvesTo.type === "chorus" || entry.resolvesTo.type === "hook")
  )
    score += 0.12;
...
function scorePairCompatibility(exit, entry) {
  ...
  const lowDiff =
    Number.isFinite(exitLow) && Number.isFinite(entryLow)
      ? Math.abs(exitLow - entryLow)
      : 0.18;
  const energyDiff =
    Number.isFinite(exitEnergy) && Number.isFinite(entryEnergy)
      ? Math.abs(exitEnergy - entryEnergy)
      : lowDiff;
...
function scoreLyricHandoff(exit, entry) {
  ...
  const shared = Array.from(exitContent).filter((token) =>
    entryContent.has(token),
  );
  let score = shared.length ? Math.min(0.78, 0.48 + shared.length * 0.16) : 0;

  const directionalExit = includesAny(exitText, [
    "break through",
    "through",
    "help me",
    "rise",
  ...
  const pickupEntry = includesAny(entryText, [
    "take me to",
    "take me",
    "to the",
    "into",
...
function profileForTitle(title) {
  ...
  const profiles = [
    {
      patterns: ["avicii"],
      families: ["electronic", "dance"],
      moods: ["uplifting", "anthemic"],
    },
    {
      patterns: ["bingo players"],
      families: ["electronic", "dance"],
      moods: ["uplifting"],
    },
    {
      patterns: ["odesza"],
      families: ["electronic", "cinematic"],
      moods: ["uplifting", "warm"],
    },
    {
      patterns: ["apashe"],
      families: ["electronic", "cinematic"],
      moods: ["dark", "theatrical"],
    },
    {
      patterns: ["acdc", "highway to hell"],
      families: ["rock"],
      moods: ["raw", "anthemic"],
    },
    { patterns: ["nirvana"], families: ["rock"], moods: ["grunge", "dark"] },
    {
      patterns: ["marilyn manson", "sweet dreams"],
      families: ["rock"],
      moods: ["gothic", "dark"],
    },
    {
      patterns: ["glass animals", "heat waves"],
      families: ["alt-pop", "electronic"],
      moods: ["warm", "melodic"],
    },
    {
      patterns: ["flume", "never be like you"],
      families: ["electronic", "alt-pop"],
      moods: ["warm", "future"],
    },
  ...
  return (
    profiles.find((profile) =>
      profile.patterns.some((pattern) => text.includes(pattern)),
    ) || null
  );
}
...
function inferStyleCompatibility(fromAnalysis, toAnalysis) {
  ...
  const upliftingIntoDark =
    hasTag(from, "uplifting") && (hasTag(to, "dark") || hasTag(to, "gothic"));
  const rawRockJump =
    hasTag(from, "electronic") && hasTag(to, "rock") && !familyOverlap;
...
function classifyTier(score, recipe, dimensions, risks) {
  ...
  if ((risks || []).includes("style bridge mismatch"))
    return "usable_but_not_magic";
  const magicRecipe =
    recipe === "lyric-handoff" || recipe === "instrumental-outro-to-vocal-hook";
  if (
    magicRecipe &&
...
function evaluateTransitionPair(opts = {}) {
  ...
  if (styleCompatibility != null)
    dimensions.styleCompatibility = styleCompatibility;
  const ctx = {
    entry,
    ...
    exitLateEnough:
      !fromDuration || (exit && toNumber(exit.time) / fromDuration >= 0.89),
  };
  const recipe = recipeFor(ctx);
  const reasons = [];
  const risks = [];
  if (dimensions.lyricHandoff >= 0.75) reasons.push("lyric handoff");
  if (recipe === "instrumental-outro-to-vocal-hook")
    reasons.push("instrumental outro to vocal hook");
  if (dimensions.pairCompatibility >= 0.8)
    reasons.push("stable energy handoff");
  if (isClosedOutgoingPhrase(textOf(exit)))
    risks.push("closed outgoing phrase");
  if (dimensions.directionality < 0.5) risks.push("directionality mismatch");
  if (styleCompatibility != null && styleCompatibility < 0.62)
    risks.push("style bridge mismatch");

  const recipeBonus =
    recipe === "lyric-handoff"
      ? 0.1
      : recipe === "instrumental-outro-to-vocal-hook"
        ? 0.18
        : 0;
  const score = round(
    clamp01(
