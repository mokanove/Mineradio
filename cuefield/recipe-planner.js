...
function densityAt(windows, time) {
  ...
  const found = list.find(
    (window) => time >= toNumber(window.start) && time < toNumber(window.end),
  );
  ...
  const nearest = list
    ...
    .sort(
      (a, b) =>
        Math.abs(toNumber(a.start) - time) - Math.abs(toNumber(b.start) - time),
    )[0];
...
function barsInRange(profile, start, end) {
  ...
  return bars.filter(
    (bar) => toNumber(bar.end) > start && toNumber(bar.start) < end,
  );
}
...
function nearestCandidate(profile, role, time) {
  return (
    (profile.candidates || [])
      ...
      .sort(
        (a, b) =>
          Math.abs(toNumber(a.time) - time) - Math.abs(toNumber(b.time) - time),
      )[0] || null
  );
}
...
function nearestDownbeat(profile, time) {
  const downbeats =
    profile && Array.isArray(profile.downbeats) ? profile.downbeats : [];
  const step = Math.max(0.001, toNumber(profile && profile.gridStep, 0.5));
  const nearest = downbeats
    .slice()
    .sort(
      (a, b) =>
        Math.abs(toNumber(a.time) - time) - Math.abs(toNumber(b.time) - time),
...
function pickAnchors(fromProfile, toProfile, opts = {}) {
  ...
  const hook = firstCandidate(
    toProfile,
    (candidate) =>
      candidate.role === "entry" &&
      candidate.time <= maxEntryTime &&
      (candidate.type === "hook" || candidate.type === "chorus"),
  );
  const intro = firstCandidate(
    toProfile,
    (candidate) =>
      candidate.role === "entry" &&
      candidate.time <= Math.min(12, maxEntryTime) &&
      candidate.type === "intro",
  );
  const rawExit = round(
    toNumber(
      sectionChoice.exit && sectionChoice.exit.time,
      toNumber(fromCue.outroStart, Math.max(0, fromProfile.duration - 12)),
    ),
  );
  const rawEntry = toNumber(
    sectionChoice.entry && sectionChoice.entry.time,
    NaN,
  );
  const boundedEntry =
    Number.isFinite(rawEntry) && rawEntry <= maxEntryTime
      ? rawEntry
      : toNumber(
          hook && hook.time,
          toNumber(toCue.firstStrongDownbeat, toNumber(intro && intro.time, 0)),
        );
  const aSnap = nearestDownbeat(fromProfile, rawExit);
  const bSnap = nearestDownbeat(
    toProfile,
    Math.max(0, Math.min(maxEntryTime, boundedEntry)),
  );
  const aExit =
    aSnap.residual <= Math.max(0.2, toNumber(fromProfile.gridStep, 0.5))
      ? aSnap.time
      : rawExit;
  const bAnchor =
    bSnap.residual <= Math.max(0.2, toNumber(toProfile.gridStep, 0.5))
      ? bSnap.time
      : Math.max(0, Math.min(maxEntryTime, boundedEntry));
...
function commonScores(fromProfile, toProfile, anchors) {
  const aEnergy = densityAt(
    fromProfile.windows && fromProfile.windows.energy,
    anchors.aExit,
  );
  const bEnergy = densityAt(
    toProfile.windows && toProfile.windows.energy,
    anchors.bAnchor,
  );
  const aBass = densityAt(
    fromProfile.windows && fromProfile.windows.bass,
    anchors.aExit,
  );
  const bBass = densityAt(
    toProfile.windows && toProfile.windows.bass,
    anchors.bStart,
  );
  ...
  const outroTexture = textureAt(
    fromProfile,
    anchors.aExit,
    toNumber(fromProfile.duration, anchors.aExit) + 0.001,
  );
  const introTexture = textureAt(
    toProfile,
    anchors.bStart,
    anchors.bStart + 12,
  );
  const exitCandidate = nearestCandidate(fromProfile, "exit", anchors.aExit);
  const outroLengthScore = clamp(
    (toNumber(fromProfile.duration) - anchors.aExit) / 16,
  );
  const outroConfidence = clamp(
    exitCandidate && exitCandidate.confidence,
    0,
    1,
  );
  const outroCompleteness = clamp(
    outroLengthScore * 0.45 +
      outroConfidence * 0.35 +
      (1 - outroTexture.energy) * 0.2,
  );
  const bIntroAggression = clamp(
    introTexture.energy * 0.55 +
      introTexture.low * 0.25 +
      introTexture.snap * 0.2,
  );
  const styleTextureDistance = clamp(
    average([
  ...
  const aVocal = densityAt(
    fromProfile.windows && fromProfile.windows.vocal,
    Math.max(0, anchors.aExit - 2),
  );
  const bVocal = densityAt(
    toProfile.windows && toProfile.windows.vocal,
    Math.max(0, anchors.bAnchor - 2),
  );
...
function makeLongBlend(anchors, scores) {
  ...
  const score =
    0.28 +
    scores.beatScore * 0.28 +
    scores.energyScore * 0.18 +
    scores.bassScore * 0.18 +
...
function makeFilteredPickup(anchors, scores) {
  ...
  const score =
    0.34 +
    scores.beatScore * 0.3 +
    scores.energyScore * 0.18 +
    scores.bassScore * 0.12 +
    scores.bpmScore * 0.06;
  const risks = [];
...
function makeBassHandoff(anchors, scores) {
  ...
  const score =
    0.3 +
    scores.beatScore * 0.28 +
    scores.bassScore * 0.24 +
    scores.energyScore * 0.12 +
    scores.bpmScore * 0.06;
  const risks = [];
  if (scores.aBass > 0.65 && scores.bBass > 0.55)
    risks.push("requires bass swap");
  return baseCandidate(
    "bass-eq-handoff",
    score,
...
function makeQuickFade(anchors, scores) {
  ...
  const score =
    0.24 +
    scores.beatScore * 0.32 +
    scores.energyScore * 0.16 +
    scores.bpmScore * 0.1;
  return baseCandidate(
    "quick-safe-fade",
    score,
...
function makeSafetyLongBlend(anchors, scores, sectionChoice = {}) {
  ...
  const tier =
    (sectionChoice.evaluation && sectionChoice.evaluation.tier) || "";
...
function makeAnchorAlignedBeatmix(
  fromProfile,
  toProfile,
  anchors,
  scores,
  ...
  const requestedFade = Math.max(
    1.8,
    toNumber(fromProfile.gridStep, 0.5) * 4 * (twoBars ? 2 : 1),
  );
  ...
  return {
    ...baseCandidate(
      ...
      scores.mixConfidence,
      [
        "A/B anchors are scheduled on their own downbeats",
        twoBars ? "two-bar high-confidence blend" : "one-bar tempo-safe blend",
      ],
      risks,
      {
        ...anchors,
        bStart: preparedStart,
        bFadeStart: bStart,
        anchorLead,
        warmupSec: actualWarmup,
        lead: anchorLead + actualWarmup,
      },
      [
        {
          t: -anchorLead - actualWarmup,
          deck: "B",
          op: "play",
          at: preparedStart,
          volume: 0,
        },
        {
          ...
          op: "crossfade",
          value: 1,
          duration: Math.round(fadeSec * 1000),
        },
...
function makeSimpleCrossfade(fromProfile, toProfile, scores) {
  ...
  const fadeSec = round(
    Math.max(
      ...
      Math.min(
        2.4,
        1.65 + (1 - energyGap) * 0.45 - scores.vocalOverlapRisk * 0.4,
      ),
    ),
  );
  const exitTime = round(
    Math.max(fadeSec + 0.2, toNumber(fromProfile.duration, fadeSec + 0.2)),
  );
  const warmupSec = 0.7;
  return {
    ...baseCandidate(
      ...
      0.9,
      [
        "low-confidence pairs keep the natural tail",
        "short fade avoids forced beat or phrase matching",
      ],
      ["analysis fallback"],
      {
        aExit: exitTime,
...
function planRecipeCandidates(fromProfile, toProfile, opts = {}) {
  ...
  const sectionRisks =
    (opts.sectionChoice &&
      opts.sectionChoice.evaluation &&
      opts.sectionChoice.evaluation.risks) ||
    [];
  const hardRisk =
    sectionRisks.includes("closed outgoing phrase") ||
    sectionRisks.includes("near closed outgoing phrase");
  const anchorCandidate = makeAnchorAlignedBeatmix(
    fromProfile || {},
    toProfile || {},
    anchors,
    scores,
  ...
  const simpleCandidate = makeSimpleCrossfade(
    fromProfile || {},
    toProfile || {},
    scores,
  );
  const exitTail = Math.max(
    0,
    toNumber(fromProfile && fromProfile.duration) - anchors.aExit,
  );
