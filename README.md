...
| 下载入口 | 推荐人群 | 链接 |
| --- | --- | --- |
| 蓝奏云满速下载 | 国内用户优先 | [下载 Mineradio 2.0 安装包](https://xxhuber.lanzout.com/s/Mineradio) |
| GitHub Release | GitHub 用户 | [Mineradio 2.0 Release](https://github.com/XxHuberrr/Mineradio/releases/tag/v2.0.0) |

安装时只需要下载并运行 `Mineradio-2.0.0-Setup.exe`。不要把 `.blockmap`、`latest.yml` 或 `win-unpacked` 当成正式安装包。

## 下载或安装被拦截怎么办

小众 Electron 桌面软件、未签名安装包有时会被浏览器、Windows Defender 或 SmartScreen 提示风险。请先确认安装包来自上面的蓝奏云或 GitHub Release 官方入口，文件名是 `Mineradio-2.0.0-Setup.exe`。
...
状态：Mineradio 2.0 正式版。

> 安全提示：`v1.0.10` 及更早旧安装包不再建议继续安装或传播。请使用本页提供的 `Mineradio-2.0.0-Setup.exe`。
...
正式分发以 `Mineradio-2.0.0-Setup.exe` 为准，不建议直接使用 `win-unpacked` 目录。安装包会创建桌面快捷方式。

已经安装过旧版本的用户可直接运行 `Mineradio-2.0.0-Setup.exe` 完成更新。
