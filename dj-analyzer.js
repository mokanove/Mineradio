...
function buildBeatMapFromLowEnergy(lowEnergy, hitEnergy, hopSec, durationSec) {
  ...
  let phaseSource = strong
    .filter((c) => c.time < Math.min(durationSec || nFrames * hopSec, 180))
    .slice(0, 72);
  ...
  for (let si = 0; si < sectionCount; si++) {
    ...
    const prevStep = sectionSteps.length
  ...
  for (let gridT = anchor; gridT < duration - 0.04;) {
    ...
    const lowTone = bestCand
      ? Math.max(gridLowTone * 0.62, bestCand.lowTone)
      : gridLowTone;
    const hitTone = bestCand
    ...
    const powerRel = clamp01(
      (basePower - p30 * 0.78) / Math.max(0.001, p96 - p30 * 0.78),
    );
    const lowRel = clamp01(
      (gridLow - lowFloor) / Math.max(0.0001, lowCeil - lowFloor),
    );
    const kickRel = clamp01(
      powerRel * 0.74 + lowRel * 0.22 + clamp01((hitTone - 0.26) / 1.7) * 0.04,
    );
  ...
  const pulseBeats = beats
    .filter(
      (b) => b.pulse !== false && (b.impact >= 0.16 || b.combo === "downbeat"),
    )
    .map((b) => ({
      time: b.time,
      strength: b.strength,
...
async function decodePodcastDjEnergyRange(audioUrl, opts) {
  ...
  function processDecoded(result) {
    ...
    const n = Math.min(
      result.samplesDecoded,
      left ? left.length : 0,
      right ? right.length : left ? left.length : 0,
    );
    decodedSamples += n;
  ...
  try {
    ...
    while (true) {
      ...
      if (
        limitSec &&
        effectiveSr &&
        effectiveSamples / effectiveSr >= limitSec
      ) {
        try {
          await reader.cancel();
        } catch (e) {}
...
async function analyzePodcastDjIntro(audioUrl, opts) {
  ...
  const frameLimit = Math.max(
    1,
    Math.min(
      decoded.lowEnergy.length,
      Math.ceil((introSec + 2) / Math.max(0.001, decoded.hopSec || 0.01)),
    ),
  );
...
async function analyzePodcastDjRangeSamples(audioUrl, opts) {
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    const shaped =
      i === 0 ? 0 : i === sampleCount - 1 ? 0.88 : 0.08 + pos * 0.8;
  ...
  for (let i = 0; i < sampleStarts.length; i++) {
    const targetTime = Math.max(
      0,
      Math.min(duration - sampleWindow, sampleStarts[i]),
    );
    const bytePerSec = contentLength / Math.max(1, duration);
    const prerollBytes =
      i === 0 ? 0 : Math.min(384 * 1024, Math.floor(bytePerSec * 4));
    const startByte = Math.max(
      0,
      Math.floor(targetTime * bytePerSec) - prerollBytes,
    );
    const windowBytes = Math.max(
      768 * 1024,
      Math.floor(sampleWindow * bytePerSec) + prerollBytes + 128 * 1024,
    );
  ...
  function phaseFromMap(map, baseStep) {
    ...
    if (total <= 0)
      return { phase: ((beats[0].time % step) + step) % step, step };
  ...
  sampleMaps.forEach((s) => {
    const w = Math.max(
      1,
      Math.min(16, Math.round((s.map.visualBeatCount || 0) / 16)),
    );
    for (let i = 0; i < w; i++) stepVotes.push(s.map.gridStep);
  });
  let globalStep = clampRange(
    median(stepVotes) || sampleMaps[0].map.gridStep || 0.5,
    0.32,
    0.86,
  );
  ...
  const profiles = sampleMaps
    .map((s) => {
      ...
      const impacts = beats
        .map((b) => (b.impact == null ? b.strength : b.impact))
        .filter((v) => Number.isFinite(v));
      const activeImpacts = impacts.filter((v) => v >= 0.1);
      const avgImpact = activeImpacts.length
        ? activeImpacts.reduce((a, b) => a + b, 0) / activeImpacts.length
        : 0.16;
      const hiImpact = impacts.length
        ? percentile(impacts, 0.9, 4000)
        : Math.max(0.55, avgImpact);
      const activity = beats.length / Math.max(20, s.map.duration || 20);
      const phaseInfo = phaseFromMap(s.map, globalStep);
      return {
        time: s.offset,
        avg: clampRange(
          avgImpact * clampRange(activity / 1.65, 0.38, 1.05),
          0.08,
          0.72,
        ),
  ...
  function profileAt(time) {
    ...
    const mix = clamp01(
      (time - prev.time) / Math.max(1, next.time - prev.time),
    );
    return {
      time,
  ...
  function pushRangeBeat(t, stepOverride) {
      ...
      slot === 0
        ...
        : slot === 1
          ...
          : slot === 2
            ...
            : "rebound";
    const sectionEnergy =
      clamp01((p.avg - 0.055) / 0.54) * clampRange(p.activity || 0.5, 0.3, 1.1);
    const motion =
      (Math.sin(gridIndex * 1.618 + p.avg * 9.7) * 0.5 +
        Math.sin(gridIndex * 0.317) * 0.28) *
      (0.08 + sectionEnergy * 0.17);
    const rel = clamp01(
      0.12 + sectionEnergy * 0.7 + motion + (combo === "downbeat" ? 0.06 : 0),
    );
      ...
      combo === "downbeat"
        ...
        : combo === "drop"
          ...
          : combo === "accent"
            ? 0.075 * sectionEnergy
            : 0;
    const impact = clampRange(
      0.026 + Math.pow(visualRel, 1.48) * (0.42 + p.hi * 0.34) + comboLift,
      0.02,
      0.9,
    );
    const strength = clampRange(
      0.15 + Math.pow(visualRel, 1.02) * 0.66 + comboLift * 0.68,
      0.12,
      0.93,
    );
    const cameraActive =
      impact >= 0.105 || (combo === "downbeat" && sectionEnergy >= 0.16);
    const low = clampRange(
      0.5 +
        visualRel * 0.32 +
        (combo === "downbeat" ? 0.05 * sectionEnergy : 0) -
        (combo === "accent" ? 0.12 : 0),
      0.42,
      0.9,
    );
    const body = clampRange(
      0.06 +
        visualRel * 0.15 +
        (combo === "push" ? 0.22 * sectionEnergy : 0) +
        (combo === "drop" ? 0.3 * sectionEnergy : 0),
      0.045,
      0.56,
    );
    ...
    beats.push({
      ...
      snap,
      mass: clampRange(
        low * 0.72 + Math.pow(visualRel, 1.22) * 0.24,
        0.36,
        0.94,
      ),
  ...
  for (let si = 0; si < profiles.length; si++) {
    ...
    const end =
      si === profiles.length - 1
        ? duration
        : (p.time + profiles[si + 1].time) * 0.5;
...
async function analyzePodcastDjStream(audioUrl, opts) {
  ...
  if (durationSec > 3300 && durationSec <= FULL_STREAM_QUALITY_LIMIT_SEC) {
    try {
      const map = await analyzePodcastDjStreamFull(
        audioUrl,
        Object.assign({}, opts, { preferQualityFullStream: true }),
      );
      map.debug = Object.assign({}, map.debug || {}, {
        fullStreamQuality: true,
