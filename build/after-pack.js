...
function resolveRcedit(projectDir) {
  ...
  if (localAppData) {
    var cached = findNewestRceditInCache(
  ...
  if (!hit)
    throw new Error(
      "No usable rcedit executable was found for Mineradio icon injection.",
    );
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  if (!fs.existsSync(exePath))
    throw new Error(`Mineradio executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath))
    throw new Error(`Mineradio icon was not found: ${iconPath}`);
