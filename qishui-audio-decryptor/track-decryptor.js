...
class TrackDecryptor {
  ...
  buildFlacFile(flacMetadata, decryptedSamples) {
    ...
    const metadataBody =
      flacMetadata.length > 4 ? flacMetadata.subarray(4) : flacMetadata;
  ...
  decrypt({ encryptedBuffer, spadeA, media = {} }) {
    ...
    const trak = Mp4Box.findBox(
      encryptedBuffer,
      "trak",
      moov.offset + 8,
      moov.offset + moov.size,
    );
    const mdia = Mp4Box.findBox(
      encryptedBuffer,
      "mdia",
      trak.offset + 8,
      trak.offset + trak.size,
    );
    const minf = Mp4Box.findBox(
      encryptedBuffer,
      "minf",
      mdia.offset + 8,
      mdia.offset + mdia.size,
    );
    const stbl = Mp4Box.findBox(
      encryptedBuffer,
      "stbl",
      minf.offset + 8,
      minf.offset + minf.size,
    );
    const stsd = Mp4Box.findBox(
      encryptedBuffer,
      "stsd",
      stbl.offset + 8,
      stbl.offset + stbl.size,
    );
    const stsz = Mp4Box.findBox(
      encryptedBuffer,
      "stsz",
      stbl.offset + 8,
      stbl.offset + stbl.size,
    );
    const stsc = Mp4Box.findBox(
      encryptedBuffer,
      "stsc",
      stbl.offset + 8,
      stbl.offset + stbl.size,
    );
    const stco = Mp4Box.findBox(
      encryptedBuffer,
      "stco",
      stbl.offset + 8,
      stbl.offset + stbl.size,
    ...
    if (sampleSizes.length !== ivs.length) {
      throw new Error(
        `Decrypt failed: sample count ${sampleSizes.length} does not match iv count ${ivs.length}.`,
      );
    }
