...
const QQ_COOKIE_FILE =
  process.env.QQ_COOKIE_FILE || path.join(__dirname, ".qq-cookie");
...
function normalizeQQProfile(body, cookieObj) {
  ...
  if (!vipType) {
    ...
    if (
      vipFlag === true ||
      Number(vipFlag) > 0 ||
      String(vipFlag || "").toLowerCase() === "true"
    )
      vipType = 1;
  }
  return {
...
async function handleQQUserPlaylists() {
  ...
  const [createdRaw, collectRaw] = await Promise.allSettled([
    createdReq,
    collectReq,
  ]);
  const created =
    createdRaw.status === "fulfilled" &&
    createdRaw.value &&
...
async function handleQQSongUrl(mid, mediaMid, qualityPreference) {
  ...
  if (purl) {
    const sip =
      (data.sip && data.sip[0]) || "https://ws.stream.qqmusic.qq.com/";
    const fileMeta =
...
async function getLoginInfo() {
  ...
  try {
    const st = await login_status({
      cookie: userCookie,
      timestamp: Date.now(),
    });
    const body = st.body || {};
    const data = body.data || body;
    const info = normalizeLoginInfo(
    ...
    try {
      ...
      if (typeof playlist_track_all === "function") {
        try {
          ...
          console.warn(
            "[PlaylistTracks] playlist_track_all failed, fallback to detail:",
            err.message,
          );
        }
      }
