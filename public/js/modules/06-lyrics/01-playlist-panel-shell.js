...
function smoothScrollToItem(scroller, item, opts) {
  ...
  var target =
    ...
    Math.max(
      0,
      (scroller.clientHeight - item.offsetHeight) *
        (opts.align == null ? 0.42 : opts.align),
    );
  target = Math.max(
    0,
    Math.min(
  ...
  if (window.gsap) {
    if (typeof scroller.__syncSmoothWheelTarget === "function")
      scroller.__syncSmoothWheelTarget(target);
    ...
    function (e) {
      ...
      if (
        ...
        ((delta < 0 && scroller.scrollTop <= 0) ||
          (delta > 0 && scroller.scrollTop >= max - 1))
      ) {
...
function animateVisiblePanelList(
  listEl,
  selector,
  scroller,
  activeSelector,
  ...
  requestAnimationFrame(function () {
    animateListItems(listEl, selector, {
      x: -8,
      y: 6,
      stagger: 0.01,
      duration: 0.2,
    ...
    if (active && scroller && opts.scrollActive !== false)
      smoothScrollToItem(scroller, active, { duration: 0.32 });
  });
}
...
function queueHydrationExpectedTotal() {
  ...
  if (!queueHydrationState || queueHydrationState.queueRef !== playQueue)
    return loaded;
  if (
    !queueHydrationState.active &&
...
function queueHydrationFooterHtml(compact) {
  ...
  var retry = queueHydrationState.error
    ...
    : queueHydrationState.active && !queueHydrationState.loading
      ...
      : "";
  return (
    '<div class="queue-hydration-status' +
    (compact ? " compact" : "") +
    '">' +
    '<span class="queue-hydration-spinner' +
    (queueHydrationState.loading ? " spinning" : "") +
    '"></span>' +
    "<span>" +
    label +
    "</span>" +
...
function togglePlaylistPanel(force) {
  ...
  if (el.classList.contains("show")) {
    ...
    scheduleUiWarmTask(function () {
      ...
      if (runPlaylistOpenAnimation)
        animatePlaylistPanelCurrentTab(el, { scrollActive: false });
    }, 180);
  }
}
function closePlaylistPanelSoft(reason) {
  ...
  if (!panel.classList.contains("peek") && !panel.classList.contains("show"))
    return false;
  if (peekTimers.pl) {
    clearTimeout(peekTimers.pl);
    peekTimers.pl = null;
  }
  if (typeof resetSecondaryPlaylistEdgeGuard === "function")
  ...
  setTimeout(
    ...
    playlistPanelMotionMs("close") + 80,
  );
  return true;
}
...
function setPlaylistPanelPinned(on, silent) {
  ...
  if (!silent)
    showToast(
      playlistPanelPinned ? "左侧歌单已常开" : "左侧歌单已恢复自动隐藏",
    );
}
...
function scrollPlaylistPanelToCurrent() {
  ...
  if (panel.__lastCurrentScrollAt && now - panel.__lastCurrentScrollAt < 650)
    return;
  panel.__lastCurrentScrollAt = now;
  requestAnimationFrame(function () {
    renderQueuePanel({ animate: false, scrollCurrent: true });
    smoothScrollToItem(panel, list.querySelector(".queue-item.now"), {
      duration: 0.28,
      align: 0.34,
    });
  });
...
function animatePlaylistPanelCurrentTab(panel, opts) {
  ...
  if (queueViewTab === "queue") {
    animateVisiblePanelList(
      ...
      panel,
      ".queue-item.now",
      { scrollActive: opts.scrollActive !== false },
    );
  } else if (queueViewTab === "playlists") {
    animateVisiblePanelList(
      document.getElementById("pl-list"),
      ".pl-card",
      panel,
    );
  } else {
...
function preparePlaylistPanelTabOnOpen(panel) {
  ...
  else if (queueViewTab === "playlists" || queueViewTab === "podcasts")
...
function switchPlaylistTab(tab, opts) {
  ...
  if (playlistPane)
    playlistPane.style.display = tab === "playlists" ? "" : "none";
  var podcastPane = document.getElementById("podcast-pane");
  if (podcastPane) podcastPane.style.display = tab === "podcasts" ? "" : "none";
  if ((tab === "playlists" || tab === "podcasts") && opts.refresh !== false)
    refreshUserPlaylists();
  if (opts.animate !== false)
    animatePlaylistPanelCurrentTab(document.getElementById("playlist-panel"));
}
...
function openPlaylistPanelTab(tab, preserve) {
  tab = normalizePlaylistPanelTab(tab);
  var panel = document.getElementById("playlist-panel");
  if (panel && panel.dataset && preserve !== false)
    panel.dataset.preserveTabOnOpen = "1";
...
function renderMiniQueuePanel(opts) {
  ...
  $count.textContent = total
    ? (expectedTotal > total ? total + "/" + expectedTotal : total) +
      " 首" +
      (currentIdx >= 0 ? " · 正在播放 " + (currentIdx + 1) : "")
    : "0 首";
  if (!miniQueueOpen && !opts.animate && !opts.scrollCurrent) return;
  if (!total) {
    $list.innerHTML =
      '<div class="mini-queue-empty">队列为空，先搜索或打开歌单</div>';
    return;
  }
  var windowInfo = queuePanelVirtualWindow(
    $list,
    $list,
    total,
    true,
    opts.scrollCurrent ? currentIdx : -1,
  );
  var visibleQueue = playQueue.slice(windowInfo.start, windowInfo.end);
  $list.innerHTML =
    queueVirtualSpacerHtml(windowInfo.top) +
    visibleQueue
      .map(function (song, localIndex) {
        ...
        return (
          ...
          '" data-queue-index="' +
          i +
          '" onclick="if(window.__mineradioSuppressReorderClick)return;playQueueAt(' +
          i +
          ')">' +
          imgTag +
          '<div class="mini-queue-info"><div class="mini-queue-name">' +
          escHtml(song.name) +
          '</div><div class="mini-queue-sub">' +
          escHtml(song.artist || "") +
          "</div></div>" +
          '<button class="mini-queue-remove mini-queue-next" onclick="event.stopPropagation();queueIndexNext(' +
          i +
          ')" title="下一首播放">下</button>' +
          '<button class="mini-queue-remove" onclick="event.stopPropagation();removeFromQueue(' +
          i +
          ')" title="移除">×</button>' +
          "</div>"
        );
      ...
      .join("") +
    queueVirtualSpacerHtml(windowInfo.bottom) +
    queueHydrationFooterHtml(true);
  if (opts.animate || opts.scrollCurrent) {
    requestAnimationFrame(function () {
      if (opts.animate)
        animateListItems($list, ".mini-queue-item", {
          x: 0,
          y: 6,
          stagger: 0.01,
          duration: 0.2,
      ...
      if (opts.scrollCurrent)
        smoothScrollToItem($list, $list.querySelector(".mini-queue-item.now"), {
          duration: 0.3,
          align: 0.42,
        });
    });
...
function clearPanelReorderClasses() {
  ...
  Array.prototype.forEach.call(
    document.querySelectorAll(
      ".reorder-pressing,.is-reordering,.is-reordering-list",
    ),
    function (node) {
...
function markPanelReorderSuppressed(ms) {
  ...
  setTimeout(function () {
    if (performance.now() >= panelReorderState.suppressClickUntil)
      window.__mineradioSuppressReorderClick = false;
  }, ms || 420);
}
...
function cancelPanelReorder(markSuppress) {
  ...
  if (
    markSuppress &&
    panelReorderState.active &&
    panelReorderState.kind === "queue" &&
...
function panelReorderBlockedTarget(target) {
  return !!(
    ...
    target.closest &&
    target.closest(
      "button,a,input,textarea,select,[data-pl-load-more],[data-pl-detail-load-more],[data-pl-detail-top],[data-pl-detail-play],[data-pl-detail-artist],[data-pl-detail-row],.pl-inline-detail",
    )
  );
}
function panelReorderHitFromTarget(target) {
  if (!target || !target.closest || panelReorderBlockedTarget(target))
    return null;
  var queueItem = target.closest(
    ".queue-item[data-queue-index],.mini-queue-item[data-queue-index]",
  );
  if (queueItem) {
...
function panelReorderHitAtPoint(x, y, state) {
  ...
  if (!hit || !state || hit.kind !== state.kind || hit.rootId !== state.rootId)
    return null;
  if (
    state.kind === "playlist" &&
    state.provider &&
    hit.provider !== state.provider
  )
    return null;
  return hit;
}
function markPanelReorderItem(state) {
  ...
  requestAnimationFrame(function () {
    ...
    var attr =
      state.kind === "playlist" ? "data-playlist-index" : "data-queue-index";
    var item = root.querySelector(
      "[" + attr + '="' + state.currentIndex + '"]',
    );
...
function bindLongPressPanelReorder() {
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      if (
        !e.target ||
        !e.target.closest ||
        !e.target.closest("#queue-list,#mini-queue-list,#pl-list")
      )
        return;
      e.preventDefault();
    ...
    function (e) {
      ...
      panelReorderState.timer = setTimeout(function () {
        ...
        if (typeof markRenderInteraction === "function")
          markRenderInteraction("panel-reorder", 900);
      }, reorderLongPressMs);
    },
    true,
  );
  document.addEventListener(
    ...
    function (e) {
      ...
      var moved =
        panelReorderState.kind === "queue"
          ? typeof moveQueueIndex === "function" &&
            moveQueueIndex(panelReorderState.currentIndex, hit.index, {
              rebuildShelf: true,
              renderPanel: true,
              persistSnapshot: false,
            })
          : typeof moveUserPlaylistIndex === "function" &&
            moveUserPlaylistIndex(panelReorderState.currentIndex, hit.index, {
              rebuildShelf: true,
              renderPanel: true,
            });
      if (!moved) return;
...
document.addEventListener("click", function (e) {
  if (
    miniQueueOpen &&
    !(e.target && e.target.closest && e.target.closest("#bottom-bar"))
  )
    closeMiniQueue();
});
...
function renderQueuePanel(opts) {
  ...
  if (!playQueue.length) {
    ...
    if (
      ...
      switchPlaylistTab("playlists", { save: false });
    return;
  }
  var total = playQueue.length;
    ...
    visibleQueue
      .map(function (song, localIndex) {
        ...
        return (
          ...
          ')">' +
          imgTag +
          '<div class="qi-info"><div class="qi-name">' +
          escHtml(song.name) +
          '</div><div class="qi-sub"><button class="queue-artist-link" type="button" onclick="event.stopPropagation();openQueueArtist(' +
          i +
          ')">' +
          ...
          '<div class="qi-act">' +
          '<button class="' +
          (isSongLiked(song) ? "liked" : "") +
          '" onclick="event.stopPropagation();toggleLikeQueueIndex(' +
          i +
          ')" title="' +
          (isSongLiked(song) ? "取消红心" : "红心喜欢") +
          '">' +
          heartIconSvg() +
          "</button>" +
          '<button class="queue-next" onclick="event.stopPropagation();queueIndexNext(' +
          i +
          ')" title="下一首播放">下</button>' +
          '<button onclick="event.stopPropagation();collectQueueIndex(' +
          i +
          ')" title="收藏到歌单">' +
          playlistPlusIconSvg() +
          "</button>" +
          '<button onclick="event.stopPropagation();removeFromQueue(' +
          i +
          ')" title="移除">×</button>' +
          "</div>" +
          "</div>"
      ...
      .join("") +
    queueVirtualSpacerHtml(windowInfo.bottom) +
    queueHydrationFooterHtml(false);
  ...
  renderMiniQueuePanel({
    scrollCurrent: opts.scrollCurrent !== false && miniQueueOpen,
  });
}
...
function playlistCatalogPageUrl(provider, offset, limit) {
  ...
  if (provider === "netease")
    return "/api/user/playlists?paged=1&limit=" + limit + "&offset=" + offset;
  if (provider === "spotify")
    return (
      "/api/spotify/user/playlists?limit=" +
      Math.min(500, limit) +
      "&offset=" +
      offset
    );
...
function rebuildUserPlaylistsFromCatalog(opts) {
  opts = opts || {};
  userPlaylists = neteasePlaylists.concat(
    qqPlaylists,
    kugouPlaylists,
    qishuiPlaylists,
    spotifyPlaylists,
...
async function loadPlaylistCatalogProviderPage(provider, reason) {
  ...
  var limit = first
    ? PLAYLIST_CATALOG_FIRST_PAGE_SIZE
    : PLAYLIST_CATALOG_BACKGROUND_PAGE_SIZE;
  ...
  try {
    ...
    state.nextOffset =
      r && r.nextOffset != null
        ? Math.max(0, Number(r.nextOffset) || 0)
    ...
    rebuildUserPlaylistsFromCatalog({
      animate: first && isPlaylistPanelVisibleForRender(),
      reset: first,
      preserveScroll: !first,
      reason: reason || "playlist-catalog-page",
    });
...
function playlistCatalogHasPendingPages() {
  ...
  return Object.keys(providers).some(function (key) {
    return providers[key] && (providers[key].loading || providers[key].hasMore);
  });
}
function requestNextPlaylistCatalogPage(reason) {
  ...
  loadPlaylistCatalogProviderPage(provider, reason || "background").finally(
    function () {
      if (
        playlistCatalogSyncState !== root ||
        !playlistCatalogHasPendingPages()
      ) {
        root.loading = false;
...
async function refreshUserPlaylists(force) {
  if (
    ...
    !qishuiLoginStatus.loggedIn &&
    !spotifyLoginStatus.loggedIn
  ) {
    resetPlaylistPanelRenderLimit();
    document.getElementById("pl-list").innerHTML =
      '<div style="text-align:center;padding:24px 0;color:rgba(255,255,255,.32);font-size:11.5px">登录后显示个人歌单</div>';
    var podcastListLoggedOut = document.getElementById("podcast-list");
    if (podcastListLoggedOut)
      podcastListLoggedOut.innerHTML =
        '<div style="text-align:center;padding:14px 0;color:rgba(255,255,255,.28);font-size:11.5px">登录后显示我的播客</div>';
    return;
  }
  var catalogNeedsNewProvider =
    playlistCatalogSyncState.loading &&
    ["netease", "qq", "kugou", "qishui", "spotify"].some(function (provider) {
      var state =
        playlistCatalogSyncState.providers &&
        playlistCatalogSyncState.providers[provider];
      return (
        playlistCatalogProviderLoggedIn(provider) && (!state || !state.enabled)
      );
    });
  if (
    playlistCatalogSyncState.loading &&
    !catalogNeedsNewProvider &&
    (!force ||
      Date.now() - Number(playlistCatalogSyncState.startedAt || 0) < 1200)
  ) {
    if (userPlaylists.length)
      renderUserPlaylistsList({
        animate: isPlaylistPanelVisibleForRender(),
        preserveScroll: true,
      });
    return;
  }
  ...
  if ($pl) {
    ...
    if (window.gsap)
      animateListItems($pl, ".mini-queue-skeleton", {
        x: 0,
        y: 6,
        stagger: 0.018,
        duration: 0.18,
  ...
  if (playlistCatalogSyncState.timer)
    clearTimeout(playlistCatalogSyncState.timer);
  var token = playlistCatalogSyncState.token + 1;
  playlistCatalogSyncState = {
    token: token,
    loading: true,
    timer: 0,
    providers: {},
  ...
  ["netease", "qq", "kugou", "qishui", "spotify"].forEach(function (provider) {
    if (force && playlistCatalogProviderLoggedIn(provider))
  ...
  var firstPageTasks = Object.keys(playlistCatalogSyncState.providers)
    .filter(playlistCatalogProviderLoggedIn)
    .map(function (provider) {
      return loadPlaylistCatalogProviderPage(provider, "first-page");
    });
  var podcastTask = loginStatus.loggedIn
    ? apiJson("/api/podcast/my", { timeoutMs: 12000 })
        .then(function (r) {
  ...
  if (playlistCatalogSyncState.loading)
    requestNextPlaylistCatalogPage("after-first-pages");
}
