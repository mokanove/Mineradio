...
if (podcastListEl) {
  podcastListEl.addEventListener("click", function (e) {
    if (
      e.target &&
      e.target.closest &&
      e.target.closest("[data-podcast-back]")
    ) {
    ...
    var radioCard =
      e.target && e.target.closest
        ? e.target.closest("[data-podcast-radio-id]")
        : null;
    if (radioCard) {
      loadPodcastRadioIntoQueue(
        radioCard.getAttribute("data-podcast-radio-id"),
        true,
        radioCard.getAttribute("data-podcast-title") || "",
      );
      return;
    }
    var card =
      e.target && e.target.closest
        ? e.target.closest("[data-podcast-key]")
        : null;
    if (!card) return;
    openMyPodcastCollection(
      card.getAttribute("data-podcast-key"),
      card.getAttribute("data-podcast-title") || "",
    );
  });
}
function renderMyPodcastRadioItems(key, title, items) {
  ...
  if (!items.length) {
    $pod.innerHTML =
      '<div class="podcast-inline-head"><div class="pl-section-label">' +
      escHtml(title || "我的播客") +
      '</div><button class="fx-mini-btn ghost" data-podcast-back="1" style="height:24px;padding:0 9px;font-size:10.5px">返回</button></div>' +
  ...
  $pod.innerHTML =
    ...
    items
      .map(function (r) {
        ...
        return (
          '<div class="pl-card podcast-card podcast-child" data-podcast-radio-id="' +
          escHtml(String(r.id || r.radioId || "")) +
          '" data-podcast-title="' +
          escHtml(r.name || "") +
          '">' +
          imgTag +
          ...
          escHtml(
            (r.djName || r.artist || "Podcast") +
              (r.programCount ? " · " + r.programCount + " 集" : ""),
          ) +
          "</div></div>" +
          "</div>"
        );
...
function playlistQueueSource(id) {
  var raw = String(id || "");
  if (raw.indexOf("qq:") === 0)
    return { provider: "qq", id: raw.slice(3), requestId: raw };
  if (raw.indexOf("kugou:") === 0)
    return { provider: "kugou", id: raw.slice(6), requestId: raw };
  if (raw.indexOf("qishui:") === 0)
    return { provider: "qishui", id: raw.slice(7), requestId: raw };
  if (raw.indexOf("spotify:") === 0)
    return { provider: "spotify", id: raw.slice(8), requestId: raw };
  return { provider: "netease", id: raw, requestId: raw };
}
function playlistQueuePageSize(provider, initial) {
  if (initial)
    return provider === "kugou" || provider === "qishui"
      ? 50
      : provider === "spotify"
        ? 96
        : PLAYLIST_QUEUE_INITIAL_BATCH_SIZE;
...
function playlistQueuePageUrl(source, offset, limit) {
  return playlistTracksEndpoint(source.provider, source.id, {
    offset: Math.max(0, offset || 0),
    limit: Math.max(1, limit || PLAYLIST_QUEUE_INITIAL_BATCH_SIZE),
  });
}
...
function schedulePlaylistQueueHydration(delay, reason) {
  ...
  if (!state || !state.active || state.error || state.queueRef !== playQueue)
    return false;
...
async function hydratePlaylistQueueNextPage(reason) {
  ...
  state.promise = apiJson(playlistQueuePageUrl(source, offset, limit), {
    timeoutMs: 16000,
  })
    .then(function (r) {
      ...
      if (playMode === "shuffle" && pageTracks.length > 1)
        shuffleArrayInPlace(pageTracks);
      ...
      if (!state.active) {
        ...
        state.promise = null;
        state.pausedForBuffer = false;
        safeRenderQueuePanel("playlist-queue-hydrate-complete", {
          animate: false,
          scrollCurrent: false,
        });
      }
    ...
    .catch(function (e) {
      ...
      state.active = false;
      state.pausedForBuffer = false;
      safeRenderQueuePanel("playlist-queue-hydrate-error", {
        animate: false,
        scrollCurrent: false,
      });
      return false;
  ...
  safeRenderQueuePanel("playlist-queue-hydrate-start", {
    animate: false,
    scrollCurrent: false,
  });
...
function ensurePlaylistQueueHydratedAhead(index) {
  var state = queueHydrationState;
  if (!state || state.queueRef !== playQueue || !state.active || state.error)
    return false;
  if (
    playQueue.length - Math.max(0, Number(index) || 0) <=
    PLAYLIST_QUEUE_PLAYBACK_AHEAD_THRESHOLD
  ) {
    state.pausedForBuffer = false;
...
async function loadPlaylistIntoQueueById(id, autoplay, title, opts) {
  ...
  var r = null;
  var seedTracks =
    Array.isArray(opts.seedTracks) && opts.seedTracks.length
      ? opts.seedTracks.map(cloneSong)
      : [];
  try {
    if (!seedTracks.length) {
      r = await apiJson(
        playlistQueuePageUrl(
          source,
          0,
          playlistQueuePageSize(source.provider, true),
        ),
        { timeoutMs: 16000 },
      );
  ...
  try {
    ...
    var catalogPlaylist = userPlaylists.find(function (pl) {
      return (
        normalizePlaylistProvider(pl && pl.provider) === source.provider &&
        String((pl && pl.id) || "") === String(source.id || "")
      );
    });
    var total = Math.max(
      playQueue.length,
      Number(r && (r.total || (r.playlist && r.playlist.trackCount))) ||
        Number(opts.total) ||
        Number(catalogPlaylist && catalogPlaylist.trackCount) ||
        0,
    );
    var nextOffset = Math.max(
      Number(r && r.nextOffset) || Number(opts.nextOffset) || playQueue.length,
      playQueue.length,
    );
    ...
    currentIdx = Math.max(
      0,
      Math.min(playQueue.length - 1, Number(opts.startIndex) || 0),
    );
    safeRenderQueuePanel("playlist-load-first-page", {
      animate: true,
      scrollCurrent: true,
