...
function lyricTranslationTextFromAliases(source) {
  ...
  return (
    source.tlyric ||
    source.trans ||
    source.translatedLyric ||
    source.translation ||
    source.translated_lyric ||
    ""
  );
}
function lyricEndpointForSong(songOrId) {
  ...
  if (provider === "qq") {
    ...
    var qqId =
      song.qqId || (/^\d+$/.test(String(song.id || "")) ? song.id : "");
    return (
      "/api/qq/lyric?mid=" +
  ...
  if (provider === "kugou") {
    return (
      ...
      "&duration=" +
      encodeURIComponent(playbackDurationFromSong(song) || "")
    );
  }
  if (provider === "qishui") {
    return (
  ...
  if (provider === "spotify") {
    return (
      "/api/spotify/lyric?id=" +
      encodeURIComponent(song.id || song.providerSongId || song.spotifyId || "")
    );
  }
...
function persistentLyricCacheKey(song) {
  ...
  var provider =
    typeof songProviderKey === "function"
      ...
      : song.source || song.provider || "netease";
...
function readPersistentLyricCache(song) {
  ...
  return window.desktopWindow
    .readLyricCache(persistentLyricCacheKey(song))
    .then(function (result) {
      return result && result.ok && result.hit && result.payload
        ? result.payload
        : null;
    })
    .catch(function () {
...
function writePersistentLyricCache(song, payload) {
  if (
    ...
    return;
  window.desktopWindow
    .writeLyricCache(persistentLyricCacheKey(song), payload)
    .catch(function () {});
}
...
function nextQueueLyricPrefetchSong(fromIndex) {
  ...
  var from = isFinite(Number(fromIndex))
    ? Math.round(Number(fromIndex))
    : currentIdx;
...
async function runQueueLyricPrefetch(fromIndex, token) {
  ...
  try {
    ...
    if (token !== lyricQueuePrefetchToken) return false;
    var merged = mergeInlineLyricResponseForSong(
      candidate.song,
      response || {},
    );
...
function applyFetchedLyricResponse(song, token, response, options) {
  ...
  if (state.usableLyric && options.persist !== false)
    writePersistentLyricCache(song, mergedResponse);
  return state;
}

function refreshPersistentLyricCache(song) {
  apiJson(lyricEndpointForSong(song))
    .then(function (response) {
      var mergedResponse = mergeInlineLyricResponseForSong(
        song,
        response || {},
      );
      var state = parseLyricResponseToOriginalState(song, mergedResponse);
...
function mergeInlineLyricResponseForSong(song, response) {
  ...
  if (!response.tlyric)
    response.tlyric = lyricTranslationTextFromAliases(response);
  ...
  if (!response.yrc && (song.yrc || song.qrc))
    response.yrc = song.yrc || song.qrc;
...
async function findNeteaseLyricFallbackCandidate(song) {
  ...
  var data = await apiJson(
    "/api/search?keywords=" + encodeURIComponent(query) + "&limit=8",
    { timeoutMs: 4800 },
  );
  ...
  list = list.filter(function (candidate) {
    return !(
      typeof sourceCandidateRejectReason === "function" &&
      sourceCandidateRejectReason(song, candidate, "netease")
    );
  });
  ...
  list = list.slice().sort(function (a, b) {
    var sa =
      typeof scoreSongSearchResult === "function"
        ? scoreSongSearchResult(a, query, 0)
        : 0;
    var sb =
      typeof scoreSongSearchResult === "function"
        ? scoreSongSearchResult(b, query, 0)
        : 0;
  ...
  var bestScore =
    typeof scoreSongSearchResult === "function"
      ? scoreSongSearchResult(best, query, 0)
      : 0;
  return best && best.id && bestScore >= 28 ? best : null;
}
function mergeNeteaseFallbackTranslationsIntoCurrent(
  song,
  token,
  payload,
  cacheKey,
  ...
  var currentSong =
    typeof currentLyricSong === "function" ? currentLyricSong() : null;
  if (lyricTranslationFallbackKey(currentSong) !== cacheKey) return false;
  if (
    originalLyricsState &&
    originalLyricsState.translationLines &&
    originalLyricsState.translationLines.length
  )
    return false;
  var mergedLines = attachLyricTranslations(
    originalLyricsState.lines || [],
    payload.lines,
  );
  var attached = mergedLines.some(function (line) {
...
async function fetchNeteaseLyricTranslationFallback(song, token, cacheKey) {
  ...
  if (cached)
    return mergeNeteaseFallbackTranslationsIntoCurrent(
      song,
      token,
      cached,
      cacheKey,
  ...
  try {
    ...
    var response = await apiJson(
      "/api/lyric?id=" + encodeURIComponent(candidate.id),
      { timeoutMs: 5200 },
    );
    if (token !== trackSwitchToken) return false;
...
function scheduleNeteaseLyricTranslationFallback(song, token, state) {
  ...
  if (cached) {
    setTimeout(function () {
      mergeNeteaseFallbackTranslationsIntoCurrent(
        song,
        token,
        cached,
        cacheKey,
  ...
  var start = function () {
    if (token === trackSwitchToken)
      fetchNeteaseLyricTranslationFallback(song, token, cacheKey);
  };
  setTimeout(function () {
...
function lyricFallbackTextForSong(song) {
  ...
  if (!title && document)
    title = String(
      (document.getElementById("thumb-title") &&
        document.getElementById("thumb-title").textContent) ||
        "",
    ).trim();
  if (!artist && document)
    artist = String(
      (document.getElementById("thumb-artist") &&
        document.getElementById("thumb-artist").textContent) ||
        "",
    ).trim();
  if (!title) return "";
...
function parseLyricResponseToOriginalState(song, response) {
  ...
  var hasNativeKaraoke = nativeLines.some(function (line) {
    return line.words && line.words.length;
  });
  ...
  var lines = withLyricFallbackForSong(
    song,
    attachLyricTranslations(primaryLines, translationLines),
  );
...
function resetLyricsForTrackSwitch() {
  ...
  if (typeof invalidateStageLyricPayloadForNewLyrics === "function")
    invalidateStageLyricPayloadForNewLyrics("track-switch-pending");
...
function scheduleTrackSwitchFallbackLyrics(song, token, delay) {
  cancelPendingTrackFallbackLyrics();
  var multiLineDelay =
    typeof stageLyricMultiLineWarmupLoad === "function" &&
    stageLyricMultiLineWarmupLoad()
      ? 1850
      : 180;
  pendingTrackFallbackLyricTimer = setTimeout(
    function () {
      pendingTrackFallbackLyricTimer = 0;
      ...
      if (hasUsableLyricLines(originalLyricsState && originalLyricsState.lines))
        return;
      setOriginalLyricsState(
        withLyricFallbackForSong(song || currentLyricSong(), []),
        false,
        "fallback",
        [],
        "none",
...
async function fetchLyric(songOrId, token, attempt) {
  ...
  try {
    ...
    if (
      !state.usableLyric &&
      shouldRetryStartupLyricFetch(song, token, attempt)
    )
      scheduleStartupLyricFetchRetry(song, token, attempt);
  } catch (e) {
    if (token !== trackSwitchToken) return;
    cancelPendingTrackFallbackLyrics();
    var fallbackLines = withLyricFallbackForSong(
      song || currentLyricSong(),
      [],
    );
    setOriginalLyricsState(fallbackLines, false, "fallback", [], "none");
    applyPreferredLyricsForCurrent(true);
    if (shouldRetryStartupLyricFetch(song, token, attempt))
      scheduleStartupLyricFetchRetry(song, token, attempt);
  }
}
...
function lyricTagTimeToSeconds(min, sec, frac) {
  ...
  if (frac)
    t += (parseInt(frac, 10) || 0) / Math.pow(10, Math.min(3, frac.length));
  return t;
}
function finalizeLyricLineDurations(lines) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    lines[i].charCount = Math.max(
      1,
      lines[i].charCount || String(lines[i].text || "").length,
    );
  }
  return lines;
}
function parseLyricText(text) {
  ...
  text.split(/\r?\n/).forEach(function (line) {
    ...
    if (hasInterleavedText) {
      for (var si = 0; si < tags.length; si++) {
        var segment = line
          .slice(
            tags[si].end,
            si + 1 < tags.length ? tags[si + 1].index : line.length,
          )
          .trim();
        if (segment)
          lines.push({ t: tags[si].t, text: segment, source: "lrc" });
      }
      return;
    }
    ...
    times.forEach(function (t) {
      lines.push({ t: t, text: txt, source: "lrc" });
    });
  });
...
function isLyricCreditLineText(text) {
  ...
  var split = cleaned.split(
    /\s*(?:\/|,|&|\+|;|\band\b|\bfeat\.?\b|\bft\.?\b|\bwith\b)\s*/i,
  );
  var tokens = [];
  ...
  var lyricWords =
    /\b(?:i|me|my|mine|you|your|yours|we|us|our|ours|he|him|his|she|her|they|them|it|love|die|want|need|know|gotta|gonna|wanna|would|could|should|can|cant|can't|dont|don't|ain't|is|are|am|be|been|being|was|were|do|does|did|go|come|make|take|throw|lock|away|world|force|key|place|baby|girl|boy|night|heart|life)\b/i;
  if (!hasCreditPunctuation && tokens.length > 1 && lyricWords.test(cleaned))
    return false;
  var nameLike = 0;
  ...
  return (
    tokens.length >= 2 &&
    nameLike >= tokens.length - 1 &&
    /(?:\/|,|&|\+|\band\b|\bfeat\.?\b|\bft\.?\b|\bwith\b)/i.test(cleaned)
  );
}
...
function mergeLyricTranslationLineSources() {
  ...
  function hasSameLine(candidate) {
    var candidateText = normalizeLyricTranslationText(
    ...
    for (var i = 0; i < out.length; i++) {
      var item = out[i];
      if (
        Math.abs((Number(item.t) || 0) - candidateTime) <= 0.12 &&
        normalizeLyricTranslationText(item.text) === candidateText
      )
        return true;
    }
    return false;
...
function buildLyricTranslationPayload(response) {
  response = response || {};
  var lrcTranslations = markLyricLineSource(
    parseLyricText(lyricTranslationTextFromAliases(response)),
    "tlyric",
  );
  var yrcTranslations = markLyricLineSource(
    parseYrcText(response.ytlrc || ""),
    "ytlrc",
  );
  var lines = mergeLyricTranslationLineSources(
    lrcTranslations,
    yrcTranslations,
  );
  var sources = [];
...
function attachLyricTranslations(primaryLines, translationLines) {
  ...
  function translationToleranceForLine(line) {
    var lineDuration = Math.max(
      0.9,
      Math.min(5.5, Number(line && line.duration) || 3.2),
    );
  ...
  primary.forEach(function (line, lineIndex) {
    ...
    if (bestIndex >= 0)
      assignTranslation(lineIndex, bestIndex, bestDelta, "time");
  });
  if (translations.length >= Math.max(2, primary.length * 0.58)) {
    var orderedPrimaryIndexes = [];
    primary.forEach(function (line, lineIndex) {
      if (line && !line.fallback && !isLyricCreditLineText(line.text))
        orderedPrimaryIndexes.push(lineIndex);
    });
    ...
    orderedPrimaryIndexes.forEach(function (lineIndex, orderPos) {
      ...
      for (var trIndex = 0; trIndex < translations.length; trIndex++) {
        ...
        if (orderGap > 5 && translations.length <= primary.length * 1.25)
          continue;
        var delta = Math.abs((Number(tr.t) || 0) - (Number(line.t) || 0));
        var fallbackTolerance = Math.max(
          translationToleranceForLine(line) * 1.35,
          2.8,
        );
      ...
      if (bestIndex >= 0)
        assignTranslation(lineIndex, bestIndex, bestDelta, "order");
    });
  }
...
function renderLyrics(options) {
  options = options || {};
  var renderSignature =
    typeof stageLyricRenderSignatureForCurrentState === "function"
      ? stageLyricRenderSignatureForCurrentState()
      : "";
  if (
    options.preserveSame &&
    typeof stageLyricCanPreserveSameRender === "function" &&
    stageLyricCanPreserveSameRender(renderSignature)
  ) {
    if (typeof markStageLyricsPlaybackResume === "function")
      markStageLyricsPlaybackResume(options.reason || "preserve-same-lyrics");
    return;
  }
  ...
  var restoreWarmup =
    typeof stageLyricRestoreWarmupSeconds === "function" &&
    stageLyricRestoreWarmupSeconds() != null;
  var prewarmReason = restoreWarmup ? "startup-restore-lyrics" : warmupReason;
  if (typeof invalidateStageLyricPayloadForNewLyrics === "function")
    invalidateStageLyricPayloadForNewLyrics("renderLyrics");
  else clearStageLyrics();
  if (typeof stageLyrics !== "undefined" && stageLyrics && renderSignature)
    stageLyrics.renderSignature = renderSignature;
  if (typeof requestStageLyricWarmup === "function")
    requestStageLyricWarmup(prewarmReason, fallbackTitleOnly ? 120 : 900);
  if (restoreWarmup && typeof scheduleStageLyricRestorePrewarm === "function") {
    scheduleStageLyricRestorePrewarm(
      prewarmReason,
      fallbackTitleOnly ? 40 : 16,
    );
  ...
  if (
    !fallbackTitleOnly &&
    typeof scheduleStageLyricSingleLineBootstrapPrewarm === "function"
  ) {
    scheduleStageLyricSingleLineBootstrapPrewarm(
      prewarmReason,
      restoreWarmup ? 24 : 44,
    );
  }
  if (
    ...
    scheduleStageLyricFullTrackWarmup(
      restoreWarmup ? "track-ready-fast" : "lyrics-ready-preload",
      restoreWarmup ? 120 : 24,
    );
  }
...
function toggleLyricsPanel(force) {
  ...
  if (fx.particleLyrics) {
    ...
    if (typeof requestStageLyricWarmup === "function")
      requestStageLyricWarmup("toggleLyricsPanel", 150);
    if (typeof scheduleStageLyricPrewarm === "function")
      scheduleStageLyricPrewarm("toggleLyricsPanel", 48);
