...
function queueVirtualSpacerHtml(height) {
  ...
  return height
    ? '<div class="queue-virtual-spacer" aria-hidden="true" style="height:' +
        height +
        'px"></div>'
    : "";
}
function queuePanelVirtualWindow(
  ...
  var viewport = Math.max(
    rowStep * 4,
    Number(scroller && scroller.clientHeight) || rowStep * 9,
  );
  ...
  if (forceIndex != null && forceIndex >= 0 && forceIndex < total) {
    start = Math.max(
      0,
      Math.floor(forceIndex - visibleRows * 0.42) - QUEUE_VIRTUAL_OVERSCAN,
    );
  } else if (selfScroll) {
    start = Math.max(
      0,
      Math.floor((Number(scroller && scroller.scrollTop) || 0) / rowStep) -
    ...
    start = Math.max(
      0,
      Math.floor(visibleTop / rowStep) - QUEUE_VIRTUAL_OVERSCAN,
    );
  }
  ...
  return {
    ...
    top: start * rowStep,
    bottom: Math.max(0, total - end) * rowStep,
  };
}
function scheduleQueuePanelVirtualRender() {
  ...
  queuePanelVirtualState.raf = requestAnimationFrame(function () {
    ...
    if (miniQueueOpen)
      renderMiniQueuePanel({ animate: false, scrollCurrent: false });
...
function maybeRequestPlaylistQueuePageFromScroller(scroller) {
  if (!scroller || typeof requestPlaylistQueueHydrationForBrowse !== "function")
    return false;
  if (
    scroller.scrollTop + scroller.clientHeight <
    scroller.scrollHeight - QUEUE_VIRTUAL_ROW_STEP * 6
  )
    return false;
...
function queuePanelItemKey(song, fallback) {
  try {
    if (typeof queueItemKey === "function")
      return queueItemKey(song) || fallback;
  } catch (e) {}
  return (
    (song && (song.id || song.mid || song.localKey || song.name)) || fallback
  );
}
...
function queuePanelVisibleLimit(total) {
  ...
  var base = Math.max(
    QUEUE_PANEL_BATCH_SIZE,
    queuePanelRenderLimit || QUEUE_PANEL_BATCH_SIZE,
  );
  if (currentIdx >= 0 && currentIdx < total) {
    base = Math.max(
      base,
      Math.ceil((currentIdx + 1) / QUEUE_PANEL_BATCH_SIZE) *
        QUEUE_PANEL_BATCH_SIZE,
    );
  }
...
function maybeGrowQueuePanelRenderLimit() {
  ...
  if (panel.scrollTop + panel.clientHeight >= panel.scrollHeight - 220)
...
function playlistCardPriority(pl) {
  ...
  if (
    pl.virtual ||
    String(pl.id || "") === "spotify-liked" ||
    Number(pl.specialType || 0) === 5
  )
    return 0;
  return 1;
}
...
function playlistPanelNoticeHtml(text, isError) {
  ...
  return (
    '<div style="text-align:center;padding:14px 10px;color:' +
    (isError ? "rgba(255,180,160,.82)" : "rgba(255,255,255,.30)") +
...
function playlistPanelDetailRowsHtml(options) {
  ...
  if (!tracks.length)
    return playlistPanelNoticeHtml(st.message || st.error || "", !!st.error);
  var viewport = Math.max(
    280,
    Number(options.viewport) ||
      Math.min(620, Math.round((window.innerHeight || 800) * 0.72)),
  );
  var localScrollTop = Math.max(0, Number(options.scrollTop) || 0);
  var start = Math.max(
    0,
    Math.floor(localScrollTop / PLAYLIST_DETAIL_ROW_STEP) -
      PLAYLIST_DETAIL_VIRTUAL_OVERSCAN,
  );
  var maxRows =
    Math.ceil(viewport / PLAYLIST_DETAIL_ROW_STEP) +
    PLAYLIST_DETAIL_VIRTUAL_OVERSCAN * 2;
  ...
  var rows =
    '<div class="pl-detail-virtual-spacer" aria-hidden="true" style="height:' +
    start * PLAYLIST_DETAIL_ROW_STEP +
    'px"></div>';
  rows += tracks
    .slice(start, end)
    .map(function (song, localIndex) {
      var i = start + localIndex;
      var thumb = songCoverSrc(song, 60);
      var imgTag = thumb
        ? '<img src="' +
          escHtml(thumb) +
          '" alt="" loading="lazy" decoding="async" onerror="this.style.opacity=0.2">'
        : '<div style="width:34px;height:34px;border-radius:7px;background:rgba(255,255,255,.06);flex:0 0 auto"></div>';
      return (
        '<div class="pl-detail-row" data-pl-detail-row="' +
        i +
        '">' +
        imgTag +
        '<div style="flex:1;min-width:0"><div class="pl-detail-row-title">' +
        escHtml(song.name || "") +
        "</div>" +
        '<button type="button" class="pl-detail-row-artist" data-pl-detail-artist="' +
        i +
        '">' +
  ...
  rows +=
    '<div class="pl-detail-virtual-spacer" aria-hidden="true" style="height:' +
    Math.max(0, tracks.length - end) * PLAYLIST_DETAIL_ROW_STEP +
    'px"></div>';
  if (st.error) {
    rows +=
      '<div class="pl-detail-progress">后续歌曲载入失败，重新打开歌单可继续</div>';
  } else if (st.hasMore || st.loadingMore) {
    rows +=
      '<div class="pl-detail-progress"><span class="queue-hydration-spinner' +
      (st.loadingMore ? " spinning" : "") +
      '"></span><span>' +
    ...
    rows +=
      '<div class="pl-detail-progress">已加载全部 ' +
      tracks.length +
      " 首</div>";
  }
  return rows;
...
function savePlaylistReorderKeys() {
  try {
    localStorage.setItem(
      PLAYLIST_REORDER_STORE_KEY,
      JSON.stringify(userPlaylists.map(playlistReorderKey).filter(Boolean)),
    );
  } catch (e) {}
}
...
function playlistPanelDetailHtml(pl, provider, detailWindow) {
  ...
  var cover =
    pl && pl.cover
      ? provider === "netease"
        ? pl.cover + "?param=96y96"
        : pl.cover
      : "";
  var img = cover
    ? '<img class="pl-detail-cover" src="' +
      escHtml(cover) +
      '" alt="" decoding="async" onerror="this.style.opacity=0.2">'
    : '<div class="pl-detail-cover"></div>';
  var expectedTotal = Math.max(
    tracks.length,
    Number(playlistPanelDetailState.total) || Number(pl.trackCount) || 0,
  );
  ...
  var canUncollect = !!(
    ...
    pl.subscribed &&
    !pl.virtual &&
    (provider === "netease" || provider === "qishui" || provider === "spotify")
  );
  var collectionButton = canUncollect
    ...
    : "";
  return (
    '<div class="pl-inline-detail" data-pl-detail="' +
    escHtml(key) +
    '" style="height:' +
    playlistPanelDetailShellHeight() +
    'px">' +
    '<div class="pl-detail-sticky">' +
    '<div class="pl-detail-head">' +
    img +
    '<div style="flex:1;min-width:0"><div class="pl-detail-title">' +
    escHtml(pl.name || "歌单详情") +
    '</div><div class="pl-detail-sub">' +
    escHtml(
      (expectedTotal || tracks.length || 0) +
        " 首 · " +
        (pl.creator || playlistProviderName(provider)),
    ) +
    '</div></div><div class="pl-detail-count">' +
    (loading && !tracks.length
      ? "载入中"
      : tracks.length +
        (expectedTotal > tracks.length ? "/" + expectedTotal : "")) +
    "</div></div>" +
    '<div class="pl-detail-actions"><button class="pl-detail-play" type="button" data-pl-detail-play="' +
    escHtml(key) +
    '"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>播放歌单</button>' +
    collectionButton +
    '<button class="fx-mini-btn ghost pl-detail-top-btn" type="button" data-pl-detail-top="1">回到顶部</button></div>' +
    "</div>" +
    '<div class="pl-detail-list" data-pl-detail-scroll="' +
    escHtml(key) +
    '">' +
    rows +
    "</div>" +
...
function scrollPlaylistPanelDetailIntoView(key) {
  ...
  requestAnimationFrame(function () {
    ...
    Array.prototype.some.call(
      panel.querySelectorAll("[data-pl-detail]"),
      function (node) {
    ...
    if (toolbar) {
      var toolbarTop = 82;
      try {
        toolbarTop = parseFloat(getComputedStyle(toolbar).top) || toolbarTop;
      } catch (e) {}
...
function cancelPlaylistPanelDetailRequest() {
  if (playlistPanelDetailState.warmTimer)
    clearTimeout(playlistPanelDetailState.warmTimer);
  playlistPanelDetailState.warmTimer = 0;
  if (playlistPanelDetailState.controller) {
    try {
      playlistPanelDetailState.controller.abort();
    } catch (e) {}
  }
...
function appendPlaylistPanelDetailTracks(target, incoming) {
  ...
  (target || []).forEach(function (song, index) {
    seen[queuePanelItemKey(song, "old:" + index)] = true;
  });
  var added = 0;
...
async function loadMorePlaylistPanelDetailTracks(reason) {
  var st = playlistPanelDetailState;
  if (!st.key || st.loadingMore || (reason !== "initial" && !st.hasMore))
    return false;
  ...
  var offset =
    reason === "initial"
      ? 0
      : Math.max(0, Number(st.nextOffset) || st.tracks.length);
  var token = st.token;
  ...
  try {
    var r = await apiJson(
      playlistTracksEndpoint(provider, pid, {
        limit: PLAYLIST_DETAIL_BATCH_SIZE,
        offset: offset,
      }),
      controller ? { signal: controller.signal } : { timeoutMs: 12000 },
    );
    if (
      playlistPanelDetailState.token !== token ||
      playlistPanelDetailState.key !== st.key
    )
      return false;
    ...
    var responseTotal =
      Number(r && (r.total || (r.playlist && r.playlist.trackCount))) || 0;
    st.total = Math.max(st.total || 0, responseTotal, st.tracks.length);
    st.nextOffset = Math.max(
      offset + rawTracks.length,
      Number(r && r.nextOffset) || 0,
    );
    st.hasMore = !!(r && r.hasMore);
    if (!rawTracks.length || (!added && st.nextOffset <= offset))
      st.hasMore = false;
    st.loading = false;
    ...
    if (r && r.playlist)
      st.playlist = Object.assign({}, st.playlist || {}, r.playlist);
    if (reason === "initial") {
      ...
      if (st.hasMore) {
        st.warmTimer = setTimeout(function () {
          ...
          if (
            ...
            loadMorePlaylistPanelDetailTracks("warm");
        }, 320);
      }
    } else {
    ...
    if (
      playlistPanelDetailState.token === token &&
      playlistPanelDetailState.controller === controller
    )
      playlistPanelDetailState.controller = null;
  }
}
async function openPlaylistPanelDetail(provider, pid, title) {
  ...
  var pl = userPlaylists.find(function (item) {
    return (
      playlistPanelKey(normalizePlaylistProvider(item.provider), item.id) ===
      key
    );
  }) || { id: pid, provider: provider, name: title || "歌单详情" };
...
function playPlaylistPanelDetail() {
  ...
  loadPlaylistIntoQueueById(
    playlistPanelProviderId(provider, pid),
    true,
    (st.playlist && st.playlist.name) || "",
  );
}
...
function playPlaylistPanelDetailTrack(index) {
  ...
  loadPlaylistIntoQueueById(
    playlistPanelProviderId(provider, pid),
    true,
    (playlistPanelDetailState.playlist &&
      playlistPanelDetailState.playlist.name) ||
      "",
    {
      seedTracks: tracks,
      startIndex: index,
...
function openPlaylistPanelDetailArtist(index) {
  var song =
    playlistPanelDetailState.tracks && playlistPanelDetailState.tracks[index];
...
function maybeGrowPlaylistPanelDetailRenderLimit() {
  var panel = document.getElementById("playlist-panel");
  var detail =
    panel && panel.querySelector(".pl-inline-detail[data-pl-detail]");
  if (
    !panel ||
    !detail ||
    !playlistPanelDetailState.hasMore ||
    playlistPanelDetailState.loadingMore
  )
    return;
  var panelRect = panel.getBoundingClientRect();
  var detailRect = detail.getBoundingClientRect();
  if (detailRect.bottom <= panelRect.bottom + PLAYLIST_DETAIL_ROW_STEP * 8)
    loadMorePlaylistPanelDetailTracks("scroll");
}
function resetPlaylistPanelRenderLimit() {
  playlistPanelRenderLimit = PLAYLIST_PANEL_BATCH_SIZE;
}
var playlistPanelVirtualCache = {
  revision: -1,
  detailKey: "",
  detailSig: "",
  entries: [],
...
function playlistPanelDetailShellHeight() {
  var st = playlistPanelDetailState || {};
  var rows = Math.max(
    st.loading && !(st.tracks && st.tracks.length) ? 1 : 0,
    (st.tracks && st.tracks.length) || 0,
  );
  ...
  var footerHeight =
    ...
    st.hasMore ||
    st.loadingMore ||
    rows > PLAYLIST_DETAIL_INITIAL_RENDER
      ? PLAYLIST_DETAIL_OUTER_FOOTER_HEIGHT
      : 12;
  return (
    PLAYLIST_DETAIL_OUTER_CHROME_HEIGHT +
    rows * PLAYLIST_DETAIL_ROW_STEP +
    noticeHeight +
    footerHeight
  );
}
...
function playlistPanelBuildVirtualEntries() {
  var detailSig = [
    ...
    (playlistPanelDetailState.tracks &&
      playlistPanelDetailState.tracks.length) ||
      0,
  ...
  if (
    playlistPanelVirtualCache.revision === playlistCatalogRevision &&
    playlistPanelVirtualCache.detailKey === playlistPanelDetailState.key &&
    playlistPanelVirtualCache.detailSig === detailSig
  )
  ...
  order.forEach(function (key) {
    ...
    items.forEach(function (entry) {
      ...
      var cardKey = playlistPanelKey(
        normalizePlaylistProvider(entry.pl.provider),
        entry.pl.id,
      );
      if (playlistPanelDetailState.key === cardKey) {
        entries.push({
          type: "detail",
          pl: entry.pl,
          provider: normalizePlaylistProvider(entry.pl.provider),
          height: playlistPanelDetailShellHeight(),
        });
      }
    });
  });
...
function playlistPanelOffsetIndex(offsets, value) {
  var lo = 0,
    hi = Math.max(0, offsets.length - 1);
  while (lo < hi) {
...
function playlistCatalogFooterHtml() {
  ...
  var totals = Object.keys(providerStates).reduce(
    function (acc, key) {
  ...
  return (
    '<div class="playlist-catalog-status"><span class="queue-hydration-spinner spinning"></span><span>' +
    label +
    "</span></div>"
  );
}
...
function renderUserPlaylistsList(opts) {
  ...
  if (!userPlaylists.length) {
    $pl.innerHTML =
      playlistCatalogSyncState && playlistCatalogSyncState.loading
  ...
  function playlistCardHtml(pl, sourceIndex) {
    ...
    return (
      ...
      '">' +
      imgTag +
      '<div style="flex:1;min-width:0"><div class="pl-name">' +
      escHtml(pl.name) +
      '<span class="tag-source ' +
      provider +
      '" style="margin-left:6px;vertical-align:1px">' +
      providerLabel +
      '</span></div><div class="pl-sub">' +
      pl.trackCount +
      " 首 · " +
      escHtml(pl.creator || "") +
      "</div></div>" +
  ...
  var start = playlistPanelOffsetIndex(
    cache.offsets,
    Math.max(0, visibleTop - PLAYLIST_CARD_VIRTUAL_OVERSCAN_PX),
  );
  var end = Math.min(
    cache.entries.length,
    playlistPanelOffsetIndex(
      cache.offsets,
      visibleTop + viewport + PLAYLIST_CARD_VIRTUAL_OVERSCAN_PX,
    ) + 1,
  );
  var topHeight = cache.offsets[start] || 0;
  var bottomHeight = Math.max(
    0,
    cache.totalHeight - (cache.offsets[end] || cache.totalHeight),
  );
  var html =
    '<div class="playlist-virtual-spacer" aria-hidden="true" style="height:' +
    Math.round(topHeight) +
    'px"></div>';
  for (var entryIndex = start; entryIndex < end; entryIndex++) {
    var entry = cache.entries[entryIndex];
    if (entry.type === "label")
      html += '<div class="pl-section-label">' + entry.label + "</div>";
    else if (entry.type === "card")
      html += playlistCardHtml(entry.pl, entry.sourceIndex);
    else if (entry.type === "detail") {
      var entryTop = cache.offsets[entryIndex] || 0;
      var detailRowScrollTop = Math.max(
        0,
        visibleTop - entryTop - PLAYLIST_DETAIL_OUTER_CHROME_HEIGHT,
      );
      html += playlistPanelDetailHtml(entry.pl, entry.provider, {
        scrollTop: detailRowScrollTop,
        viewport: viewport,
      });
    }
  }
  html +=
    ...
    'px"></div>' +
    playlistCatalogFooterHtml();
  $pl.innerHTML = html;
  if (panel && opts.preserveScroll) panel.scrollTop = keepTop;
  bindPlaylistPanelDetailScroller();
  if (
    typeof requestNextPlaylistCatalogPage === "function" &&
    end >= cache.entries.length - 8
  )
    requestNextPlaylistCatalogPage("panel-near-end");
  if (opts.animate && seq === playlistRenderSeq)
...
function renderMyPodcastCollections(opts) {
  ...
  if (!loginStatus.loggedIn) {
    $pod.innerHTML =
      '<div style="text-align:center;padding:14px 0;color:rgba(255,255,255,.28);font-size:11.5px">登录后显示我的播客</div>';
    return;
  }
  var items = myPodcastCollections || [];
  if (!items.length) {
    $pod.innerHTML =
      '<div style="text-align:center;padding:14px 0;color:rgba(255,255,255,.28);font-size:11.5px">暂无播客数据</div>';
    return;
  }
  $pod.innerHTML = items
    .map(function (pc) {
      ...
      return (
        '<div class="pl-card podcast-card" data-podcast-key="' +
        escHtml(pc.key || "") +
        '" data-podcast-title="' +
        escHtml(pc.title || "") +
        '">' +
...
document.getElementById("pl-list").addEventListener("click", function (e) {
  var loadMore =
    e.target && e.target.closest
      ? e.target.closest("[data-pl-load-more]")
      : null;
  if (loadMore) {
    e.preventDefault();
    e.stopPropagation();
  ...
  var detailLoadMore =
    e.target && e.target.closest
      ? e.target.closest("[data-pl-detail-load-more]")
      : null;
  if (detailLoadMore) {
    e.preventDefault();
    e.stopPropagation();
  ...
  var detailTop =
    e.target && e.target.closest
      ? e.target.closest("[data-pl-detail-top]")
      : null;
  if (detailTop) {
    e.preventDefault();
    e.stopPropagation();
  ...
  var playDetail =
    e.target && e.target.closest
      ? e.target.closest("[data-pl-detail-play]")
      : null;
  if (playDetail) {
    e.preventDefault();
    e.stopPropagation();
  ...
  var collection =
    e.target && e.target.closest
      ? e.target.closest("[data-pl-detail-collection]")
      : null;
  if (collection) {
    e.preventDefault();
    e.stopPropagation();
    togglePlaylistPanelCollection(
      collection.getAttribute("data-pl-detail-collection") === "1",
    );
    return;
  }
  var artist =
    e.target && e.target.closest
      ? e.target.closest("[data-pl-detail-artist]")
      : null;
  if (artist) {
    e.preventDefault();
    e.stopPropagation();
    openPlaylistPanelDetailArtist(
      Number(artist.getAttribute("data-pl-detail-artist")),
    );
    return;
  }
  var row =
    e.target && e.target.closest
      ? e.target.closest("[data-pl-detail-row]")
      : null;
  if (row) {
    e.preventDefault();
    e.stopPropagation();
    playPlaylistPanelDetailTrack(
      Number(row.getAttribute("data-pl-detail-row")),
    );
    return;
  }
