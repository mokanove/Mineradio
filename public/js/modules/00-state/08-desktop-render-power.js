...
function detectRuntimeHardwareProfile() {
  ...
  var cssPixels = Math.max(
    1,
    (Number(window.innerWidth) || 1) * (Number(window.innerHeight) || 1),
  );
  ...
  var lowSpec =
    lowCore || lowMemory || (cores > 0 && cores <= 6 && veryLargeSurface);
...
function refreshRuntimeHardwareSurfaceProfile() {
  ...
  runtimeHardwareProfile.lowSpec =
    runtimeHardwareProfile.lowCore ||
    runtimeHardwareProfile.lowMemory ||
    (runtimeHardwareProfile.cores > 0 &&
      runtimeHardwareProfile.cores <= 6 &&
      next.veryLargeSurface);
  runtimeHardwareProfile.balancedSpec =
    runtimeHardwareProfile.lowSpec ||
    (runtimeHardwareProfile.cores > 0 && runtimeHardwareProfile.cores <= 8) ||
    next.largeSurface;
  return runtimeHardwareProfile;
}
function performanceQualityRank() {
  var quality =
    typeof normalizePerformanceQuality === "function"
...
function currentPerformanceBackgroundMode() {
  return normalizePerformanceBackgroundMode(
    fx && fx.performanceBackground,
    fx && fx.liveBackgroundKeep === true,
  );
}
...
function updateRenderPowerClasses() {
  ...
  document.body.classList.toggle(
    "render-background-eco",
    isVisibleBackgroundMode(),
  );
}
...
function collectProtectedCoverUrls() {
  ...
  function mark(url) {
    if (url) keep[String(url)] = true;
  }
  try {
    var song =
      typeof currentCoverSong === "function"
        ? currentCoverSong()
        : playQueue && currentIdx >= 0
    ...
    if (
      ...
      mark(currentCoverSource.src);
    if (
      typeof playlistPanelDetailState !== "undefined" &&
  ...
  try {
    if (typeof beatMapSongKey === "function" && playQueue && playQueue.length) {
      ...
      for (var i = start; i <= end; i++)
        markProtectedKey(keep, beatMapSongKey(playQueue[i]));
    }
    if (typeof beatPrefetchLastKey !== "undefined")
      markProtectedKey(keep, beatPrefetchLastKey);
    if (typeof djMode !== "undefined" && djMode && djMode.songKey)
      markProtectedKey(keep, djMode.songKey);
    if (
      typeof localBeatAnalysis !== "undefined" &&
      localBeatAnalysis &&
      localBeatAnalysis.song &&
      typeof beatMapSongKey === "function"
    ) {
...
function collectProtectedCoverDepthIds() {
  ...
  try {
    ...
    if (
      ...
      candidates.push(currentCoverSource.src);
    var song =
      typeof currentCoverSong === "function" ? currentCoverSong() : null;
    ...
    candidates.forEach(function (src) {
      if (src)
        markProtectedKey(keep, coverDepthCacheId(src + "|tex=" + w + "x" + h));
    });
  } catch (e) {}
  return keep;
}
...
function trimCoverDepthCache(keep, protectedKeys) {
  ...
  var keys = coverDepthCacheKeys.filter(function (key) {
    return !!coverDepthCache[key];
  });
  if (keys.length <= keep) {
  ...
  Object.keys(protectedKeys || {}).forEach(function (key) {
    keepSet[key] = true;
  });
  var dropped = 0;
  ...
  coverDepthCacheKeys = keys.filter(function (key) {
    return !!coverDepthCache[key];
  });
  return dropped;
}
function collectRuntimePerfSnapshot(now) {
  ...
  runtimePerfState.cacheCounts = {
    ...
    stageLyricTrack:
      typeof stageLyricTrackCache !== "undefined" &&
      stageLyricTrackCache &&
      stageLyricTrackCache.entries
        ? stageLyricTrackCache.entries.length
        : 0,
  };
  if (
    performance &&
    performance.memory &&
    now - runtimePerfState.lastHeapSampleAt > 12000
  ) {
    runtimePerfState.lastHeapSampleAt = now;
    runtimePerfState.heapMB = Math.round(
      (performance.memory.usedJSHeapSize || 0) / 1048576,
    );
  }
  return {
    render:
      typeof renderPerfState !== "undefined"
        ? {
    ...
    gpu:
      ...
      (runtimeGpuDiagnosticsError
        ? { error: runtimeGpuDiagnosticsError }
        : null),
    ...
    renderer:
      typeof renderer !== "undefined" && renderer && renderer.info
        ? {
      ...
      typeof renderer !== "undefined" && renderer && renderer.domElement
        ? {
            ...
            renderPixelRatio: renderer.getPixelRatio
              ? Number(renderer.getPixelRatio().toFixed(3))
              : 0,
            ...
            targetFps:
              typeof getAdaptiveRenderFps === "function"
                ? getAdaptiveRenderFps(now)
                : 0,
            displayHz:
              typeof estimatedDisplayRefreshHz === "function"
                ? Math.round(estimatedDisplayRefreshHz() * 10) / 10
                : 0,
            adaptiveLoad:
              typeof adaptiveFrameLoadSnapshot === "function"
                ? adaptiveFrameLoadSnapshot()
                : null,
            foregroundFpsMode:
            ...
            interactionBoost:
              typeof isRenderInteractionActive === "function"
                ...
                : false,
            interactionReason:
              typeof renderInteractionReason !== "undefined"
                ? renderInteractionReason
                : "",
          }
        : null,
    frameGates:
      typeof collectFrameGateSnapshot === "function" &&
      typeof mainFrameGates !== "undefined"
        ? collectFrameGateSnapshot(mainFrameGates)
        : null,
    deepSleep: isDeepBackgroundMode(),
    probe:
      window.__mineradioPerf && window.__mineradioPerf.summary
...
function requestBackgroundAppMemoryTrim(reason, delayMs) {
  if (
    ...
    typeof window.desktopWindow.trimAppMemory !== "function"
  )
    return;
  ...
  if (
    backgroundAppMemoryTrimInFlight ||
    now - runtimePerfState.lastAppMemoryTrimAt < 30000
  )
    return;
  if (backgroundAppMemoryTrimTimer) clearTimeout(backgroundAppMemoryTrimTimer);
  backgroundAppMemoryTrimTimer = setTimeout(
    function () {
      ...
      if (
        ...
        typeof configureMemoryReductFromFx === "function"
      ) {
      ...
      window.desktopWindow
        .trimAppMemory({ reason: runtimePerfState.lastAppMemoryTrimReason })
        .then(function (result) {
          runtimePerfState.lastAppMemoryTrimResult = result || null;
        })
        .catch(function (error) {
          runtimePerfState.lastAppMemoryTrimResult = {
            ok: false,
            error: String(
...
function trimRuntimeCaches(reason, aggressive) {
  ...
  dropped += trimCoverDepthCache(
    aggressive ? 4 : 10,
    collectProtectedCoverDepthIds(),
  );
  dropped += trimObjectCache(
    beatMapCache,
    aggressive ? 12 : 36,
  ...
  runtimePerfState.lastCacheTrimReason =
    reason || (aggressive ? "deep" : "active");
...
function trimVisualCachesForBackground() {
  ...
  requestBackgroundAppMemoryTrim(
    "deep-background",
    isBackgroundReleaseMode() ? 900 : 1800,
  );
}
...
function applyRendererPowerMode() {
  ...
  if (
    ...
    Math.abs(renderPowerState.pixelRatio - pixelRatio) < 0.001
  )
    return;
  renderPowerState = {
    mode: mode,
  ...
  if (deep) {
    ...
    requestBackgroundAppMemoryTrim(
      "renderer-deep-sleep",
      isBackgroundReleaseMode() ? 900 : 2200,
    );
  }
}
function updateDesktopRuntimeState(state) {
  ...
  if (
    (desktopRuntimeState.minimized || !desktopRuntimeState.visible) &&
    typeof flushLyricLayoutSave === "function"
  ) {
    flushLyricLayoutSave();
  }
  if (fx && (fx.desktopLyrics || fx.wallpaperMode))
    setTimeout(syncDesktopOverlayState, 0);
  if (wasDeep && !isDeepBackgroundMode())
    recoverVisualsAfterBackground("desktop-runtime-state");
  if (desktopRuntimeState.fullscreen !== wasFullscreen)
    scheduleMainRendererViewportRefresh("desktop-runtime-state");
}
function installRenderPowerHooks() {
  updateRenderPowerClasses();
  if (
    window.desktopWindow &&
    typeof window.desktopWindow.getGpuDiagnostics === "function"
  ) {
    window.desktopWindow
      .getGpuDiagnostics()
      .then(function (info) {
      ...
      .catch(function (error) {
        runtimeGpuDiagnosticsError = String(
          (error && error.message) || error || "GPU_DIAGNOSTICS_FAILED",
        );
      });
  }
  document.addEventListener("visibilitychange", function () {
    ...
    if (!isDeepBackgroundMode())
      recoverVisualsAfterBackground("visibilitychange");
  });
