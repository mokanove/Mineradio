...
function readAudioFadePreference() {
  ...
  try {
    var raw =
      JSON.parse(localStorage.getItem(AUDIO_FADE_STORE_KEY) || "{}") || {};
    return {
...
function readDiyModePreference() {
  try {
    return localStorage.getItem(DIY_MODE_STORE_KEY) === "1";
  } catch (e) {
    return false;
  }
}
function saveDiyModePreference(on) {
  try {
    localStorage.setItem(DIY_MODE_STORE_KEY, on ? "1" : "0");
  } catch (e) {}
}
...
function readPlaylistPanelTabPreference() {
  try {
    return normalizePlaylistPanelTab(
      localStorage.getItem(PLAYLIST_PANEL_TAB_STORE_KEY) || "queue",
    );
  } catch (e) {
    return "queue";
  }
...
function savePlaylistPanelTabPreference(tab) {
  try {
    localStorage.setItem(
      PLAYLIST_PANEL_TAB_STORE_KEY,
      normalizePlaylistPanelTab(tab),
    );
  } catch (e) {}
}
...
function readCloseBehaviorPreference() {
  try {
    return normalizeCloseBehavior(
      localStorage.getItem(CLOSE_BEHAVIOR_STORE_KEY) || "exit",
    );
  } catch (e) {
    return "exit";
  }
...
function saveCloseBehaviorPreference(value) {
  try {
    localStorage.setItem(
      ...
      normalizeCloseBehavior(value),
    );
  } catch (e) {}
}
function syncCloseBehaviorUi() {
  document
    .querySelectorAll("#close-behavior-seg [data-close-behavior]")
    .forEach(function (btn) {
      btn.classList.toggle(
        "active",
        btn.getAttribute("data-close-behavior") === closeBehaviorPreference,
      );
    });
}
function setCloseBehaviorPreference(value, opts) {
  ...
  if (
    ...
    typeof window.desktopWindow.setCloseBehavior === "function"
  ) {
    window.desktopWindow
      .setCloseBehavior(closeBehaviorPreference)
      .catch(function (e) {
        console.warn("[CloseBehavior]", e);
      });
  }
  if (opts.toast)
    showToast(
      closeBehaviorPreference === "tray"
        ? "关闭按钮将放到后台托盘"
        : "关闭按钮将直接退出",
    );
}
  ...
  seg.addEventListener("click", function (e) {
    var btn =
      e.target && e.target.closest
        ? e.target.closest("[data-close-behavior]")
        : null;
    if (!btn) return;
    setCloseBehaviorPreference(btn.getAttribute("data-close-behavior"), {
      toast: true,
    });
  });
  syncCloseBehaviorUi();
...
function readStartupResumeModePreference() {
  try {
    return normalizeStartupResumeMode(
      localStorage.getItem(STARTUP_RESUME_MODE_STORE_KEY) || "resume",
    );
  } catch (e) {
    return "resume";
  }
...
function saveStartupResumeModePreference(value) {
  try {
    localStorage.setItem(
      STARTUP_RESUME_MODE_STORE_KEY,
      normalizeStartupResumeMode(value),
    );
  } catch (e) {}
}
...
function applyStartupResumeModeToRestoredSnapshot() {
  ...
  pendingPlaybackResumeAt = startupResumeSecondsFromSnapshot(
    restoredLastPlaybackSnapshot,
  );
  applyRestoredPlaybackProgressUi(
    Object.assign({}, restoredLastPlaybackSnapshot, {
      currentTime: pendingPlaybackResumeAt,
    }),
  );
}
function syncStartupResumeModeUi() {
  document
    .querySelectorAll("#startup-resume-mode-seg [data-startup-resume-mode]")
    .forEach(function (btn) {
      btn.classList.toggle(
        "active",
        btn.getAttribute("data-startup-resume-mode") ===
...
function setStartupResumeModePreference(value, opts) {
  ...
  if (opts.toast)
    showToast(
      startupResumeModePreference === "restart"
        ? "恢复播放将重播整首"
        : "恢复播放将按上次进度继续",
    );
}
function bindStartupResumeModeControls() {
  ...
  seg.addEventListener("click", function (e) {
    var btn =
      e.target && e.target.closest
        ? e.target.closest("[data-startup-resume-mode]")
        : null;
    if (!btn) return;
    setStartupResumeModePreference(
      btn.getAttribute("data-startup-resume-mode"),
      { toast: true },
    );
  });
...
function toggleStartupAutoplay() {
  ...
  showToast(
    startupAutoplayPreference ? "启动自动播放已开启" : "启动自动播放已关闭",
  );
...
function toggleFxFabAutoHide(e) {
  ...
  showToast(
    fxFabAutoHide ? "视觉控制台按钮已自动隐藏" : "视觉控制台按钮已固定显示",
  );
}
function updateFxFabAutoHideFromPointer(x, y) {
  ...
  var panelOpen = !!(
    panel &&
    (panel.classList.contains("peek") || panel.classList.contains("show"))
  );
  var nearBottomRight = x > innerWidth - 126 && y > innerHeight - 158;
  if (!nearBottomRight) fxFabAutoHideRevealArmed = true;
  document.body.classList.toggle(
    "fx-fab-peek",
    panelOpen || (nearBottomRight && fxFabAutoHideRevealArmed),
  );
}
function layoutFullscreenDiyZone() {
  ...
  var top = 24;
  var anchor =
    document.querySelector("#top-right .top-account-pill") ||
...
function shouldSuppressFullscreenDiyPeek() {
  ...
  var fxPanelOpen = !!(
    fxPanel &&
    (fxPanel.classList.contains("peek") || fxPanel.classList.contains("show"))
  );
...
function applyDiyMode(on, opts) {
  ...
  if (opts.toast)
    showToast(diyPlayerMode ? "DIY 玩家模式已开启" : "已切回简约模式");
    ...
    ["diy-mode-btn", "fullscreen-diy-btn"].forEach(function (id) {
      ...
      if (btn)
        window.gsap.fromTo(
          btn,
          { scale: 0.94 },
          { scale: 1, duration: 0.34, ease: "back.out(1.8)", overwrite: true },
        );
    });
  }
}
