...
function normalizeMemorySystemMask(mask) {
  var value =
    Math.round(Number(mask) || MEMORY_REDUCT_MASK_DEFAULT) &
    MEMORY_REDUCT_MASK_DEFAULT;
...
function ensureMemoryFxDefaults() {
  ...
  if (fx.memoryAutoTrimOnBackground !== false)
    fx.memoryAutoTrimOnBackground = true;
  ...
  fx.memorySystemIntervalMin = clampRange(
    Math.round(
  ...
  fx.memorySystemThresholdPercent = clampRange(
    Math.round(
  ...
  fx.memorySystemMask = normalizeMemorySystemMask(
...
function memoryAutoConfigPayload(runNow) {
  ...
  return {
    ...
    enabled: !!(
      ...
      fx.memoryAutoTrimOnBackground !== false
    ),
    mask: normalizeMemorySystemMask(fx && fx.memorySystemMask),
    intervalMin: Math.max(
      5,
      Math.round(Number(fx && fx.memorySystemIntervalMin) || 30),
    ),
    thresholdPercent: Math.max(
      50,
      Math.min(
        98,
        Math.round(Number(fx && fx.memorySystemThresholdPercent) || 78),
      ),
    ),
...
function rememberMemoryStatusPayload(payload) {
  ...
  if (
    payload.snapshot ||
    Object.prototype.hasOwnProperty.call(payload, "systemPurgeAvailable")
  ) {
    memoryLastStatusPayload = Object.assign(
      {},
      memoryLastStatusPayload || {},
      payload,
    );
    return;
  }
  if (memoryLastStatusPayload) {
    memoryLastStatusPayload.auto =
      payload.state || payload.auto || memoryLastStatusPayload.auto;
  }
}
...
function configureMemoryReductFromFx(reason, runNow) {
  if (
    ...
    typeof window.desktopWindow.configureMemoryReduct !== "function"
  )
    return Promise.resolve(null);
  return window.desktopWindow
    .configureMemoryReduct(memoryAutoConfigPayload(runNow))
...
function refreshMemorySnapshot(force) {
  if (
    !window.desktopWindow ||
    typeof window.desktopWindow.getMemorySnapshot !== "function"
  ) {
  ...
  return window.desktopWindow
    ...
    .then(function (payload) {
      ...
      var status =
        payload && payload.ok
          ? memoryFormatSnapshot(payload.snapshot)
          : "系统内存读取失败";
    ...
    .catch(function (error) {
      updateMemoryStatusText(
        "系统内存读取失败: " + String((error && error.message) || error || ""),
      );
      return null;
    });
}
...
function updateMemoryControls() {
  ...
  var systemAvailable =
    !memoryLastStatusPayload ||
    memoryLastStatusPayload.systemPurgeAvailable !== false;
  ...
  document
    .querySelectorAll("#memory-mask-seg [data-memory-mask]")
    .forEach(function (btn) {
      var bit =
        MEMORY_REDUCT_MASK_BITS[btn.getAttribute("data-memory-mask")] || 0;
      btn.classList.toggle(
        "active",
        !!(normalizeMemorySystemMask(fx.memorySystemMask) & bit),
      );
    });
...
function runAppMemoryTrim(reason) {
  ...
  window.desktopWindow
    .trimAppMemory({ reason: reason || "manual" })
    .then(function (payload) {
      if (payload && payload.ok && payload.after)
        updateMemoryStatusText(
          "播放器已压缩: " + memoryFormatSnapshot(payload.after),
        );
      else if (
        payload &&
        payload.skipped &&
        payload.reason === "foreground-visible"
      )
        updateMemoryStatusText(
          "前台可见时不压缩，避免操作卡顿；最小化/隐藏后自动处理",
        );
    ...
    .catch(function (error) {
      updateMemoryStatusText(
        "播放器压缩失败: " + String((error && error.message) || error || ""),
      );
    });
}
  ...
  window.desktopWindow
    ...
    .then(function (payload) {
      ...
      if (result && result.skipped && result.reason === "foreground-visible") {
        ...
        updateMemoryStatusText(
          autoElevate
            ? "提权释放未完成：可能取消了管理员权限或被系统拦截"
            : "当前权限只能完成部分释放；需要时可最小化后点提权释放",
        );
        showToast(
          autoElevate ? "提权释放未完成" : "需要管理员权限的部分已跳过",
        );
      } else {
        updateMemoryStatusText(
          "系统释放未完成: " +
            String(
              (result && result.message) || (payload && payload.error) || "",
            ),
        );
      }
      refreshMemorySnapshot(true);
    ...
    .catch(function (error) {
      updateMemoryStatusText(
        "系统释放失败: " + String((error && error.message) || error || ""),
      );
    });
}
...
function bindSystemMemoryControls() {
  ...
  [
    ...
    input.addEventListener("input", function () {
      fx[item[1]] = clampRange(
        Math.round(Number(input.value) || fxDefaults[item[1]]),
        item[2],
        item[3],
      );
