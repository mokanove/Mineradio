function buildPresetGrid() {
  ...
  grid.innerHTML = order
    .map(function (i) {
      ...
      return (
        '<div class="preset-card" data-preset="' +
        i +
        '" onclick="setPreset(' +
        i +
        ')">' +
...
function triggerPresetParticleTransition(fromPreset, toPreset) {
  ...
  presetTransition.duration = toPreset === 5 ? 0.3 : 0.24;
  ...
  var card = document.querySelector(
    '.preset-card[data-preset="' + toPreset + '"]',
  );
  if (card) {
...
function tickPresetTransition() {
  ...
  var raw =
    (uniforms.uTime.value - presetTransition.start) / presetTransition.duration;
  ...
  uniforms.uPointScale.value =
    fx.point * (1 + wave * (wallpaperFlow ? 0.016 : 0.048));
  if (raw >= 1) {
...
function setPreset(p, opts) {
  ...
  var changed = prev !== p;
  fx.preset = p;
  if (changed && prev === SKULL_PRESET_INDEX && p !== SKULL_PRESET_INDEX)
    clearSkullPresetResidue();
...
function syncFxUniforms() {
  ...
  if (bloomParticles)
    bloomParticles.visible = fx.bloom && fx.bloomStrength > 0.01;
  uniforms.uEdgeEnabled.value = fx.edge ? 1 : 0;
  if (uniforms.uTintColor)
    uniforms.uTintColor.value.set(
      normalizeHexColor(fx.visualTintColor || "#9db8cf"),
    );
  if (uniforms.uTintStrength)
