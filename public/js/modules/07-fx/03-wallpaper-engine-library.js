...
function flushWallpaperEnginePointerActivity() {
  wallpaperEnginePointerActivityTimer = 0;
  if (
    !wallpaperEnginePointerActivityHasPoint ||
    !wallpaperEnginePointerActivityReady()
  )
    return;
  var api = wallpaperEngineDesktopApi();
  if (!api || typeof api.reportWallpaperEnginePointerActivity !== "function")
    return;
  wallpaperEnginePointerActivityLastSentAt =
...
function rememberWallpaperEnginePointerPosition(event) {
  ...
  var xRatio = Math.max(
    0,
    Math.min(1, Number(event.clientX) / Math.max(1, width - 1)),
  );
  var yRatio = Math.max(
    0,
    Math.min(1, Number(event.clientY) / Math.max(1, height - 1)),
  );
...
function queueWallpaperEnginePointerActivity(event) {
  ...
  if (
    !wallpaperEnginePointerActivityHasPoint ||
    !wallpaperEnginePointerActivityReady() ||
    wallpaperEnginePointerActivityTimer
  )
    return;
  var now =
    typeof performance !== "undefined" && performance.now
  ...
  var delay = Math.max(
    0,
    WALLPAPER_ENGINE_POINTER_ACTIVITY_INTERVAL_MS -
      Math.max(0, now - wallpaperEnginePointerActivityLastSentAt),
  );
  wallpaperEnginePointerActivityTimer = setTimeout(
...
function cancelWallpaperEngineHostRecovery(resetAttempts) {
  if (wallpaperEngineHostRecoveryRetryTimer)
    clearTimeout(wallpaperEngineHostRecoveryRetryTimer);
...
function readWallpaperEngineIdSet(key) {
  try {
    ...
    return new Set(
      (Array.isArray(raw) ? raw : []).map(String).filter(function (id) {
...
var hiddenWallpaperEngineIds = readWallpaperEngineIdSet(
  WALLPAPER_ENGINE_HIDDEN_STORE_KEY,
);
var favoriteWallpaperEngineIds = readWallpaperEngineIdSet(
  WALLPAPER_ENGINE_FAVORITE_STORE_KEY,
);
...
function readWallpaperEngineSelection() {
  try {
    return normalizeWallpaperEngineSelection(
      JSON.parse(
        localStorage.getItem(WALLPAPER_ENGINE_SELECTION_STORE_KEY) || "{}",
      ),
    );
  } catch (e) {
    return normalizeWallpaperEngineSelection({});
...
function saveWallpaperEngineSelection() {
  try {
    localStorage.setItem(
      WALLPAPER_ENGINE_SELECTION_STORE_KEY,
      JSON.stringify(
        normalizeWallpaperEngineSelection(wallpaperEngineSelection),
      ),
    );
  } catch (e) {}
}
...
function wallpaperEngineUsesDesktopHostLifecycle() {
  var api = wallpaperEngineDesktopApi();
  return !!(
    api && typeof api.onWallpaperEngineHostBoundsChanged === "function"
  );
}
...
function normalizeWallpaperEngineProject(item) {
  ...
  var projectType =
    String(item.projectType || "unknown")
      .toLowerCase()
      .replace(/[^a-z0-9_-]/g, "")
      .slice(0, 32) || "unknown";
  var mediaType =
  ...
  return {
    id: id,
    title:
      String(item.title || "Wallpaper Engine")
        .replace(/[\u0000-\u001f\u007f]/g, " ")
        .replace(/\s+/g, " ")
        .trim()
        .slice(0, 160) || "Wallpaper Engine",
    ...
    audioPropertyCount: Math.max(
      0,
      Math.min(256, Number(item.audioPropertyCount) || 0),
    ),
    mutedAudioPropertyCount: Math.max(
      0,
      Math.min(256, Number(item.mutedAudioPropertyCount) || 0),
    ),
    updatedAt: Math.max(0, Number(item.updatedAt) || 0),
    safetyMode:
      item.safetyMode === "native-engine"
        ? "native-engine"
        : item.safetyMode === "direct-media"
          ? "direct-media"
          : "preview-only",
  };
}
...
function wallpaperEngineMediaUrl(item, kind) {
  ...
  return (
    ...
    "?v=" +
    encodeURIComponent(String(item.updatedAt || 0)) +
    "&token=" +
    encodeURIComponent(wallpaperEngineMediaToken)
  );
}
...
function wallpaperEngineProjectLabel(item) {
  ...
  if (item.projectType === "scene" && item.enginePlayable)
    return "Scene · Wallpaper Engine 原生实时运行";
  if (item.projectType === "scene")
    return "Scene · 预览（未找到有效 PKGV 场景包）";
  if (item.projectType === "web") return "Web · 安全预览（未执行 HTML）";
  if (item.projectType === "application")
    return "Application · 安全预览（未运行程序）";
  return "本地项目 · 安全预览";
}
...
function updateWallpaperEngineEntryUi(message) {
  ...
  if (value) {
    if (message) value.textContent = message;
    else if (active && wallpaperEngineRuntimeError)
      value.textContent = wallpaperEngineRuntimeError + " · 已显示原背景";
    else if (
      active &&
      wallpaperEngineSelection.kind === "engine" &&
      ...
      value.textContent =
        (wallpaperEngineSelection.title || "已选择") +
        (wallpaperEngineDesktopPreviewUsesAsset
          ? " · 桌面被动模式 · 项目预览"
          : " · 桌面被动模式 · 原背景");
    } else if (active && wallpaperEngineSelection.kind === "engine")
      value.textContent =
        (wallpaperEngineSelection.title || "已选择") + " · WE 引擎实时运行";
    else if (active)
      value.textContent =
        (wallpaperEngineSelection.title || "已选择") + " · 原背景保留";
...
function cancelWallpaperEngineVideoRetry() {
  if (wallpaperEngineVideoRetryTimer)
    clearTimeout(wallpaperEngineVideoRetryTimer);
...
function cancelWallpaperEngineFirstFrameWait() {
  ...
  if (
    wait.video &&
    wait.callbackId &&
    typeof wait.video.cancelVideoFrameCallback === "function"
  ) {
    try {
      wait.video.cancelVideoFrameCallback(wait.callbackId);
    } catch (e) {}
  }
  if (wait.video && wait.loadedDataHandler) {
    try {
      wait.video.removeEventListener("loadeddata", wait.loadedDataHandler);
    } catch (e2) {}
  }
  if (wait.raf1 && typeof cancelAnimationFrame === "function")
    cancelAnimationFrame(wait.raf1);
  if (wait.raf2 && typeof cancelAnimationFrame === "function")
    cancelAnimationFrame(wait.raf2);
}
...
function wallpaperEngineCaptureContentSize(runtime) {
  ...
  var host =
    runtime.hostWindowRect && typeof runtime.hostWindowRect === "object"
      ? runtime.hostWindowRect
      : null;
  var width = host
    ? Math.abs((Number(host.right) || 0) - (Number(host.left) || 0))
    : 0;
  var height = host
    ? Math.abs((Number(host.bottom) || 0) - (Number(host.top) || 0))
    : 0;
...
function captureWallpaperEngineFreezeFrame() {
  ...
  if (
    ...
    return false;
  if (wallpaperEngineFreezeReleaseTimer)
    clearTimeout(wallpaperEngineFreezeReleaseTimer);
  wallpaperEngineFreezeReleaseTimer = 0;
  ++wallpaperEngineFreezeGeneration;
  try {
    var freezeScale = Math.min(
      1,
      3840 / Math.max(1, video.videoWidth),
      2160 / Math.max(1, video.videoHeight),
    );
    ...
    var sourceWidth = Math.max(
      ...
      Math.min(
        video.videoWidth,
        Math.round(
          video.videoWidth / Math.max(1, wallpaperEngineCaptureViewportScaleX),
        ),
      ),
    );
    var sourceHeight = Math.max(
      ...
      Math.min(
        video.videoHeight,
        Math.round(
          video.videoHeight / Math.max(1, wallpaperEngineCaptureViewportScaleY),
        ),
      ),
    );
    canvas.width = width;
...
function clearWallpaperEngineFreezeFrame(immediate) {
  ...
  function releaseCanvas() {
    if (
      generation !== wallpaperEngineFreezeGeneration ||
      wallpaperEngineFreezeVisible ||
      !canvas
    )
      return;
    canvas.width = 1;
  ...
  if (immediate) releaseCanvas();
    ...
    wallpaperEngineFreezeReleaseTimer = setTimeout(function () {
      wallpaperEngineFreezeReleaseTimer = 0;
      releaseCanvas();
...
function storeWallpaperEnginePreparedGlassCaptureStream(sessionId, stream) {
  ...
  var entry = { stream: stream, timer: 0 };
  entry.timer = setTimeout(function () {
    if (wallpaperEnginePreparedGlassCaptureStreams.get(sessionId) !== entry)
      return;
...
function stopWallpaperEngineGlassCaptureStream(keepPreparedStreams) {
  ...
  if (wallpaperEngineGlassCaptureRetryTimer)
    clearTimeout(wallpaperEngineGlassCaptureRetryTimer);
...
async function hardenWallpaperEngineCaptureStream(stream) {
  ...
  audioTracks.forEach(function (track) {
    try {
      if (stream.removeTrack) stream.removeTrack(track);
    } catch (e) {}
    try {
      track.stop();
    } catch (e2) {}
  ...
  var cursorResults = await Promise.all(
    tracks.map(function (track) {
      if (!track || typeof track.applyConstraints !== "function")
        return Promise.resolve(false);
      return Promise.resolve(track.applyConstraints({ cursor: "never" }))
        .then(function () {
          try {
            var settings =
  ...
  try {
    stream.__mineradioCursorSuppressed =
      !!tracks.length &&
      cursorResults.every(function (value) {
        return value === true;
      });
  } catch (e4) {}
  return stream;
...
function wallpaperEngineCaptureFpsPreference() {
  ...
  var fixed =
    typeof foregroundFixedFpsForMode === "function"
      ...
      : /^(45|60|75|90|120)$/.test(String(mode))
        ? Number(mode)
        : 0;
  return Number(fixed) > 0
    ? Math.max(24, Math.min(240, Math.round(Number(fixed))))
    : 0;
}
...
async function syncWallpaperEngineCaptureFrameRate() {
  ...
  try {
    ...
    if (range && Number(range.min) > 0)
      target = Math.max(Number(range.min), target);
    if (range && Number(range.max) > 0)
      target = Math.min(Number(range.max), target);
  } catch (e) {}
  try {
    ...
    return {
      ...
      error: String(
        (error && (error.message || error.name)) ||
          error ||
          "FRAME_RATE_CONSTRAINT_FAILED",
      ).slice(0, 240),
    };
  }
}

window.__mineradioSyncWallpaperEngineCaptureFrameRate =
  syncWallpaperEngineCaptureFrameRate;
...
function stopWallpaperEngineNativeSession(sessionId) {
  ...
  if (stopAll || !sessionId || expected === wallpaperEngineNativeSessionId)
    wallpaperEngineNativeSessionId = "";
  if (!api || typeof api.stopWallpaperEngineScene !== "function")
    return Promise.resolve({ ok: true });
  return Promise.resolve(
    api.stopWallpaperEngineScene({ sessionId: expected, all: stopAll }),
  ).catch(function () {
    return { ok: false };
  });
}

async function openWallpaperEngineCaptureStream(
  sessionId,
  fps,
  sourceId,
  options,
  ...
  try {
    if (diagnostics.purpose === "dwm-glass")
      window.__mineradioWallpaperEngineGlassCaptureDiagnostics = diagnostics;
  ...
  function recordAttempt(path, stream, error) {
    ...
    var entry = {
      ...
      constraints: null,
      error: error
        ? String((error && (error.message || error.name)) || error).slice(
            0,
            500,
          )
        : "",
    ...
    try {
      entry.constraints =
        track && typeof track.getConstraints === "function"
  ...
  if (
    ...
    try {
      ...
      if (
        ...
        var hardenedSourceStream =
          await hardenWallpaperEngineCaptureStream(sourceStream);
        try {
          hardenedSourceStream.__mineradioCapturePath = "source-id-media";
        } catch (e4) {}
        var sourceAttempt = recordAttempt(
          "source-id-media",
          hardenedSourceStream,
          null,
        );
        if (
          sourceAttempt.cursorSuppressed ||
          options.trustedCursorFreeSurface === true ||
          window.__mineradioAllowUnverifiedSourceCapture === true
        ) {
          try {
            hardenedSourceStream.__mineradioUnverifiedCursorCapture =
              !sourceAttempt.cursorSuppressed;
          } catch (e5) {}
        ...
        sourceError = new Error(
          "WALLPAPER_CAPTURE_CURSOR_SUPPRESSION_UNVERIFIED",
        );
      } else {
  ...
  if (
    ...
    try {
      ...
      if (
        ...
        var hardenedDisplayStream =
          await hardenWallpaperEngineCaptureStream(displayStream);
        try {
          hardenedDisplayStream.__mineradioCapturePath = "display-media";
        } catch (e6) {}
  ...
  var sourceMessage = String(
    (sourceError && (sourceError.message || sourceError.name)) ||
      "source-id-media unavailable",
  );
  throw new Error(
    "WALLPAPER_CAPTURE_FAILED: " +
      displayName +
      ": " +
      displayMessage +
      " (source: " +
...
window.__mineradioPrepareWallpaperEngineCapture = async function (
  ...
  try {
    var stream = await openWallpaperEngineCaptureStream(
      sessionId,
      fps,
      sourceId,
    );
...
window.__mineradioPrepareWallpaperEngineHostBoundsChange = function (
  ...
  var suspendingHost =
    /(?:^|[-_])(hidden|hide|minimize|minimized|tray|document-hidden)(?:$|[-_])/i.test(
      reason,
    ) ||
  ...
  if (wallpaperEngineHostBoundsPreparing) {
    return {
      ...
      error: wallpaperEngineFreezeVisible
        ? ""
  ...
  if (!frozen) {
    return {
      ok: false,
      frozen: false,
      error: "WALLPAPER_BOUNDS_FREEZE_UNAVAILABLE",
    };
  }
  wallpaperEngineHostBoundsPreparing = true;
...
window.__mineradioPrepareWallpaperEngineDesktopPreview = function (
  sessionId,
  reason,
) {
  sessionId = String(sessionId || "");
  ...
  return new Promise(function (resolve) {
    ...
    image.onload = function () {
      if (
        token !== wallpaperEngineLayerToken ||
        !wallpaperEngineSelection.active ||
        wallpaperEngineSelection.kind !== "engine" ||
    ...
    image.onerror = function () {
      fallback("WALLPAPER_DESKTOP_PREVIEW_LOAD_FAILED");
    };
    timer = setTimeout(function () {
      fallback("WALLPAPER_DESKTOP_PREVIEW_LOAD_TIMEOUT");
    }, 5000);
...
function reportWallpaperEngineCaptureResult(sessionId, ok) {
  var api = wallpaperEngineDesktopApi();
  if (!api || typeof api.reportWallpaperEngineCaptureResult !== "function")
    return Promise.resolve({ ok: false });
  return Promise.resolve(
    api.reportWallpaperEngineCaptureResult({
      sessionId: String(sessionId || ""),
      ok: ok === true,
    }),
  ).catch(function () {
...
function waitForWallpaperEngineVideoFirstFrame(
  video,
  item,
  token,
  sessionId,
  ...
  function firstFrameReady() {
    ...
    if (
      !wallpaperEngineNativeStartIsCurrent(item, token) ||
      wallpaperEngineNativeSessionId !== sessionId ||
      wallpaperEngineCaptureStream !== video.srcObject
    )
      return;
    reportWallpaperEngineCaptureResult(sessionId, true).then(
      function (acknowledgement) {
        ...
        if (
          ...
          wallpaperEngineRuntimeError = String(
            (acknowledgement && acknowledgement.error) ||
              "WALLPAPER_CAPTURE_CONFIRMATION_FAILED",
          );
  ...
  wait.timer = setTimeout(
    firstFrameTimedOut,
    WALLPAPER_ENGINE_FIRST_FRAME_TIMEOUT_MS,
  );
  if (typeof video.requestVideoFrameCallback === "function") {
    wait.callbackId = video.requestVideoFrameCallback(function () {
      firstFrameReady();
    });
  } else {
    wait.loadedDataHandler = function () {
      ...
      wait.raf1 = requestAnimationFrame(function () {
        wait.raf2 = requestAnimationFrame(function () {
          firstFrameReady();
        });
      });
    };
    video.addEventListener("loadeddata", wait.loadedDataHandler, {
      once: true,
    });
  }
}
...
function wallpaperEngineGlassSamplerIsCurrent(
  sessionId,
  layerToken,
  captureToken,
) {
  return (
    captureToken === wallpaperEngineGlassCaptureToken &&
...
function sampleWallpaperEngineGlassSamplerPixels(video) {
  ...
  try {
    ...
    for (
      var sourceIndex = 0, targetIndex = 0;
...
function wallpaperEngineGlassSamplerPixelDifference(first, second) {
  if (!first || !second || first.length !== second.length || !first.length)
    return 0;
  var difference = 0;
...
function waitForWallpaperEngineGlassSamplerPixelChange(
  video,
  stream,
  baseline,
  timeoutMs,
  ...
  return new Promise(function (resolve) {
    ...
    function poll() {
      ...
      var difference = wallpaperEngineGlassSamplerPixelDifference(
        baseline,
        current,
      );
...
function scheduleWallpaperEngineGlassSamplerCapture(
  sessionId,
  layerToken,
  attempt,
) {
  ...
  if (
    ...
    return false;
  wallpaperEngineGlassCaptureRetryTimer = setTimeout(
    function () {
      wallpaperEngineGlassCaptureRetryTimer = 0;
...
async function ensureWallpaperEngineGlassSamplerCapture(
  sessionId,
  layerToken,
  attempt,
) {
  ...
  if (
    !api ||
    typeof api.prepareWallpaperEngineGlassCapture !== "function" ||
    !video ||
    ...
    return false;
  var activeTrack =
    wallpaperEngineGlassCaptureStream &&
    wallpaperEngineGlassCaptureStream.getVideoTracks
      ? wallpaperEngineGlassCaptureStream.getVideoTracks()[0]
      : null;
  if (
    activeTrack &&
  ...
  try {
    ...
    var frameReady = await waitForWallpaperEngineGlassSamplerFrame(
      video,
      stream,
      5000,
    );
    if (
      !frameReady ||
      !wallpaperEngineGlassSamplerIsCurrent(
        sessionId,
        layerToken,
        captureToken,
      ) ||
    ...
    var activated = await api.activateWallpaperEngineDwmSurface({
      sessionId: sessionId,
    });
    if (
      !activated ||
      ...
      throw new Error(
        (activated && activated.error) || "WALLPAPER_ENGINE_DWM_SURFACE_FAILED",
      );
    }
    ...
    var livePixels = await waitForWallpaperEngineGlassSamplerPixelChange(
      video,
      stream,
      primingPixels,
      3600,
      ...
      try {
        window.__mineradioWallpaperEngineGlassSamplerState = {
          ...
          error: String(
            (error && (error.message || error.name)) ||
              error ||
              "WALLPAPER_GLASS_CAPTURE_FAILED",
          ).slice(0, 500),
        };
      } catch (e3) {}
      stopWallpaperEngineGlassCaptureStream(false);
      scheduleWallpaperEngineGlassSamplerCapture(
        sessionId,
        layerToken,
        attempt + 1,
      );
...
async function startWallpaperEngineNativeBackground(item, token) {
  var api = wallpaperEngineDesktopApi();
  if (!api || typeof api.startWallpaperEngineScene !== "function")
    throw new Error("WALLPAPER_ENGINE_RUNTIME_UNAVAILABLE");
  ...
  var result = await api.startWallpaperEngineScene({
    ...
    height: Math.max(
      360,
      Math.min(2160, Math.round(window.innerHeight || 1080)),
    ),
  ...
  if (!result || result.ok === false) {
    ...
    throw new Error(
      (result && result.error) || "WALLPAPER_ENGINE_SCENE_START_FAILED",
    );
  }
  var sessionId = String(result.sessionId || "");
  if (!/^[a-f0-9]{24}$/i.test(sessionId))
    throw new Error("WALLPAPER_ENGINE_SESSION_INVALID");
  if (!wallpaperEngineNativeStartIsCurrent(item, token)) {
    stopWallpaperEnginePreparedCaptureStreams(sessionId);
  ...
  if (result.captureMode === "dwm-thumbnail") {
    ...
    wallpaperEngineCaptureMode = "dwm-thumbnail";
    wallpaperEngineNativeSessionId = sessionId;
    var dwmAcknowledgement = await reportWallpaperEngineCaptureResult(
      sessionId,
      true,
    );
    if (
    ...
    if (
      ...
      wallpaperEngineRuntimeError = String(
        (dwmAcknowledgement && dwmAcknowledgement.error) ||
  ...
  if (!stream) {
    ...
    await stopWallpaperEngineNativeSession(sessionId);
    throw new Error(
      result.captureError || "WALLPAPER_CAPTURE_PREPARED_STREAM_MISSING",
    );
  }
  if (!wallpaperEngineNativeStartIsCurrent(item, token)) {
    stopWallpaperEngineMediaStream(stream);
  ...
  wallpaperEngineCaptureMode =
    result.captureMode === "main-prepared"
      ? "main-prepared"
      : "renderer-prepared";
  wallpaperEngineNativeSessionId = sessionId;
  video.muted = true;
...
function wallpaperEnginePlayWasInterrupted(error) {
  ...
  return (
    name === "AbortError" ||
    /interrupted|pause\(\)|new load request/i.test(message)
  );
}

function wallpaperEngineRuntimeErrorText(error) {
  var code = String((error && (error.code || error.message)) || error || "");
  if (/WALLPAPER_ENGINE_HOST_ELEVATED/.test(code))
    return "Mineradio 正以管理员身份运行，无法捕获 WE 实时窗口；请取消“以管理员身份运行”后重启播放器";
  if (/WALLPAPER_ENGINE_NOT_INSTALLED/.test(code))
    return "未找到 Wallpaper Engine 本体";
  if (/WALLPAPER_ENGINE_SIGNATURE_INVALID/.test(code))
    return "Wallpaper Engine 运行时签名无效";
  if (/WALLPAPER_ENGINE_WINDOW_CLOSE_FAILED/.test(code))
    return "上一次 Mineradio 实时壁纸窗口仍在收尾，请稍后重试；Wallpaper Engine 本体会保留";
  if (
    /WALLPAPER_ENGINE_DWM_SURFACE_FAILED|WALLPAPER_ENGINE_PARALLAX_RELAY_FAILED/.test(
      code,
    )
  )
    return "WE 原生鼠标视差连接失败，本次会话已关闭；请再次点击重连";
  if (/WALLPAPER_ENGINE_CONTROL_FAILED/.test(code))
    return "WE 场景控制暂时未就绪，请稍后重试";
  if (/WALLPAPER_ENGINE_WINDOW_TIMEOUT/.test(code))
    return "WE 场景窗口启动超时";
  if (
    /WALLPAPER_ENGINE_CAPTURE_UNAVAILABLE|WALLPAPER_CAPTURE_UNSUPPORTED/.test(
      code,
    )
  )
    return "当前系统不支持实时窗口捕获";
  if (/InvalidStateError/.test(code))
    return "WE 实时画面连接需要 Mineradio 保持在前台";
  if (/NotAllowedError|Permission denied|PermissionDismissed/i.test(code))
    return "WE 实时画面捕获权限被拒绝";
  if (/NotReadableError/.test(code))
    return "WE 实时捕获通道暂时忙，已清理本次会话；请再次点击重连";
  if (/WALLPAPER_ENGINE_REFRESH_SUPERSEDED/.test(code))
    return "WE 实时窗口正在切换，请重试";
  if (/WALLPAPER_CAPTURE_PREPARE_TIMEOUT/.test(code))
    return "WE 实时画面连接超时";
  if (
    /WALLPAPER_CAPTURE_PREPARE_HANDLER_MISSING|WALLPAPER_CAPTURE_PREPARED_STREAM_MISSING/.test(
      code,
    )
  )
    return "WE 实时画面连接尚未准备完成";
  if (/WALLPAPER_CAPTURE_FAILED|WALLPAPER_CAPTURE_STREAM_EMPTY/.test(code))
    return "WE 实时画面连接失败";
  if (/WALLPAPER_SCENE_PACKAGE_INVALID/.test(code))
    return "所选 .pkg/.pak 不是有效的 Wallpaper Engine PKGV 场景包";
  if (/WALLPAPER_SCENE_MANIFEST_INVALID/.test(code))
    return "该场景缺少有效的 project.json";
  if (/WALLPAPER_SCENE_NOT_FOUND/.test(code))
    return "没有找到该项目的有效场景包";
  return "WE 引擎运行失败";
}

function requestWallpaperEngineVideoPlayback(
  video,
  item,
  kind,
  token,
  ...
  try {
    ...
    handleWallpaperEngineVideoPlayFailure(
      error,
      video,
      item,
      kind,
...
function handleWallpaperEngineVideoPlayFailure(
  error,
  video,
  item,
  kind,
  ...
  if (token !== wallpaperEngineLayerToken || !wallpaperEngineSelection.active)
    return;
  var hostUnavailable =
    kind === "engine"
      ? wallpaperEngineNativeHostUnavailable()
  ...
  wallpaperEngineVideoRetryTimer = setTimeout(function () {
    wallpaperEngineVideoRetryTimer = 0;
    requestWallpaperEngineVideoPlayback(
      video,
      item,
      kind,
      token,
...
function restoreOriginalBackgroundAfterWallpaperEngine() {
  document.body.classList.remove(
    "wallpaper-engine-active",
    "wallpaper-engine-dwm-active",
  );
  stopWallpaperEngineGlassCaptureStream(false);
  if (typeof syncWallpaperEngineControlGlassSurface === "function") {
    syncWallpaperEngineControlGlassSurface(true);
...
function wallpaperEngineLayerReady(kind, token) {
  ...
  if (kind !== "dwm") {
    stopWallpaperEngineGlassCaptureStream(false);
    layer.classList.add(
      kind === "video" ? "video-ready" : "image-ready",
      "ready",
    );
    if (kind === "video" && wallpaperEngineSelection.kind === "engine")
      layer.classList.add("engine-ready");
    if (kind === "video" && wallpaperEngineSelection.kind === "engine")
      queueWallpaperEnginePointerActivity();
  }
  document.body.classList.add("wallpaper-engine-active");
  if (
    kind === "dwm" &&
    typeof animateWallpaperEngineControlGlassSurface === "function"
  ) {
...
function wallpaperEngineLayerFailed(item, attemptedKind, token) {
  ...
  if (attemptedKind === "engine") {
    ...
    if (/^[a-f0-9]{24}$/i.test(failedSessionId))
      reportWallpaperEngineCaptureResult(failedSessionId, false);
    stopWallpaperEngineCaptureStream();
    nativeStopPromise = stopWallpaperEngineNativeSession();
    if (
      wallpaperEngineHostRecoveryInFlight &&
      wallpaperEngineHostRecoveryAttempt <
        WALLPAPER_ENGINE_HOST_RECOVERY_MAX_ATTEMPTS &&
      wallpaperEngineDesktopHostIsVisible()
    ) {
      wallpaperEngineHostBoundsPreparing = true;
      wallpaperEngineRuntimeError = "";
      ...
      clearWallpaperEngineLayerMedia(0);
      updateWallpaperEngineEntryUi(
        "正在恢复 " + ((item && item.title) || "Wallpaper Engine") + "…",
      );
      Promise.resolve(nativeStopPromise).finally(function () {
        if (
          !wallpaperEngineHostRecoveryInFlight ||
          wallpaperEngineHostRecoveryRetryTimer
        )
          return;
  ...
  if (
    (attemptedKind === "media" || attemptedKind === "engine") &&
    item &&
    item.hasPreview
  ) {
    ...
    showToast(
      attemptedKind === "engine"
        ? (wallpaperEngineRuntimeError || "Wallpaper Engine 实时运行失败") +
            "，已切换到项目预览；再次点击可重试"
        : "动态媒体解码失败，已切换到安全预览",
    );
  ...
  wallpaperEngineRuntimeError =
    attemptedKind === "engine" ? "WE 引擎运行失败" : "媒体不可用";
  restoreOriginalBackgroundAfterWallpaperEngine();
  clearWallpaperEngineLayerMedia(0);
  updateWallpaperEngineEntryUi();
...
function applyWallpaperEngineBackground(item, quiet) {
  ...
  var kind =
    wallpaperEngineSelection.kind === "engine" && item.enginePlayable
      ? "engine"
      : wallpaperEngineSelection.kind === "media" && item.playable
        ? "media"
        : "preview";
  ...
  var image = document.getElementById("wallpaper-engine-image");
  var video = document.getElementById("wallpaper-engine-video");
  var preserveOutgoingFrame = !!(
    layer &&
    (layer.classList.contains("ready") || wallpaperEngineSwitchTimer)
  );
  cancelWallpaperEngineSwitchTimer();
  var token = ++wallpaperEngineLayerToken;
  ...
  function beginWallpaperEngineMediaLoad() {
    ...
    if (kind === "engine") {
      startWallpaperEngineNativeBackground(item, token).catch(function (error) {
        ...
        if (
          wallpaperEngineNativeHostUnavailable() ||
          /WALLPAPER_ENGINE_START_SUPERSEDED/.test(
      ...
      video.playsInline = true;
      if (item.hasPreview)
        video.poster = wallpaperEngineMediaUrl(item, "preview");
      ...
      image.src = wallpaperEngineMediaUrl(
        item,
        kind === "media" ? "media" : "preview",
      );
    }
  }
  ...
  if (!quiet)
    showToast(
      kind === "engine"
        ? "正在用 Wallpaper Engine 原生引擎载入 Scene…"
        : kind === "media"
          ? "Wallpaper Engine 壁纸已启用"
          : "已启用安全预览，原背景仍保留",
    );
  return true;
}
...
function restartWallpaperEngineAfterHostBoundsChange() {
  ...
  wallpaperEngineRuntimeError = "";
  updateWallpaperEngineEntryUi(
    "正在恢复 " + (item.title || "Wallpaper Engine") + "…",
  );
  startWallpaperEngineNativeBackground(item, token).catch(function (error) {
    if (token !== wallpaperEngineLayerToken) return;
    if (
...
function handleWallpaperEngineHostBoundsChange(payload) {
  ...
  if (phase === "restart") {
    ...
    if (document.hidden && !(payload && payload.forceVisibleHost === true))
      return;
    wallpaperEngineDesktopPreviewActive = false;
    wallpaperEngineDesktopPreviewUsesAsset = false;
    wallpaperEngineHostBoundsPreparing = false;
  ...
  if (
    phase === "prepare" &&
    typeof window.__mineradioPrepareWallpaperEngineHostBoundsChange ===
      "function"
  ) {
    window.__mineradioPrepareWallpaperEngineHostBoundsChange(
...
function wallpaperEngineFilteredProjects() {
  ...
  var query = String((search && search.value) || "")
    .trim()
    .toLowerCase();
  return wallpaperEngineProjects
    ...
    .sort(function (a, b) {
      var activeA =
        wallpaperEngineSelection.active && wallpaperEngineSelection.id === a.id
          ? 1
          : 0;
      var activeB =
        wallpaperEngineSelection.active && wallpaperEngineSelection.id === b.id
          ? 1
          : 0;
...
function disconnectWallpaperEnginePreviewObserver() {
  if (wallpaperEnginePreviewObserver)
    wallpaperEnginePreviewObserver.disconnect();
...
function loadWallpaperEnginePreviewsNearViewport() {
  ...
  grid.querySelectorAll("img[data-src]").forEach(function (image) {
    ...
    var nearby =
      rect.bottom >= viewport.top - 220 && rect.top <= viewport.bottom + 220;
    if (nearby) {
...
function extendWallpaperEngineLibraryNearEnd() {
  var grid = document.getElementById("wallpaper-engine-grid");
  if (!grid || !grid.querySelector('[data-wallpaper-action="load-more"]'))
    return;
...
function observeWallpaperEnginePreviews() {
  ...
  wallpaperEnginePreviewObserver = new IntersectionObserver(
    function (entries) {
...
function renderWallpaperEngineManualRoots() {
  ...
  var roots =
    ...
    Array.isArray(wallpaperEngineLibrarySnapshot.manualRoots)
      ? wallpaperEngineLibrarySnapshot.manualRoots
      : [];
  host.innerHTML = roots
    .map(function (root) {
      return (
        '<span class="wallpaper-engine-root-chip"><span title="手动导入目录">' +
        escHtml(root.name || "导入目录") +
        "</span>" +
        '<button type="button" data-wallpaper-action="remove-root" data-root-id="' +
        escHtml(root.id || "") +
        '" title="移除此索引目录">×</button></span>'
      );
    })
    .join("");
}
...
function renderWallpaperEngineLibrary(preserveRenderLimit) {
  ...
  if (!preserveRenderLimit)
    wallpaperEngineRenderLimit = WALLPAPER_ENGINE_RENDER_BATCH;
  disconnectWallpaperEnginePreviewObserver();
  if (wallpaperEngineLibraryBusy) {
    grid.innerHTML =
      '<div class="wallpaper-engine-empty">正在读取 project.json 元数据，不扫描 94GB 素材文件…</div>';
    return;
  }
  var items = wallpaperEngineFilteredProjects();
  if (!items.length) {
    grid.innerHTML =
      '<div class="wallpaper-engine-empty">' +
      (wallpaperEngineProjects.length
        ? "没有符合筛选条件的壁纸"
        : "没有识别到 Wallpaper Engine 项目<br>可以点击“导入目录”手动选择项目或素材库") +
      "</div>";
    return;
  }
  ...
  grid.innerHTML =
    visibleItems
      .map(function (item) {
        ...
        var active =
          wallpaperEngineSelection.active &&
          wallpaperEngineSelection.id === item.id;
        var preview = item.hasPreview
          ? wallpaperEngineMediaUrl(item, "preview")
          : "";
        return (
          '<article class="wallpaper-engine-card' +
          (favorite ? " favorite" : "") +
          (active ? " active" : "") +
          '" tabindex="0" role="button" data-wallpaper-id="' +
          item.id +
          '">' +
          (preview
            ? '<img class="wallpaper-engine-card-preview" data-src="' +
              escHtml(preview) +
              '" data-animated="' +
              (item.previewAnimated ? "1" : "0") +
              '" alt="" loading="lazy" decoding="async">'
            : '<div class="wallpaper-engine-card-placeholder"></div>') +
          '<button class="wallpaper-engine-card-star' +
          (favorite ? " active" : "") +
          '" type="button" data-wallpaper-action="favorite" data-wallpaper-id="' +
          item.id +
          '" title="' +
          (favorite ? "取消星标" : "星标并置顶") +
          '">' +
          (favorite ? "★" : "☆") +
          "</button>" +
          '<button class="wallpaper-engine-card-settings" type="button" data-wallpaper-action="details" data-wallpaper-id="' +
          item.id +
          '" title="读取项目设置">⚙</button>' +
          '<button class="wallpaper-engine-card-hide" type="button" data-wallpaper-action="hide" data-wallpaper-id="' +
          item.id +
          '" title="从列表隐藏">×</button>' +
          '<div class="wallpaper-engine-card-meta">' +
          escHtml(item.title) +
          "<small>" +
          escHtml(wallpaperEngineProjectLabel(item)) +
          "</small></div>" +
          "</article>"
        );
      })
    ...
    (visibleItems.length < items.length
      ? '<button type="button" class="wallpaper-engine-load-more" data-wallpaper-action="load-more">继续加载 ' +
        visibleItems.length +
        " / " +
        items.length +
        "</button>"
...
function normalizeWallpaperEngineProjectDetails(details) {
  ...
  var id = String(details.id || "")
    ...
    .slice(0, 24);
  if (id.length !== 24) return null;
  var properties = Array.isArray(details.properties)
    ? details.properties
        .slice(0, 256)
        .map(function (property) {
          ...
          if (typeof value === "string")
            value = value.replace(/[\u0000-\u001f\u007f]/g, " ").slice(0, 512);
          var options = Array.isArray(property.options)
            ? property.options
                .slice(0, 64)
                .map(function (option) {
  ...
  return {
    id: id,
    title:
      String(details.title || "Wallpaper Engine")
        .replace(/[\u0000-\u001f\u007f]/g, " ")
        .replace(/\s+/g, " ")
        .trim()
        .slice(0, 160) || "Wallpaper Engine",
    projectType:
      String(details.projectType || "unknown")
        .replace(/[^a-z0-9_-]/gi, "")
        .slice(0, 32) || "unknown",
    workshopId: String(details.workshopId || "")
      .replace(/\D/g, "")
      .slice(0, 32),
    propertyCount: Math.max(
      0,
      Math.min(256, Number(details.propertyCount) || properties.length),
    ),
    audioPropertyCount: Math.max(
      0,
      Math.min(256, Number(details.audioPropertyCount) || 0),
    ),
    mutedAudioPropertyCount: Math.max(
      0,
      Math.min(256, Number(details.mutedAudioPropertyCount) || 0),
    ),
...
function wallpaperEnginePropertyValueLabel(property) {
  if (property.options && property.options.length) {
    var selected = property.options.find(function (option) {
      return String(option.value) === String(property.value);
    });
  ...
  if (typeof property.value === "number")
    return String(Math.round(property.value * 1000) / 1000);
...
function renderWallpaperEngineProjectDetails(details, error) {
  ...
  var properties = document.getElementById(
    "wallpaper-engine-details-properties",
  );
  var weButton = document.getElementById("wallpaper-engine-details-we");
  var workshopButton = document.getElementById(
    "wallpaper-engine-details-workshop",
  );
  ...
  if (error) {
    ...
    properties.innerHTML =
      '<div class="wallpaper-engine-details-empty">无法读取此项目的 project.json 设置。</div>';
  ...
  if (!details) {
    ...
    properties.innerHTML =
      '<div class="wallpaper-engine-details-empty">读取中…</div>';
    if (weButton) weButton.disabled = true;
    if (workshopButton) workshopButton.disabled = true;
    return;
  }
  ...
  summary.textContent =
    "已读取 " +
    details.propertyCount +
    " 项设置 · 检测到 " +
    details.audioPropertyCount +
    " 项音频控制 · 每次加载自动静音 " +
    details.mutedAudioPropertyCount +
    " 项";
  properties.innerHTML = details.properties.length
    ? details.properties
        .map(function (property) {
          var badge = property.audio
            ? '<span class="wallpaper-engine-property-badge' +
              (property.autoMuted ? "" : " warning") +
              '">' +
              (property.autoMuted ? "加载时静音" : "音频相关") +
              "</span>"
            : "";
          return (
            '<div class="wallpaper-engine-property-row">' +
            '<div class="wallpaper-engine-property-copy"><strong>' +
            escHtml(property.label) +
            "</strong><small>" +
            escHtml(property.key + " · " + property.type) +
            "</small></div>" +
            badge +
            '<span class="wallpaper-engine-property-value">' +
            escHtml(wallpaperEnginePropertyValueLabel(property)) +
            "</span></div>"
          );
        })
        .join("")
    : '<div class="wallpaper-engine-details-empty">这个项目没有声明可调整的用户属性。</div>';
...
async function showWallpaperEngineProjectDetails(id) {
  ...
  if (!api || typeof api.getWallpaperEngineProjectDetails !== "function") {
    renderWallpaperEngineProjectDetails(
      null,
      "当前环境不支持读取 Wallpaper Engine 项目设置",
    );
    return;
  }
  try {
    ...
    if (!response || response.ok === false)
      throw new Error((response && response.error) || "读取失败");
    ...
    if (wallpaperEngineProjectDetailsId === id)
      renderWallpaperEngineProjectDetails(null, error.message || "读取失败");
  }
}
...
async function launchWallpaperEngineProjectDetails(target) {
  ...
  if (
    !id ||
    !api ||
    typeof api.openWallpaperEngineProjectDetails !== "function"
  )
    return;
  try {
    var response = await api.openWallpaperEngineProjectDetails(
      id,
      target === "workshop" ? "workshop" : "we",
    );
    if (!response || response.ok === false)
      throw new Error((response && response.error) || "打开失败");
    if (response.opened === "wallpaper-engine")
      showToast("已在 Wallpaper Engine 中定位此壁纸；可打开项目设置栏调整");
    else if (response.fallback)
      showToast("当前 WE 版本无法直接定位，已打开创意工坊详情");
    else showToast("已打开创意工坊详情");
  } catch (error) {
    showToast(
      error.message === "WALLPAPER_ENGINE_WORKSHOP_DETAILS_UNAVAILABLE"
...
function updateWallpaperEngineLibraryStatus(snapshot, error) {
  ...
  if (wallpaperEngineLibraryBusy) {
    ...
    var runtimeText =
      snapshot.runtime && snapshot.runtime.available === false
        ? " · 未找到可用的 Wallpaper Engine 本体"
        : "";
    status.textContent =
      "已识别 " +
      (snapshot.count || 0) +
      " 个项目 · " +
      (snapshot.dynamicCount || 0) +
      " 个媒体动态 · " +
      (snapshot.enginePlayableCount || 0) +
      " 个 Scene 原生运行 · " +
      (snapshot.previewOnlyCount || 0) +
      " 个安全预览 · 用时 " +
...
function consumeWallpaperEngineSnapshot(snapshot) {
  ...
  wallpaperEngineMediaToken = /^[a-f0-9]{48}$/i.test(
    String((snapshot && snapshot.mediaToken) || ""),
  )
    ? String(snapshot.mediaToken).toLowerCase()
    : "";
  wallpaperEngineProjects =
    snapshot && Array.isArray(snapshot.projects)
  ...
  if (wallpaperEngineSelection.active) {
    ...
    if (selected) {
      wallpaperEngineSelection = normalizeWallpaperEngineSelection(
        Object.assign({}, wallpaperEngineSelection, {
          title: selected.title,
          kind:
            wallpaperEngineSelection.kind === "engine" &&
            !selected.enginePlayable
              ? selected.playable
                ? "media"
                : "preview"
              : wallpaperEngineSelection.kind,
          mediaType:
            wallpaperEngineSelection.kind === "engine" &&
            selected.enginePlayable
              ? "video"
              : wallpaperEngineSelection.kind === "media"
                ? selected.mediaType
                : "image",
...
async function loadWallpaperEngineLibrary(force, showNotice) {
  ...
  renderWallpaperEngineLibrary();
  try {
    var snapshot = await api.listWallpaperEngineProjects({
      force: force === true,
    });
    if (!snapshot || snapshot.ok === false)
      throw new Error((snapshot && snapshot.error) || "扫描失败");
    consumeWallpaperEngineSnapshot(snapshot);
    if (showNotice)
      showToast(
        snapshot.count
          ? "已识别 " + snapshot.count + " 个 Wallpaper Engine 项目"
          : "没有识别到 Wallpaper Engine 项目",
      );
    return wallpaperEngineProjects;
  } catch (e) {
...
async function openWallpaperEngineLibrary() {
  ...
  if (!wallpaperEngineLibrarySnapshot)
    await loadWallpaperEngineLibrary(false, false);
...
function closeWallpaperEngineLibrary() {
  ...
  disconnectWallpaperEnginePreviewObserver();
  document
    .querySelectorAll('#wallpaper-engine-grid img[data-animated="1"]')
...
async function chooseWallpaperEngineDirectory() {
  ...
  try {
    ...
    if (!snapshot || snapshot.ok === false)
      throw new Error((snapshot && snapshot.error) || "导入失败");
    consumeWallpaperEngineSnapshot(snapshot);
    showToast(
      "目录已加入壁纸索引，共识别 " + (snapshot.count || 0) + " 个项目",
    );
  } catch (e) {
...
async function chooseWallpaperEngineProjectFile() {
  ...
  try {
    ...
    consumeWallpaperEngineSnapshot(snapshot);
    showToast(
      "Wallpaper Engine 项目已加入索引；Scene 将由本机官方引擎实时运行",
    );
  } catch (e) {
...
async function removeWallpaperEngineDirectory(rootId) {
  ...
  try {
    ...
    if (!snapshot || snapshot.ok === false)
      throw new Error((snapshot && snapshot.error) || "移除失败");
    consumeWallpaperEngineSnapshot(snapshot);
...
function toggleFavoriteWallpaperEngineItem(id) {
  ...
  saveWallpaperEngineIdSet(
    WALLPAPER_ENGINE_FAVORITE_STORE_KEY,
    favoriteWallpaperEngineIds,
  );
  renderWallpaperEngineLibrary();
}
...
function bindWallpaperEngineLibraryEvents() {
  ...
  if (
    ...
    desktopApi &&
    typeof desktopApi.onWallpaperEngineHostBoundsChanged === "function"
  ) {
    wallpaperEngineHostBoundsUnsubscribe =
      desktopApi.onWallpaperEngineHostBoundsChanged(function (payload) {
  ...
  if (grid && !grid._wallpaperEngineBound) {
    ...
    grid.addEventListener(
      "scroll",
      scheduleWallpaperEnginePreviewViewportUpdate,
      { passive: true },
    );
    grid.addEventListener("click", function (event) {
      var action =
        event.target && event.target.closest
          ? event.target.closest("[data-wallpaper-action]")
          : null;
      if (action) {
        ...
        if (actionName === "favorite") toggleFavoriteWallpaperEngineItem(id);
          ...
          showWallpaperEngineProjectDetails(id);
      ...
      var card =
        event.target && event.target.closest
          ? event.target.closest("[data-wallpaper-id]")
          : null;
      if (card)
    ...
    grid.addEventListener("keydown", function (event) {
      ...
      var card =
        event.target && event.target.closest
          ? event.target.closest(".wallpaper-engine-card[data-wallpaper-id]")
          : null;
  ...
  if (roots && !roots._wallpaperEngineBound) {
    ...
    roots.addEventListener("click", function (event) {
      var button =
        event.target && event.target.closest
          ? event.target.closest('[data-wallpaper-action="remove-root"]')
          : null;
      if (button)
        removeWallpaperEngineDirectory(button.getAttribute("data-root-id"));
    });
  }
  if (!document._wallpaperEngineKeyBound) {
    ...
    document.addEventListener("visibilitychange", function () {
      ...
      var animatedImage =
        wallpaperEngineSelection.kind === "preview"
          ? wallpaperEngineSelection.previewAnimated
          : wallpaperEngineSelection.mediaAnimated;
