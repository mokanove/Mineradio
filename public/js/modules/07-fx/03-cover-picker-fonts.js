...
function currentCoverPickerCanvas() {
  if (coverPickerCanvas && coverPickerCanvas.getContext)
    return coverPickerCanvas;
  if (coverTex && coverTex.image && coverTex.image.getContext)
    return coverTex.image;
  return null;
}
...
function setCoverPickerPreview(hex) {
  ...
  if (preview)
    preview.style.setProperty("--picked", normalizeHexColor(hex || "#9db8cf"));
}
function renderCoverPickerSwatches() {
  ...
  wrap.innerHTML = colors
    .map(function (c) {
      return (
        '<button type="button" style="--c:' +
        c +
        '" title="' +
        c.toUpperCase() +
        '" onclick="applyCoverPickerColor(\'' +
        c +
        "')\"></button>"
      );
    })
...
function openCoverColorPicker(target) {
  ...
  var imgSrc = "";
  try {
    imgSrc = cv.toDataURL("image/jpeg", 0.84);
  } catch (e) {}
  if (!imgSrc && currentCoverSource && currentCoverSource.src)
    imgSrc = currentCoverSource.src;
  art.style.backgroundImage = imgSrc
    ? 'url("' + cssImageUrl(imgSrc) + '")'
    : "";
  setCoverPickerPreview(
    fx.visualTintColor ||
      (stageLyrics.coverPalette && stageLyrics.coverPalette.primary) ||
      "#9db8cf",
  );
...
function updateLyricFontControls() {
  ...
  document.querySelectorAll("#lyric-font-grid button").forEach(function (btn) {
    btn.classList.toggle(
      "active",
      btn.dataset.font === normalizeLyricFontKey(fx.lyricFont),
    );
  });
}
function setLyricFont(key) {
  ...
  if (customFont) {
    registerCustomLyricFont(customFont).then(function (ok) {
      if (
        ok &&
        normalizeLyricFontKey(fx.lyricFont) ===
          customLyricFontKey(customFont.id)
      ) {
...
function renderCustomLyricFontButtons() {
  ...
  if (!grid) return;
  grid.querySelectorAll('button[data-custom-font="1"]').forEach(function (btn) {
    btn.remove();
  });
  ...
  (customLyricFonts || []).forEach(function (font) {
    ...
    btn.innerHTML =
      "<span>" +
      escHtml(font.name) +
      '</span><span class="font-remove" title="删除字体" onclick="removeCustomLyricFont(event,\'' +
      font.id +
      "')\">×</span>";
...
function isSupportedLyricFontFile(file) {
  ...
  return (
    ...
    /^font\/(ttf|otf|woff2?)$/i.test(file.type || "")
  );
}
function readFileAsDataUrl(file) {
  return new Promise(function (resolve, reject) {
    ...
    reader.onerror = function () {
      reject(reader.error || new Error("FONT_READ_FAILED"));
    };
...
async function handleLyricFontFiles(files) {
  ...
  try {
    ...
    customLyricFonts = [record]
      .concat(
        (customLyricFonts || []).filter(function (item) {
...
function removeCustomLyricFont(event, id) {
  ...
  customLyricFonts = (customLyricFonts || []).filter(function (font) {
    return font && font.id !== id;
  });
  if (normalizeLyricFontKey(fx.lyricFont) === key || fx.lyricFont === key)
    fx.lyricFont = "sans";
...
function applyLyricPaletteLive(reason) {
  if (typeof setStageLyricPalette === "function") {
    setStageLyricPalette(currentLyricPaletteSource(), {
      immediate: true,
      durationMs: 1,
      reason: reason || "live",
    });
...
function setLyricGlowLinked(linked, openPicker) {
  ...
  if (!fx.lyricGlowLinked)
    fx.lyricGlowColor = normalizeHexColor(
      fx.lyricGlowColor || fx.lyricHighlightColor || "#9db8cf",
    );
