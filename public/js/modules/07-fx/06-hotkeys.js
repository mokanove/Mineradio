...
function readHotkeySettings() {
  ...
  try {
    var raw =
      JSON.parse(localStorage.getItem(HOTKEY_SETTINGS_STORE_KEY) || "{}") || {};
    return {
...
function saveHotkeySettings() {
  try {
    localStorage.setItem(
      ...
      JSON.stringify(hotkeySettings || getHotkeyDefaults()),
    );
  } catch (e) {}
}
...
function isModifierKeyCode(code) {
  return /^(ControlLeft|ControlRight|ShiftLeft|ShiftRight|AltLeft|AltRight|MetaLeft|MetaRight)$/i.test(
    String(code || ""),
  );
}
      ...
      .then(function (state) {
        ...
        return applyWallpaperModeState(true).then(function (result) {
          if (result && result.ok === true)
            showToast("完整桌面模式已开启 · " + desktopInteractionHotkeyHint());
          return result;
        });
      })
      .catch(function () {});
...
function desktopInteractionHotkeyHint() {
  var binding =
    hotkeySettings &&
    hotkeySettings.global &&
    hotkeySettings.global.toggleDesktopInteraction;
  return binding
    ? "按 " + formatHotkey(binding) + " 进入 / 退出完整桌面模式"
    : "可在热键设置中配置完整桌面模式切换";
}
function handleConfiguredLocalHotkey(e) {
  if (!hotkeySettings || !hotkeySettings.local || isTypingTarget(e.target))
    return false;
  if (
    hotkeyCaptureState ||
    (document.getElementById("hotkey-modal") &&
      document.getElementById("hotkey-modal").classList.contains("show"))
  )
    return false;
  if (
    freeCamera &&
...
function shouldSuppressDefaultConfiguredHotkey(e) {
  ...
  for (var i = 0; i < HOTKEY_ACTIONS.length; i++) {
    var action = HOTKEY_ACTIONS[i];
    if (action.local === combo && hotkeySettings.local[action.key] !== combo)
      return true;
  }
  return false;
}
function ensureHotkeySettingsButton() {
  ...
  if (head.firstElementChild)
    head.firstElementChild.classList.add("fx-head-main");
...
function ensureHotkeyModal() {
  ...
  modal.addEventListener("click", function (e) {
    if (e.target === modal || e.target.closest("[data-hotkey-close]"))
      closeHotkeySettings();
    var scopeBtn = e.target.closest("[data-hotkey-scope]");
    if (scopeBtn)
      setHotkeyModalScope(scopeBtn.getAttribute("data-hotkey-scope"));
    var bindBtn = e.target.closest("[data-hotkey-bind]");
    if (bindBtn)
      startHotkeyCapture(
        bindBtn.getAttribute("data-hotkey-action"),
        bindBtn.getAttribute("data-hotkey-bind"),
      );
    var resetBtn = e.target.closest("[data-hotkey-reset]");
    if (resetBtn)
      resetHotkeyBinding(
        resetBtn.getAttribute("data-hotkey-action"),
        resetBtn.getAttribute("data-hotkey-reset"),
      );
  });
  return modal;
}
function hotkeyStatusMarkup(scope, actionKey, binding, duplicate) {
  ...
  if (duplicate && duplicate[binding] > 1)
    return '<span class="hotkey-status conflict"><span class="source-icon">!</span>Mineradio 内部重复</span>';
  ...
  var source =
    (status.conflict && status.conflict.sourceName) || "系统 / 其他软件";
  return (
    '<span class="hotkey-status conflict"><span class="source-icon">!</span>' +
    escHtml(source) +
    "</span>"
  );
}
function renderHotkeyScope(scope) {
  var wrap = document.getElementById(
    scope === "global" ? "hotkey-global-section" : "hotkey-local-section",
  );
  if (!wrap) return;
  ...
  Object.keys(groups).forEach(function (category) {
    html +=
      '<div class="hotkey-group"><div class="hotkey-group-title">' +
      escHtml(category) +
      "</div>";
    groups[category].forEach(function (action) {
      var binding =
        (hotkeySettings[scope] && hotkeySettings[scope][action.key]) || "";
      html +=
        '<div class="hotkey-row">' +
        '<div class="hotkey-name">' +
        escHtml(action.label) +
        "</div>" +
        '<button class="hotkey-key' +
        (hotkeyCaptureState &&
        hotkeyCaptureState.scope === scope &&
        hotkeyCaptureState.action === action.key
          ? " capturing"
          : "") +
        '" type="button" data-hotkey-bind="' +
        scope +
        '" data-hotkey-action="' +
        action.key +
        '">' +
        ...
        "</button>" +
        '<button class="hotkey-reset" type="button" data-hotkey-reset="' +
        scope +
        '" data-hotkey-action="' +
        action.key +
        '">默认</button>' +
...
function renderHotkeySettings() {
  ...
  modal.querySelectorAll("[data-hotkey-scope]").forEach(function (btn) {
    btn.classList.toggle(
      "active",
      btn.getAttribute("data-hotkey-scope") === active,
    );
  });
...
function resetHotkeyBinding(action, scope) {
  ...
  setHotkeyBinding(
    action,
    scope,
    scope === "global" ? meta.global : meta.local,
  );
}
function registerGlobalHotkeys() {
  ...
  HOTKEY_ACTIONS.forEach(function (action) {
    ...
    if (accelerator)
      bindings.push({ action: action.key, accelerator: accelerator });
  });
  return api
