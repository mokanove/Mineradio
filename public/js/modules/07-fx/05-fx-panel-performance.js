...
function updateHomeAudioVisual(dt) {
  ...
  if (homeWaveTrackState.lastAt && nowMs - homeWaveTrackState.lastAt < 80)
    return;
      ...
      bin =
        (frequencyData[
          Math.min(
            ...
            Math.floor(Math.pow(ratio, 1.2) * (frequencyData.length - 1)),
          )
        ] || 0) / 255;
    } else {
      bin = 0.16 + Math.sin(nowT * 1.4 + i * 0.34) * 0.06;
    }
    var target = clampRange(
      Math.max(bin, smoothBass * 0.35 + smoothMid * 0.18 + beatPulse * 0.24),
      0.03,
      1,
    );
...
function setRange(id, value) {
  ...
  if (id === "fx-glassaberration")
    value = normalizeControlGlassChromaticOffset(value);
  if (id === "fx-lyriccustomlines") value = lyricCustomLineCountValue();
  if (
    id === "fx-memory-interval" ||
    id === "fx-memory-threshold" ||
    id === "fx-bgcropx" ||
    id === "fx-bgcropy" ||
    /^fx-sonic(?!we)/.test(id) ||
    id === "fx-sonicwegain"
    ...
    out.textContent =
      id === "fx-coverres"
        ...
        : id === "fx-lyricweight" ||
            ...
            id === "fx-lyrictilty" ||
            id === "fx-shelfangle" ||
            id === "fx-shelfdetailanglex" ||
            id === "fx-shelfdetailangley" ||
            id === "fx-memory-interval" ||
            id === "fx-memory-threshold" ||
            /^fx-sonic(?!we)/.test(id) ||
            id === "fx-sonicwegain"
          ? String(Math.round(Number(value) || 0))
          : Number(value).toFixed(id === "fx-lyricspacing" ? 3 : 2);
}
function updateDevelopmentFxControls() {
  [
    ...
    [
      ...
      "锁定后防误触；鼠标移到桌面歌词上按中键可锁定/解锁",
    ],
    [
      "desktopLyricsCinema",
      "t-desktopLyricsCinema",
      "桌面歌词绑定鼓点电影震动，基础漂浮始终保留",
    ],
    [
  ...
  var wallpaperFpsLocked =
    isDevelopmentLockedFx("wallpaperMode") ||
    (typeof desktopWallpaperRuntimeState !== "undefined" &&
      desktopWallpaperRuntimeState.supported === false);
  document
...
function updateWallpaperFpsControls() {
  ...
  document
    ...
    .forEach(function (btn) {
      var active =
        normalizeWallpaperFps(btn.getAttribute("data-wallpaper-fps")) ===
        fx.wallpaperFps;
...
function updateLyricTextureClarityControls() {
  ...
  document
    .querySelectorAll("#lyric-texture-quality-seg [data-lyric-texture-clarity]")
    .forEach(function (btn) {
      var active =
        normalizeLyricTextureClarity(
          btn.getAttribute("data-lyric-texture-clarity"),
        ) === fx.lyricTextureClarity;
      btn.classList.toggle("active", active);
      btn.setAttribute("aria-pressed", active ? "true" : "false");
    });
}
...
function updatePerformanceControls() {
  fx.performanceBackground = normalizePerformanceBackgroundMode(
  ...
  document
    ...
    .forEach(function (btn) {
      var active =
        btn.getAttribute("data-performance-quality") === fx.performanceQuality;
      btn.classList.toggle("active", active);
      btn.setAttribute("aria-pressed", active ? "true" : "false");
    });
  document
    ...
    .forEach(function (btn) {
      var active =
        normalizeForegroundFpsMode(btn.getAttribute("data-foreground-fps")) ===
        fx.foregroundFpsMode;
      btn.classList.toggle("active", active);
      btn.setAttribute("aria-pressed", active ? "true" : "false");
    });
  var liveBackgroundKeepToggle = document.getElementById(
    "t-liveBackgroundKeep",
  );
  if (liveBackgroundKeepToggle)
    liveBackgroundKeepToggle.classList.toggle(
      "on",
      fx.liveBackgroundKeep === true,
    );
}
var SONIC_ORIGINAL_FX_CONTROL_IDS = [
  "fx-sonic-ground-section",
  "fx-sonicamp",
  "fx-sonicspeed",
  "fx-sonicdensity",
  "fx-sonicrange",
  ...
  "fx-sonicdepth",
  "fx-sonicautorotate",
  "fx-sonic-audio-section",
  "sonic-audio-toggle-grid",
  "sonic-audio-monitor",
  "fx-sonicaudiosensitivity",
  "fx-sonicaudiobandstart",
  "fx-sonicaudiobandend",
  "fx-sonicaudiothreshold",
  "fx-sonicaudiopulse",
  "fx-sonicsubbass",
  "fx-sonicbass",
  "fx-soniclowmid",
  ...
  "fx-sonicbrilliance",
  "fx-sonicair",
  "fx-sonic-color-section",
  "sonic-ground-base-row",
  "sonic-ground-cool-row",
  "sonic-ground-warm-row",
  "sonic-ground-accent-row",
  "fx-sonicglow",
  "fx-sonic-floating-section",
  "sonic-floating-toggle-grid",
  "fx-sonicfloatcount",
  "fx-sonicfloatintensity",
  "fx-sonicfloatmin",
  "fx-sonicfloatmax",
  "fx-sonicfloatspeed",
];
function fxPanelControlBlockById(id) {
  ...
  if (
    el.classList &&
    (el.classList.contains("fx-section-label") ||
      el.classList.contains("fx-slider") ||
      el.classList.contains("fx-toggle-grid") ||
      el.classList.contains("sonic-audio-monitor") ||
      el.classList.contains("lyric-color-row") ||
      el.classList.contains("fx-seg"))
  )
    return el;
  return el.closest
    ? el.closest(
        ".fx-slider,.fx-toggle-grid,.sonic-audio-monitor,.lyric-color-row,.fx-seg,.fx-section-label",
      )
    : null;
}
...
function setPerformanceBackgroundMode(mode, silent) {
  ...
  if (next === "keep")
    recoverVisualsAfterBackground("performance-background-keep");
  else if (next === "release" && isDeepBackgroundMode())
    trimRuntimeCaches("performance-release", true);
  if (!silent) {
    showToast(
      next === "keep"
        ? "后台策略: 保持运行"
...
function setForegroundFpsMode(mode, silent) {
  ...
  if (typeof syncWallpaperEngineCaptureFrameRate === "function") {
    Promise.resolve(syncWallpaperEngineCaptureFrameRate()).catch(
      function () {},
    );
  }
  if (!silent)
...
function updateFxInputs() {
  ...
  if (lyricGlowBeatToggle)
    lyricGlowBeatToggle.classList.toggle("on", fx.lyricGlowBeat);
  var lyricGlowEnableBtn = document.getElementById("lyric-glow-enable-btn");
  if (lyricGlowEnableBtn)
    lyricGlowEnableBtn.classList.toggle("active", fx.lyricGlow);
  var lyricGlowBeatBtn = document.getElementById("lyric-glow-beat-btn");
  if (lyricGlowBeatBtn)
    lyricGlowBeatBtn.classList.toggle("active", fx.lyricGlowBeat);
  var lyricGlowParticlesToggle = document.getElementById(
    "t-lyricGlowParticles",
  );
  if (lyricGlowParticlesToggle)
    lyricGlowParticlesToggle.classList.toggle("on", fx.lyricGlowParticles);
  var backgroundStarRiverToggle = document.getElementById(
    "t-backgroundStarRiver",
  );
  if (backgroundStarRiverToggle)
    backgroundStarRiverToggle.classList.toggle(
      "on",
      fx.backgroundStarRiver !== false,
    );
  var lyricVerticalFloatToggle = document.getElementById(
    "t-lyricVerticalFloat",
  );
  if (lyricVerticalFloatToggle)
    lyricVerticalFloatToggle.classList.toggle(
      "on",
      fx.lyricVerticalFloat !== false,
    );
  var lyricPauseHoldToggle = document.getElementById("t-lyricPauseHold");
  if (lyricPauseHoldToggle)
    lyricPauseHoldToggle.classList.toggle("on", fx.lyricPauseHold !== false);
  var lyricCameraLockToggle = document.getElementById("t-lyricCameraLock");
  if (lyricCameraLockToggle)
    lyricCameraLockToggle.classList.toggle("on", fx.lyricCameraLock);
  ...
  if (desktopLyricsToggle)
    desktopLyricsToggle.classList.toggle("on", fx.desktopLyrics);
  var desktopLyricsClickToggle = document.getElementById(
    "t-desktopLyricsClickThrough",
  );
  if (desktopLyricsClickToggle)
    desktopLyricsClickToggle.classList.toggle(
      "on",
      fx.desktopLyricsClickThrough !== false,
    );
  var desktopLyricsCinemaToggle = document.getElementById(
    "t-desktopLyricsCinema",
  );
  if (desktopLyricsCinemaToggle)
    desktopLyricsCinemaToggle.classList.toggle(
      "on",
      fx.desktopLyricsCinema !== false,
    );
  var desktopLyricsHighlightToggle = document.getElementById(
    "t-desktopLyricsHighlight",
  );
  if (desktopLyricsHighlightToggle)
    desktopLyricsHighlightToggle.classList.toggle(
      "on",
  ...
  if (wallpaperModeToggle)
    wallpaperModeToggle.classList.toggle("on", fx.wallpaperMode);
  var shelfPodcastsToggle = document.getElementById("t-shelfShowPodcasts");
  if (shelfPodcastsToggle)
    shelfPodcastsToggle.classList.toggle("on", fx.shelfShowPodcasts !== false);
  var shelfMergeToggle = document.getElementById("t-shelfMergeCollections");
  if (shelfMergeToggle)
    shelfMergeToggle.classList.toggle("on", fx.shelfMergeCollections === true);
  var liveBackgroundKeepToggle = document.getElementById(
    "t-liveBackgroundKeep",
  );
  if (liveBackgroundKeepToggle)
  ...
  document.querySelectorAll("#shelf-seg button").forEach(function (b) {
    b.classList.toggle("active", b.dataset.shelf === fx.shelf);
  });
  updateShelfControlUi();
  document.querySelectorAll("#cam-seg button").forEach(function (b) {
    b.classList.toggle("active", b.dataset.cam === fx.cam);
  });
  refreshPresetGrid();
...
function animateFxResetButton(btn) {
  ...
  window.gsap.fromTo(
    btn,
    { rotate: -120, scale: 0.88 },
    { rotate: 0, scale: 1, duration: 0.48, ease: "expo.out", overwrite: true },
  );
  window.gsap.fromTo(
    btn,
    { boxShadow: "0 0 0 0 rgba(244,210,138,.38)" },
    {
      boxShadow: "0 0 0 8px rgba(244,210,138,0)",
      duration: 0.55,
      ease: "sine.out",
      overwrite: true,
    },
...
function syncLyricRealtimeFxChange(key, opts) {
  ...
  if (key === "lyricMotionSoftness" || /^lyricGlitch/.test(key))
    updateLyricMotionStyleControls();
  if (isStageLyricRealtimeFxKey(key))
    scheduleStageLyricRealtimeRefresh(!!opts.deferred);
  else if (
    key === "lyricLetterSpacing" ||
    key === "lyricLineHeight" ||
    key === "lyricWeight"
...
function resetFxSliderValue(id, key, btn) {
  ...
  if (key === "coverResolution")
    applyCoverParticleResolution(fx[key], { reload: true });
  if (
    key === "backgroundOpacity" ||
    key === "windowBackgroundOpacity" ||
    key === "backgroundGlassOpacity" ||
    key === "backgroundMediaCropX" ||
    key === "backgroundMediaCropY" ||
    key === "backgroundMediaZoom"
  )
    updateCustomBackgroundControls();
  ...
  syncFxUniforms();
  if (/^shelf/.test(key) && shelfManager && shelfManager.refreshTheme)
    shelfManager.refreshTheme();
  syncLyricRealtimeFxChange(key);
  saveLyricLayout({
    syncDisk: key === "controlGlassChromaticOffset",
    user: true,
    reason: "reset:" + key,
  });
...
function ensureFxSliderResetButton(id, key) {
  ...
  if (
    !el ||
    !el.parentElement ||
    el.parentElement.querySelector(".fx-reset-one")
  )
    return;
...
function setFxPanelTab(tab) {
  ...
  if (
    panel &&
    previousTab !== nextTab &&
    panel.getAttribute("data-console-layout") === "task-first-v2"
  ) {
  ...
  document
    .querySelectorAll("#fx-panel-tabs [data-fx-tab]")
    .forEach(function (btn) {
      var active = btn.getAttribute("data-fx-tab") === fxPanelTab;
      btn.classList.toggle("active", active);
      btn.setAttribute("aria-selected", active ? "true" : "false");
      btn.setAttribute("tabindex", active ? "0" : "-1");
      if (active && previousTab !== fxPanelTab)
  ...
  if (
    panel &&
    previousTab !== fxPanelTab &&
    panel.getAttribute("data-console-layout") === "task-first-v2"
  ) {
    requestAnimationFrame(function () {
      panel.scrollTop = Object.prototype.hasOwnProperty.call(
        fxPanelTabScroll,
        fxPanelTab,
      )
        ? fxPanelTabScroll[fxPanelTab]
        : 0;
    });
  }
...
function fxPanelInputId(node) {
  var input =
    node && node.querySelector ? node.querySelector("input[id]") : null;
...
function fxPanelTargetForNode(node, current) {
  ...
  if (id === "fx-advanced" || node.classList.contains("fx-actions"))
    return "advanced";
  if (
    node.classList.contains("lyric-color-row") ||
    node.classList.contains("cover-color-pop") ||
    node.classList.contains("color-lab-pop") ||
    node.classList.contains("cover-color-loupe")
  )
    return "appearance";
  if (
    inputId === "fx-bgopacity" ||
    inputId === "fx-bgcropx" ||
    inputId === "fx-bgcropy" ||
    inputId === "fx-bgzoom" ||
    inputId === "fx-windowbgopacity" ||
    inputId === "fx-bgglassopacity" ||
    inputId === "fx-glassaberration" ||
    /^fx-playlist/.test(inputId)
  )
    return "appearance";
  if (inputId === "fx-lyricglow" || inputId === "fx-lyricbgadapt")
    return "lyrics";
  if (/^fx-sonic/.test(inputId)) return "motion";
  if (/^fx-(intensity|depth|coverres|cineshake|shelf)/.test(inputId))
    return "motion";
...
function organizeFxPanel() {
  ...
  var original = Array.prototype.slice
    .call(panel.children)
    .filter(function (child) {
      return (
        child !== head &&
        child !== tabs &&
        !child.classList.contains("fx-tab-page")
      );
    });
  var current = "presets";
  ...
  ["fx-lyric-fold", "fx-overlay-fold", "fx-stage-fold", "fx-advanced"].forEach(
    function (id) {
...
function fxControlBlock(id) {
  ...
  return (
    el.closest(
      ".fx-slider,.lyric-color-row,.lyric-color-grid,.fx-seg,.preset-grid,.user-archive-grid,.fx-font-grid",
    ) || el
  );
}
function setFxSectionBefore(id, text) {
  ...
  if (
    !prev ||
    !prev.classList ||
    !prev.classList.contains("fx-section-label")
  ) {
...
function setFxSliderLabel(id, text) {
  var block = fxControlBlock(id);
  var label =
    block && block.querySelector ? block.querySelector("label") : null;
...
function applyBackgroundMediaHint() {
  ...
  if (value)
    value.title =
      "\u652f\u6301\u5c01\u9762\u539f\u56fe\u3001\u56fe\u7247 / \u89c6\u9891\u4e0a\u4f20\uff0c\u5df2\u8bbe\u7f6e\u5a92\u4f53\u53ef\u91cd\u590d\u88c1\u5207";
  var label =
    value && value.closest ? value.closest(".fx-color-row-label") : null;
