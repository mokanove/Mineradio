...
function applyIconAccentColors() {
  var homeColor = normalizeHexColor(
    fx.homeIconColor || fxDefaults.homeIconColor || "#f4d28a",
    "#f4d28a",
  );
  var visualColor = normalizeHexColor(
    fx.visualIconColor || fxDefaults.visualIconColor || "#7fd8ff",
    "#7fd8ff",
  );
  ...
  root.style.setProperty(
    "--home-icon-rgb",
    homeRgb.r + "," + homeRgb.g + "," + homeRgb.b,
  );
  root.style.setProperty("--visual-icon-color", visualColor);
  root.style.setProperty(
    "--visual-icon-rgb",
    visualRgb.r + "," + visualRgb.g + "," + visualRgb.b,
  );
}
...
function setHomeIconColor(color, silent) {
  fx.homeIconColor = normalizeHexColor(
    color || fxDefaults.homeIconColor || "#f4d28a",
    "#f4d28a",
  );
...
function setVisualIconColor(color, silent) {
  fx.visualIconColor = normalizeHexColor(
    color || fxDefaults.visualIconColor || "#7fd8ff",
    "#7fd8ff",
  );
  updateIconAccentControls();
...
function customBackgroundAlbumCoverSource() {
  ...
  try {
    if (
      typeof albumBackgroundCurrentSrc !== "undefined" &&
      albumBackgroundCurrentSrc
    )
      src = String(albumBackgroundCurrentSrc || "");
  } catch (e) {}
  if (!src) {
    try {
      if (
        typeof currentCoverSource !== "undefined" &&
        currentCoverSource &&
        currentCoverSource.src
      )
        src = String(currentCoverSource.src || "");
    } catch (e2) {}
  }
  if (!src) {
    try {
  ...
  if (!src) {
    try {
      var song =
        typeof sonicWorkshopCurrentSong === "function"
          ? sonicWorkshopCurrentSong()
          : null;
      if (!song)
        song =
      ...
      if (song)
        src =
          typeof songCoverSrc === "function"
            ? songCoverSrc(song, 640)
            : song.customCover || song.cover || "";
    } catch (e4) {}
  }
  if (src && /^https?:\/\//i.test(src) && typeof coverProxySrc === "function")
    return coverProxySrc(src, false) || src;
  return src || "";
}
function customBackgroundActiveMedia() {
  var media = normalizeCustomBackgroundMedia(
...
function applyCustomBackgroundCropVars(root, layer) {
  var cropX = customBackgroundCropNumber(
    "backgroundMediaCropX",
    fxDefaults.backgroundMediaCropX == null
      ? 50
      : fxDefaults.backgroundMediaCropX,
    0,
    100,
  );
  var cropY = customBackgroundCropNumber(
    "backgroundMediaCropY",
    fxDefaults.backgroundMediaCropY == null
      ? 50
      : fxDefaults.backgroundMediaCropY,
    0,
    100,
  );
  var zoom = customBackgroundCropNumber(
    "backgroundMediaZoom",
    fxDefaults.backgroundMediaZoom == null ? 1 : fxDefaults.backgroundMediaZoom,
    1,
    2.8,
  );
  var vars = [
...
function applyCustomBackground() {
  ...
  var opacity = clampRange(
    fx.backgroundOpacity == null ? 1 : Number(fx.backgroundOpacity),
    0,
    1,
  );
  var windowOpacity = clampRange(
  ...
  var overlayOpacity = Math.max(
    media || albumMode ? 0.18 : 0,
    glassActive ? glassOpacity * 0.42 : 0,
  );
  ...
  root.style.setProperty(
    "--custom-bg-album-opacity",
    albumMode ? opacity.toFixed(3) : "1",
  );
  ...
  if (layer) {
    layer.style.setProperty(
      "--custom-bg-image",
      image ? 'url("' + cssImageUrl(image) + '")' : "none",
    );
    layer.style.setProperty(
      "--custom-bg-image-opacity",
      image ? opacity.toFixed(3) : "0",
    );
    layer.style.setProperty(
      "--custom-bg-video-opacity",
      hasVideo ? opacity.toFixed(3) : "0",
    );
    layer.style.setProperty(
      "--custom-bg-base-opacity",
      windowOpacity.toFixed(3),
    );
    layer.style.setProperty(
      "--custom-bg-overlay-opacity",
      overlayOpacity.toFixed(3),
    );
    layer.style.setProperty(
      "--custom-bg-glass-opacity",
      glassOpacity.toFixed(3),
    );
    layer.style.setProperty(
      "--custom-bg-glass-blur",
      glassBlur.toFixed(1) + "px",
    );
    layer.style.setProperty(
      "--custom-bg-glass-saturate",
      glassSaturate.toFixed(3),
    );
    layer.style.setProperty(
      "--custom-bg-glass-brightness",
      glassBrightness.toFixed(3),
    );
  ...
  var wallpaperEngineActive = document.body.classList.contains(
  ...
  function setVideoSrc(src) {
    ...
    if (customBgObjectUrl && customBgObjectUrl !== src) {
      URL.revokeObjectURL(customBgObjectUrl);
      customBgObjectUrl = "";
    }
  ...
  if (media.src) {
    ...
    getCustomBackgroundBlob(media.id)
      .then(function (blob) {
...
function updateCustomBackgroundControls() {
  ...
  setRange(
    "fx-bgopacity",
    fx.backgroundOpacity == null ? 1 : fx.backgroundOpacity,
  );
  setRange(
    "fx-windowbgopacity",
    fx.windowBackgroundOpacity == null
  ...
  setRange(
    ...
    fx.backgroundGlassOpacity == null
      ? fxDefaults.backgroundGlassOpacity
      : fx.backgroundGlassOpacity,
  );
  setRange(
    "fx-bgcropx",
    customBackgroundCropNumber(
      "backgroundMediaCropX",
      fxDefaults.backgroundMediaCropX == null
        ? 50
        : fxDefaults.backgroundMediaCropX,
  ...
  setRange(
    ...
    customBackgroundCropNumber(
      "backgroundMediaZoom",
      fxDefaults.backgroundMediaZoom == null
        ? 1
  ...
  if (imageValue)
    imageValue.textContent = customBackgroundMediaLabel(
      fx.backgroundMedia || fx.backgroundImage,
    );
  if (albumBtn) {
    albumBtn.classList.toggle(
      "active",
      typeof customBackgroundUsesAlbumCover === "function" &&
        customBackgroundUsesAlbumCover(),
    );
    albumBtn.setAttribute(
      "aria-pressed",
      albumBtn.classList.contains("active") ? "true" : "false",
    );
  }
  if (cropBtn) {
    cropBtn.disabled = !activeMedia;
    cropBtn.title = activeMedia
      ? "\u91cd\u65b0\u88c1\u5207\u5df2\u8bbe\u7f6e\u7684\u80cc\u666f\u5a92\u4f53"
      : "\u5148\u9009\u62e9\u5c01\u9762\u3001\u56fe\u7247\u6216\u89c6\u9891";
  }
...
function setCustomBackgroundCoverMode(silent) {
  ...
  fx.backgroundColor = normalizeHexColor(
    fx.backgroundColor || fxDefaults.backgroundColor || "#000000",
    "#000000",
  );
  updateCustomBackgroundControls();
...
function setCustomBackgroundOpacity(value, silent) {
  ...
  if (!silent)
    showToast("背景透明度: " + Math.round(fx.backgroundOpacity * 100) + "%");
}
function setWindowBackgroundOpacity(value, silent) {
  ...
  if (!silent)
    showToast(
      "\u7a97\u53e3\u80cc\u666f\u900f\u660e: " +
        Math.round(fx.windowBackgroundOpacity * 100) +
        "%",
    );
}
function setBackgroundGlassOpacity(value, silent) {
  ...
  if (!silent)
    showToast(
      "\u6bdb\u73bb\u7483\u900f\u660e: " +
        Math.round(fx.backgroundGlassOpacity * 100) +
        "%",
    );
}
function setCustomBackgroundAlbumCover(enabled, silent) {
  ...
  if (!silent)
    showToast(
      fx.backgroundAlbumCover
        ? "\u80cc\u666f\u5a92\u4f53: \u5c01\u9762\u539f\u56fe"
        : "\u80cc\u666f\u5a92\u4f53: \u5df2\u5173\u95ed\u5c01\u9762",
    );
}
function toggleCustomBackgroundAlbumCover() {
  setCustomBackgroundAlbumCover(
    !(
      typeof customBackgroundUsesAlbumCover === "function" &&
      customBackgroundUsesAlbumCover()
    ),
...
function resetCustomBackgroundCrop() {
  fx.backgroundMediaCropX =
    fxDefaults.backgroundMediaCropX == null
      ? 50
      : fxDefaults.backgroundMediaCropX;
  fx.backgroundMediaCropY =
    fxDefaults.backgroundMediaCropY == null
      ? 50
      : fxDefaults.backgroundMediaCropY;
  fx.backgroundMediaZoom =
    fxDefaults.backgroundMediaZoom == null ? 1 : fxDefaults.backgroundMediaZoom;
  updateCustomBackgroundControls();
  saveLyricLayout({ user: true, reason: "backgroundMediaCrop" });
  showToast("\u80cc\u666f\u88c1\u5207\u5df2\u590d\u4f4d");
...
function customBackgroundCropSnapshot() {
  return {
    ...
    zoom: customBackgroundCropNumber(
      "backgroundMediaZoom",
      fxDefaults.backgroundMediaZoom == null
        ? 1
        : fxDefaults.backgroundMediaZoom,
...
function customBackgroundCropMediaSrc(media) {
  ...
  if (media.type === "video") {
    ...
    var activeSrc =
      (activeVideo &&
        (activeVideo.currentSrc || activeVideo.getAttribute("src"))) ||
      "";
...
function bindCustomBackgroundCropModal() {
  ...
  stage.addEventListener("pointermove", function (e) {
    if (
      ...
      !customBackgroundCropModalState.dragging
    )
      return;
    e.preventDefault();
    ...
    var nextX =
      (Number(fx.backgroundMediaCropX) || 50) -
      ((dx / Math.max(1, rect.width)) * 100) / zoomFactor;
    var nextY =
      (Number(fx.backgroundMediaCropY) || 50) -
      ((dy / Math.max(1, rect.height)) * 100) / zoomFactor;
  ...
  stage.addEventListener(
    ...
    function (e) {
      ...
      current.zoom = clampRange(
        current.zoom + (e.deltaY < 0 ? 0.08 : -0.08),
        1,
        2.8,
      );
  ...
  if (media.type === "video" && !src && media.id) {
    getCustomBackgroundBlob(media.id)
      .then(function (blob) {
        if (
          !customBackgroundCropModalState ||
          customBackgroundCropModalState.media !== media ||
          !blob
        )
          return;
        releaseCustomBackgroundCropModalObjectUrl();
        customBackgroundCropModalObjectUrl = URL.createObjectURL(blob);
        setCustomBackgroundCropModalSource(
          media,
          customBackgroundCropModalObjectUrl,
        );
      })
...
function closeCustomBackgroundCropModal(restoreOriginal) {
  var modal = document.getElementById("background-crop-modal");
  var original =
    customBackgroundCropModalState && customBackgroundCropModalState.original;
...
function setCustomBackgroundImage(src, silent) {
  ...
  if (!silent)
    showToast(fx.backgroundImage ? "背景图片已应用" : "背景图片已清除");
}
...
function readBackgroundVideoFile(file) {
  ...
  putCustomBackgroundBlob(id, file, {
    name: file.name || "",
    mime: file.type || "",
    size: file.size || 0,
  })
...
function applyUiAccentColor() {
  ...
  root.style.setProperty(
    "--glass-border",
    "rgba(" + rgb.r + "," + rgb.g + "," + rgb.b + ",.30)",
  );
  root.style.setProperty(
    "--glass-shadow-focus",
    "0 24px 72px rgba(0,0,0,.34),0 0 0 1px rgba(" +
      rgb.r +
      "," +
      rgb.g +
      "," +
      ...
      "," +
      rgb.b +
      ",.075),inset 0 1px 0 rgba(255,255,255,.20)",
  );
}
...
function updateVisualTintControls() {
  ...
  if (value)
    value.textContent =
      fx.visualTintMode === "custom" ? color.toUpperCase() : "封面取色";
  if (autoBtn)
    autoBtn.classList.toggle("active", fx.visualTintMode !== "custom");
}
...
function resetVisualTintColor() {
  fx.visualTintMode = "auto";
  fx.visualTintColor = normalizeHexColor(
    fxDefaults.visualTintColor || "#9db8cf",
  );
  updateVisualTintControls();
  syncFxUniforms();
...
var SONIC_GROUND_COLOR_CONTROLS = [
  {
    key: "sonicGroundBaseColor",
    picker: "sonic-ground-base-picker",
    value: "sonic-ground-base-value",
    label: "地形暗部",
  },
  {
    key: "sonicGroundCoolColor",
    picker: "sonic-ground-cool-picker",
    value: "sonic-ground-cool-value",
    label: "冷色峰值",
  },
  {
    key: "sonicGroundWarmColor",
    picker: "sonic-ground-warm-picker",
    value: "sonic-ground-warm-value",
    label: "暖色峰值",
  },
  {
    key: "sonicGroundAccentColor",
    picker: "sonic-ground-accent-picker",
    value: "sonic-ground-accent-value",
    label: "涟漪高光",
  },
];
function sonicGroundColorControl(key) {
  for (var i = 0; i < SONIC_GROUND_COLOR_CONTROLS.length; i++) {
    if (
      SONIC_GROUND_COLOR_CONTROLS[i].key === key ||
      SONIC_GROUND_COLOR_CONTROLS[i].picker === key
    )
      return SONIC_GROUND_COLOR_CONTROLS[i];
  }
  return null;
}
...
function sonicMixHex(a, b, t) {
  ...
  return (
    ...
    [
      ...
      .map(function (v) {
        return Math.round(clampRange(v, 0, 255))
          .toString(16)
          .padStart(2, "0");
      })
      .join("")
...
function sonicPaletteHex(value, fallback, minLum) {
  if (typeof lyricPaletteColorToHex === "function")
    return lyricPaletteColorToHex(value, fallback, minLum);
...
function sonicGroundCoverPreviewColors() {
  ...
  var primary = sonicPaletteHex(
    pal.primary || pal.secondary || pal.highlight,
    "#33e6ff",
    0.42,
  );
  var secondary = sonicPaletteHex(
    pal.secondary || pal.primary || pal.highlight,
    "#7fd8ff",
    0.4,
  );
  var highlight = sonicPaletteHex(
    pal.highlight || pal.primary || pal.secondary,
    "#ffd070",
    0.48,
  );
  return {
...
function updateSonicGroundColorControls() {
  ...
  SONIC_GROUND_COLOR_CONTROLS.forEach(function (item) {
    ...
    var color = customMode
      ? normalizeHexColor(fx[item.key] || fallback, fallback)
      : normalizeHexColor(coverPreview[item.key] || fallback, fallback);
    var picker = document.getElementById(item.picker);
    var value = document.getElementById(item.value);
    if (picker) picker.value = color;
    if (value && !customMode) value.textContent = "封面 " + color.toUpperCase();
    if (value)
      value.textContent = customMode ? color.toUpperCase() : "封面取色";
  });
  if (!customMode) {
    SONIC_GROUND_COLOR_CONTROLS.forEach(function (item) {
      var fallback = fxDefaults[item.key] || "#33e6ff";
      var color = normalizeHexColor(
        (coverPreview && coverPreview[item.key]) || fallback,
        fallback,
      );
      var value = document.getElementById(item.value);
...
function resetSonicGroundColor(key) {
  ...
  SONIC_GROUND_COLOR_CONTROLS.forEach(function (control) {
    fx[control.key] = normalizeHexColor(
      fxDefaults[control.key] || "#33e6ff",
      "#33e6ff",
    );
  });
  updateSonicGroundColorControls();
...
var SONIC_WORKSHOP_COLOR_CONTROLS = [
  {
    id: "theme",
    role: "primary",
    modeKey: "sonicWorkshopColorMode",
    colorKey: "sonicWorkshopCustomColor",
    picker: "sonic-workshop-cover-picker",
    value: "sonic-workshop-theme-value",
    button: "sonic-workshop-cover-btn",
    fallback: "#cb6c89",
    label: "\u4e3b\u9898\u57fa\u8272",
  },
  {
    id: "base",
    role: "base",
    modeKey: "sonicWorkshopBaseColorMode",
    colorKey: "sonicWorkshopBaseColor",
    picker: "sonic-workshop-base-picker",
    value: "sonic-workshop-base-value",
    button: "sonic-workshop-base-btn",
    fallback: "#16060f",
    label: "\u5730\u5f62\u5e95\u8272",
  },
  {
    id: "warm",
    role: "warm",
    modeKey: "sonicWorkshopWarmColorMode",
    colorKey: "sonicWorkshopWarmColor",
    picker: "sonic-workshop-warm-picker",
    value: "sonic-workshop-warm-value",
    button: "sonic-workshop-warm-btn",
    fallback: "#cb6c89",
    label: "\u6696\u8272\u4e3b\u4f53",
  },
  {
    id: "cool",
    role: "cool",
    modeKey: "sonicWorkshopCoolColorMode",
    colorKey: "sonicWorkshopCoolColor",
    picker: "sonic-workshop-cool-picker",
    value: "sonic-workshop-cool-value",
    button: "sonic-workshop-cool-btn",
    fallback: "#99c4ff",
    label: "\u4e0a\u5c42\u9ad8\u5149",
  },
  {
    id: "ripple",
    role: "ripple",
    modeKey: "sonicWorkshopRippleColorMode",
    colorKey: "sonicWorkshopRippleColor",
    picker: "sonic-workshop-ripple-picker",
    value: "sonic-workshop-ripple-value",
    button: "sonic-workshop-ripple-btn",
    fallback: "#f8d8ff",
    label: "\u6ce2\u7eb9\u4eae\u533a",
  },
  {
    id: "peak",
    role: "peak",
    modeKey: "sonicWorkshopPeakColorMode",
    colorKey: "sonicWorkshopPeakColor",
    picker: "sonic-workshop-peak-picker",
    value: "sonic-workshop-peak-value",
    button: "sonic-workshop-peak-btn",
    fallback: "#99c4ff",
    label: "\u5cf0\u503c\u9ad8\u5149",
  },
];
...
function normalizeSonicWorkshopTheme(theme) {
  ...
  return SONIC_WORKSHOP_THEMES[theme]
    ? theme
    : fxDefaults.sonicWorkshopTheme || "coral-mirage";
}
...
function sonicWorkshopCurrentCoverDomSource() {
  try {
    if (
      ...
      return String(currentCoverSource.src || "");
  } catch (e) {}
  try {
    var thumb = document.getElementById("thumb-cover");
    var thumbSrc =
      thumb && (thumb.currentSrc || thumb.src || thumb.getAttribute("src"));
...
function sonicWorkshopCurrentCoverSampleSrc() {
  ...
  if (domSrc) {
    if (
      /^data:image\//i.test(domSrc) ||
      /^blob:/i.test(domSrc) ||
      /^\/api\/cover\?/i.test(domSrc) ||
      /^https?:\/\/[^/]+\/api\/cover\?/i.test(domSrc)
    )
      return domSrc;
    if (typeof coverProxySrc === "function")
      return coverProxySrc(domSrc, false) || domSrc;
    return domSrc;
  }
  var song = sonicWorkshopCurrentSong();
  if (!song) return "";
...
function sonicWorkshopCssFromSample(sample, fallback) {
  ...
  if (typeof lyricCoverSampleCss === "function")
    return lyricCoverSampleCss(sample, fallback || "#cb6c89");
  return (
    "#" +
    ["r", "g", "b"]
      .map(function (key) {
        return Math.round(clampRange(Number(sample[key]) || 0, 0, 255))
          .toString(16)
          .padStart(2, "0");
      })
      .join("")
  ...
  try {
    ...
    for (var y = 0; y < h; y += step) {
      for (var x = 0; x < w; x += step) {
        ...
        sumG += g;
        sumB += b;
        if (typeof lyricCoverAddAreaBucket === "function")
          lyricCoverAddAreaBucket(buckets, r, g, b);
        else fallbackList.push({ r: r, g: g, b: b, score: 1 });
      }
    }
    if (!count) return null;
    var areaPalette =
      typeof lyricCoverAreaPaletteFromBuckets === "function"
        ? lyricCoverAreaPaletteFromBuckets(buckets)
        : null;
    if (!areaPalette) {
      fallbackList.sort(function (a, b) {
    ...
    var palette = {
      rawAreaPrimary: sonicWorkshopCssFromSample(
        areaPalette.primary,
        "#cb6c89",
      ),
      ...
      sonicWorkshopColors: Array.isArray(areaPalette.colors)
        ? areaPalette.colors.slice(0, 10)
        : [],
      coverColors: [],
      coverSourceKey: key || "",
    ...
    [
      ...
      if (typeof lyricCoverPushUniqueColor === "function")
        lyricCoverPushUniqueColor(palette.coverColors, color);
      else if (palette.coverColors.indexOf(color) < 0)
        palette.coverColors.push(color);
    });
    return palette;
  } catch (e) {
    return null;
...
function sonicWorkshopPaletteLooksCurrent(pal, key) {
  ...
  var hasArea = !!(
    pal.rawAreaPrimary ||
    pal.rawPrimary ||
    (pal.sonicWorkshopColors && pal.sonicWorkshopColors.length)
  );
...
function sonicWorkshopUiPaletteForKey(key) {
  ...
  return sonicWorkshopPaletteLooksCurrent(
    pal,
    key || sonicWorkshopCurrentCoverKey(),
  )
    ? pal
    : null;
}
  ...
  try {
    ...
    if (uiPalette) {
      ...
      if (typeof stageLyrics !== "undefined" && stageLyrics) {
        stageLyrics.coverPalette = Object.assign(
          {},
    ...
    if (stageLyrics && stageLyrics.coverPalette) {
      ...
      stageLyrics.coverPalette.sonicWorkshopCoverKey =
        key || stageLyrics.coverPalette.sonicWorkshopCoverKey || "";
      stageLyrics.coverPalette.coverSourceKey =
        key || stageLyrics.coverPalette.coverSourceKey || "";
    }
    sonicWorkshopCoverUiSample.key =
      key || sonicWorkshopCoverUiSample.key || "";
...
function ensureSonicWorkshopCoverPaletteForUi() {
  ...
  if (
    !src ||
    (sonicWorkshopCoverUiSample.loading &&
...
function sonicWorkshopPaletteForUi() {
  return (
    sonicWorkshopUiPaletteForKey(sonicWorkshopCurrentCoverKey()) ||
    (stageLyrics && (stageLyrics.coverPalette || stageLyrics.palette)) ||
    {}
  );
}
...
function sonicWorkshopThemeForColor(hex) {
  ...
  if (hsl.h < 0.1) return "coral-mirage";
...
function currentSonicWorkshopTheme() {
  if (fx.sonicWorkshopColorMode === "custom")
    return normalizeSonicWorkshopTheme(fx.sonicWorkshopTheme);
...
function sonicWorkshopRegionHex(item) {
  ...
  if (fx[item.modeKey] === "custom")
    return normalizeHexColor(fx[item.colorKey] || fallback, fallback);
...
function updateSonicWorkshopColorControls() {
  ...
  var meta =
    SONIC_WORKSHOP_THEMES[theme] || SONIC_WORKSHOP_THEMES["coral-mirage"];
  ...
  document
    .querySelectorAll("#sonic-workshop-theme-seg [data-sonic-workshop-theme]")
    .forEach(function (btn) {
      btn.classList.toggle(
        "active",
        fx.sonicWorkshopColorMode === "custom" &&
          normalizeSonicWorkshopTheme(
            btn.getAttribute("data-sonic-workshop-theme"),
          ) === theme,
      );
    });
}
function pushSonicWorkshopColorChange(reason) {
  ...
  if (workshop && typeof workshop.pushProperties === "function")
    workshop.pushProperties(true);
  if (typeof syncFxUniforms === "function") syncFxUniforms();
  saveLyricLayout({
    user: true,
    reason: reason || "sonicWorkshopRegionColors",
  });
}
function setSonicWorkshopRegionColorMode(id, mode, silent) {
  ...
  pushSonicWorkshopColorChange(
    item.id === "theme" ? "sonicWorkshopColorMode" : item.colorKey,
  );
  if (!silent)
    showToast(
      "\u97f3\u57df\u56de\u54cd\u00b7WE " +
...
function setSonicWorkshopTheme(theme, silent) {
  ...
  SONIC_WORKSHOP_COLOR_CONTROLS.forEach(function (item) {
    ...
    fx[item.colorKey] = normalizeHexColor(
      meta[item.id] || item.fallback,
      item.fallback,
    );
  });
...
function setSonicWorkshopThemeFromPicker(color, silent) {
  ...
  if (!silent)
    showToast(
      "\u97f3\u57df\u56de\u54cd\u00b7WE " +
        SONIC_WORKSHOP_COLOR_CONTROLS[0].label +
        ": " +
        fx.sonicWorkshopCustomColor.toUpperCase(),
    );
}
