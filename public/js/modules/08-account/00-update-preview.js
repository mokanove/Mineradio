...
function formatUpdateBytes(bytes) {
  ...
  if (bytes >= 1024 * 1024 * 1024)
    return (
      (bytes / (1024 * 1024 * 1024)).toFixed(2).replace(/\.00$/, "") + " GB"
    );
  if (bytes >= 1024 * 1024)
...
function updateProgressDetailText() {
  ...
  if (updatePreviewState.attempts > 1 && updatePreviewState.attempt > 0) {
    parts.push(
      "线路 " + updatePreviewState.attempt + "/" + updatePreviewState.attempts,
    );
  }
  if (updatePreviewState.sourceLabel)
    parts.push(updatePreviewState.sourceLabel);
  if (updatePreviewState.received > 0) {
    parts.push(
      updatePreviewState.total > 0
        ? formatUpdateBytes(updatePreviewState.received) +
            " / " +
            formatUpdateBytes(updatePreviewState.total)
...
function applyLatestUpdateInfo(data) {
  ...
  updatePreviewState.currentVersion =
    data.currentVersion || updatePreviewState.currentVersion;
  updatePreviewState.version =
    data.latestVersion || release.version || updatePreviewState.currentVersion;
  ...
  updatePreviewState.downloadUrl =
    release.downloadUrl || data.downloadUrl || "";
  updatePreviewState.patchAvailable = !!(
    release.patchAvailable &&
    release.patch &&
    release.patch.downloadUrl
  );
  updatePreviewState.patchUrl = updatePreviewState.patchAvailable
    ? release.patch.downloadUrl
    : "";
  updatePreviewState.patchFallbackTried = false;
  updatePreviewState.hero =
    release.summary ||
  ...
  renderUpdatePreviewPanel();
  setUpdatePreviewVisible(
    updatePreviewState.updateAvailable || updatePreviewState.preview,
  );
}
...
function startUpdateIconBreathing() {
  ...
  window.gsap.to(entry, {
    y: -1.4,
    boxShadow:
      "0 16px 44px rgba(0,0,0,.32),0 0 24px rgba(244,210,138,.18),0 0 13px rgba(157,184,207,.06),inset 0 1px 0 rgba(255,255,255,.11)",
    duration: 2.6,
    repeat: -1,
    yoyo: true,
    ease: "sine.inOut",
...
function renderUpdatePreviewPanel() {
  ...
  if (hero)
    hero.textContent = updatePreviewState.hero || "当前版本，更新检测已就绪。";
  if (list) {
    var notes =
      Array.isArray(updatePreviewState.notes) && updatePreviewState.notes.length
        ? updatePreviewState.notes
        : ["更新检测已就绪"];
    list.innerHTML = notes
      .map(function (text, i) {
        return (
          '<div class="update-item"><span class="update-item-dot" data-index="' +
...
function syncUpdatePreviewStateClass() {
  ...
  var canDownloadUpdate =
    updatePreviewState.configured &&
    updatePreviewState.updateAvailable &&
    updatePreviewState.downloadUrl;
  var canOpenRelease =
    updatePreviewState.configured &&
  ...
  if (label) {
    if (isDownloading)
      label.textContent =
        (isPatch ? "快速补丁 " : "正在下载 ") +
        Math.round(updatePreviewState.progress) +
        "%";
      ...
      label.textContent = "下载完整安装包";
    else if (isError)
      label.textContent =
        updatePreviewState.mode === "installer" ? "重试下载" : "重试更新";
    else if (isReady && isPatch && updatePreviewState.restartRequired)
      label.textContent = "重启生效";
    else if (isReady && isPatch) label.textContent = "补丁已应用";
    else if (isReady && updatePreviewState.installerOpened)
      label.textContent = "安装包已打开";
    else if (isReady && updatePreviewState.installerPath)
      label.textContent = updatePreviewState.cached
        ? "打开已下载安装包"
        : "打开安装包";
    else if (isReady)
      label.textContent = updatePreviewState.configured
        ? "打开安装包"
        : "预览完成";
    else
      label.textContent = updatePreviewState.patchAvailable
        ? "安装快速补丁"
        : canDownloadUpdate || canOpenRelease
          ? "下载完整安装包"
          : "立即更新";
  }
  ...
  if (foot) {
    if (isDownloading)
      ...
      foot.textContent =
        ...
        updatePreviewState.failedAttempts.length
          ? " · 已尝试 " + updatePreviewState.failedAttempts.length + " 条线路"
          : "");
    else if (isReady && isPatch)
      foot.textContent = updatePreviewState.restartRequired
        ? "快速补丁已应用，重启 Mineradio 后生效。"
        : "快速补丁已应用。";
    else if (isReady)
      foot.textContent = updatePreviewState.cached
        ? "已复用上次校验通过的安装包，不会重复下载。"
        : "安装包已准备好，点击按钮后再打开安装。";
    else if (updatePreviewState.patchAvailable)
      foot.textContent =
        "优先使用轻量补丁，只更新缺失或变更的资源文件；不适用时可下载完整安装包。";
    else
      foot.textContent = updatePreviewState.updateAvailable
        ? "没有可用快速补丁时会下载完整安装包。"
        : "当前版本已是最新。";
  }
}
...
function updateUpdatePreviewProgress(progress) {
  ...
  if (ring) {
    ...
    ring.style.strokeDashoffset = (
      circumference *
      (1 - updatePreviewState.progress / 100)
    ).toFixed(2);
  }
  syncUpdatePreviewStateClass();
}
...
function openUpdatePanel() {
  ...
  if (entry && window.gsap) {
    window.gsap.fromTo(
      entry,
      { scale: 0.93 },
      { scale: 1, duration: 0.42, ease: "back.out(1.7)", overwrite: "auto" },
    );
  }
  openGsapModal(mask);
...
function animateUpdatePanelContents() {
  ...
  var items = Array.prototype.slice.call(
    modal.querySelectorAll(".update-item"),
  );
...
async function startRealUpdateDownload() {
  ...
  try {
    ...
    if (!job || job.ok === false || !job.id)
      throw new Error((job && job.error) || "UPDATE_DOWNLOAD_START_FAILED");
...
async function startRealUpdatePatch() {
  ...
  try {
    ...
    if (!job || job.ok === false || !job.id)
      throw new Error((job && job.error) || "UPDATE_PATCH_START_FAILED");
...
async function pollUpdateDownloadJob(id) {
  if (!id) return;
  try {
    var job = await apiJson(
      "/api/update/download/status?id=" +
    ...
    if (updatePreviewState.pollTimer)
      clearInterval(updatePreviewState.pollTimer);
    updatePreviewState.pollTimer = null;
    updatePreviewState.status = "error";
    updatePreviewState.errorReason = "更新下载状态读取失败";
    updatePreviewState.errorDetail =
      (e && e.message) || updatePreviewState.errorReason;
    updatePreviewState.message = updatePreviewState.errorReason;
...
function applyUpdateDownloadJob(job) {
  if (!job || job.ok === false || job.status === "error") {
    if (updatePreviewState.pollTimer)
      clearInterval(updatePreviewState.pollTimer);
    updatePreviewState.pollTimer = null;
    updatePreviewState.mode =
      (job && job.mode) || updatePreviewState.mode || "installer";
    ...
    updatePreviewState.sourceLabel =
      (job && job.sourceLabel) || updatePreviewState.sourceLabel || "";
    updatePreviewState.attempt = Number((job && job.attempt) || 0);
    updatePreviewState.attempts = Number((job && job.attempts) || 0);
    updatePreviewState.errorReason =
      (job && (job.errorReason || job.message || job.error)) || "请稍后重试";
    updatePreviewState.errorDetail = (job && job.errorDetail) || "";
    updatePreviewState.failedAttempts = Array.isArray(job && job.failedAttempts)
      ? job.failedAttempts
      : [];
    updatePreviewState.message =
      (job && job.message) || updatePreviewState.errorReason;
    updatePreviewState.status = "error";
    updateUpdatePreviewProgress(
      (job && job.progress) || updatePreviewState.progress || 0,
    );
    if (
      updatePreviewState.mode === "patch" &&
      updatePreviewState.downloadUrl &&
      !updatePreviewState.patchFallbackTried
    ) {
      updatePreviewState.patchFallbackTried = true;
      showToast(
        "快速补丁失败，可手动下载完整安装包：" + updatePreviewState.errorReason,
      );
      return;
    }
  ...
  if (updatePreviewState.mode === "patch")
    updatePreviewState.patchJobId = job.id || updatePreviewState.patchJobId;
  ...
  updatePreviewState.failedAttempts = Array.isArray(job.failedAttempts)
    ? job.failedAttempts
    : [];
  ...
  if (job.status === "ready") {
    ...
    if (updatePreviewState.mode === "patch") {
      showToast(
        updatePreviewState.restartRequired
          ? "快速补丁已应用，重启后生效"
          : "快速补丁已应用",
      );
    } else if (updatePreviewState.installerPath) {
      showToast(
        updatePreviewState.cached
          ? "已复用上次下载的安装包"
          : "安装包已下载，点击按钮打开",
      );
    }
  ...
  try {
    if (window.desktopWindow && window.desktopWindow.openUpdateInstaller) {
      ...
      if (!result || result.ok === false)
        throw new Error((result && result.error) || "OPEN_UPDATE_FAILED");
    ...
    updatePreviewState.status = "ready";
    syncUpdatePreviewStateClass();
    if (updatePreviewState.releaseUrl)
      window.open(updatePreviewState.releaseUrl, "_blank");
...
function startUpdatePreviewDownload() {
  var releaseLink =
    updatePreviewState.downloadUrl || updatePreviewState.releaseUrl;
  if (
    updatePreviewState.status === "ready" &&
    updatePreviewState.mode === "patch"
  ) {
    ...
    if (window.gsap) {
      ...
      if (modal)
        window.gsap.fromTo(
          ...
          {
            boxShadow:
              "0 30px 100px rgba(0,0,0,.62),0 0 0 1px rgba(244,210,138,.16)",
          },
          {
            boxShadow:
              "0 30px 100px rgba(0,0,0,.62),0 0 34px rgba(244,210,138,.18)",
            duration: 0.52,
            yoyo: true,
            repeat: 1,
            ease: "sine.inOut",
  ...
  if (btn && window.gsap)
    window.gsap.fromTo(
      btn,
      { scale: 0.985 },
      { scale: 1, duration: 0.34, ease: "back.out(1.45)", overwrite: true },
    );
...
function pulseUpdateReady() {
  ...
  if (entry) {
    window.gsap.fromTo(
      ...
      {
        scale: 1.04,
        filter: "drop-shadow(0 0 14px rgba(244,210,138,.28))",
        duration: 0.34,
        yoyo: true,
        repeat: 1,
        ease: "sine.inOut",
  ...
  if (btn) {
    window.gsap.fromTo(
      ...
      {
        boxShadow:
          "0 0 0 rgba(244,210,138,0), inset 0 1px 0 rgba(255,255,255,.09)",
      },
      {
        boxShadow:
          "0 0 24px rgba(244,210,138,.16), inset 0 1px 0 rgba(255,255,255,.11)",
        duration: 0.42,
        yoyo: true,
        repeat: 1,
        ease: "sine.inOut",
