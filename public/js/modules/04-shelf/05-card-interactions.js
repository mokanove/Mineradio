...
function pointerCardHit(rc, e, screenPad) {
  ...
  return (
    shelfManager.raycastCards(rc) ||
    (shelfManager.pickCardAtScreen &&
      shelfManager.pickCardAtScreen(e.clientX, e.clientY, screenPad))
  );
}
function isSideShelfFocusHit(e) {
  ...
  if (shelfAlwaysVisible())
    return !!pointerCardHit(raycasterFromPointerEvent(e), e, 18);
  if (!shelfAutoHiddenInputReady()) return false;
  if (
    shelfVisibility > 0.34 &&
    (isShelfClickZone(e) || isShelfPreviewUseZone(e))
  )
    return true;
  return !!(
    shelfPreviewIsVisible() &&
    pointerCardHit(raycasterFromPointerEvent(e), e, 24)
  );
}
function updateShelfCardHoverSelection(e) {
  if (!shelfManager || !shelfManager.clearSelected || !shelfManager.setSelected)
    return;
  if (
    typeof shelfPlaybackSwitchGuardActive === "function" &&
    shelfPlaybackSwitchGuardActive()
...
function isShelfPlaylistPlayHit(hit) {
  if (
    ...
    !hit.uv ||
    !hit.card.item ||
    hit.card.item.type !== "playlist"
  )
    return false;
  return (
    hit.uv.x >= 0.49 && hit.uv.x <= 0.72 && hit.uv.y >= 0.13 && hit.uv.y <= 0.42
  );
}
renderer.domElement.addEventListener("click", function (e) {
  ...
  if (shelfManager.hasOpenContent()) {
    ...
    if (cl) {
      ...
      if (!rowHit && cl.pickRowAtScreen)
        rowHit = cl.pickRowAtScreen(e.clientX, e.clientY);
      if (rowHit) {
        if (cl.pulseRow) cl.pulseRow(rowHit.row, 0.72);
        var selectedRow = Math.abs(rowHit.row.index - cl.getCenterIdx()) < 0.5;
        var rowIsPodcastRadio = !!(
          rowHit.row.song && rowHit.row.song.type === "podcast-radio"
        );
        var hitLikeButton =
          rowHit.uv &&
          rowHit.uv.x > 0.61 &&
        ...
        var hitPlayButton =
          ...
          rowHit.uv.y < 0.82;
        var screenAction =
          !rowHit.uv && cl.rowActionAtScreen
            ? cl.rowActionAtScreen(rowHit.row, e.clientX, e.clientY)
            : null;
        ...
        if (selectedRow && !rowIsPodcastRadio && hitLikeButton) {
          ...
          (rowHit.row.song && rowHit.row.song.id) ||
          rowIsPodcastRadio ||
          (selectedRow && hitPlayButton)
        ) {
      ...
      if (returnHit && returnHit.card) {
        shelfManager.scrollBy(
          returnHit.card.index - shelfManager.getCenterIdx(),
        );
      }
      return;
    }
  ...
  // 一级卡片
  var hit = pointerCardHit(
    rc,
    e,
    mode === "side" && !shelfPinnedOpen && shelfAlwaysVisible()
      ? 18
      : undefined,
  );
  if (
  ...
  if (hit) {
    ...
    if (Math.abs(idx - shelfManager.getCenterIdx()) < 0.5) {
      if (
        isShelfPlaylistPlayHit(hit) &&
        shelfManager.playPlaylistAt &&
        shelfManager.playPlaylistAt(idx)
      )
        return;
...
renderer.domElement.addEventListener("contextmenu", function (e) {
  ...
  e.preventDefault();
  e.stopPropagation();
  if (typeof suppressBottomControlsForShelf === "function")
  ...
  if (shelfManager.hasOpenContent && shelfManager.hasOpenContent()) {
    ...
    if (
      ...
      rowHit.row.song &&
      rowHit.row.song.id &&
      rowHit.row.song.type !== "podcast-radio"
    ) {
  ...
  if (!shelfPinnedOpen && typeof setFocusZone === "function")
  ...
  function (e) {
    ...
    if (shelfManager.hasOpenContent()) {
      ...
      if (cl) {
        ...
        var panelScreenHit =
          !rowHit && !panelHit && cl.screenContainsPanel
            ? cl.screenContainsPanel(e.clientX, e.clientY)
            : false;
    ...
    var cardWheelHit = canScrollShelf
      ? pointerCardHit(
          rc,
          e,
          mode === "side" && !shelfPinnedOpen && shelfAlwaysVisible()
    ...
    if (
      ...
      else if (shelfPreviewActive)
        inShelfArea = isShelfWheelZone(e) || !!cardWheelHit;
    } else if (canScrollShelf && mode === "stage" && cardWheelHit)
      inShelfArea = true;
    if (inShelfArea) {
      e.preventDefault();
      e.stopImmediatePropagation();
