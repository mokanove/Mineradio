...
function scheduleShelfRebuild(reason, asyncCards) {
  deferredShelfRebuild.reason =
    reason || deferredShelfRebuild.reason || "deferred";
  ...
  deferredShelfRebuild.raf = requestAnimationFrame(function () {
    ...
    scheduleUiWarmTask(function () {
      ...
      safeShelfRebuild(
        deferredShelfRebuild.reason,
        deferredShelfRebuild.asyncCards,
      );
    }, 260);
  });
}
function safeShelfCloseContent(reason) {
  if (!shelfManager || typeof shelfManager.closeContent !== "function")
    return false;
  try {
    shelfManager.closeContent();
    if (
      !shelfPinnedOpen &&
      typeof restoreBottomControlsAfterShelfExit === "function"
    ) {
      requestAnimationFrame(function () {
        restoreBottomControlsAfterShelfExit(reason || "shelf-content-close");
      });
    }
    return true;
  } catch (e) {
