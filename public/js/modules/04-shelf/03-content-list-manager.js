...
function makeContentListManager() {
  ...
  var CONTENT_PREFETCH_AHEAD = Math.max(
    24,
    Math.floor(
      (typeof PLAYLIST_LAZY_BATCH_SIZE === "number"
  ...
  var DETAIL_BASE = { x: 1.28, y: 0.18, z: 1.36, rx: -0.008, ry: 0.02 };
  ...
  function placeDynamicDetailFromCamera(layout, intro, parX, parY) {
    ...
    var rightOffset = portrait ? 0.02 : narrow ? 0.06 : 0.1;
    var upOffset = portrait ? -0.06 : -0.04;
    distance += clampRange((layout.z - DETAIL_BASE.z) * 0.12, -0.2, 0.24);
    rightOffset += clampRange((layout.x - DETAIL_BASE.x) * 0.08, -0.24, 0.24);
    upOffset += clampRange((layout.y - DETAIL_BASE.y) * 0.08, -0.16, 0.16);
    rightOffset +=
      introMix * (portrait ? 0.34 : 0.46) +
      parX * (portrait ? 0.045 : 0.055) * parallax;
    upOffset -= introMix * (portrait ? 0.05 : 0.04);
    upOffset += parY * (portrait ? 0.042 : 0.045) * parallax;
    distance +=
      introMix * 0.12 + parY * 0.018 * parallax - parX * 0.012 * parallax;
  ...
  function ensurePanel() {
    ...
    mesh.position.set(-0.02, 0.0, 0.2);
  ...
  function drawPanel() {
    ...
    bg.addColorStop(
      0,
      "rgba(0,0,0," + Math.min(0.98, panelBgAlpha + 0.02).toFixed(3) + ")",
    );
    bg.addColorStop(0.42, "rgba(0,0,0," + panelBgAlpha.toFixed(3) + ")");
    bg.addColorStop(
      1,
      "rgba(0,0,0," + Math.max(0.2, panelBgAlpha - 0.04).toFixed(3) + ")",
    );
    ctx.fillStyle = bg;
    ctx.fill();
    ...
    var playableCount = allTracks.filter(function (song) {
      return song && song.id && song.type !== "podcast-radio";
    }).length;
    var contentCount = allTracks.filter(function (song) {
      return song && song.id;
    }).length;
    var isLoading =
      allTracks.length === 1 &&
      isLoadingLabel(allTracks[0] && allTracks[0].name);
    var countLabel =
      contentKind === "podcast"
        ? contentCount
          ? contentCount + " 项播客内容"
    ...
    if (
      contentKind !== "podcast" &&
      contentTotalCount &&
      contentTotalCount > playableCount
    ) {
      countLabel =
        playableCount +
        "/" +
        contentTotalCount +
        (contentLoadingMore ? " loading" : " loaded");
    }
  ...
  function disposePanelObject(targetPanel) {
    ...
    if (targetPanel.mesh && targetPanel.mesh.parent)
      targetPanel.mesh.parent.remove(targetPanel.mesh);
    if (targetPanel.texture) targetPanel.texture.dispose();
    if (targetPanel.mesh && targetPanel.mesh.material)
      targetPanel.mesh.material.dispose();
    if (targetPanel.mesh && targetPanel.mesh.geometry)
      targetPanel.mesh.geometry.dispose();
  }
  ...
  function prefetchContentCoversFrom(start, count) {
    ...
    count = Math.max(
      1,
      Math.round(Number(count) || CONTENT_COVER_PREFETCH_LIMIT),
    );
  ...
  function prefetchContentCoversAround(center, reason) {
    ...
    prefetchContentCoversFrom(
      Math.max(0, center - 2),
      CONTENT_COVER_PREFETCH_LIMIT,
    );
    prefetchContentCoversFrom(
      Math.max(0, center + CONTENT_VISIBLE_RADIUS),
      CONTENT_COVER_PREFETCH_LIMIT,
    );
    prefetchContentCoversFrom(
      Math.max(0, allTracks.length - CONTENT_PREFETCH_AHEAD),
      Math.ceil(CONTENT_COVER_PREFETCH_LIMIT * 0.75),
    );
  }
  ...
  function contentPageUrl(offset) {
    ...
    if (contentSource.provider === "qq") {
      return (
        ...
        encodeURIComponent(contentSource.id) +
        "&limit=" +
        limit +
        "&offset=" +
        Math.max(0, offset || 0)
    ...
    if (contentSource.provider === "spotify") {
      return (
        "/api/spotify/playlist/tracks?id=" +
        encodeURIComponent(contentSource.id) +
        "&limit=" +
        limit +
        "&offset=" +
  ...
  async function loadMoreContentRows(reason) {
    if (!open || contentLoadingMore || !contentHasMore || !contentSource)
      return false;
    ...
    try {
      ...
      contentNextOffset = Math.max(
        contentNextOffset + tracks.length,
        Number(r && r.nextOffset) || contentNextOffset + tracks.length,
      );
      var total =
      ...
      if (!added && (!tracks.length || allTracks.length === before))
  ...
  function drawPanelIfNeeded(force, nowT) {
    ...
    if (
      !force &&
      !panelDirty &&
      (!isLoadingContent() || nowT - panelDrawAt < LOADING_ANIM_INTERVAL)
    )
      return;
    drawPanel();
    panelDirty = false;
  ...
  function drawRow(row, song, isCenter) {
    ...
    if (isCenter) {
      rowGrad.addColorStop(
        0,
        "rgba(8,14,24," +
          Math.min(0.985, centerRowBgAlpha + 0.04).toFixed(3) +
          ")",
      );
      rowGrad.addColorStop(
        0.48,
        "rgba(0,0,0," +
          Math.min(0.985, centerRowBgAlpha + 0.03).toFixed(3) +
          ")",
      );
      rowGrad.addColorStop(
        1,
        "rgba(0,0,0," +
          Math.min(0.98, centerRowBgAlpha + 0.015).toFixed(3) +
          ")",
      );
    } else {
      rowGrad.addColorStop(
        0,
        "rgba(16,16,20," + Math.max(0.2, rowBgAlpha - 0.02).toFixed(3) + ")",
      );
      rowGrad.addColorStop(
        1,
        "rgba(0,0,0," + Math.max(0.2, rowBgAlpha - 0.04).toFixed(3) + ")",
      );
    }
    if (isCenter) {
      ctx.shadowColor = canvasAccent(0.2);
      ctx.shadowBlur = 18;
    }
    ...
    if (actionReady || hasSongCover) {
      ...
      if (hasSongCover) {
        ...
        if (songCoverRec && songCoverRec.loaded && songCoverRec.img) {
          ...
          !songCoverRec ||
          (!songCoverRec.loading && !songCoverRec.failed)
        ) {
          requestPlaylistCover(songCover, function () {
            if (row && row.mesh && row.mesh.parent)
              drawRow(row, row.song, !!row.lastCenter);
          });
        }
      }
    }
    ...
    if (loadingRow) {
      ...
      for (var sk = 0; sk < 3; sk++) {
        ...
        skGrad.addColorStop(Math.max(0, hot - 0.18), canvasAccent(0.1));
    ...
    ctx.font = isCenter
      ? '800 24px Inter, "Microsoft YaHei", Arial'
      : '600 20px Inter, "Microsoft YaHei", Arial';
    ...
    if (isCenter && actionReady) {
      if (!isPodcastRadio) {
        ...
        ctx.fill();
        ctx.strokeStyle = liked
          ? "rgba(255,122,144,0.52)"
        ...
        drawCanvasHeart(
          ...
          btnY + 26,
          20,
          liked ? "#ff7a90" : "rgba(255,255,255,0.76)",
        );

        makeRoundRect(ctx, collectX, btnY + 2, miniBtn, btnH - 4, 15);
        var collectGrad = ctx.createLinearGradient(
          collectX,
          btnY + 2,
          collectX + miniBtn,
          btnY + btnH,
        ...
        var nextGrad = ctx.createLinearGradient(
          nextX,
          btnY + 2,
          nextX + miniBtn,
          btnY + btnH,
      ...
      var btnGrad = ctx.createLinearGradient(
        btnX,
        btnY,
        btnX + btnW,
        btnY + btnH,
  ...
  function place(row, i) {
    ...
    row.mesh.renderOrder =
      240 +
      Math.round(
        (CONTENT_VISIBLE_RADIUS +
          1 -
          Math.min(absD, CONTENT_VISIBLE_RADIUS + 1)) *
          14,
      );
    ...
    var revealRaw = Math.max(
      ...
      Math.min(
        1,
        (nowT - rowAnimAt - absD * 0.04) / (layout.rowDuration || 0.72),
      ),
    );
    ...
    var rowSpreadX = skullDetail ? 0.03 : 0.014;
    ...
    var px =
      rowBaseX +
      absD * rowSpreadX +
      (1 - reveal) * (rowIntroX + absD * rowSpreadX) * introStrength;
    var py =
      -delta * layout.rowStep +
      (1 - reveal) * (0.2 + (delta < 0 ? -0.1 : 0.1)) * introStrength;
    var pz =
      (absD < 0.5 ? rowCenterZ : rowBackZ - absD * rowDepthStep) -
      (1 - reveal) * (skullDetail ? 0.1 : 0.16) * introStrength;
    px +=
      settle *
      ((skullDetail ? 0.11 : 0.12) + absD * (skullDetail ? 0.01 : 0.012));
    ...
    pz +=
      (parY * (skullDetail ? 0.014 : 0.024) -
        parX * (skullDetail ? 0.01 : 0.02)) *
      parWeight *
      parallax;
    var scale =
      (absD < 0.5 ? 1.0 : Math.max(0.66, 0.94 - absD * 0.07)) *
      (0.9 + reveal * 0.1) *
      (1 + pulse * 0.052) *
      (1 - settle * 0.025) *
      layout.rowScale;
    ...
    var rowOpacityBase = Math.min(
      1,
      (absD < 0.5 ? 1.0 : Math.max(0.34, 1.0 - absD * 0.12)) * reveal +
        pulse * 0.14,
    );
    var rowOpacityScale =
      absD < 0.5 ? Math.max(0.94, shelfLook.opacity) : shelfLook.opacity;
    row.mesh.material.opacity = Math.min(1, rowOpacityBase * rowOpacityScale);
    row.mesh.rotation.y =
      (skullDetail ? -0.07 : 0.1) +
      (1 - reveal) * (skullDetail ? 0.018 : 0.052) * introStrength +
      parX * (skullDetail ? 0.01 : 0.018) * parWeight * parallax;
    row.mesh.rotation.x =
      (skullDetail ? 0.01 : 0) -
      delta * (skullDetail ? 0.01 : 0.022) -
      parY * (skullDetail ? 0.006 : 0.014) * parWeight * parallax;
  }
  ...
  function disposeCapturedDetail(targetGroup, targetRows, targetPanel) {
    if (targetGroup && targetGroup.parent)
      targetGroup.parent.remove(targetGroup);
  ...
  function startRowsLoadedIntro() {
    ...
    if (window.gsap) {
      ...
      window.gsap.to(group.userData, {
        rowSettle: 0,
        duration: rowDuration + 0.04,
        ease: "expo.out",
      });
    } else {
  ...
  function syncRenderedRows(force) {
    ...
    var refreshLoading =
      isLoadingContent() && nowT - rowDrawAt >= LOADING_ANIM_INTERVAL;
    ...
    rows.forEach(function (row) {
      existingByIndex[row.index] = row;
    });
    var reusable = rows.filter(function (row) {
  ...
  return {
    ...
    open: async function (playlistId, title, fromCard) {
      ...
      contentNextOffset = 0;
      contentTotalCount =
        Number(fromCard && fromCard.item && fromCard.item.trackCount) || 0;
      contentHasMore = false;
      contentLoadingMore = false;
      clearContentWarmPrefetch();
      ...
      var openDynamicDetail =
        !openSkullDetail &&
        shouldUseShelfDynamicCamera("shelf-detail") &&
        camera;
      var openCoverRx =
        particles && particles.rotation ? particles.rotation.x : 0;
      var openCoverRy =
        particles && particles.rotation ? particles.rotation.y : 0;
      var openCoverRz =
        particles && particles.rotation ? particles.rotation.z : 0;
      group.userData.detailIntro = 1;
      if (openDynamicDetail) {
        placeDynamicDetailFromCamera(openLayout, 1, 0, 0);
      } else {
        group.position.set(
          openLayout.x + (openSkullDetail ? 0.1 : 0.16),
          openLayout.y - (openSkullDetail ? 0.02 : 0.024),
          openLayout.z - (openSkullDetail ? 0.05 : 0.07),
        );
      }
      if ((openSkullDetail || openDynamicDetail) && camera) {
        ...
        group.rotation.z = openCoverRz * 0.7;
      }
      ...
      if (window.gsap) {
        ...
        window.gsap.to(group.userData, {
          detailIntro: 0,
          duration: openLayout.openDuration || 0.48,
          ease: "power3.out",
        });
      } else {
      ...
      try {
        ...
        console.warn(
          "[ShelfContentLoadingRender]",
          playlistId,
          renderLoadingErr,
        );
      }
      var podcastCollectionKey =
        String(playlistId || "").indexOf("podcast:") === 0
          ? String(playlistId).slice(8)
          : "";
      var qqPlaylistId =
        String(playlistId || "").indexOf("qq:") === 0
          ? String(playlistId).slice(3)
          : "";
      var kugouPlaylistId =
        String(playlistId || "").indexOf("kugou:") === 0
          ? String(playlistId).slice(6)
          : "";
      var qishuiPlaylistId =
        String(playlistId || "").indexOf("qishui:") === 0
          ? String(playlistId).slice(7)
          : "";
      var spotifyPlaylistId =
        String(playlistId || "").indexOf("spotify:") === 0
          ? String(playlistId).slice(8)
          : "";
      contentKind = podcastCollectionKey ? "podcast" : "playlist";
      contentSource = podcastCollectionKey
        ? null
        : {
            provider: qqPlaylistId
              ? "qq"
              : kugouPlaylistId
                ? "kugou"
                : qishuiPlaylistId
      ...
      try {
        r = podcastCollectionKey
          ? await apiJson(
              "/api/podcast/my/items?key=" +
                encodeURIComponent(podcastCollectionKey) +
                "&limit=" +
                PLAYLIST_LAZY_BATCH_SIZE,
            )
          : qqPlaylistId
            ? await apiJson(
                "/api/qq/playlist/tracks?id=" +
                  encodeURIComponent(qqPlaylistId) +
                  "&limit=" +
                  PLAYLIST_LAZY_BATCH_SIZE +
                  "&offset=0",
              )
            : kugouPlaylistId
              ? await apiJson(
                  "/api/kugou/playlist/tracks?id=" +
                    encodeURIComponent(kugouPlaylistId) +
                    "&limit=" +
                    PLAYLIST_LAZY_BATCH_SIZE +
                    "&offset=0",
                )
              : qishuiPlaylistId
                ? await apiJson(
                    "/api/qishui/playlist/tracks?id=" +
                      encodeURIComponent(qishuiPlaylistId) +
                      "&limit=" +
                      PLAYLIST_LAZY_BATCH_SIZE +
                      "&offset=0",
                  )
                : spotifyPlaylistId
                  ? await apiJson(
                      "/api/spotify/playlist/tracks?id=" +
                        encodeURIComponent(spotifyPlaylistId) +
                        "&limit=" +
                        PLAYLIST_LAZY_BATCH_SIZE +
                        "&offset=0",
                    )
      ...
      try {
        ...
        if (!tracks.length) {
          allTracks = [
            {
              name: podcastCollectionKey ? "播客为空" : "歌单为空",
              artist: "",
            },
          ];
          panelDirty = true;
        ...
        if (!podcastCollectionKey) {
          ...
          contentTotalCount = Math.max(
            contentTotalCount || 0,
            Number(r && (r.total || (r.playlist && r.playlist.trackCount))) ||
              0,
          );
          contentHasMore = !!(r && r.hasMore);
        }
    ...
    close: function () {
      ...
      var materials = targetRows
        .map(function (row) {
          return row.mesh && row.mesh.material;
        })
        .filter(Boolean);
      if (targetPanel && targetPanel.mesh && targetPanel.mesh.material)
        materials.push(targetPanel.mesh.material);
      if (window.gsap) {
        window.gsap.killTweensOf(targetGroup.position);
        window.gsap.killTweensOf(targetGroup.scale);
        window.gsap.to(targetGroup.scale, {
          x: 0.965,
          y: 0.965,
          z: 0.965,
        ...
        window.gsap.to(targetGroup.position, {
          ...
          z: targetGroup.position.z - 0.1 * closeIntro,
          duration: closeDuration,
          ease: "power2.in",
        });
    ...
    update: function (dt) {
      ...
      var parallax = layout.parallax == null ? 1 : layout.parallax;
      var dynamicDetail =
        !skullDetail && shouldUseShelfDynamicCamera("shelf-detail") && camera;
      var coverBoundDetail =
        !skullDetail && !dynamicDetail && particles && particles.rotation;
      var coverBindX = coverBoundDetail ? particles.rotation.y * 0.18 : 0;
      var coverBindY = coverBoundDetail ? particles.rotation.x * -0.16 : 0;
      var coverBindZ = coverBoundDetail
        ? Math.abs(particles.rotation.y) * 0.03
        : 0;
      if (dynamicDetail) {
        ...
        group.position.set(
          layout.x +
            coverBindX +
            introMix * (skullDetail ? 0.1 : 0.16) +
            parX * (skullDetail ? 0.024 : 0.03) * parallax,
          layout.y +
            coverBindY -
            introMix * (skullDetail ? 0.02 : 0.024) +
            parY * (skullDetail ? 0.026 : 0.026) * parallax,
          layout.z +
            coverBindZ -
            introMix * (skullDetail ? 0.05 : 0.07) +
            parY * (skullDetail ? 0.014 : 0.016) * parallax -
            parX * (skullDetail ? 0.01 : 0.01) * parallax,
        );
      }
      if (skullDetail && camera) {
        ...
        group.rotation.y +=
          (coverRy * 0.82 +
            ...
            group.rotation.y) *
          0.16;
        group.rotation.z += (coverRz * 0.7 - group.rotation.z) * 0.14;
      }
      group.scale.setScalar(
        layout.scale * (1 - introMix * (skullDetail ? 0.02 : 0.035)),
      );
      ...
      if (panel && panel.mesh) {
        var pr = Math.max(
          ...
          Math.min(
            1,
            (uniforms.uTime.value - openAnimAt) / (layout.openDuration || 0.72),
          ),
        );
        pr = pr * pr * (3 - 2 * pr);
    ...
    scrollBy: function (d) {
      if (allTracks.length) {
        ...
        centerTarget = Math.max(
          0,
          Math.min(allTracks.length - 1, centerTarget + d),
        );
        var nextTarget = Math.round(centerTarget);
        syncRenderedRows(false);
    ...
    screenContainsPanel: function (sx, sy) {
      ...
      var params =
        (panel.mesh.geometry && panel.mesh.geometry.parameters) || {};
    ...
    rowActionAtScreen: function (row, sx, sy) {
      ...
      if (!isCenter || !((song && song.id) || song.type === "podcast-radio"))
        return null;
      var params = (row.mesh.geometry && row.mesh.geometry.parameters) || {};
      var hw = (params.width || 2.5) / 2;
      var hh = (params.height || 0.36) / 2;
    ...
    playRow: function (row) {
      ...
      if (row.song && row.song.type === "podcast-radio") {
        loadPodcastRadioIntoQueue(
          row.song.id || row.song.radioId,
          true,
          row.song.name || playlistTitle,
        );
      ...
      var playIndex =
        allTracks.slice(0, idx + 1).filter(function (song) {
          return song && song.id;
        }).length - 1;
      var allSongs = allTracks
      ...
      var queuePlaylistId =
        ...
        contentSource.provider !== "netease"
  ...
  function makeRow(song, i) {
    ...
    var geo = new THREE.PlaneGeometry(2.5, 0.36, 1, 1);
    var mesh = new THREE.Mesh(geo, mat);
