...
function playbackRestrictionLooksVipLocked(song, data) {
  ...
  var text = [
    ...
    .join(" ");
  return /vip_required|paid_required|trial_only|need_vip|only_vip|member|vip|会员|付费|购买|数字专辑|专辑/.test(
    text,
  );
}
...
function playbackRestrictionCategory(song, data) {
  ...
  var mergedStatus = Object.assign(
    {},
    status,
    data || {},
  ...
  var vipLevel =
    typeof providerVipLevel === "function"
      ? providerVipLevel(provider, mergedStatus)
      : "none";
  ...
  if (vipLocked && !playbackRestrictionMissingPlaybackKey(data)) {
    ...
    if (
      ...
      if (loggedIn && vipLevel === "none" && !membershipUnknown)
        return "vip_required";
    }
  }
  if (
    ...
    data &&
    data.error &&
    /401|403|login_required|auth|cookie|credential|unauthorized|forbidden/i.test(
      String(data.error),
    )
  )
    return loggedIn && vipLocked ? "vip_required" : "login_required";
  if (
    !category &&
    data &&
    data.error &&
    /vip|member|paid|trial|会员|付费|购买/i.test(String(data.error))
  )
    return loggedIn ? "vip_required" : "login_required";
...
function playbackRestrictionNotice(song, data) {
  ...
  if (
    ...
    var body =
      ...
      provider +
        ...
        needText +
        "。";
    if (loggedIn && body.indexOf("当前状态") < 0)
      body += " 当前状态是 " + membership + "。";
    return {
      category: category,
      title: title,
      body: body + " 可以登录会员账号、降低音质或切换到其它音源。",
      action: "upgrade",
      toast: title,
    };
  }
  if (category === "login_required") {
    ...
    return {
      ...
      title: "当前平台未登录",
      body:
        (message || provider + " 需要登录后才能获取播放地址。") +
        " 正在打开对应登录入口。",
      action: "login",
      toast: "当前平台未登录",
    };
  ...
  if (category === "provider_limited") {
    return {
      ...
      body:
        message ||
        provider + " 当前只提供搜索/匹配信息，播放会自动寻找其它可播版本。",
  ...
  if (category === "copyright_unavailable") {
    return {
      ...
      title: "当前平台版权不可播",
      body:
        (message || provider + " 当前版权暂不可播。") + " 可以换一个平台版本。",
      action: "switch_source",
      toast: "版权不可播",
    };
  }
  return {
    ...
    title: "当前平台没有可用音源",
    body:
      (message || provider + " 没有返回可播放地址。") +
      " 可能是版权、地区、会员或网络限制，可以换源或稍后重试。",
    action: "switch_source",
    toast: "当前平台没有可用音源",
  };
}
function playbackRestrictionMessage(song, data) {
  ...
  if (!message) {
    if (category === "login_required")
      message = provider + "需要登录后再尝试播放";
    else if (category === "vip_required")
      message = provider + "歌曲需要会员权限";
    else if (category === "paid_required")
      message = provider + "歌曲需要购买或更高权限";
    else if (category === "trial_only") message = provider + "仅返回试听片段";
    else if (category === "copyright_unavailable")
      message = provider + "版权暂不可播";
    else if (category === "provider_limited")
      message = provider + "当前只作为匹配源，正在寻找其它可播版本";
...
function qqPlaybackRetryQualities(requestedQuality, resolvedLevel) {
  requestedQuality = normalizePlaybackQualityForProvider(
    requestedQuality || getProviderPlaybackQuality("qq"),
    "qq",
  );
...
async function retryQQPlaybackWithCompatibleQuality(
  ...
  opts = opts || {};
  if (
    playbackRestrictionCategory(song, data) === "login_required" ||
    playbackRestrictionMissingPlaybackKey(data)
  )
    return false;
  var tried = Array.isArray(opts.qqQualityTried)
    ? opts.qqQualityTried.slice()
    : [];
  [requestedQuality, data && data.level].forEach(function (q) {
    q = normalizePlaybackQuality(q || "");
    if (q && tried.indexOf(q) < 0) tried.push(q);
  });
  var candidates = qqPlaybackRetryQualities(
    requestedQuality,
    data && data.level,
  ).filter(function (q) {
    return tried.indexOf(q) < 0;
  });
  ...
  if (!opts.startupAutoplay)
    showSourceFallbackNotice(
      "QQ 音质自动兼容",
      "当前音质启动失败，正在切到 " +
        playbackQualityLabel(nextQuality, "qq") +
        "。",
    );
  ...
  if (
    ...
    retryResumeAt = pendingPlaybackResumeAt;
  var retryStarted = await playQueueAt(
    idx,
    Object.assign({}, opts, {
...
function closeSourceFallbackNotice() {
  ...
  if (sourceFallbackNoticeTimer) {
    clearTimeout(sourceFallbackNoticeTimer);
  ...
  if (stack)
    Array.prototype.slice
      .call(stack.children || [])
      .forEach(removeSourceFallbackCard);
}
...
function isSameTitleArtist(source, candidate) {
  ...
  if (
    normalizeMatchText(source.name || source.title) !==
    normalizeMatchText(candidate.name || candidate.title)
  )
    return false;
  var a = artistNameParts(source);
  var b = artistNameParts(candidate);
  if (!a.length || !b.length) return false;
  return a.some(function (name) {
...
function completeSourceFallbackRecovery(recovery) {
  ...
  if (activeSourceFallbackRecovery === recovery)
    activeSourceFallbackRecovery = null;
  return true;
}
...
function ensureSourceFallbackRecovery(opts, song, idx, token) {
  var recovery = sourceFallbackRecoveryFromOptions(opts);
  if (recovery)
    return sourceFallbackRecoveryIdentityActive(recovery) ? recovery : null;
...
function settleExpiredSourceFallbackPlayback(idx, token, opts, message) {
  ...
  if (opts && opts.fallbackOriginalSong && opts.fallbackCandidateSong) {
    restoreSourceFallbackQueueItem(
      idx,
      opts.fallbackOriginalSong,
      opts.fallbackCandidateSong,
      token,
    );
  }
...
function sourceFallbackProviderAttemptKey(recovery, song, provider) {
  return (
    (sourceFallbackRecoveryContentKey(song) || sourceFallbackSongKey(song)) +
    "|" +
...
function beginSourceFallbackProviderAttempt(recovery, song, provider) {
  ...
  if (recovery.providerAttempts >= SOURCE_FALLBACK_MAX_PROVIDER_ATTEMPTS)
    return false;
...
function sourceFallbackProviderReady(provider) {
  ...
  var status =
    typeof platformStatus === "function" ? platformStatus(provider) : null;
  if (!status || !status.loggedIn) return false;
  if (provider === "qq" || provider === "kugou")
    return status.playbackKeyReady === true;
  return true;
}
function alternatePlaybackProviders(song) {
  ...
  ordered.concat(SOURCE_FALLBACK_DIRECT_PROVIDERS).forEach(function (provider) {
    ...
    if (
      ...
      !sourceFallbackProviderReady(provider)
    )
      return;
    seen[provider] = true;
...
async function searchAlternatePlatformSong(song, requestedTarget, recovery) {
  ...
  var query = [song.name || song.title || "", song.artist || artist]
    .filter(Boolean)
    .join(" ")
    .trim();
  ...
  if (
    data === sourceFallbackBudgetTimeoutResult ||
    (recovery && !sourceFallbackRecoveryCanContinue(recovery))
  )
    return null;
  ...
  for (var i = 0; i < list.length; i++) {
    if (
      ...
      sourceCandidateRejectReason(song, list[i], target)
    )
      continue;
...
function sourceFallbackSongKey(song) {
  ...
  return [
    ...
    song.id || song.mid || song.hash || "",
    song.name || song.title || "",
    song.artist || "",
  ].join(":");
}
function restoreSourceFallbackQueueItem(
  idx,
  originalSong,
  candidateSong,
  expectedToken,
...
function settleSourceFallbackTerminal(idx, token, message, opts) {
  ...
  if (typeof clearAlbumGaplessPreload === "function")
    clearAlbumGaplessPreload("source-fallback-terminal");
  if (typeof resetCuefieldAutoMix === "function")
    resetCuefieldAutoMix("source-fallback-terminal");
  if (typeof clearPlaybackResumeWatchdogs === "function")
  ...
  if (typeof syncPlaybackStateFromAudioEvent === "function")
    syncPlaybackStateFromAudioEvent("source-fallback-terminal");
  if (!opts.silent)
    showSourceFallbackNotice(
      "当前没有可用音源",
      message || "当前歌曲不可播放，并且没有其它已登录、已授权的音源可接管。",
    );
  return false;
}
function markQueueItemPlaybackFailed(idx, recovery) {
  ...
  playQueue[idx]._lastPlaybackFailRecoveryId =
    recovery && recovery.id ? recovery.id : "";
}
...
function recentQueuePlaybackFailureCount(recovery) {
  ...
  for (var index = 0; index < playQueue.length; index++) {
    var failedAt =
      Number(playQueue[index] && playQueue[index]._lastPlaybackFailAt) || 0;
    if (
      recovery &&
      playQueue[index] &&
      playQueue[index]._lastPlaybackFailRecoveryId !== recovery.id
    )
      continue;
...
function nextUnblockedQueueIndex(idx, recovery) {
  ...
  for (var step = 1; step < playQueue.length; step++) {
    ...
    var failedAt =
      Number(playQueue[nextIdx] && playQueue[nextIdx]._lastPlaybackFailAt) || 0;
    var recoveryKey = sourceFallbackRecoveryContentKey(playQueue[nextIdx]);
    if (recovery && recoveryKey && recovery.visitedSongKeys[recoveryKey])
      continue;
    var failedInRecovery =
      !recovery ||
      (playQueue[nextIdx] &&
        playQueue[nextIdx]._lastPlaybackFailRecoveryId === recovery.id);
    if (!failedInRecovery || !failedAt || now - failedAt > 18000)
      return nextIdx;
  }
  return -1;
}
function isQueueItemRecentlyPlaybackFailed(idx) {
  var failedAt =
    Number(playQueue[idx] && playQueue[idx]._lastPlaybackFailAt) || 0;
...
async function skipFailedQueueItem(idx, token, message, opts) {
  ...
  var terminalOpts = Object.assign({}, opts, {
    sourceFallbackRecovery: recovery,
  });
  ...
  if (playQueue.length <= 1) {
    return settleSourceFallbackTerminal(
      idx,
      token,
      message || "当前歌曲不可播放，队列里没有其他歌曲。",
      terminalOpts,
    );
  }
  if (
    recentQueuePlaybackFailureCount(recovery) >=
    Math.min(MAX_RECENT_AUTO_QUEUE_FAILURES, playQueue.length)
  ) {
  ...
  if (!opts.silent)
    showSourceFallbackNotice(
      "已跳过受限歌曲",
      message || "未找到同名同歌手的另一个平台版本，正在播放下一首。",
    );
  ...
  if (nextStarted === true) completeSourceFallbackRecovery(recovery);
    ...
    !sourceFallbackRecoveryCanContinue(recovery)
  ) {
    return settleSourceFallbackTerminal(
      currentIdx,
      trackSwitchToken,
...
async function tryAutoPlaybackFallback(song, data, idx, token, opts) {
  ...
  if (!opts.startupAutoplay) {
    showSourceFallbackNotice(
      ...
      fromLabel +
        " 当前不可播，正在检查 " +
        alternateProviders.map(sourceFallbackProviderTitle).join("、") +
        " 的同名同歌手版本。",
    );
  }
  for (
    var providerIndex = 0;
    providerIndex < alternateProviders.length;
    providerIndex++
  ) {
    var alternateProvider = alternateProviders[providerIndex];
    if (
      !beginSourceFallbackProviderAttempt(recovery, song, alternateProvider)
    ) {
      if (!sourceFallbackRecoveryCanContinue(recovery)) {
        return settleSourceFallbackTerminal(
          idx,
    ...
    try {
      var alternate = await searchAlternatePlatformSong(
        song,
        alternateProvider,
        recovery,
      );
      ...
      var alternateData =
        typeof resolveAlbumGaplessPlaybackData === "function"
          ? await awaitSourceFallbackBudget(
              resolveAlbumGaplessPlaybackData(alternate),
              recovery,
            )
          : null;
      if (token !== trackSwitchToken) return false;
      if (
        alternateData === sourceFallbackBudgetTimeoutResult ||
        !sourceFallbackRecoveryCanContinue(recovery)
      ) {
        return settleSourceFallbackTerminal(
          idx,
      ...
      safeRenderQueuePanel("source-fallback-provisional", {
        scrollCurrent: miniQueueOpen,
      });
...
function handlePlaybackUnavailable(song, data) {
  ...
  showToast(
    notice.toast || notice.title || playbackRestrictionMessage(song, data),
  );
