...
function playbackRestrictionCategory(song, data) {
  ...
  if (vipLocked && !playbackRestrictionMissingPlaybackKey(data)) {
    if (category === "login_required" && loggedIn && vipLevel === "none")
      return "vip_required";
    if (
      !category ||
...
async function searchAlternatePlatformSong(song, requestedTarget) {
  ...
  var data = await apiJson(url, {
    timeoutMs: SOURCE_FALLBACK_SEARCH_TIMEOUT_MS,
  });
...
async function skipFailedQueueItem(idx, token, message, opts) {
  ...
  if (
    recentQueuePlaybackFailureCount() >=
  ...
  var nextPlaybackOpts = Object.assign(
    {},
    opts.playbackOpts || { fallbackDepth: 0 },
...
async function tryAutoPlaybackFallback(song, data, idx, token, opts) {
  ...
  var skipOpts = opts.startupAutoplay
    ? { silent: true, playbackOpts: skipPlaybackOpts }
    : null;
