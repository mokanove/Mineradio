...
function audioGraphHealthy() {
  return !!(
    ...
    analyser &&
    beatAnalyser &&
    (gainNode || analysisSinkNode)
  );
}
function disconnectAudioGraphNodes(keepSource) {
  [source, analyser, beatAnalyser, gainNode, analysisSinkNode].forEach(
    function (node) {
      if (!node) return;
      try {
        node.disconnect();
    ...
    try {
      if (media.duration && isFinite(media.duration))
        media.currentTime = Math.min(
          seconds,
          Math.max(0, media.duration - 0.25),
        );
...
function replaceAudioElementForGraphRecovery(reason, opts) {
  ...
  var seconds =
    preservePlayback && isFinite(oldAudio.currentTime)
...
function resetPlaybackAudioGraphForSourceSwitch(reason) {
  ...
  if (sourceUsesCapture && previousSourceMedia === audio) {
    replaceAudioElementForGraphRecovery(reason || "capture-track-switch", {
...
function initAudio() {
  ...
  if (audioCtx && audioCtx.state === "closed")
    replaceAudioElementForGraphRecovery("closed-context");
  if (!audioCtx || audioCtx.state === "closed")
  ...
  if (!source) {
    ...
    if (
      !forceCapture &&
      !audio.__mineradioMediaSourceBound &&
      ...
      try {
        ...
        audio.__mineradioMediaSourceBound = true;
        console.warn(
          "media element source unavailable:",
    ...
    if (!forceCapture && !mediaSource && audio.__mineradioMediaSourceBound) {
      ...
      try {
        ...
        mediaSource = null;
        console.warn(
          "media element source recovery failed:",
          rebindingErr && (rebindingErr.message || rebindingErr),
        );
      }
    }
    ...
    if (!mediaSource) {
      ...
      try {
        if (audio && audio.captureStream && audioCtx.createMediaStreamSource)
          capturedStream = audio.captureStream();
      } catch (captureErr) {
        capturedStream = null;
        console.warn(
          "capture stream source unavailable:",
          captureErr && (captureErr.message || captureErr),
        );
      }
    }
    ...
    try {
      ...
      audioSourceMedia = null;
      console.warn(
        "capture stream graph unavailable:",
        captureSourceErr && (captureSourceErr.message || captureSourceErr),
      );
      return false;
    }
...
function readPlaybackAnalyserSignal() {
  ...
  try {
    ...
    for (var i = 0; i < timeDomainData.length; i += step)
      timeSum += Math.abs(timeDomainData[i] - 128);
    var freqStep = Math.max(1, Math.floor(frequencyData.length / 256));
    for (var j = 0; j < frequencyData.length; j += freqStep)
      freqSum += frequencyData[j];
    return (
      timeSum / Math.max(1, Math.ceil(timeDomainData.length / step)) / 128 +
      freqSum / Math.max(1, Math.ceil(frequencyData.length / freqStep)) / 255
    );
  } catch (e) {
    return 0;
  }
...
function rebuildPlaybackGraphWithCapture(reason) {
  ...
  if (audio.__mineradioMediaSourceBound) {
    if (!source || source.__mineradioUsesCapture || audioSourceMedia !== audio)
      return false;
    disconnectAudioGraphNodes(true);
    var reused = initAudio();
    if (reused)
      console.warn(
        "audio analyser reconnected with lifetime media source:",
        reason || "silent-graph",
      );
    return reused;
  }
  ...
  if (ok)
    console.warn(
      "audio analyser recovered with capture stream:",
      reason || "silent-graph",
    );
  return ok;
}
...
function schedulePlaybackAnalyserRecovery(reason) {
  ...
  var recoverySrc =
    recoveryMedia && (recoveryMedia.currentSrc || recoveryMedia.src || "");
  var previousMediaTime =
    recoveryMedia && isFinite(recoveryMedia.currentTime)
      ? recoveryMedia.currentTime
      : 0;
  var advancingSilentSamples = 0;
  [720, 1600, 2800].forEach(function (delay) {
    setTimeout(function () {
      if (
        serial !== playbackAnalyserRecoverySerial ||
      ...
      if (!audioReady || !analyser || !source) {
        ensurePlaybackAudioGraph(
          "analyser-health-missing-" + (reason || "playback"),
        );
        return;
      }
      ...
      if (source && !source.__mineradioUsesCapture && audio.captureStream) {
        ...
        ensurePlaybackAudioGraph(
          "analyser-health-capture-" + (reason || "playback"),
        );
      }
    }, delay);
  });
...
async function ensurePlaybackAudioGraph(reason) {
  ...
  if (!audioGraphHealthy())
    console.warn("audio graph still unhealthy:", reason || "playback");
...
function ensureUiSfxContext() {
  ...
  applyAudioOutputDevice(audio);
  if (uiSfxCtx.state === "suspended" && uiSfxCtx.resume)
    uiSfxCtx.resume().catch(function () {});
  return uiSfxCtx;
}
...
function playShelfSelectTick(direction, variant) {
  ...
  var volumeScale =
    0.38 +
    Math.max(0, Math.min(1, targetVolume == null ? 0.65 : targetVolume)) * 0.62;
  ...
  clickOsc("triangle", 720, 0.0, 0.03, 0.18, 0.7);
  clickOsc("square", 2180, 0.004, 0.022, 0.3, 0.86);
  clickOsc("triangle", 4200, 0.011, 0.018, 0.18, 0.94);
  clickOsc("square", 7100, 0.018, 0.012, 0.07, 0.98);
  setTimeout(function () {
    try {
      out.disconnect();
    } catch (_) {}
...
function clearAudioAudibilityRecoveryTimers() {
  if (!audioAudibilityRecoveryTimers || !audioAudibilityRecoveryTimers.length)
    return;
  audioAudibilityRecoveryTimers.forEach(function (timer) {
    clearTimeout(timer);
  });
  audioAudibilityRecoveryTimers = [];
}
function currentAudioOutputGain() {
  if (isFinite(audioFadeEnvelope))
    return clampRange(targetVolume * audioFadeEnvelope, 0, 1);
  if (audio && isFinite(audio.volume))
    return clampRange(Number(audio.volume), 0, 1);
  if (gainNode && gainNode.gain && isFinite(gainNode.gain.value))
    return clampRange(Number(gainNode.gain.value), 0, 1);
...
function writeAudioOutputGain(value) {
  ...
  audioFadeEnvelope =
    targetVolume > 0.001
      ? clampRange(value / targetVolume, 0, 1)
      : value > 0.001
        ? 1
        : 0;
...
function ensureAudiblePlaybackGain(reason) {
  ...
  if (
    ...
    return false;
  var current = currentAudioOutputGain();
  var floor = Math.max(0.004, targetVolume * 0.1);
  if (current > floor) return false;
  setAudioOutputGainImmediate(targetVolume);
  console.warn(
    "[AudioFade] restored silent playback gain:",
    reason || "playback",
  );
  return true;
}
...
function updateVolumeUi() {
  ...
  if (slider && Math.abs(parseFloat(slider.value) - targetVolume) > 0.001)
...
function updateAudioFadeUi() {
  ...
  if (
    fadeInSlider &&
    Math.abs(Number(fadeInSlider.value) - Number(inSeconds)) > 0.001
  )
    fadeInSlider.value = inSeconds;
  if (
    fadeOutSlider &&
    Math.abs(Number(fadeOutSlider.value) - Number(outSeconds)) > 0.001
  )
    fadeOutSlider.value = outSeconds;
  if (fadeInValue)
    fadeInValue.textContent = audioFadeSecondsLabel(AUDIO_FADE_IN_MS);
  if (fadeOutValue)
    fadeOutValue.textContent = audioFadeSecondsLabel(AUDIO_FADE_OUT_MS);
}
function setAudioFadeSetting(kind, seconds, silent) {
  var ms = normalizeAudioFadeMs(
    Number(seconds) * 1000,
    kind === "in" ? 460 : 420,
  );
  ...
  updateAudioFadeUi();
  if (!silent)
    showToast((kind === "in" ? "淡入 " : "淡出 ") + audioFadeSecondsLabel(ms));
}

function setVolume(value, silent) {
  var next = Math.max(0, Math.min(1, Number(value) || 0));
  var previous = targetVolume;
  var shouldRestoreAudibleEnvelope =
    next > 0.001 &&
    !audioFadeTimer &&
    (previous <= 0.001 || clampRange(audioFadeEnvelope, 0, 1) <= 0.0015);
  targetVolume = next;
  if (next > 0.01) lastNonZeroVolume = next;
  try {
    localStorage.setItem("apex-player-volume", String(next));
  } catch (e) {}
...
function bindVolumeControls() {
  ...
  if (slider) {
    ...
    slider.addEventListener("change", function () {
      showToast("音量 " + Math.round(targetVolume * 100) + "%");
    });
  }
  if (fadeInSlider) {
    fadeInSlider.addEventListener("input", function () {
      setAudioFadeSetting("in", fadeInSlider.value, true);
    });
    ...
    fadeInSlider.addEventListener("change", function () {
      setAudioFadeSetting("in", fadeInSlider.value, false);
    });
  }
  if (fadeOutSlider) {
    fadeOutSlider.addEventListener("input", function () {
      setAudioFadeSetting("out", fadeOutSlider.value, true);
    });
    ...
    fadeOutSlider.addEventListener("change", function () {
      setAudioFadeSetting("out", fadeOutSlider.value, false);
    });
  }
  if (btn) {
