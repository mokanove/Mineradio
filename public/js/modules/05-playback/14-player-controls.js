...
function isSameAudioPlaybackTarget(media, src) {
  return !!(
    ...
    media &&
    audio === media &&
    (audio.currentSrc || audio.src || "") === src
  );
}
...
function clearPlaybackResumeWatchdogs() {
  if (
    ...
    !Array.isArray(playbackResumeRecovery.timerIds)
  )
    return;
  playbackResumeRecovery.timerIds.forEach(function (timerId) {
...
function updatePlaybackResumePauseMarker(reason) {
  ...
  if (reason === "pause" || reason === "manual-pause") {
    ...
    playbackResumeRecovery.pausedSongKey =
      typeof queueItemKey === "function" ? queueItemKey(song) : "";
    playbackResumeRecovery.pausedSrc = src;
    playbackResumeRecovery.pausedPosition = isFinite(audio.currentTime)
...
function playbackResumeLongPauseThresholdMs(song) {
  ...
  var providerMs =
    PLAYBACK_RESUME_LONG_PAUSE_PROVIDER_MS &&
    PLAYBACK_RESUME_LONG_PAUSE_PROVIDER_MS[provider];
  return Math.max(
    30000,
    Number(providerMs || PLAYBACK_RESUME_LONG_PAUSE_MS || 0) || 8 * 60 * 1000,
  );
}
...
function playbackResumePausedLongEnough(song) {
  ...
  return (
    Date.now() - playbackResumeRecovery.pausedAt >=
    playbackResumeLongPauseThresholdMs(song)
  );
}
...
function isQishuiTrackStartStalled(song, opts, media, startTime, current) {
  opts = opts || {};
  if (
    !(opts.trackSwitch || opts.manual || opts.fastResume) ||
    opts.resumeRecovery
  )
    return false;
...
async function nudgeQishuiTrackStart(media, src, token) {
  if (
    !isSameAudioPlaybackTarget(media, src) ||
  ...
  try {
    ...
    console.warn(
      "[PlaybackResumeRecovery] qishui start nudge failed:",
      err && (err.message || err),
    );
    return false;
  }
...
function qishuiTrackStartResumeSeconds(media, current, startTime) {
  var target = Math.max(
    Number(current) || 0,
    Number(startTime) || 0,
    qishuiTrackStartNudgeSeconds(media),
  );
  var duration = media && isFinite(media.duration) ? Number(media.duration) : 0;
...
function showQishuiTrackStartStallNotice() {
  ...
  if (now - (playbackResumeRecovery.lastQishuiStartNoticeAt || 0) < 8000)
    return;
  playbackResumeRecovery.lastQishuiStartNoticeAt = now;
  var title = "汽水播放未响应";
  var body =
    "音频开头解码卡住，已尝试重新接入；如果仍不播放，请拖动一下进度或切换音质。";
...
function playbackFreshUrlRecoverySongKey(song) {
  ...
  return song
    ? [songProviderKey(song), song.id || song.mid || song.hash || ""].join(":")
    : "";
}
...
function resetPlaybackFreshUrlRecoveryBudget(song) {
  ...
  playbackResumeRecovery.freshUrlSongKey =
    playbackFreshUrlRecoverySongKey(song);
...
function playbackStallRecoveryTransaction(song, opts) {
  ...
  var recovery =
    typeof sourceFallbackRecoveryFromOptions === "function"
      ...
      : null;
  var recoverySongKey =
    typeof sourceFallbackRecoveryContentKey === "function"
  ...
  if (
    ...
    (!recoverySongKey ||
      activeSourceFallbackRecovery.visitedSongKeys[recoverySongKey])
  ) {
    recovery = activeSourceFallbackRecovery;
  }
  if (!recovery && typeof ensureSourceFallbackRecovery === "function") {
    recovery = ensureSourceFallbackRecovery(
      {},
      song,
      currentIdx,
      trackSwitchToken,
...
async function recoverCurrentTrackPlaybackFromFreshUrl(reason, opts) {
  ...
  if (playbackResumeRecovery.freshUrlSongKey !== songKey)
    resetPlaybackFreshUrlRecoveryBudget(song);
  var now = performance.now();
  if (
    playbackResumeRecovery.pending ||
    now - (playbackResumeRecovery.lastAttemptAt || 0) < 1200
  )
    return false;
  ...
  playbackResumeRecovery.freshUrlAttemptCount =
    (Number(playbackResumeRecovery.freshUrlAttemptCount) || 0) + 1;
...
function playbackStallRecoveryOwnerStillCurrent(
  media,
  src,
  token,
  recoverySerial,
  ...
  if (
    token !== trackSwitchToken ||
    recoverySerial !== playbackResumeRecovery.serial
  )
    return false;
  if (media.paused || media.ended || media.seeking) return false;
  if (queueKey && String(media.__mineradioQueueItemKey || "") !== queueKey)
    return false;
  if (
    typeof playbackMediaMatchesCurrentQueueItem === "function" &&
    !playbackMediaMatchesCurrentQueueItem(media)
  )
    return false;
  return true;
}
...
function schedulePlaybackStallRecovery(reason, opts) {
  ...
  if (
    opts.ownerToken != null &&
    Number(opts.ownerToken) !== Number(trackSwitchToken)
  )
    return;
  var queueKey = String(
    opts.ownerQueueItemKey || media.__mineradioQueueItemKey || "",
  );
  if (queueKey && String(media.__mineradioQueueItemKey || "") !== queueKey)
    return;
  if (
  ...
  clearPlaybackResumeWatchdogs();
  playbackResumeRecovery.serial =
    (Number(playbackResumeRecovery.serial) || 0) + 1;
  ...
  PLAYBACK_RESUME_STALL_DELAYS.forEach(function (delayMs) {
    var timerId = setTimeout(async function () {
      if (
        !playbackStallRecoveryOwnerStillCurrent(
          media,
          src,
          token,
          recoverySerial,
      ...
      var qishuiStartStall = isQishuiTrackStartStalled(
        song,
        opts,
        media,
        startTime,
      ...
      if (qishuiStartStall && delayMs < 3000) {
        try {
          ...
          console.warn(
            "[PlaybackResumeRecovery] qishui graph precheck failed:",
            nudgeGraphErr,
          );
        }
        if (
      ...
      if (
        delayMs < 3000 &&
        media.readyState >= 2 &&
        media.networkState !== media.NETWORK_NO_SOURCE
      )
        return;
      try {
        ...
        console.warn(
          "[PlaybackResumeRecovery] graph precheck failed:",
          graphErr,
        );
      }
      if (
      ...
      if (current >= startTime + minAdvance) return;
      qishuiStartStall = isQishuiTrackStartStalled(
        song,
        opts,
        media,
        startTime,
      ...
      var recovered = await recoverCurrentTrackPlaybackFromFreshUrl(
        qishuiStartStall
          ? "qishui-track-start-stalled"
          : reason || "resume-stalled",
        {
          resumeAt: qishuiStartStall
            ? qishuiTrackStartResumeSeconds(media, current, startTime)
            : current || startTime,
          silent: opts.silent,
        },
      );
...
function awaitMediaPlayWithTimeout(media, playPromise, token, timeoutMs) {
  timeoutMs = Math.max(
    1000,
    Number(timeoutMs) || AUDIO_PLAY_REQUEST_TIMEOUT_MS,
  );
  return new Promise(function (resolve, reject) {
    ...
    var timer = setTimeout(function () {
      ...
      var timeoutError = new Error(
        "AUDIO_PLAY_TIMEOUT: media.play() did not start within " +
          timeoutMs +
          "ms",
      );
...
function playbackMediaMatchesCurrentQueueItem(media) {
  if (!media || !media.src || currentIdx < 0 || currentIdx >= playQueue.length)
    return false;
  var song = playQueue[currentIdx];
  var expectedKey =
    typeof queueItemKey === "function" ? queueItemKey(song) : "";
...
async function completeAudioPlayStart(
  opts,
  reason,
  expectedMedia,
  expectedToken,
  ...
  if (typeof markStageLyricsPlaybackResume === "function")
    markStageLyricsPlaybackResume(reason || "playback-started");
  if (opts.trackSwitch) primeCinemaAfterTrackStart(reason || "track-switch");
  if (
    opts.trackSwitch &&
    !opts.resumeRecovery &&
    typeof resetPlaybackFreshUrlRecoveryBudget === "function"
  ) {
    var startedSong =
      playQueue && currentIdx >= 0 && currentIdx < playQueue.length
        ? playQueue[currentIdx]
  ...
  setTimeout(async function () {
    ...
    try {
      ...
      await ensurePlaybackAudioGraph(
        (reason || "manual-resume-fast") + "-deferred-graph",
      );
      ensureAudiblePlaybackGain(
        (reason || "manual-resume-fast") + "-deferred-gain",
      );
    } catch (err) {
...
async function resumePausedAudioFast(opts) {
  ...
  try {
    ...
    if (!isSameAudioPlaybackTarget(media, src) || token !== trackSwitchToken)
      return false;
    switchPlaybackVisualToEmily();
    playing = true;
    setPlayIcon(true);
    ...
    schedulePausedAudioResumeMaintenance(
      ...
      token,
      "manual-resume-fast",
      { manual: true, silent: true, fastResume: true },
    );
    return true;
  } catch (err) {
...
async function retryTrackSwitchAudioPlayOnce(
  opts,
  originalErr,
  expectedMedia,
  expectedToken,
  ...
  if (
    !playbackAttemptStillCurrent(retryAudio, expectedToken) ||
    !isSameAudioPlaybackTarget(retryAudio, retrySrc)
  )
    return null;
  if (
    retryAudio.readyState === 0 ||
    retryAudio.networkState === retryAudio.NETWORK_EMPTY
  ) {
    try {
      retryAudio.load();
    } catch (e) {}
  ...
  if (!playbackAttemptStillCurrent(retryAudio, expectedToken)) return null;
  return await completeAudioPlayStart(
    opts,
    "track-switch-retry-started",
    retryAudio,
    expectedToken,
  );
}
...
async function attemptAudioPlay(opts) {
  ...
  var expectedToken =
    opts.expectedToken == null ? trackSwitchToken : Number(opts.expectedToken);
  try {
    if (!playbackAttemptStillCurrent(expectedMedia, expectedToken))
      return false;
    var currentSongForResume =
      playQueue && currentIdx >= 0 && currentIdx < playQueue.length
        ? playQueue[currentIdx]
        : null;
    if (
      ...
      audio.paused &&
      !audio.ended &&
      playbackResumePausedLongEnough(currentSongForResume)
    ) {
      var staleResumeAt = currentResumeSeconds(
        playbackResumeRecovery && playbackResumeRecovery.pausedPosition,
      );
      var refreshedResume = await recoverCurrentTrackPlaybackFromFreshUrl(
        "long-pause-stale-source",
        {
    ...
    if (opts.manual || opts.trackSwitch) {
      ...
      await ensurePlaybackAudioGraph(
        opts.manual
          ? "manual-after-play-request"
          : "track-switch-after-play-request",
      );
      ...
      if (!playbackAttemptStillCurrent(expectedMedia, expectedToken))
        return false;
      await ensurePlaybackAudioGraph(
        opts.startupAutoplay ? "startup-before-play" : "auto-before-play",
      );
      if (!playbackAttemptStillCurrent(expectedMedia, expectedToken))
        return false;
      ...
      await ensurePlaybackAudioGraph(
        opts.startupAutoplay
          ? "startup-after-play-request"
          : "auto-after-play-request",
      );
    ...
    if (opts.trackSwitch && expectedMedia && expectedMedia.src) {
      try {
        var recovered = await retryTrackSwitchAudioPlayOnce(
          opts,
          err,
          expectedMedia,
          expectedToken,
    ...
    if (!opts.trackSwitch && !opts.resumeRecovery) {
      var resumed = await recoverCurrentTrackPlaybackFromFreshUrl(
        "play-rejected",
        { originalError: err, silent: opts.silent },
      );
...
async function playAudio(opts) {
  ...
  return attemptAudioPlay({
    manual: !!opts.manual,
    silent: !!opts.silent || !!opts.startupAutoplay || !!opts.trackSwitch,
    startupAutoplay: !!opts.startupAutoplay,
    fade: opts.fade,
    preserveGain: !!opts.preserveGain,
    trackSwitch: !!opts.trackSwitch,
    resumeRecovery: !!opts.resumeRecovery,
    expectedMedia: opts.expectedMedia || audio,
    expectedToken:
      opts.expectedToken == null ? trackSwitchToken : opts.expectedToken,
  });
}
async function togglePlay() {
  ...
  try {
    ...
    if (
      (!audio || !audio.src) &&
      currentLocalSong &&
      (currentLocalSong.localMissing || !currentLocalSong.localUrl)
    ) {
    ...
    if (audio.paused || audio.ended) {
      ...
      if (
        ...
        typeof resetCuefieldAutoMix === "function"
      ) {
      ...
      setPlayIcon(false);
      hideLoading();
      safePlaybackStep("listen-stats-pause", function () {
        updateListenStatsTick(true);
      });
      forcePlaybackControlsInteractive();
      safePlaybackStep("sync-pause-state", function () {
        syncPlaybackStateFromAudioEvent("manual-pause");
      });
      safePlaybackStep("pause-controls-hide", function () {
...
function reorderQueueForShufflePlaybackOrder(startIdx, opts) {
  ...
  if (!isFinite(startIdx) || startIdx < 0 || startIdx >= playQueue.length) {
    startIdx =
      currentIdx >= 0 && currentIdx < playQueue.length ? currentIdx : 0;
  }
  ...
  if (opts.renderPanel !== false)
    safeRenderQueuePanel(opts.reason || "shuffle-playback-order", {
      animate: false,
      scrollCurrent: false,
      deferWhenHidden: false,
    });
  if (opts.rebuildShelf !== false)
    safeShelfRebuild(opts.reason || "shuffle-playback-order", true);
  if (
    opts.persistSnapshot !== false &&
    typeof saveLastPlaybackSnapshot === "function"
  )
    saveLastPlaybackSnapshot(true, opts.reason || "shuffle-playback-order");
  return currentIdx;
}
function nextTrack(userInitiated) {
  ...
  if (
    ...
    Promise.resolve(hydratePlaylistQueueNextPage("queue-tail"))
      .then(function () {
        if (
          playQueue.length <= previousTail + 1 &&
          queueHydrationState &&
          queueHydrationState.error
        ) {
        ...
        var tailOpts = userInitiated
          ? { manual: true, suppressPlayFailureNotice: true }
          : { suppressPlayFailureNotice: true };
  ...
  if (playMode === "shuffle")
    currentIdx = currentIdx < 0 ? 0 : (currentIdx + 1) % playQueue.length;
  else currentIdx = (currentIdx + 1) % playQueue.length;
  var opts = userInitiated
    ? { manual: true, suppressPlayFailureNotice: true }
    : { suppressPlayFailureNotice: true };
  if (playMode === "shuffle") opts.skipShuffleOrder = true;
  Promise.resolve(playQueueAt(currentIdx, opts)).finally(
    forcePlaybackControlsInteractive,
  );
}
...
function clearQueue() {
  if (typeof cancelPlaylistQueueHydration === "function")
    cancelPlaylistQueueHydration("clear-queue");
  playQueue = [];
  currentIdx = -1;
...
function playModeLabel(mode) {
  return (
    { loop: "顺序循环", shuffle: "随机播放", single: "单曲循环" }[mode] ||
    "顺序循环"
  );
}
  ...
  if (window.gsap) {
    ...
    window.gsap
      ...
      .fromTo(
        btn,
        { scale: 0.86, rotate: -8 },
        { scale: 1.12, rotate: 4, duration: 0.16, ease: "power2.out" },
      )
    ...
    window.gsap.fromTo(
      ...
      { boxShadow: "0 0 0 0 rgba(255,63,85,.36)" },
      {
        ...
        overwrite: false,
        onComplete: function () {
          window.gsap.set(btn, { clearProps: "boxShadow" });
        },
      },
    );
    if (icon)
      window.gsap.fromTo(
        icon,
        { y: 4, autoAlpha: 0.32, rotate: -22, scale: 0.74 },
        {
          y: 0,
          autoAlpha: 1,
          rotate: 0,
...
function cyclePlayMode() {
  ...
  if (playMode === "shuffle" && prevMode !== "shuffle") {
    reorderQueueForShufflePlaybackOrder(currentIdx, {
      reason: "play-mode-shuffle",
    });
  }
