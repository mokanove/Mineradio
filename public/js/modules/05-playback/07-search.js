...
function syncSearchAreaResultState() {
  ...
  var hasVisibleResults =
    $results.classList.contains("show") && $results.children.length > 0;
  var hasIntent =
    !!($input && String($input.value || "").trim()) || searchMode === "podcast";
  searchArea.classList.toggle("has-results", hasVisibleResults && hasIntent);
}
if (window.MutationObserver && $results) {
  new MutationObserver(syncSearchAreaResultState).observe($results, {
    childList: true,
    attributes: true,
...
function readSearchHistoryState() {
  ...
  try {
    var raw = JSON.parse(
      localStorage.getItem(SEARCH_HISTORY_STORE_KEY) || "[]",
    );
    ...
    var sourceModes =
      raw.modes && typeof raw.modes === "object" ? raw.modes : raw;
    var migratedItems = [];
    SEARCH_HISTORY_MODES.forEach(function (mode) {
      migratedItems = migratedItems.concat(
        Array.isArray(sourceModes[mode]) ? sourceModes[mode] : [],
      );
    });
...
function writeSearchHistoryState(state) {
  ...
  try {
    localStorage.setItem(SEARCH_HISTORY_STORE_KEY, JSON.stringify(normalized));
  } catch (e) {}
}
...
function rememberSearchQuery(q) {
  q = String(q || "").trim();
  if (!q) return;
  var items = readSearchHistory().filter(function (item) {
...
function renderSearchHistory() {
  ...
  $results.innerHTML =
    ...
    items
      .map(function (q) {
        return (
          '<button class="search-history-chip" type="button" data-history-query="' +
          escHtml(q) +
          '">' +
          escHtml(q) +
          "</button>"
...
function updateSearchModeTabs() {
  ...
  if (songBtn) {
    ...
    songBtn.setAttribute(
      "aria-selected",
      searchMode === "song" ? "true" : "false",
    );
  }
  if (neteaseBtn) {
    neteaseBtn.classList.toggle("active", searchMode === "netease");
    neteaseBtn.setAttribute(
      "aria-selected",
      searchMode === "netease" ? "true" : "false",
    );
  }
  if (qqBtn) {
  ...
  if (kugouBtn) {
    ...
    kugouBtn.setAttribute(
      "aria-selected",
      searchMode === "kugou" ? "true" : "false",
    );
  }
  if (qishuiBtn) {
    qishuiBtn.classList.toggle("active", searchMode === "qishui");
    qishuiBtn.setAttribute(
      "aria-selected",
      searchMode === "qishui" ? "true" : "false",
    );
  }
  if (spotifyBtn) {
    spotifyBtn.classList.toggle("active", searchMode === "spotify");
    spotifyBtn.setAttribute(
      "aria-selected",
      searchMode === "spotify" ? "true" : "false",
    );
  }
  if (podcastBtn) {
    podcastBtn.classList.toggle("active", searchMode === "podcast");
    podcastBtn.setAttribute(
      "aria-selected",
      searchMode === "podcast" ? "true" : "false",
    );
  }
  if ($input) {
    $input.placeholder =
  ...
  if ($input && searchMode === "qishui")
    $input.placeholder = "搜索汽水音乐匹配源...";
  if ($input && searchMode === "spotify")
    $input.placeholder = "搜索 Spotify 匹配源...";
  requestAnimationFrame(updateSearchPillGlassDisplacementMap);
}
...
function programMetaText(item) {
  ...
  if (item.duration)
    bits.push(formatProgramTime(Math.round(item.duration / 1000)));
  return bits.join("  ·  ");
}
function searchThumbHtml(src) {
  return src
    ? '<img src="' +
        coverUrlWithSize(src, 80) +
        '" alt="" loading="lazy" onerror="this.style.opacity=0.2">'
...
function renderPodcastRadios(items, label) {
  ...
  $results.innerHTML = podcastResults
    .map(function (p, i) {
      return (
        '<div class="search-result">' +
        '<div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0" onclick="openPodcastPrograms(' +
        i +
        ')">' +
        ...
        escHtml(p.name || "") +
        '<span class="tag-podcast">Podcast</span></div>' +
        '<div class="search-result-meta">' +
        escHtml(podcastMetaText(p) || label || "NetEase Radio") +
        "</div>" +
        "</div>" +
        "</div>" +
        '<button class="add-btn" title="Open" onclick="event.stopPropagation();openPodcastPrograms(' +
        i +
        ')">›</button>' +
        "</div>"
      );
  ...
  $results.classList.add("show");
  if (window.gsap)
    animateListItems($results, ".search-result", {
      x: 0,
      y: 6,
      stagger: 0.012,
      duration: 0.18,
...
async function loadPodcastHot() {
  ...
  try {
    ...
    if (requestSeq === searchRequestSeq)
      $results.innerHTML =
        '<div class="search-empty">Podcast load failed</div>';
  }
}
async function doPodcastSearch(q) {
  ...
  try {
    var data = await apiJson(
      "/api/podcast/search?keywords=" + encodeURIComponent(q) + "&limit=18",
    );
    if (
...
async function openPodcastPrograms(i) {
  ...
  try {
    var data = await apiJson(
      "/api/podcast/programs?id=" +
        encodeURIComponent(radio.id) +
        "&limit=" +
        PLAYLIST_LAZY_BATCH_SIZE,
    );
    if (requestSeq !== searchRequestSeq || searchMode !== "podcast") return;
    ...
    if (requestSeq === searchRequestSeq)
      $results.innerHTML =
        '<div class="search-empty">Episodes load failed</div>';
  }
}
function renderPodcastPrograms() {
  ...
  if (!podcastPrograms.length) {
    $results.innerHTML =
      '<div class="podcast-result-head"><button class="podcast-back-btn" onclick="event.stopPropagation();renderPodcastRadios(podcastResults)">‹</button><div class="search-result-info"><div class="search-result-title">' +
      escHtml(radio.name || "Podcast") +
      '</div><div class="search-result-meta">No playable episodes</div></div></div>';
    $results.classList.add("show");
    return;
  }
  $results.innerHTML =
    ...
    '<div class="search-result-info"><div class="search-result-title">' +
    escHtml(radio.name || "Podcast") +
    '<span class="tag-podcast">Podcast</span></div><div class="search-result-meta">' +
    escHtml(radio.djName || podcastPrograms.length + " episodes") +
    "</div></div>" +
    "</div>" +
    podcastPrograms
      .map(function (p, i) {
        return (
          '<div class="search-result">' +
          '<div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0" onclick="playPodcastProgram(' +
          i +
          ')">' +
          searchThumbHtml(p.cover) +
          '<div class="search-result-info">' +
          ...
          "</div>" +
          '<div class="search-result-meta">' +
          escHtml(programMetaText(p)) +
          "</div>" +
          "</div>" +
          "</div>" +
          '<button class="add-btn" title="下一首播放" onclick="event.stopPropagation();queuePodcastProgram(' +
          i +
          ')">+</button>' +
          "</div>"
        );
...
$input.addEventListener("input", function () {
  ...
  if (isMusicSearchMode(searchMode)) {
    setSearchHistorySurface(false);
    $results.innerHTML =
      '<div class="search-empty">正在搜索 “' + escHtml(q) + "”…</div>";
    $results.classList.add("show");
  }
...
$results.addEventListener("click", function (e) {
  var loadMore =
    e.target && e.target.closest
      ? e.target.closest("[data-search-load-more]")
      : null;
  if (loadMore) {
    e.preventDefault();
    e.stopPropagation();
  ...
  var clearBtn =
    e.target && e.target.closest
      ? e.target.closest("[data-clear-history]")
      : null;
  if (clearBtn) {
    e.preventDefault();
    e.stopPropagation();
  ...
  var item =
    e.target && e.target.closest
      ? e.target.closest("[data-history-query]")
      : null;
  if (item) {
    e.preventDefault();
    e.stopPropagation();
...
function songProviderKey(song) {
  ...
  if (
    song &&
    (song.provider === "qq" || song.source === "qq" || song.type === "qq")
  )
    return "qq";
  if (
    song &&
...
function songSourceTagHtml(song, opts) {
  ...
  var rawKey =
    ...
    (song.resolvedPlaybackProvider ||
      ...
      "");
  var key = /^(netease|qq|kugou|qishui|spotify)$/.test(String(rawKey || ""))
    ? String(rawKey)
  ...
  if (opts.switcher) {
    return (
      '<button type="button" class="tag-source ' +
      key +
      ' control-source-chip" title="切换音源" aria-haspopup="true" onclick="toggleControlSourceSwitcher(event)">' +
      label +
      "</button>"
    );
  }
...
var controlSourceSwitcherState = {
  open: false,
  loading: false,
  requestId: 0,
  anchor: null,
...
function controlSourceProviderTitle(provider) {
  var item = controlSourceProviders().filter(function (p) {
...
function controlSourceSearchUrl(provider, query) {
  if (provider === "qq")
    return "/api/qq/search?keywords=" + encodeURIComponent(query) + "&limit=8";
  if (provider === "kugou")
    return (
  ...
  if (provider === "qishui")
    return (
      "/api/qishui/search?keywords=" + encodeURIComponent(query) + "&limit=8"
    );
  if (provider === "spotify")
    return (
...
function currentControlSong() {
  return Array.isArray(playQueue) &&
    currentIdx >= 0 &&
...
function controlSourceSwitchQuery(song) {
  song = song || {};
  var artist =
    String(song.artist || "").split(/\s*\/\s*|\s*,\s*|\s*&\s*/)[0] || "";
  return [song.name || song.title || "", artist]
    .filter(Boolean)
    .join(" ")
    .trim();
}
...
function renderControlSourceSwitcher(matches) {
  ...
  el.innerHTML =
    '<div class="control-source-switcher-head"><span>切换音源</span><small>' +
    (controlSourceSwitcherState.loading ? "正在匹配" : "保留当前进度") +
    "</small></div>" +
    ...
    controlSourceProviders()
      .map(function (provider) {
        ...
        var cleanStatus = active
          ? "当前"
          : providerLimited
            ? "匹配源"
            : match
              ? "可切换"
              : controlSourceSwitcherState.loading
                ? "检测中"
                : controlSourceIssueLabel(issue);
        var title = active
          ? "当前音源"
          : providerLimited
            ? provider.title + ": 播放将自动换源"
            : match
              ? "切换到 " + provider.title
              : provider.title + ": " + controlSourceIssueLabel(issue);
        var status = active
          ? "当前"
          : providerLimited
            ? "匹配源"
            : match
              ...
              : controlSourceSwitcherState.loading
                ...
                : "无匹配";
        return (
          '<button type="button" class="control-source-option' +
          (active ? " active" : "") +
          (!ready ? " disabled" : "") +
          ...
          '" ' +
          (!ready ? "disabled " : "") +
          "onclick=\"switchCurrentSongSource('" +
          provider.key +
          "')\">" +
          ...
          provider.label +
          "</span>" +
          '<span class="control-source-option-title">' +
          provider.title +
          "</span>" +
          "<small>" +
          cleanStatus +
...
async function findControlSourceMatchResult(song, provider) {
  ...
  var data = await apiJson(controlSourceSearchUrl(provider, query), {
    timeoutMs: 6000,
  });
  var list = data && (data.songs || data.result || []);
  if (!Array.isArray(list) || !list.length)
    return { song: null, issue: "no_source" };
  var best = null;
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    var score =
      typeof scoreSongSearchResult === "function"
        ? scoreSongSearchResult(candidate, query, i)
        : 0;
    if (
      typeof isSameTitleArtist === "function" &&
  ...
  return best && bestScore >= 24
    ? { song: cloneSong(best), issue: "" }
    : { song: null, issue: bestIssue };
}
...
async function loadControlSourceMatches(song, requestId) {
  ...
  await Promise.all(
    providers.map(async function (provider) {
      ...
      try {
        matches[provider.key] = await findControlSourceMatchResult(
          song,
          provider.key,
        );
      } catch (err) {
  ...
  if (
    requestId !== controlSourceSwitcherState.requestId ||
    !controlSourceSwitcherState.open
  )
    return;
  controlSourceSwitcherState.loading = false;
...
function toggleControlSourceSwitcher(e) {
  ...
  renderControlSourceSwitcher({
    [songProviderKey(song)]: { song: song, issue: "" },
  });
...
async function switchCurrentSongSource(provider) {
  ...
  try {
    var entry =
      controlSourceSwitcherState.matches &&
      controlSourceSwitcherState.matches[provider];
    var match = controlSourceMatchSong(entry);
    var issue = controlSourceMatchIssue(entry);
    if (!match) {
      ...
      if (controlSourceSwitcherState.matches)
        controlSourceSwitcherState.matches[provider] = lookup || {
          song: null,
          issue: issue || "no_source",
        };
    }
    ...
    if (!match) {
      showSourceFallbackNotice(
        "未找到可切换音源",
        controlSourceProviderTitle(provider) +
          " 暂时没有匹配到同名同歌手版本。",
      );
      showSourceFallbackNotice(
        "该平台无正版音源",
    ...
    closeControlSourceSwitcher();
    safeRenderQueuePanel("manual-source-switch", {
      scrollCurrent: miniQueueOpen,
    });
    updateControlTrackInfo(playQueue[currentIdx]);
    showSourceFallbackNotice(
      "正在切换音源",
      (song.name || "当前歌曲") + " -> " + controlSourceProviderTitle(provider),
    );
    ...
    if (currentIdx >= 0 && currentIdx < playQueue.length) {
      ...
      safeRenderQueuePanel("manual-source-switch-restore", {
        scrollCurrent: miniQueueOpen,
      });
      updateControlTrackInfo(playQueue[currentIdx]);
    }
...
document.addEventListener("click", function (e) {
  ...
  if (
    controlSourceSwitcherState.anchor &&
    controlSourceSwitcherState.anchor.contains &&
    controlSourceSwitcherState.anchor.contains(e.target)
  )
    return;
  closeControlSourceSwitcher();
});
...
function songRequiresVip(song) {
  ...
  if (
    ...
    song.needVip === true ||
    song.need_vip === true ||
    song.onlyVipPlayable === true ||
    song.only_vip_playable === true
  )
    return true;
  if (song.trial === true || song.preview === true) return true;
  var labelInfo = song.label_info || song.labelInfo || {};
  if (
    labelInfo.only_vip_playable === true ||
    labelInfo.onlyVipPlayable === true ||
    labelInfo.vip === true
  )
    return true;
  ...
  return /vip_required|paid_required|trial_only|need_vip|only_vip|vip/i.test(
    text,
  );
}
...
function searchResultMetaText(song) {
  ...
  if (songProviderKey(song) === "qq" && !song.playable)
    bits.push("QQ 播放需会话/授权");
  if (songProviderKey(song) === "kugou" && !song.playable)
    bits.push("酷狗播放需会话/授权");
  if (songProviderKey(song) === "qishui" && !song.playable)
    bits.push("汽水匹配源，播放会自动换源");
  if (songProviderKey(song) === "spotify" && !song.playable)
    bits.push("Spotify 匹配源，播放会自动换源");
...
function searchResultMetaHtml(song, index) {
  ...
  return (
    '<button class="search-artist-link" type="button" onclick="event.stopPropagation();openSearchResultArtist(' +
    index +
    ')">' +
    escHtml(artist) +
    "</button>" +
...
var MUSIC_SEARCH_PROVIDER_ORDER = [
  "netease",
  "qq",
  "kugou",
  "qishui",
...
function searchProviderLoginNotice(mode) {
  ...
  if (specific) {
    var meta =
      typeof platformMeta === "function"
        ? platformMeta(specific)
        : { label: specific };
...
function searchProviderUrl(provider, q, limit, offset) {
  var suffix =
    "&limit=" + limit + "&offset=" + Math.max(0, Number(offset) || 0);
  if (provider === "qq")
    return "/api/qq/search?keywords=" + encodeURIComponent(q) + suffix;
  if (provider === "kugou")
    return "/api/kugou/search?keywords=" + encodeURIComponent(q) + suffix;
  if (provider === "qishui")
    return "/api/qishui/search?keywords=" + encodeURIComponent(q) + suffix;
  if (provider === "spotify")
    return "/api/spotify/search?keywords=" + encodeURIComponent(q) + suffix;
...
function searchQueryTokens(text) {
  ...
  return raw
    ...
    .filter(function (token, index, all) {
      if (
        !token ||
        /^(qq|网易云|网易云音乐|酷狗|酷狗音乐|汽水|汽水音乐|spotify)$/.test(
          token,
        )
      )
        return false;
...
function searchMentionsKnownArtist(q, artist) {
  ...
  if (
    /周杰伦|周杰倫|jay\s*chou/.test(rawQ) &&
    /周杰伦|周杰倫|jay\s*chou/.test(rawArtist)
  )
    return true;
...
function searchLooksLikeDerivative(text) {
  return /(翻唱|cover|伴奏|instrumental|remix|片段|demo|女声|男声|karaoke|完整版\s*cover|抖音版|dj版|合唱版|改编版|赵露思版|超燃|硬曲|剪辑|二创|氛围|浴室|节奏版|进行曲|加速版|慢速版|变速|串烧|tribute|made\s*famous\s*by)/i.test(
    String(text || ""),
  );
}
var SOURCE_SWITCH_BLOCKED_ARTIST_TOKENS = ["asablue"];
var SOURCE_SWITCH_STRICT_ARTIST_ALIASES = [["周杰伦", "jaychou", "zhoujielun"]];
function sourceSwitchArtistParts(song) {
  ...
  if (song && Array.isArray(song.artists)) {
    song.artists.forEach(function (a) {
      if (a && a.name) parts.push(a.name);
    });
  }
  if (song && song.artist) {
    String(song.artist)
      .split(/\s*\/\s*|\s*,\s*|\s*&\s*| feat\.? | ft\.? /i)
...
function sourceSwitchPartsOverlap(sourceParts, candidateParts) {
  return sourceParts.some(function (sourcePart) {
    return candidateParts.some(function (candidatePart) {
      return sourceSwitchPartMatches(sourcePart, candidatePart);
    });
  });
}
function sourceSwitchSongHasBlockedArtist(song) {
  ...
  return SOURCE_SWITCH_BLOCKED_ARTIST_TOKENS.some(function (token) {
    return (
      raw.indexOf(token) >= 0 || norm.indexOf(simpleSearchNorm(token)) >= 0
    );
  });
}
  ...
  for (var i = 0; i < SOURCE_SWITCH_STRICT_ARTIST_ALIASES.length; i++) {
    ...
    if (
      aliases.some(function (alias) {
        return joined.indexOf(simpleSearchNorm(alias)) >= 0;
      })
    )
      return aliases;
  }
...
function sourceCandidateRejectReason(source, candidate, provider) {
  ...
  var candidateTitle = simpleSearchNorm(
  ...
  if (
    ...
    !sourceSwitchPartsOverlap(sourceParts, candidateParts)
  )
    return "artist_mismatch";
  if (
    sourceSwitchStrictArtistRuleForSong(source) &&
    sourceSwitchCandidateHasUnexpectedArtist(sourceParts, candidateParts)
  )
    return "artist_extra";
  return "";
}
function controlSourceIssueLabel(issue) {
  ...
  if (issue === "artist_mismatch" || issue === "artist_extra")
    return "非原唱版本";
  return "无正版音源";
}
var SEARCH_ORIGINAL_ARTIST_HINTS = [
  ...
  {
    titles: ["beautyandabeat", "beauty and a beat"],
...
function canonicalOriginalArtistsForSearch(q, song) {
  ...
  SEARCH_ORIGINAL_ARTIST_HINTS.forEach(function (rule) {
    var matched = (rule.titles || []).some(function (title) {
      ...
      var titleMatches = !!(
        titleNorm &&
        (titleNorm === nt || titleNorm.indexOf(nt) >= 0)
      );
...
function songArtistMatchesAny(song, artists) {
  ...
  return artists.some(function (artist) {
    ...
    return !!(
      na &&
      (songArtist.indexOf(na) >= 0 || na.indexOf(songArtist) >= 0)
    );
  });
}
function searchLooksLikeSameTitleCover(
  ...
  var sameTitle =
    name === nq || nq.indexOf(name) >= 0 || name.indexOf(nq) === 0;
  if (!sameTitle) return false;
  var selfTitledSingle = !!(
    album &&
    (album === name ||
      album === nq ||
      album.indexOf(name) >= 0 ||
      name.indexOf(album) >= 0)
  );
  return (
    selfTitledSingle || searchLooksLikeDerivative(raw) || (sourceIndex || 0) > 0
  );
}
function searchPopularityScore(song, sourceIndex) {
  ...
  if (isFinite(hot) && hot > 0) {
    score +=
      hot <= 100
        ? Math.min(12, hot / 8)
        : Math.min(14, Math.log(hot + 1) * 1.2);
  }
...
function searchCanonicalSongKey(song) {
  ...
  var version =
    searchVersionSignature(
      ((song && song.name) || "") + " " + ((song && song.album) || ""),
    ) || "studio";
...
function mergeSongSearchResults(
  neteaseSongs,
  qqSongs,
  kugouSongs,
  qishuiSongs,
  ...
  function push(song, sourceIndex) {
    ...
    var key =
      songProviderKey(song) +
      ":" +
      (song.mid || song.id || song.name + "|" + song.artist);
    ...
    if (canonicalKey && canonicalSeen[canonicalKey] != null) {
      ...
      if ((song._searchScore || 0) > (out[existingIndex]._searchScore || 0))
  ...
  (neteaseSongs || []).forEach(function (song, i) {
    push(song, i);
  });
  (qqSongs || []).forEach(function (song, i) {
    push(song, i);
  });
  (kugouSongs || []).forEach(function (song, i) {
    push(song, i);
  });
  (qishuiSongs || []).forEach(function (song, i) {
    push(song, i);
  });
  (spotifySongs || []).forEach(function (song, i) {
    push(song, i);
  });
...
function mergeUniqueSearchSongPools(existing, incoming) {
  ...
  function push(song) {
    ...
    var canonicalKey = searchCanonicalSongKey(song);
    if (
      providerSeen[providerKey] ||
      (canonicalKey && canonicalSeen[canonicalKey])
    )
      return;
...
async function fetchMusicSearchResults(q, mode, previousPages) {
  ...
  var fetchProviders = providers.filter(function (provider) {
    return (
      !previousPages ||
      !previousPages[provider] ||
      previousPages[provider].hasMore
    );
  });
  var result = await Promise.allSettled(
    fetchProviders.map(function (provider) {
      ...
      return apiJson(searchProviderUrl(provider, q, limit, offset)).then(
        function (value) {
          return {
            provider: provider,
            offset: offset,
  ...
  fetchProviders.forEach(function (provider, index) {
    ...
    if (!entry || entry.status !== "fulfilled") {
      console.warn(
        controlSourceProviderTitle(provider) + " search failed:",
        entry && entry.reason,
      );
      providerPages[provider] = Object.assign(
        {},
        providerPages[provider] || {},
        { hasMore: false, failed: true },
      );
      return;
    }
    ...
    if (value.message && !songs.length && !searchProviderNotice)
      searchProviderNotice = value.message;
  });
...
function searchSongResultHtml(s, i) {
  ...
  return (
    ...
    sourceClass +
    '">' +
    '<div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0" onclick="playSearchResult(' +
    i +
    ')">' +
    imgTag +
    '<div class="search-result-info">' +
    ...
    i +
    '" title="' +
    (isSongLiked(s) ? "取消红心" : "红心喜欢") +
    '" onclick="event.stopPropagation();toggleLikeSearchResult(' +
    i +
    ')">' +
    heartIconSvg() +
    "</button>" +
    '<button class="song-action-btn" title="收藏到歌单" onclick="event.stopPropagation();collectSearchResult(' +
    i +
    ')">' +
    playlistPlusIconSvg() +
    "</button>" +
    '<button class="add-btn" title="下一首播放" onclick="event.stopPropagation();queueSearchResult(' +
    i +
    ')">+</button>' +
    "</div>"
  );
...
function searchLoadMoreSentinelHtml() {
  var remaining = Math.max(
    0,
    searchMusicRenderState.songs.length - searchMusicRenderState.visibleCount,
  );
  if (
    !remaining &&
    !searchMusicRenderState.remoteHasMore &&
    !searchMusicRenderState.loadingMore
  )
    return "";
  var label = searchMusicRenderState.loadingMore
    ? "正在加载更多歌曲…"
    : remaining
      ? "继续滚动加载 · 当前还有 " + remaining + " 首"
      : "继续滚动加载更多歌曲";
  return (
    '<div class="search-empty search-load-more" data-search-load-more="1" role="status">' +
    label +
    "</div>"
  );
}
...
function observeSearchLoadMoreSentinel() {
  ...
  searchLoadMoreObserver = new IntersectionObserver(
    function (entries) {
      if (
        ...
        appendNextSearchResults(expectedKey);
    },
...
async function loadNextMusicSearchPage(expectedKey) {
  if (
    !expectedKey ||
    expectedKey !== searchMusicRenderState.key ||
    expectedKey !== searchLastResultQuery
  )
    return false;
  if (
    searchMusicRenderState.loadingMore ||
  ...
  try {
    var page = await fetchMusicSearchResults(
      q,
      mode,
      searchMusicRenderState.providerPages,
    );
    ...
    var merged = mergeUniqueSearchSongPools(
      searchMusicRenderState.songs,
      page.songs || [],
    );
    ...
    playlist = merged;
    searchMusicRenderState.remoteHasMore =
      !!page.hasMore && merged.length < MUSIC_SEARCH_MAX_RESULTS;
...
function appendNextSearchResults(expectedKey) {
  ...
  if (
    ...
    return false;
  if (searchMusicRenderState.appending || searchMusicRenderState.loadingMore)
    return false;
  if (
    searchMusicRenderState.visibleCount >= searchMusicRenderState.songs.length
  ) {
  ...
  requestAnimationFrame(function () {
    ...
    var end = Math.min(
      searchMusicRenderState.songs.length,
      start + MUSIC_SEARCH_APPEND_BATCH,
    );
    var html = "";
    for (var i = start; i < end; i++)
      html += searchSongResultHtml(searchMusicRenderState.songs[i], i);
...
function renderSongSearchResults(songs) {
  ...
  searchMusicRenderState.key =
    plan.key ||
    searchLastResultQuery ||
    searchResultKey($input && $input.value, searchMode);
  searchMusicRenderState.query =
    plan.query || String(($input && $input.value) || "").trim();
  ...
  searchMusicRenderState.visibleCount = Math.min(
    playlist.length,
    MUSIC_SEARCH_INITIAL_VISIBLE,
  );
  var html = "";
  for (var i = 0; i < searchMusicRenderState.visibleCount; i++)
    html += searchSongResultHtml(playlist[i], i);
  $results.innerHTML = html + searchLoadMoreSentinelHtml();
  $results.classList.add("show");
  syncLikeStatusForSongs(
    playlist.slice(0, searchMusicRenderState.visibleCount),
  );
  if (window.gsap)
    animateListItems($results, ".search-result", {
      x: 0,
...
async function doSearch(q, opts) {
  ...
  try {
    ...
    var songs =
      searchData && Array.isArray(searchData.songs) ? searchData.songs : [];
    if (
      requestSeq !== searchRequestSeq ||
      searchMode !== mode ||
      $input.value.trim() !== q
    ...
    if (!songs.length) {
      ...
      $results.innerHTML =
        ...
        escHtml(searchProviderNotice || "没有找到相关歌曲") +
        "</div>";
      $results.classList.add("show");
      return;
    }
    ...
    if (requestSeq === searchRequestSeq) {
      ...
      searchLastResultQuery = "";
      $results.innerHTML =
        '<div class="search-empty">搜索暂时失败，请稍后重试</div>';
      $results.classList.add("show");
    }
  }
}
