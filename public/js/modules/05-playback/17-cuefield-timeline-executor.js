(function (root, factory) {
  ...
  function normalizeAction(action, leadSec, targetVolume) {
    ...
    var normalized = {
      ...
      delayMs: Math.max(
        0,
        Math.round((toNumber(action.t, 0) + leadSec) * 1000),
      ),
  ...
  function fallbackTimeline(opts) {
    ...
    if (mode === "intro-bed") {
      ...
      return {
        ...
        bStart: Math.max(
          0,
          entryTime - Math.min(5.2, Math.max(2.2, entryTime * 0.7)),
        ),
        actions: [
          {
            t: -introLead,
            deck: "B",
            op: "play",
            at: Math.max(0, entryTime - introLead),
            volume: 0,
          },
          {
            t: -introLead,
    ...
    return {
      ...
      bStart: Math.max(
        0,
        entryTime - Math.min(2.4, Math.max(0.8, entryTime * 0.45)),
      ),
      actions: [
        {
          t: -2.8,
  ...
  function buildCuefieldTimelineExecution(opts) {
    ...
    var targetVolume = clamp(
      opts.targetVolume == null ? 1 : opts.targetVolume,
      0,
      1,
    );
    var fallback = rawTimeline.length ? null : fallbackTimeline(opts);
    var timeline = rawTimeline.length ? rawTimeline : fallback.actions;
    var leadSec = rawTimeline.length
      ? leadFromTimeline(timeline, 2.8)
      : fallback.leadSec;
    var entryTime = Math.max(0, toNumber(opts.entryTime, 0));
    var bStart = rawTimeline.length
      ? bStartFromTimeline(timeline, entryTime)
      : fallback.bStart;
    var actions = timeline
      .map(function (action) {
        return normalizeAction(action, leadSec, targetVolume);
      })
    ...
    var requiresBGraph = actions.some(function (action) {
      return (
        action.deck === "B" && (action.op === "filter" || action.op === "bass")
      );
    });
    var handoff = actions
      .filter(function (action) {
        return action.op === "handoff";
      })
      .slice(-1)[0];
    var crossfade =
      actions.filter(function (action) {
        return action.op === "crossfade";
      })[0] || null;
