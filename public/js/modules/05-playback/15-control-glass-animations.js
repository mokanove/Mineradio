...
function applyControlGlassChromaticOffsetToFilter(
  ...
  var verticalSpread =
    spread * (verticalFactor == null ? 0.08 : verticalFactor);
  setControlGlassChannelOffset(
    filter,
    "dispRedShifted",
    baseShiftX - spread,
    -verticalSpread,
...
function generateControlGlassDisplacementMap(width, height, radius) {
  ...
  var svg =
    ...
    '" rx="' +
    radius +
    '" fill="url(#glass-blue)" style="mix-blend-mode:difference"/>' +
    '<rect x="' +
    edge.toFixed(2) +
    '" y="' +
    edge.toFixed(2) +
...
function generateAccountPillGlassDisplacementMap(
  width,
  height,
  radius,
  minWidth,
  ...
  var svg =
    ...
    '" rx="' +
    radius +
    '" fill="url(#account-x)" filter="url(#account-soft)" opacity=".82"/>' +
    '<rect x="' +
    edge.toFixed(2) +
    '" y="' +
    edge.toFixed(2) +
    ...
    innerH.toFixed(2) +
    '" rx="' +
    Math.max(1, radius - edge).toFixed(2) +
    '" fill="rgb(128,128,128)" opacity=".36"/>' +
    "</svg>";
  return "data:image/svg+xml," + encodeURIComponent(svg);
}
...
function setSearchGlassReady(ready) {
  ...
  if (on)
    document.documentElement.classList.remove(
      "search-glass-priming",
      "search-glass-fallback",
    );
  var area = document.getElementById("search-area");
  if (area) {
    area.classList.toggle("search-glass-ready", on);
    if (on)
      area.classList.remove("search-glass-priming", "search-glass-fallback");
  }
}
...
function queueSearchGlassReadyAfterPaint(force) {
  if (!glassImageHasHref(document.getElementById("search-box-glass-map"))) {
    controlGlassState.searchReadyToken =
      (controlGlassState.searchReadyToken || 0) + 1;
  ...
  if (
    ...
    return true;
  if (document.documentElement.classList.contains("search-glass-priming"))
    return false;
...
function updateGlassDisplacementMapForElement(el, img, stateKey, generator) {
  ...
  var key =
    ...
    ":" +
    Math.round(radius);
  if (key === controlGlassState[stateKey] && glassImageHasHref(img))
    return true;
  controlGlassState[stateKey] = key;
  var href = (generator || generateControlGlassDisplacementMap)(
    rect.width,
    rect.height,
    radius,
  );
...
function syncWallpaperEngineGlassSamplerGeometry(
  rect,
  radius,
  visible,
  active,
  ...
  sampler.style.setProperty(
    "--wallpaper-engine-glass-radius",
    Math.max(0, radius).toFixed(3) + "px",
  );
...
function syncWallpaperEngineControlGlassSurface(force) {
  ...
  var sessionId =
    typeof wallpaperEngineNativeSessionId !== "undefined"
      ? String(wallpaperEngineNativeSessionId || "")
      : "";
  if (
    !bar ||
    !api ||
    typeof api.updateWallpaperEngineGlassSurface !== "function" ||
...
function updateSearchBoxGlassDisplacementMap() {
  ...
  var changed =
    ready && (controlGlassState.searchBoxKey !== previousKey || !hadHref);
  if (
    ready &&
    document.documentElement.classList.contains("search-glass-priming")
  )
    return ready;
...
function updateSearchPillGlassDisplacementMap() {
  ...
  var nodes = Array.prototype.slice.call(
    document.querySelectorAll(".search-mode-tabs button,.search-history-chip"),
  );
  var maxW = 0,
    maxH = 0,
    maxRadius = 14;
  ...
  var radius = Math.max(
    12,
    Math.min(Math.round(maxRadius), Math.round(height / 2) + 10),
  );
  var key = width + "x" + height + ":" + radius;
  if (key === controlGlassState.searchPillKey && glassImageHasHref(img))
    return true;
...
function updateAccountPillGlassDisplacementMap() {
  ...
  if (!img) return;
  var nodes = Array.prototype.slice.call(
    document.querySelectorAll(".top-account-pill"),
  );
  ...
  var radius = Math.max(
    20,
    Math.min(Math.round(maxRadius), Math.round(height / 2) + 8),
  );
  var key = width + "x" + height + ":" + radius;
...
function prepareSearchGlassBeforePeek() {
  if (!document.documentElement.classList.contains("control-glass-svg-ok"))
    return true;
  setSearchGlassFallback(false);
  ...
  if (
    !ready ||
    !glassImageHasHref(document.getElementById("search-box-glass-map"))
  ) {
    setSearchGlassPriming(false);
    setSearchGlassFallback(true);
    return true;
  }
  if (document.documentElement.classList.contains("search-glass-ready"))
    return true;
...
function initControlGlassSurface() {
  ...
  if (
    ...
    (bar || searchBox || searchTabs || searchResults || userBtn)
  ) {
  ...
  if (window.MutationObserver && (searchTabs || searchResults || userBtn)) {
    ...
    if (searchResults)
      mo.observe(searchResults, { childList: true, subtree: true });
    if (userBtn)
      mo.observe(userBtn, {
        childList: true,
        subtree: true,
...
function bindPlayerControlAnimations() {
  ...
  document.querySelectorAll("#bottom-bar .ctrl-btn").forEach(function (btn) {
    ...
    var iconTarget = btn.querySelector(
      "svg,.lyrics-word-icon,#quality-btn-label",
    );
    function canAnimate() {
    ...
    function hoverIn(e) {
      ...
      window.gsap.to(btn, {
        y: -2,
        scale: isPlay ? 1.07 : 1.08,
        duration: 0.2,
        ease: "power2.out",
        overwrite: "auto",
      });
      if (iconTarget)
        window.gsap.to(iconTarget, {
          scale: isPlay ? 1.08 : 1.1,
          duration: 0.22,
          ease: "power2.out",
          overwrite: "auto",
        });
