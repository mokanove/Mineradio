...
function recordListenRollupV2(record) {
  try {
    ...
    var listenMs = Math.max(
      0,
      Math.round(Number(record && record.listenMs) || 0),
    );
    var dayKey = listenDayKey(record && record.playedAt);
    var day =
      state.daily[dayKey] && typeof state.daily[dayKey] === "object"
        ? state.daily[dayKey]
    ...
    day.completed =
      Math.max(0, Number(day.completed) || 0) +
      (record && record.completed ? 1 : 0);
...
function createListenSessionId() {
  ...
  return (
    ...
    "-" +
    listenSessionSerial.toString(36) +
    "-" +
...
function listenReportProvider(snapshot) {
  var provider = String(
    (snapshot &&
      (snapshot.provider ||
        snapshot.sourceKey ||
        snapshot.resolvedPlaybackProvider)) ||
      "",
  )
    .trim()
    .toLowerCase();
  ...
  if (
    /^(netease|qq|kugou|qishui|spotify)$/.test(provider) &&
    typeof normalizePlaybackProvider === "function"
  ) {
...
function reportListenSession(record, session, durationMs) {
  ...
  try {
    Promise.resolve(fetch("/api/listen/report", options)).then(
      function () {
...
function saveListenStatsState() {
  try {
    ...
    localStorage.setItem(
      HOME_LISTEN_STATS_KEY,
      JSON.stringify(listenStatsState),
    );
  } catch (e) {}
}
function listenSongSnapshot(song) {
  ...
  return {
    ...
    albumId:
      song.albumId || song.album_id || (song.album && song.album.id) || "",
...
function beginListenSession(song, context) {
  ...
  if (!snap.key) return;
  if (listenSession && listenSession.key !== snap.key)
...
function tickListenSessionSnapshot(session, force) {
  ...
  var deltaByAudio =
    Math.max(0, audioTime - (session.lastAudioTime || 0)) * 1000;
  ...
  session.maxProgress = Math.max(
    session.maxProgress || 0,
    audio.duration ? audioTime / audio.duration : 0,
  );
}
function updateListenStatsTick(force) {
  ...
  if (!listenSession || listenSession.key !== key)
    beginListenSession(song, activeRadioContext);
...
function finalizeListenSession(completed) {
  ...
  var effective =
    completed ||
    session.listenMs >= 45000 ||
    session.maxProgress >= 0.5 ||
    (!audio || !audio.duration ? session.listenMs >= 30000 : false);
  if (!effective) return;
  var now = Date.now();
  ...
  listenStatsState.history = [record]
    .concat(
      (listenStatsState.history || []).filter(function (item) {
        return item && item.key !== record.key;
      }),
    )
    .slice(0, 180);
  var songStat = listenStatsState.songs[record.key] || {
    key: record.key,
    name: record.name,
  ...
  String(record.artist || "")
    ...
    .forEach(function (name) {
      name = name.trim();
      if (!name) return;
      var artistStat = listenStatsState.artists[name] || {
        name: name,
        plays: 0,
        listenMs: 0,
        lastPlayedAt: 0,
...
function mostPlayedSong() {
  var list = Object.keys(listenStatsState.songs || {}).map(function (key) {
    return listenStatsState.songs[key];
  });
...
function topListenArtist() {
  var list = Object.keys(listenStatsState.artists || {}).map(function (key) {
    return listenStatsState.artists[key];
  });
  list.sort(function (a, b) {
    return (
      b.plays - a.plays ||
...
function homeListenSummary() {
  ...
  var totalPlays = Object.keys(listenStatsState.songs || {}).reduce(function (
    sum,
    key,
  ) {
    return (
      sum +
      ((listenStatsState.songs[key] && listenStatsState.songs[key].plays) || 0)
    );
  }, 0);
  return {
    recent: recent,
