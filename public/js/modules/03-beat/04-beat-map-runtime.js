function smoothBeatMapHandoff(songId, map, token, song) {
  ...
  var wait = Math.max(
    260,
    Math.min(720, 340 + (beatPulse + beatCam.punch) * 260),
  );
  var apply = function () {
...
function syncBeatMapPlaybackCursor(t, preserveVisualState) {
  ...
  beatMapNextIdx = 0;
  var pulseEvents =
    currentBeatMap && (currentBeatMap.pulseBeats || currentBeatMap.kicks);
  if (pulseEvents) {
    while (
      beatMapNextIdx < pulseEvents.length &&
      beatEventTime(pulseEvents[beatMapNextIdx]) < t
    )
      beatMapNextIdx++;
  }
...
function syncPodcastDjMapCursor(t, preserveVisualState) {
  ...
  if (currentDjBeatMap) {
    var beatEvents =
      ...
      currentDjBeatMap.beats ||
      currentDjBeatMap.kicks ||
      [];
    var camSyncTime = Math.max(0, t - 0.025);
    while (
      djBeatMapNextIdx < beatEvents.length &&
      beatEventTime(beatEvents[djBeatMapNextIdx]) < camSyncTime
    )
      djBeatMapNextIdx++;
    var pulseEvents =
      currentDjBeatMap.pulseBeats || currentDjBeatMap.kicks || [];
    var pulseSyncTime = Math.max(0, t - 0.035);
    while (
      djBeatPulseNextIdx < pulseEvents.length &&
      beatEventTime(pulseEvents[djBeatPulseNextIdx]) < pulseSyncTime
    )
      djBeatPulseNextIdx++;
  }
...
function tickPodcastDjBeatMap() {
  ...
  if (
    ...
    t > currentDjBeatMap.partialUntilSec + beatCam.lookahead
  )
    return;
  var beatEvents =
    currentDjBeatMap.cameraBeats ||
  ...
  while (
    djBeatPulseNextIdx < pulseEvents.length &&
    beatEventTime(pulseEvents[djBeatPulseNextIdx]) <= t
  ) {
...
function tickBeatMap() {
  ...
  var gridTimingLocked =
    currentBeatMap.tempoSource === "music-tempo" && beatEvents.length >= 4;
  var liveFreshWindow = Math.max(
    0.5,
    rtBeat.tempoGap ? rtBeat.tempoGap * 1.18 : 0.5,
  );
  var realtimeHasLock =
    rtBeat.lastHitAt > 0 && t - rtBeat.lastHitAt < liveFreshWindow;
  ...
  while (
    ...
    if (gridTimingLocked || !realtimeHasLock)
      triggerScheduledBeat(pulseEvents[beatMapNextIdx]);
    beatMapNextIdx++;
  }
}
...
function triggerScheduledBeat(beat) {
  var strength =
    typeof beat === "number"
      ...
      : Math.max(
          0,
          Math.min(1, beat && beat.strength != null ? beat.strength : 0.42),
        );
  var impact =
    typeof beat === "number"
      ? strength
      : Math.max(
          0,
          Math.min(1, beat && beat.impact != null ? beat.impact : strength),
        );
  if (impact < 0.18 && strength < 0.52) return;
  if (
    (cinemaTrackProfile.scale || 1) < 0.52 &&
    impact < 0.46 &&
    strength < 0.74
  )
    return;
  var body =
    typeof beat === "number"
      ? 0
      : Math.max(0, Math.min(1, beat && beat.body != null ? beat.body : 0));
  ...
  var pulse =
    (0.14 + strength * 0.46 + impact * 0.18 + body * 0.08 + comboLift) *
    dynScale;
  if (djPulse)
    pulse =
      (0.12 + strength * 0.5 + impact * 0.28 + comboLift * 0.7) *
      clampRange(dynScale, 0.78, 1.18);
