...
function normalizeMusicTempoBeats(times, duration) {
  ...
  var sorted = times
    .filter(function (t) {
      return isFinite(t) && t >= 0.05 && (!duration || t < duration - 0.05);
    })
    .sort(function (a, b) {
      return a - b;
    });
  if (sorted.length < 4) return sorted;
  var gap = medianGap(sorted, 0.2, 1.2);
...
function estimateTempoPhaseOffset(tempoBeats, beatCandidates, step, duration) {
  ...
  for (var i = 0; i < beatCandidates.length; i++) {
    ...
    while (
      ti < tempoBeats.length - 1 &&
      Math.abs(tempoBeats[ti + 1] - b.time) <= Math.abs(tempoBeats[ti] - b.time)
    )
      ti++;
    var base = tempoBeats[ti];
    var offset = b.time - base;
    if (!isFinite(offset) || Math.abs(offset) > maxOffset) continue;
    var weight = 0.2 + strength * strength * 1.35;
    if (b.primary) weight *= 1.35;
    if (b.camera) weight *= 1.18;
    if (b.mass != null)
      weight *= 0.82 + Math.max(0, Math.min(1, b.mass)) * 0.42;
  ...
  return Math.abs(offsetOut) >= 0.045
    ? Math.max(-maxOffset, Math.min(maxOffset, offsetOut))
    : 0;
}
...
function getMusicTempoWorkerUrl() {
  ...
  musicTempoWorkerUrl = URL.createObjectURL(
...
async function analyzeMusicTempoInWorker(buffer, token) {
  ...
  try {
    ...
    for (var ch = 0; ch < channels; ch++)
      chDataList.push(buffer.getChannelData(ch));
    var chScale = 1 / Math.max(1, channels);
    var monoChunk = Math.max(4096, Math.floor(buffer.sampleRate * 0.7));
...
function scheduleBeatAnalysis(songId, audioUrl, token, song) {
  ...
  beatAnalysisTimer = setTimeout(function waitForQuietStart() {
    ...
    if (current < beatAnalysisConfig.minPlaybackSec) {
      beatAnalysisTimer = setTimeout(
        waitForQuietStart,
        Math.max(500, (beatAnalysisConfig.minPlaybackSec - current) * 1000),
      );
      return;
    }
    var startAnalysis = async function () {
      ...
      analyzeAudioBeats(audioUrl, null, token, {
        skipMusicTempo:
          beatAnalysisConfig.skipMusicTempoWhilePlaying && !audio.paused,
        background: true,
        song: song || null,
      })
        .then(function (map) {
...
function updateBeatDiskCacheStatus(data) {
  ...
  beatDiskCacheStatus.mode =
    data.mode || (beatDiskCacheStatus.enabled ? "disk" : "memory-only");
  ...
  if (!beatDiskCacheStatus.enabled && !beatDiskCacheNoticeLogged) {
    ...
    console.log(
      "节拍磁盘缓存不可用，已降级为本次运行内存缓存:",
      beatDiskCacheStatus.reason || "unknown",
    );
  }
}
...
async function ensureBeatDiskCacheStatus() {
  ...
  try {
    updateBeatDiskCacheStatus(
      await apiJson("/api/beatmap/cache/status?t=" + Date.now()),
    );
  } catch (e) {
    updateBeatDiskCacheStatus({
      enabled: false,
...
async function readBeatDiskCache(key) {
  ...
  try {
    var r = await apiJson(
      "/api/beatmap/cache?key=" + encodeURIComponent(key) + "&t=" + Date.now(),
    );
...
function isBeatPrefetchCandidate(song) {
  if (!song || isPodcastSong(song) || song.type === "local" || song.localUrl)
    return false;
...
async function fetchBeatPrefetchAudioUrl(song) {
  ...
  var requestedQuality = normalizePlaybackQualityForProvider(
    getPlaybackQualityForSong(song),
    isQQ ? "qq" : "netease",
  );
  if (
    !isQQ &&
  ...
  var runtimeQualityCap = playbackQualityCapValue(
    song,
    isQQ ? "qq" : "netease",
  );
  if (
  ...
  var neteaseMatchQuery =
    typeof neteasePlaybackMatchQuery === "function"
...
async function runQueueBeatPrefetch(fromIdx, token, seq, state) {
  ...
  try {
    ...
    console.warn(
      "queue beat prefetch failed:",
      err && err.message ? err.message : err,
    );
  } finally {
    beatPrefetchBusy = false;
    if (
      state.count < BEAT_PREFETCH_LIMIT &&
      token === beatMapToken &&
      seq === beatPrefetchToken &&
      playQueue.length &&
      !(audio && audio.paused)
