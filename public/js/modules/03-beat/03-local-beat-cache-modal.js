function showBeatChip(text) {
  ...
  if (localBeatAnalysis && localBeatAnalysis.active)
    setLocalBeatStatus(text || "分析中...", "warn");
}
...
function packLocalBeatMap(map) {
  if (!map) return null;
  var camera = (map.cameraBeats || map.beats || map.kicks || []).map(
    packLocalBeatEvent,
  );
  ...
  return {
    ...
    sectionSteps: (map.sectionSteps || []).map(function (v) {
      return localBeatRound(v, 1000);
    }),
...
function unpackLocalBeatMap(stored) {
  ...
  return {
    kicks: camera.map(function (b) {
      return typeof b === "number" ? b : b.time;
    }),
    beats: camera,
    pulseBeats: pulse,
    cameraBeats: camera,
...
function readLocalBeatPrefs() {
  try {
    return (
      JSON.parse(localStorage.getItem(LOCAL_BEAT_PREF_STORE_KEY) || "{}") || {}
    );
  } catch (e) {
    return {};
  }
...
function saveLocalBeatPrefs() {
  try {
    localStorage.setItem(
      LOCAL_BEAT_PREF_STORE_KEY,
      JSON.stringify(localBeatMapPrefs || {}),
    );
  } catch (e) {}
}
function readLocalBeatMapCache() {
  ...
  try {
    var raw =
      JSON.parse(localStorage.getItem(LOCAL_BEATMAP_STORE_KEY) || "{}") || {};
...
function packLocalBeatCache(maxEntries) {
  var entries = Object.keys(localBeatMapCache || {})
    .map(function (key) {
...
function saveLocalBeatMapCache() {
  ...
  for (var i = 0; i < attempts.length; i++) {
    try {
      localStorage.setItem(
        LOCAL_BEATMAP_STORE_KEY,
        JSON.stringify(packLocalBeatCache(attempts[i])),
      );
      return true;
    } catch (e) {}
  }
...
function getLocalBeatEntry(localKey, mode) {
  var entry =
    localKey && localBeatMapCache ? localBeatMapCache[localKey] : null;
...
function storeLocalBeatEntry(localKey, mode, map, song, opts) {
  ...
  if (!opts.skipDisk)
    writeBeatDiskCache(
      localBeatDiskKey(localKey, mode),
      map,
      song || { type: "local", localKey: localKey },
      mode,
    );
}
...
function applyLocalBeatMap(song, mode, map, fromCache) {
  ...
  if (fromCache)
    showToast((mode === "dj" ? "DJ" : "MR") + " 本地节奏缓存已载入");
  return true;
}
function prepareLocalBeatAnalysis(song, audioUrl) {
  ...
  var cached =
    getLocalBeatEntry(song.localKey, preferred) ||
    getLocalBeatEntry(song.localKey, preferred === "dj" ? "mr" : "dj");
  if (cached) {
    applyLocalBeatMap(
      song,
      cached === getLocalBeatEntry(song.localKey, "dj") ? "dj" : "mr",
      cached,
      true,
    );
    return;
  ...
  (async function () {
    ...
    var firstMap = await readBeatDiskCache(
      localBeatDiskKey(song.localKey, firstMode),
    );
    var mode = firstMap ? firstMode : secondMode;
    var map =
      firstMap ||
      (await readBeatDiskCache(localBeatDiskKey(song.localKey, secondMode)));
    if (
      diskToken !== trackSwitchToken ||
      !currentLocalSong ||
...
function openLocalBeatModal(song, audioUrl) {
  ...
  localBeatAnalysis.mode =
    localBeatAnalysis.song &&
    localBeatMapPrefs[localBeatAnalysis.song.localKey] === "dj"
      ? "dj"
      : "mr";
...
function updateLocalBeatModal() {
  ...
  if (sub) {
    var cachedBits = [];
    if (song.localKey && getLocalBeatEntry(song.localKey, "mr"))
      cachedBits.push("MR 已缓存");
    if (song.localKey && getLocalBeatEntry(song.localKey, "dj"))
      cachedBits.push("DJ 已缓存");
    sub.textContent = cachedBits.length
      ? cachedBits.join(" / ")
      : "选择一种电影视角分析方式";
  }
  ...
  if (start) {
    ...
    start.textContent = getLocalBeatEntry(song.localKey, mode)
      ? "使用缓存"
      : "开始分析";
  }
...
function cancelLocalBeatAnalysis() {
  ...
  hideBeatChip();
  if (localBeatAnalysis.mode === "dj")
    setDjModeActive(false, localBeatAnalysis.song || currentLocalSong);
...
async function startLocalBeatAnalysis(mode) {
  ...
  try {
    ...
    if (mode === "dj") {
      ...
      map = await analyzePodcastDjBeats(
        audioUrl,
        djToken,
      ...
      map = await analyzeAudioBeats(
        audioUrl,
        audio && isFinite(audio.duration) ? audio.duration : 0,
        mrToken,
        { background: false, song: song },
      );
