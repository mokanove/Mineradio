function schedulePodcastDjAnalysis(songKey, audioUrl, token, durationSec) {
  ...
  djBeatAnalysisTimer = setTimeout(function waitForDjStart() {
    ...
    if (
      token !== djBeatMapToken ||
      !djMode.active ||
      djMode.songKey !== songKey ||
      djBeatMapCache[songKey]
    )
      return;
    var startAnalysis = function () {
      ...
      if (
        ...
        (durationSec <= 0 || durationSec > 3300)
      ) {
        analyzePodcastDjIntroBeats(audioUrl, token, durationSec)
          .then(function (map) {
      ...
      analyzePodcastDjBeats(audioUrl, token, durationSec)
        .then(function (map) {
          if (token !== djBeatMapToken || !map) return;
...
async function analyzePodcastDjIntroBeats(audioUrl, token, durationSec) {
  ...
  var introResp = await fetch(
    "/api/podcast/dj-beatmap?url=" +
      encodeURIComponent(audioUrl) +
      "&duration=" +
      encodeURIComponent(durationSec || 0) +
      "&intro=180",
  );
  if (token !== djBeatMapToken || !djMode.active) return null;
  var introData = await introResp.json().catch(function () {
    return null;
  });
  if (
    introResp.ok &&
    ...
    introData.ok &&
    introData.map &&
    introData.map.cameraBeats &&
    introData.map.cameraBeats.length >= 4
  ) {
...
async function buildPodcastDjLowOnlyBeatMap(buffer, token) {
  ...
  var hopSec = duration > 4200 ? 0.0125 : 0.01;
  ...
  if (channels > 2) {
    ...
    for (var ch = 0; ch < channels; ch++)
      chList.push(buffer.getChannelData(ch));
  }
  ...
  function runBiquad(st, x) {
    var y =
      st.b0 * x + st.b1 * st.x1 + st.b2 * st.x2 - st.a1 * st.y1 - st.a2 * st.y2;
    st.x2 = st.x1;
    st.x1 = x;
    st.y2 = st.y1;
    st.y1 = y;
  ...
  for (var f = 0; f < nFrames; f++) {
    ...
    if (f > 0 && f % 720 === 0) {
      if (f % 4320 === 0)
        showBeatChip(
          "DJ kick scan " + Math.min(99, Math.round((f / nFrames) * 100)) + "%",
        );
      await yieldToPaint();
      if (token !== djBeatMapToken || !djMode.active) {
        hideBeatChip();
        return null;
      }
    }
  ...
  function percentile(arr, p, maxSamples) {
    ...
    if (len <= maxSamples) {
      ...
      for (var si = 0; si < maxSamples; si++)
        sample[si] = arr[Math.min(len - 1, Math.floor(si * step))] || 0;
    }
    ...
    return (
      sample[
        Math.max(0, Math.min(sample.length - 1, Math.floor(sample.length * p)))
      ] || 0.001
    );
  }
  ...
  for (var oi = 4; oi < nFrames; oi++) {
    ...
    var wideRise = Math.max(
      0,
      (lowEnergy[oi] + lowEnergy[oi - 1]) * 0.5 -
        (lowEnergy[oi - 3] + lowEnergy[oi - 4]) * 0.5,
    );
    var peakRise = Math.max(0, hitEnergy[oi] - hitEnergy[oi - 2] * 0.84);
    onset[oi] = lowRise * 1.72 + wideRise * 0.86 + peakRise * 0.1;
  }
  ...
  for (var cf = winN + 4; cf < nFrames - 4; cf++) {
    ...
    if (o > th && o >= onset[cf - 1] && o > onset[cf + 1]) {
      var peakF = cf;
      var peakScore = o + lowEnergy[cf] * 0.1;
      for (var pf = cf - 2; pf <= cf + 3; pf++) {
        var ps = (onset[pf] || 0) + (lowEnergy[pf] || 0) * 0.1;
        if (ps > peakScore) {
          peakScore = ps;
          peakF = pf;
        }
      ...
      var lowRel = clamp01(
        (bandAt(lowEnergy, peakF) - lowFloor) /
          Math.max(0.0001, lowCeil - lowFloor),
      );
      var score =
        (o - th) / Math.max(0.0006, std + mean * 0.38 + lowRef * 0.012);
  ...
  if (!candidates.length) {
    return {
      ...
      duration: duration,
      visualBeatCount: 0,
      tempoSource: "podcast-dj-low-empty",
      analyzedAt: Date.now(),
    };
  }
  ...
  var p30 = percentile(powers, 0.3);
  var p50 = percentile(powers, 0.5);
  var p90 = Math.max(p50 + 0.001, percentile(powers, 0.9));
  var p96 = Math.max(p90 + 0.001, percentile(powers, 0.965));
  var strong = candidates.filter(function (c) {
    return c.power >= p50 && c.lowTone > 0.34;
  });
    ...
    for (var ai = 0; ai < list.length; ai++) {
      for (var bi = ai + 1; bi < list.length && bi < ai + 10; bi++) {
        ...
        for (var div = 1; div <= 6; div++) {
          ...
          var weight =
            Math.sqrt(Math.max(0.001, list[ai].power * list[bi].power)) /
            Math.sqrt((bi - ai) * div);
  ...
  var globalStep = estimateStep(strong) || estimateStep(candidates) || 0.5;
  ...
  function nearestCandidate(center, windowSec, startIdx) {
    ...
    var j = startIdx || 0;
    while (j < candidates.length && candidates[j].time < center - windowSec)
      j++;
    for (
      var ni = j;
      ni < candidates.length && candidates[ni].time <= center + windowSec;
      ni++
    ) {
      var dist = Math.abs(candidates[ni].time - center);
      var score =
        candidates[ni].power * (1 - (dist / Math.max(0.001, windowSec)) * 0.42);
      if (score > bestScore) {
        best = candidates[ni];
        bestScore = score;
      }
  ...
  function scorePhase(anchorTime, step) {
    ...
    for (var gt = start; gt < end; gt += step) {
      while (cursor < candidates.length && candidates[cursor].time < gt - win)
        cursor++;
      var best = null,
        bestScore = 0;
      for (
        var pi = cursor;
        pi < candidates.length && candidates[pi].time <= gt + win;
        pi++
      ) {
  ...
  var phaseSource = strong
    .filter(function (c) {
      return c.time < Math.min(duration, 180);
    })
    .slice(0, 72);
  ...
  for (var pa = 0; pa < phaseSource.length; pa++) {
    ...
    if (sc > bestAnchorScore) {
      bestAnchorScore = sc;
      bestAnchor = phaseSource[pa].time;
    }
  }
  ...
  for (var secIdx = 0; secIdx < sectionCount; secIdx++) {
    var t0 = secIdx * sectionLen,
      t1 = Math.min(duration, t0 + sectionLen);
    var seg = strong.filter(function (c) {
      return c.time >= t0 && c.time < t1;
    });
    var prevStep = sectionSteps.length
      ? sectionSteps[sectionSteps.length - 1]
      : globalStep;
    ...
    var blended = prevStep ? localStep * 0.3 + prevStep * 0.7 : localStep;
    sectionSteps.push(blended || globalStep);
  }
  function stepAt(time) {
    var idx = Math.max(
      0,
      Math.min(sectionSteps.length - 1, Math.floor(time / sectionLen)),
    );
  ...
  for (var gridT = anchor; gridT < duration - 0.04;) {
    var localStep2 = stepAt(gridT) || globalStep || 0.5;
    var winSec = Math.max(0.06, Math.min(0.135, localStep2 * 0.2));
    while (
      cursorIdx < candidates.length &&
      candidates[cursorIdx].time < gridT - winSec
    )
      cursorIdx++;
    ...
    var lowTone2 = bestCand
      ? Math.max(gridLowTone * 0.62, bestCand.lowTone)
      : gridLowTone;
    var hitTone2 = bestCand
      ? Math.max(gridHitTone * 0.62, bestCand.hitTone)
      : gridHitTone;
    var distPenalty = bestCand
      ? 1 - Math.min(1, Math.abs(bestCand.time - gridT) / winSec) * 0.26
      : 0.54;
    var basePower = bestCand
      ? bestCand.power * distPenalty
      : gridLowTone * 0.25 + gridHitTone * 0.06;
    var powerRel = clamp01(
      (basePower - p30 * 0.78) / Math.max(0.001, p96 - p30 * 0.78),
    );
    var lowRel2 = clamp01(
      (gridLow - lowFloor) / Math.max(0.0001, lowCeil - lowFloor),
    );
    var kickRel = clamp01(
      powerRel * 0.74 +
        lowRel2 * 0.22 +
        clamp01((hitTone2 - 0.26) / 1.7) * 0.04,
    );
    var softGrid = (!bestCand && lowRel2 < 0.2) || kickRel < 0.16;
    ...
    var downLift =
      combo === "downbeat"
        ? visualRel > 0.18
          ? 0.016 + visualRel * 0.036
          : visualRel * 0.028
        : 0;
    var sectionGate = clamp01((kickRel - 0.1) / 0.58);
    var impact = Math.max(
      0.02,
      Math.min(0.88, 0.022 + Math.pow(visualRel, 1.62) * 0.86 + downLift),
    );
    var strength = Math.max(
      0.12,
      Math.min(0.93, 0.13 + Math.pow(visualRel, 1.12) * 0.68 + downLift * 0.7),
    );
    if (softGrid) {
      var softMul = combo === "downbeat" ? 0.48 : 0.3;
      impact *= softMul;
      strength *= 0.58 + sectionGate * 0.22;
    }
    var timingPull = bestCand
      ? 0.24 + clamp01((kickRel - 0.25) / 0.65) * 0.46
      : 0;
    var sourceTime = bestCand
      ? gridT * (1 - timingPull) + bestCand.time * timingPull
      : gridT;
    var cameraActive =
      impact >= 0.13 ||
      (combo === "downbeat" && kickRel >= 0.14) ||
      (bestCand && kickRel >= 0.18);
    var lowMix = Math.max(
      0.42,
      Math.min(
        0.9,
    ...
    var snapMix = Math.max(
      0.015,
      Math.min(
        0.62,
        0.026 +
          (combo === "accent" ? 0.4 : 0) +
          (combo === "rebound" ? 0.08 : 0) +
          visualRel * 0.038,
      ),
    );
    beats.push({
      ...
      confidence: Math.max(
        0.44,
        Math.min(0.99, 0.46 + kickRel * 0.43 + (bestCand ? 0.08 : -0.03)),
      ),
      impact: impact,
      ...
      mass: Math.max(
        0.36,
        Math.min(0.94, lowMix * 0.72 + Math.pow(visualRel, 1.22) * 0.24),
      ),
  ...
  var cameraBeats = beats.filter(function (b) {
    return b.camera !== false;
  });
  var pulseBeats = beats
    .filter(function (b) {
      return b.pulse !== false && (b.impact >= 0.16 || b.combo === "downbeat");
    })
    .map(function (b) {
      return {
        time: b.time,
  ...
  console.log(
    "podcast DJ low-only beatmap:",
    Math.round(duration) + "s",
    "step:",
    globalStep.toFixed(3),
    "candidates:",
    candidates.length,
    "beats:",
    beats.length,
...
async function analyzePodcastDjBeats(audioUrl, token, durationSec) {
  try {
    ...
    var preferServerAnalysis =
      /^https?:\/\//i.test(audioUrl || "") &&
      (durationSec <= 0 || durationSec > 3300);
    if (preferServerAnalysis) {
      showBeatChip("DJ 长播客后端锁拍...");
      var serverResp = await fetch(
        "/api/podcast/dj-beatmap?url=" +
          encodeURIComponent(audioUrl) +
          "&duration=" +
          encodeURIComponent(durationSec),
      );
      if (token !== djBeatMapToken || !djMode.active) {
        hideBeatChip();
        return null;
      }
      var serverData = await serverResp.json().catch(function () {
        return null;
      });
      if (serverResp.ok && serverData && serverData.ok && serverData.map)
        return serverData.map;
      console.warn(
        "podcast DJ server analysis failed:",
        serverData && serverData.error,
      );
      hideBeatChip();
      if (durationSec <= 0 || durationSec > 3300) return null;
    }
    var fetchAudioUrl = /^https?:\/\//i.test(audioUrl || "")
    ...
    var buffer = await new Promise(function (resolve, reject) {
      ...
      console.warn("podcast DJ decode failed:", e);
      return null;
    });
    ab = null;
    dc.close && dc.close();
    if (!buffer || token !== djBeatMapToken || !djMode.active) {
      hideBeatChip();
      return null;
    }
    ...
    function percentile(arr, p) {
      ...
      return copy.length
        ? copy[
            Math.max(0, Math.min(copy.length - 1, Math.floor(copy.length * p)))
          ]
        : 0.001;
    }
    ...
    for (var oi = 2; oi < nFrames; oi++) {
      ...
      onset[oi] =
        lowRise * 1.52 + lowWide * 0.58 + bodyRise * 0.16 + snapRise * 0.035;
    }
    ...
    for (var wi = 0; wi < winN; wi++) {
      sum += onset[wi] || 0;
      sq += (onset[wi] || 0) * (onset[wi] || 0);
    }
    for (var f2 = winN + 1; f2 < nFrames - 2; f2++) {
      var mean = sum / winN;
      var std = Math.sqrt(Math.max(0, sq / winN - mean * mean));
      var th = mean + std * 1.9 + lowRef * 0.006;
      var o = onset[f2];
      if (
        o > th &&
        o >= onset[f2 - 1] &&
        o > onset[f2 + 1] &&
        f2 - lastFrame >= minFrameGap
      ) {
        ...
        var lowDom =
          lowTone / Math.max(0.001, bodyTone * 0.46 + snapTone * 0.18);
    ...
    if (!candidates.length) {
      ...
      return {
        ...
        visualBeatCount: 0,
        tempoSource: "podcast-dj-empty",
        analyzedAt: Date.now(),
      };
    }

    var strong = candidates.filter(function (c) {
      return c.score > 0.52 && c.lowTone > 0.52;
    });
    ...
    for (var si = 0; si < sectionCount; si++) {
      ...
      for (var sg = 1; sg < seg.length; sg++) {
        ...
        while (gap < 0.3) gap *= 2.0;
        if (gap >= 0.3 && gap <= 0.94) gaps.push(gap);
      }
      var prevSectionStep = sectionSteps.length
        ? sectionSteps[sectionSteps.length - 1]
        : globalStep;
      var step = median(gaps) || prevSectionStep || globalStep;
      if (globalStep)
        step = clampRange(step, globalStep * 0.9, globalStep * 1.1);
      if (
        prevSectionStep &&
        Math.abs(step - prevSectionStep) / prevSectionStep > 0.08
      ) {
    ...
    var phaseCandidates = phaseSource
      .filter(function (c) {
        return c.time < Math.min(buffer.duration, 120);
      })
      .slice(0, 56);
    ...
    function nearestCandidate(center, windowSec, startIdx) {
      ...
      for (
        ...
        var score =
          candidates[ni].power *
          (1 - (dist / Math.max(0.001, windowSec)) * 0.48);
        if (score > bestScore) {
          best = candidates[ni];
          bestScore = score;
        }
    ...
    function scorePhase(anchorTime) {
      var step = globalStep || 0.5;
      var start = anchorTime;
      while (start - step > 0.05) start -= step;
      var end = Math.min(buffer.duration, 132);
      var win = Math.max(0.06, Math.min(0.13, step * 0.18));
      var score = 0,
        count = 0,
        cursor = 0;
      for (var gt = start; gt < end; gt += step) {
    ...
    var anchor = bestAnchor;
    while (anchor - (globalStep || 0.5) > 0.05) anchor -= globalStep || 0.5;

    var beats = [];
    var gridIndex = 0;
    var cursorIdx = 0;
    for (var gridT = anchor; gridT < buffer.duration - 0.05;) {
      var localStep = stepAt(gridT) || globalStep || 0.5;
      var winSec = Math.max(0.07, Math.min(0.145, localStep * 0.22));
      while (
        cursorIdx < candidates.length &&
        candidates[cursorIdx].time < gridT - winSec
      )
      ...
      var powerBase = bestCand
        ? bestCand.power
        : gridLowTone * 0.22 + gridBodyTone * 0.04 + gridSnapTone * 0.02;
      var distPenalty = bestCand
        ? 1 - Math.min(1, Math.abs(bestCand.time - gridT) / winSec) * 0.3
        : 0.58;
      var powerRel = clamp01(
        (powerBase * distPenalty - p35 * 0.78) /
          Math.max(0.001, p90 - p35 * 0.78),
      );
      var lowTone2 = bestCand
        ? Math.max(gridLowTone * 0.55, bestCand.lowTone)
        : gridLowTone;
      var bodyTone2 = bestCand
        ? Math.max(gridBodyTone * 0.5, bestCand.bodyTone)
        : gridBodyTone;
      var snapTone2 = bestCand
        ? Math.max(gridSnapTone * 0.5, bestCand.snapTone)
        : gridSnapTone;
      var toneTotal = Math.max(
        0.001,
        lowTone2 + bodyTone2 * 0.72 + snapTone2 * 0.48,
      );
      ...
      var weakGrid = !bestCand && gridLowTone < 0.5 && powerRel < 0.24;
      if (!weakGrid || comboSlot === 0 || powerRel > 0.18) {
        ...
        var strength = Math.max(
          ...
          Math.min(
            0.94,
            0.2 + Math.pow(powerRel, 1.22) * 0.54 + lowMix * 0.08 + downLift,
          ),
        );
        var impact = Math.max(
          0.1,
          Math.min(
            0.96,
            Math.pow(powerRel, 1.36) * 0.82 + lowMix * 0.12 + downLift,
          ),
        );
        beats.push({
          time: sourceTime,
          ...
          confidence: Math.max(
            ...
            Math.min(
              0.98,
              0.5 + powerRel * 0.38 + lowMix * 0.1 - (bestCand ? 0 : 0.1),
            ),
          ),
          impact: impact,
          primary: true,
          ...
          low: Math.max(0.24, Math.min(0.9, lowMix * 0.78 + powerRel * 0.18)),
          body: Math.max(0.03, Math.min(0.6, bodyMix)),
          snap: Math.max(0.02, Math.min(0.5, snapMix)),
          mass: Math.max(
            0.28,
            Math.min(0.96, lowMix * 0.74 + Math.pow(powerRel, 1.25) * 0.24),
          ),
          sharpness: Math.max(0.03, Math.min(0.62, snapMix * 1.1)),
          combo: combo,
          step: localStep,
          index: beats.length,
          dj: true,
    ...
    var pulseBeats = beats
      .filter(function (b) {
        return b.strength >= 0.38 || b.combo === "downbeat";
      })
      .map(function (b) {
        return {
          time: b.time,
...
function applyPodcastDjProfileFromMap(map) {
  ...
  var density =
    (map.cameraBeats || []).length / Math.max(20, map.duration || 20);
  cinemaTrackProfile.density = density;
  var target = 0.82 + clamp01((density - 1.25) / 1.8) * 0.16;
  target = clampRange(target, 0.76, 1.1);
