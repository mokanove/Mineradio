...
function shouldShowIdleGuide() {
  ...
  if (uniforms && uniforms.uHasCover && uniforms.uHasCover.value > 0.5)
    return false;
  return true;
}
function shouldShowShelfHoverCue(value) {
  if (document.body.classList.contains("splash-active")) return false;
  if (!shelfHoverCue.guide && document.querySelector(".modal-mask.show"))
    return false;
  if (!shelfHoverCue.guide) {
    ...
    if (shelfManager.hasOpenContent && shelfManager.hasOpenContent())
      return false;
    if (!shelfManager.getMode || shelfManager.getMode() !== "side")
      return false;
  }
  return (
    shelfHoverCue.guide ||
    shelfHoverCue.target > 0 ||
    (value || shelfHoverCue.value) > 0.015
  );
}
...
function idleGuidePointerDown(e) {
  ...
  idleGuideInteraction.pointerX =
    e.clientX / Math.max(1, idleGuideW || innerWidth);
  idleGuideInteraction.pointerY =
    e.clientY / Math.max(1, idleGuideH || innerHeight);
  document.body.classList.add("idle-guide-dragging");
}
function idleGuidePointerMove(e) {
  if (!idleGuideCanvas) return;
  var canReact =
    shouldHandleIdleGuidePointer(e) || idleGuideInteraction.dragging;
  ...
  var dt = Math.max(
    1 / 120,
    Math.min(0.08, (now - idleGuideInteraction.lastT) / 1000 || 1 / 60),
  );
  ...
  idleGuideInteraction.velocity = Math.sqrt(
    idleGuideInteraction.spinX * idleGuideInteraction.spinX +
      idleGuideInteraction.spinY * idleGuideInteraction.spinY,
  );
  idleGuideInteraction.lastX = e.clientX;
  idleGuideInteraction.lastY = e.clientY;
...
function idleGuidePointerLeave() {
  if (!idleGuideInteraction.dragging)
    idleGuideInteraction.pointerActive = false;
}
function idleGuideWheel(e) {
  ...
  guide.zoomPulse = Math.min(
    1,
    guide.zoomPulse + Math.min(0.28, Math.abs(e.deltaY) * 0.0014),
  );
  return true;
}
function resizeIdleGuideCanvas() {
  ...
  for (var i = 0; i < count; i++) {
    ...
    var wobbleAmp =
      minDim *
      (ring ? 0.012 + Math.random() * 0.035 : 0.01 + Math.random() * 0.055);
    idleGuideParticles.push({
      a: a,
      r: r,
      cx: ring ? 0.5 : Math.random(),
      cy: ring ? 0.5 : Math.random(),
      size: ring ? 0.3 + Math.random() * 0.62 : 0.18 + Math.random() * 0.44,
      speed:
        ((ring ? 0.018 : 0.01) + Math.random() * (ring ? 0.045 : 0.03)) *
        (Math.random() < 0.5 ? -1 : 1),
...
function drawIdleGuideTrail(ctx, trail, now, alpha, energy) {
  ...
  for (var i = 1; i < trail.length; i++) {
    ...
    ctx.strokeStyle =
      "rgba(255,255,255," +
      (alpha * fade * (0.18 + energy * 0.24)).toFixed(3) +
      ")";
...
function drawIdleGuideFrame() {
  ...
  var dtFrame = Math.max(
    1 / 120,
    Math.min(0.05, (nowFrame - idleGuideLastFrameAt) / 1000 || 1 / 60),
  );
  ...
  var cy = idleGuideH * 0.5;
  var guide = idleGuideInteraction;
  if (!guide.dragging) {
    guide.rotX += guide.spinX * dtFrame;
    guide.rotY += guide.spinY * dtFrame;
    guide.spinX *= Math.pow(0.9, dtFrame * 60);
    guide.spinY *= Math.pow(0.9, dtFrame * 60);
  ...
  guide.velocity = Math.sqrt(
    guide.spinX * guide.spinX + guide.spinY * guide.spinY,
  );
  ...
  var halo = ctx.createRadialGradient(
    ...
    cx,
    cy,
    Math.min(idleGuideW, idleGuideH) *
      ((0.36 + breathe * 0.035 + guide.press * 0.018) * zoom),
  );
  halo.addColorStop(
    0,
    "rgba(255,255,255," +
  ...
  for (var i = 0; i < idleGuideParticles.length; i++) {
    ...
    var wobble =
      ...
      Math.sin(t * (p.wobbleSpeed * 0.57 + 0.11) + p.phase * 1.7) *
        p.wobbleAmp *
        0.45;
    var x, y;
    var projected = null;
    ...
    if (p.ring) {
      var rr =
        (p.r + wobble + breathe * 12) *
        zoom *
        (1 + guide.press * 0.03 + zoomBoost * 0.018);
      var baseX =
        Math.cos(localA) * rr +
      ...
      var baseY =
        ...
        p.driftY;
      var baseZ =
        (Math.sin(localA * 0.84 + p.phase * 0.31) * rr * p.zAmp +
          p.z * 0.54 +
          Math.cos(wanderA * 0.91) * p.wobbleAmp) *
        zoom;
      projected = projectIdleGuidePoint(
        baseX,
        baseY,
        baseZ,
        rot,
      ...
      var nDx = pointerX - x,
        nDy = pointerY - y;
      var near =
        guide.focus * Math.max(0, 1 - Math.sqrt(nDx * nDx + nDy * nDy) / 210);
      x += nDx * near * 0.04;
      y += nDy * near * 0.04;
      ringPts.push({
        x: x,
      ...
      var fieldPt = projectIdleGuidePoint(
        driftX,
        driftY,
        driftZ,
        rot,
    ...
    var depthGlow = p.ring && projected ? 0.66 + projected.scale * 0.2 : 1;
    var aP = p.ring
      ? (0.07 +
          breathe * 0.065 +
          Math.sin(t * (0.8 + p.layer) + p.phase) * 0.024 +
          spinEnergy * 0.032) *
        depthGlow
    ...
    ctx.arc(
      ...
      p.size *
        pointScale *
        Math.sqrt(zoom) *
        (1 + spinEnergy * (p.ring ? 0.24 : 0.08) + zoomBoost * 0.12),
      0,
      Math.PI * 2,
    );
  ...
  if (guide.focus > 0.03 || spinEnergy > 0.05) {
    var orbitR =
      Math.min(idleGuideW, idleGuideH) * (0.305 + guide.press * 0.018) * zoom;
    var anchorAlpha = Math.min(
      0.68,
      0.16 + guide.focus * 0.24 + spinEnergy * 0.38,
    );
    for (var k = 0; k < 4; k++) {
      ...
      ctx.arc(
        anchorPt.x,
        anchorPt.y,
        (2.0 + spinEnergy * 1.8 + (k === 0 ? guide.press * 1.8 : 0)) *
          anchorPt.scale,
        0,
        Math.PI * 2,
      );
  ...
  if (guide.focus > 0.03) {
    ...
    var handleR =
      Math.min(idleGuideW, idleGuideH) *
      (0.315 + breathe * 0.012 + guide.press * 0.012) *
      zoom;
    ...
    var hx = handlePt.x;
    var hy = handlePt.y;
    var handleGlow = ctx.createRadialGradient(
      hx,
      hy,
      0,
      hx,
    ...
    handleGlow.addColorStop(
      0,
      "rgba(255,255,255," +
        (0.22 * guide.focus + 0.16 * guide.press).toFixed(3) +
        ")",
    );
    ...
    ctx.fillStyle =
      "rgba(255,255,255," +
      (0.54 * guide.focus + 0.24 * guide.press).toFixed(3) +
      ")";
    ctx.fill();
  }
...
function drawShelfGuideCue(ctx, t, strength) {
  strength = Math.max(
    0,
    Math.min(1, strength == null ? shelfHoverCue.value : strength),
  );
  ...
  glow.addColorStop(
    0.58,
    "rgba(255,255,255," + (0.01 * strength).toFixed(3) + ")",
  );
  glow.addColorStop(
    0.82,
    "rgba(244,210,138," +
      (0.024 * strength + pulse * 0.012 * strength).toFixed(3) +
      ")",
  );
  glow.addColorStop(
    1,
    "rgba(255,255,255," + (0.035 * strength).toFixed(3) + ")",
  );
  ctx.fillStyle = glow;
  ...
  halo.addColorStop(
    0,
    "rgba(244,210,138," +
      (0.07 * strength + pulse * 0.026 * strength).toFixed(3) +
      ")",
  );
  halo.addColorStop(
    0.45,
    "rgba(255,255,255," + (0.02 * strength).toFixed(3) + ")",
  );
  halo.addColorStop(1, "rgba(255,255,255,0)");
  ctx.fillStyle = halo;
  ...
  for (var i = 0; i < 10; i++) {
    var seed = i * 19.17;
    var phase = (t * (0.1 + (i % 4) * 0.014) + i * 0.113) % 1;
    var x =
      r.left +
      r.width * (0.45 + (i % 4) * 0.13) +
      Math.sin(t * 0.44 + seed) * 12;
    var y =
      r.top +
      r.height * (0.18 + ((i * 0.137 + Math.sin(seed)) % 0.64)) +
      floatY * (0.42 + (i % 3) * 0.1);
    var alpha = (0.035 + Math.sin(Math.PI * phase) * 0.05) * strength;
    if (alpha <= 0) continue;
    ctx.beginPath();
...
function maybeRunStartupVisualGuide(source) {
  if (visualGuideWasSeen() || visualGuideActive || immersiveMode || playing)
    return false;
...
function startVisualGuide(opts) {
  ...
  visualGuideState = {
    ...
    searchWasPeek: !!(
      ...
      document.getElementById("search-area").classList.contains("peek")
    ),
    fxWasPeek: !!(
    ...
    plWasPeek: !!(
      ...
      document.getElementById("playlist-panel").classList.contains("peek")
    ),
...
function prepareVisualGuideStep(step) {
  ...
  if (typeof setShelfGuideCueActive === "function")
    setShelfGuideCueActive(step && step.target === "shelf");
  if (step && step.selector === "#search-box") setPeek(search, true, "search");
  if (step && step.selector === "#playlist-panel")
    setPeek(playlistPanel, true, "pl");
  else if (playlistPanel && !visualGuideState.plWasPeek)
    setPeek(playlistPanel, false, "pl");
  if (step && step.selector === "#fx-panel") setPeek(fxPanel, true, "fx");
  else if (fxPanel && !visualGuideState.fxWasPeek)
    setPeek(fxPanel, false, "fx");
  if (
    step &&
    (step.selector === "#bottom-bar" ||
      step.selector === "#mini-queue-btn" ||
      step.selector === "#immersive-btn" ||
      step.selector === "#quality-control")
  ) {
...
function showVisualGuideStep(index) {
  ...
  if (hint)
    hint.textContent =
      visualGuideStep === steps.length - 1
        ? "点击空白处完成引导"
        : "点击空白处也可以继续";
  if (progress)
    progress.textContent = visualGuideStep + 1 + " / " + steps.length;
  if (next)
    next.textContent = visualGuideStep === steps.length - 1 ? "完成" : "下一步";
...
function guideTargetRect(step) {
  if (step && step.target === "stage") {
    ...
    return {
      ...
      width: stageW,
      height: stageH,
      right: stageLeft + stageW,
      bottom: stageTop + stageH,
    };
  }
  ...
  var isFullscreenDiyStep = !!(
    step &&
    step.selector === "#diy-mode-btn" &&
    (desktopRuntimeState.fullscreen ||
      desktopFullscreenActive ||
      document.fullscreenElement ||
      document.body.classList.contains("desktop-fullscreen"))
  );
  var useFullscreenDiyTarget =
    isFullscreenDiyStep && !shouldSuppressFullscreenDiyPeek();
  ...
  var target =
    step && step.selector
      ? document.querySelector(
          useFullscreenDiyTarget ? "#fullscreen-diy-btn" : step.selector,
        )
      : null;
  if (target) {
  ...
  if (step && step.selector === "#diy-mode-btn") {
    ...
    var fallbackTop = 16;
    return {
      left: fallbackRight - 88,
      top: fallbackTop,
      width: 88,
      height: 38,
      right: fallbackRight,
      bottom: fallbackTop + 38,
    };
  }
  return {
    left: innerWidth * 0.5 - 120,
    top: innerHeight * 0.5 - 40,
    width: 240,
    height: 80,
    right: innerWidth * 0.5 + 120,
    bottom: innerHeight * 0.5 + 40,
  };
}
function positionVisualGuideStep() {
  ...
  var pad =
    step && step.target === "shelf"
      ? 14
      : step && step.selector === "#bottom-bar"
        ? 10
        : 8;
  ...
  if (scrim) {
    scrim.style.setProperty(
      "--gx",
      (((rect.left + rect.width / 2) / Math.max(1, innerWidth)) * 100).toFixed(
        2,
      ) + "%",
    );
    scrim.style.setProperty(
      "--gy",
      (((rect.top + rect.height / 2) / Math.max(1, innerHeight)) * 100).toFixed(
        2,
      ) + "%",
    );
  }
...
function closeVisualGuide(markSeen) {
  ...
  if (
    search &&
    !visualGuideState.searchWasPeek &&
    document.activeElement !== $input
  )
    setPeek(search, false, "search");
  if (fxPanel && !visualGuideState.fxWasPeek) setPeek(fxPanel, false, "fx");
  if (playlistPanel && !visualGuideState.plWasPeek)
    setPeek(playlistPanel, false, "pl");
  if (bottom && !visualGuideState.bottomWasVisible && !playing)
