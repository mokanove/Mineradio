...
var DESKTOP_ICON_SHIELD_TARGETS = [
  ...
  {
    selector: "#fullscreen-diy-zone",
    kind: "fullscreen-tools",
...
var DESKTOP_ICON_SHIELD_SELECTOR = DESKTOP_ICON_SHIELD_TARGETS.map(
  function (target) {
    return target.selector;
  },
).join(",");
function getDesktopWindowApi() {
  return window.desktopWindow && window.desktopWindow.isDesktop
...
function desktopIconShieldModeState() {
  ...
  var hasWindowInteractive =
    typeof windowState.isDesktopInteractive === "boolean";
  var hasStatusEnabled =
    typeof status.enabled === "boolean" || typeof status.active === "boolean";
...
function desktopIconShieldElementVisible(element, target) {
  ...
  if (
    (!target || target.ignoreAriaHidden !== true) &&
  ...
  if (style) {
    ...
    if ((!target || target.visual !== true) && style.pointerEvents === "none")
      return false;
  }
  if (!element.getClientRects || element.getClientRects().length < 1)
    return false;
  return true;
}

function desktopIconShieldClippedRect(
  rect,
  viewportWidth,
  viewportHeight,
  kind,
  ...
  if (!isFinite(right) || !isFinite(bottom) || right <= left || bottom <= top)
    return null;
  return {
    x: left,
    y: top,
...
function desktopIconShieldMeshRect(
  mesh,
  viewportWidth,
  viewportHeight,
  kind,
...
function desktopIconShieldUnionRects(
  rects,
  viewportWidth,
  viewportHeight,
  kind,
...
function desktopIconShieldShelfRects(viewportWidth, viewportHeight) {
  var pinned =
    typeof shelfPinnedOpen !== "undefined" && shelfPinnedOpen === true;
  ...
  try {
    contentOpen = !!(
      typeof shelfManager !== "undefined" &&
      shelfManager &&
      shelfManager.hasOpenContent &&
      shelfManager.hasOpenContent()
    );
    mode = String(
      (shelfManager && shelfManager.getMode && shelfManager.getMode()) || "off",
    );
    canInteract = !!(
      shelfManager &&
      shelfManager.canInteract &&
      shelfManager.canInteract()
    );
    alwaysVisible =
  ...
  var sideActive =
    mode === "side" &&
    canInteract &&
    (pinned || contentOpen || alwaysVisible || previewVisible);
  ...
  try {
    var cards =
      shelfManager && shelfManager.getCards ? shelfManager.getCards() : [];
    for (var i = 0; i < cards.length && projected.length < 28; i++) {
      var cardRect = desktopIconShieldMeshRect(
        cards[i] && cards[i].mesh,
        viewportWidth,
        viewportHeight,
        "shelf-card",
        42,
    ...
    var content =
      shelfManager && shelfManager.getContentList
        ? shelfManager.getContentList()
        : null;
    var rows =
      contentOpen && content && content.getRows ? content.getRows() : [];
    for (var j = 0; j < rows.length && projected.length < 52; j++) {
      var rowRect = desktopIconShieldMeshRect(
        rows[j] && rows[j].mesh,
        viewportWidth,
        viewportHeight,
        "shelf-detail-row",
  ...
  var union = desktopIconShieldUnionRects(
    projected,
    viewportWidth,
    viewportHeight,
    contentOpen
    ...
    desktopIconShieldClippedRect(
      {
        ...
        right: viewportWidth,
        bottom:
          viewportHeight - Math.max(84, Math.round(viewportHeight * 0.08)),
      },
      viewportWidth,
      viewportHeight,
      contentOpen ? "shelf-detail-fallback" : "shelf-side-fallback",
    ),
  ].filter(Boolean);
}
...
function collectDesktopIconShieldRects() {
  ...
  for (
    var i = 0;
    i < DESKTOP_ICON_SHIELD_TARGETS.length && rects.length < 64;
    i++
  ) {
    ...
    for (var j = 0; j < elements.length && rects.length < 64; j++) {
      ...
      for (var k = 0; k < clientRects.length && rects.length < 64; k++) {
        var clipped = desktopIconShieldClippedRect(
          clientRects[k],
          viewportWidth,
          viewportHeight,
          target.kind,
  ...
  if (rects.length < 64) {
    ...
    for (
      var shelfIndex = 0;
      shelfIndex < shelfRects.length && rects.length < 64;
      shelfIndex++
    ) {
...
function desktopIconsAreVisible(status) {
  ...
  return (
    !Object.prototype.hasOwnProperty.call(status, "desktopIconsVisible") ||
...
function updateDesktopModeControl(status) {
  ...
  var softwareLockButton = document.getElementById(
    "desktop-software-lock-toggle",
  );
  var softwareLockState = document.getElementById(
    "desktop-software-lock-state",
  );
  ...
  if (softwareLockButton) {
    var softwareLockSupported = !!(
      api && typeof api.setDesktopSoftwareLocked === "function"
    );
    softwareLockButton.setAttribute(
      "aria-checked",
      softwareLocked ? "true" : "false",
    );
    softwareLockButton.setAttribute(
      "aria-busy",
      desktopSoftwareLockPending ? "true" : "false",
    );
    softwareLockButton.disabled =
      desktopSoftwareLockPending || !active || !softwareLockSupported;
    softwareLockButton.title = softwareLocked
      ? "恢复 Mineradio 操作"
      : "暂时把操作交给 Windows 桌面";
  }
  if (softwareLockState)
    softwareLockState.textContent = softwareLocked
      ? "软件操作已锁定，可在此解锁"
      : "软件可正常操作";
  if (document.body)
    document.body.classList.toggle(
      "desktop-software-locked",
      active && softwareLocked,
    );
  if (iconsButton) {
    var iconsSupported = !!(
      api && typeof api.setDesktopIconsVisible === "function"
    );
    iconsButton.setAttribute("aria-checked", iconsVisible ? "true" : "false");
    iconsButton.setAttribute(
      "aria-busy",
      desktopIconVisibilityPending ? "true" : "false",
    );
    iconsButton.disabled =
      desktopIconVisibilityPending || !active || !iconsSupported;
    iconsButton.title = iconsVisible
      ? "隐藏 Windows 桌面图标"
      : "显示 Windows 桌面图标";
  }
  if (iconsState)
    iconsState.textContent = iconsVisible ? "图标已显示" : "图标已隐藏";
}
...
function setDesktopIconsVisibility(desired, event) {
  ...
  if (
    !api ||
    typeof api.setDesktopIconsVisible !== "function" ||
    !mode.active
  ) {
    if (typeof showToast === "function")
      showToast("当前桌面图标显示控制不可用");
  ...
  if (desktopIconVisibilityPending)
    return Promise.resolve({
      ok: false,
      error: "DESKTOP_ICON_VISIBILITY_BUSY",
    });
  ...
  return Promise.resolve(api.setDesktopIconsVisible(desired))
    .then(function (result) {
      if (operation !== desktopIconVisibilityOperation) return result;
      result =
        result && typeof result === "object"
          ? result
          : { ok: false, error: "DESKTOP_ICON_VISIBILITY_RESULT_INVALID" };
      if (result.status) {
    ...
    .catch(function (error) {
      ...
      if (typeof showToast === "function")
        showToast(desired ? "桌面图标显示失败" : "桌面图标隐藏失败");
      return { ok: false, error: String((error && error.message) || error) };
    })
    .then(function (result) {
      if (operation === desktopIconVisibilityOperation) {
...
function toggleDesktopIconsVisibility(event) {
  return setDesktopIconsVisibility(
...
function requestDesktopKeyboardFocus(reason) {
  ...
  if (
    !api ||
    typeof api.requestDesktopKeyboardFocus !== "function" ||
    !mode.active ||
    desktopWallpaperRuntimeState.softwareInteractionLocked === true
  )
    return false;
  try {
...
function setDesktopSoftwareInteractionLocked(desired, event) {
  ...
  if (
    !api ||
    typeof api.setDesktopSoftwareLocked !== "function" ||
    !mode.active
  ) {
  ...
  if (desktopSoftwareLockPending)
    return Promise.resolve({ ok: false, error: "DESKTOP_SOFTWARE_LOCK_BUSY" });
  desired = desired === true;
  var operation = ++desktopSoftwareLockOperation;
  desktopSoftwareLockPending = true;
  updateDesktopModeControl(desktopWallpaperRuntimeState);
  return Promise.resolve(api.setDesktopSoftwareLocked(desired))
    .then(function (result) {
      if (operation !== desktopSoftwareLockOperation) return result;
      result =
        result && typeof result === "object"
          ? result
          : { ok: false, error: "DESKTOP_SOFTWARE_LOCK_RESULT_INVALID" };
      if (result.status) {
        applyDesktopWallpaperRuntimeStatus(result.status);
      } else if (result.ok === true) {
    ...
    .catch(function (error) {
      if (operation !== desktopSoftwareLockOperation)
        return { ok: false, error: String((error && error.message) || error) };
      if (typeof showToast === "function")
        showToast(desired ? "软件操作锁定失败" : "软件操作解锁失败");
      return { ok: false, error: String((error && error.message) || error) };
    })
    .then(function (result) {
      if (operation === desktopSoftwareLockOperation) {
...
function toggleDesktopSoftwareInteractionLocked(event) {
  return setDesktopSoftwareInteractionLocked(
    !(
...
function setDesktopModeControlPeek(peek) {
  ...
  if (document.body)
    document.body.classList.toggle("desktop-mode-control-peek", peek);
  scheduleDesktopIconShieldReport(false);
}

function scheduleDesktopModeControlPeekHide(delay) {
  cancelDesktopModeControlPeekHide();
  desktopModeControlDockState.peekTimer = setTimeout(
    function () {
      desktopModeControlDockState.peekTimer = 0;
      var dock = document.getElementById("desktop-mode-control-dock");
      if (
        desktopModeControlDockState.open ||
        (dock &&
          (dock.matches(":hover") || dock.contains(document.activeElement)))
      )
        return;
...
function scheduleDesktopModeControlHide(delay) {
  cancelDesktopModeControlHide();
  desktopModeControlDockState.hideTimer = setTimeout(
    function () {
      desktopModeControlDockState.hideTimer = 0;
      var dock = document.getElementById("desktop-mode-control-dock");
      if (
...
function desktopControlPointState(x, y) {
  ...
  var safeTop = rootStyle
    ? parseFloat(rootStyle.getPropertyValue("--desktop-safe-top")) || 0
    : 0;
  var safeRight = rootStyle
    ? parseFloat(rootStyle.getPropertyValue("--desktop-safe-right")) || 0
    : 0;
  var overRevealEdge =
    x >= Math.max(0, window.innerWidth - safeRight - 92) &&
    x < Math.max(0, window.innerWidth - safeRight) &&
    y >= safeTop &&
    y < safeTop + 104;
  var overHotspot = !!(
    handle &&
    desktopIconShieldElementVisible(handle, { ignoreAriaHidden: true }) &&
...
function flushDesktopPointerRouteReport(force) {
  ...
  if (mode.active && desktopPointerRouteReporter.hasPointer) {
    var pointState = desktopControlPointState(
      desktopPointerRouteReporter.x,
      desktopPointerRouteReporter.y,
    );
    ...
    if (overDesktopControls) {
      ...
      scheduleDesktopModeControlHide(
        desktopWallpaperRuntimeState &&
          desktopWallpaperRuntimeState.softwareInteractionLocked === true
          ? 120
          : 680,
    ...
    overSoftwareUi = desktopPointerOverSoftwareUi(
      desktopPointerRouteReporter.x,
      desktopPointerRouteReporter.y,
    );
  }
  var key =
    (overSoftwareUi ? "1" : "0") + "|" + (overDesktopControls ? "1" : "0");
...
function scheduleDesktopPointerRouteReport(event, force) {
  if (event && isFinite(event.clientX) && isFinite(event.clientY)) {
    ...
    var mode = desktopIconShieldModeState();
    var nextOverControls =
      mode.active &&
      desktopControlPointState(
        desktopPointerRouteReporter.x,
        desktopPointerRouteReporter.y,
      ).overControls;
    if (nextOverControls !== desktopPointerRouteReporter.overControls)
      force = true;
  ...
  desktopPointerRouteReporter.timer = setTimeout(
    function () {
...
function initDesktopModeControls(api) {
  ...
  if (softwareLockButton)
    softwareLockButton.addEventListener(
      "click",
      toggleDesktopSoftwareInteractionLocked,
    );
  if (iconsButton)
    iconsButton.addEventListener("click", toggleDesktopIconsVisibility);
  document.addEventListener(
    "pointerdown",
    function (event) {
      if (event && event.isTrusted) requestDesktopKeyboardFocus("pointerdown");
      if (!desktopModeControlDockState.open || dock.contains(event.target))
        return;
...
function desktopIconShieldPayloadKey(payload) {
  ...
  for (var i = 0; i < rects.length; i++) {
    var rect = rects[i];
    rectKey.push(
      [rect.x, rect.y, rect.width, rect.height, rect.kind || ""].join(","),
    );
  }
  return [
    payload.enabled ? 1 : 0,
...
function flushDesktopIconShieldReport(forceEmpty) {
  ...
  var payload = {
    ...
    runtimeGeneration: Math.max(
      -1,
      Number(
        desktopWallpaperRuntimeState && desktopWallpaperRuntimeState.generation,
      ) || 0,
    ),
...
function desktopIconShieldMutationRelevant(mutation) {
  ...
  try {
    if (
      target &&
      (target.matches(DESKTOP_ICON_SHIELD_SELECTOR) ||
        target.closest(DESKTOP_ICON_SHIELD_SELECTOR))
    )
      return true;
  } catch (_) {}
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    try {
      if (
        node.matches(DESKTOP_ICON_SHIELD_SELECTOR) ||
        node.querySelector(DESKTOP_ICON_SHIELD_SELECTOR)
      )
        return true;
    } catch (_) {}
  }
  ...
  for (var i = 0; i < DESKTOP_ICON_SHIELD_TARGETS.length; i++) {
    ...
    try {
      elements = document.querySelectorAll(
        DESKTOP_ICON_SHIELD_TARGETS[i].selector,
      );
    } catch (_) {
      elements = [];
    }
...
function initDesktopIconShieldReporter(api) {
  ...
  if (typeof window.MutationObserver === "function") {
    desktopIconShieldReporter.mutationObserver = new MutationObserver(function (
      mutations,
    ) {
      var relevant = false;
    ...
    desktopIconShieldReporter.mutationObserver.observe(
  ...
  desktopIconShieldReporter.shelfTimer = setInterval(function () {
    if (
      !desktopIconShieldModeState().active ||
      !desktopIconShieldShelfNeedsGeometryTracking()
    )
      return;
    scheduleDesktopIconShieldReport(false);
  }, 250);
  window.addEventListener(
    "pagehide",
    function () {
      if (desktopIconShieldReporter.shelfTimer)
        clearInterval(desktopIconShieldReporter.shelfTimer);
...
function currentDesktopSongMeta() {
  ...
  return {
    ...
    cover:
      typeof songCoverSrc === "function" && song
        ? songCoverSrc(song, 360) || song.cover || ""
        : song.cover || "",
  };
}
...
function currentDesktopLyricSnapshot() {
  ...
  var t =
    typeof getAdjustedLyricPlaybackTime === "function"
      ? getAdjustedLyricPlaybackTime(rawT)
      : rawT;
  ...
  if (playing && audio && lines.length) {
    ...
    if (idx >= 0) {
      ...
      var nextT =
        nextLine && nextLine.t > curLine.t
          ? nextLine.t
          : Math.min(
              (audio && audio.duration) || t + 4,
              curLine.t + (curLine.duration || 4.8),
            );
      var span = Math.max(0.75, nextT - curLine.t);
      return {
        text: normalizeDesktopLyricText(
          curLine.text || currentLyricFallbackText(),
        ),
  ...
  return {
    text: normalizeDesktopLyricText(
      currentDesktopSongMeta().title || "Mineradio",
    ),
    progress: 0,
    progressSpan: 4.8,
  };
...
function desktopOverlayColors() {
  ...
  return {
    primary: desktopOverlayColorValue(
      pal.primary || fx.lyricColor || "#d6f8ff",
      "#d6f8ff",
    ),
    secondary: desktopOverlayColorValue(
      pal.secondary || fx.visualTintColor || "#9cffdf",
      "#9cffdf",
    ),
    highlight: desktopOverlayColorValue(
      pal.highlight || fx.lyricHighlightColor || "#fff0b8",
      "#fff0b8",
    ),
    glow: desktopOverlayColorValue(
      pal.glowColor ||
        pal.secondary ||
        pal.primary ||
        fx.lyricGlowColor ||
...
function desktopLyricsMotionPayload() {
  return {
    ...
    lyricGlowStrength: fx.lyricGlow
      ? clampRange(Number(fx.lyricGlowStrength) || 0, 0, 0.85)
      : 0,
    highBloom:
      stageLyrics && isFinite(stageLyrics.highBloom)
        ? clampRange(stageLyrics.highBloom, 0, 1.45)
        : 0,
    beatGlow:
      stageLyrics && isFinite(stageLyrics.beatGlow)
        ? clampRange(stageLyrics.beatGlow, 0, 1.7)
        : 0,
...
function desktopLyricsPlaybackPayload() {
  ...
  var rate =
    audio && isFinite(audio.playbackRate) && audio.playbackRate > 0
      ? Number(audio.playbackRate)
      : 1;
  return {
...
function desktopLyricsBeatMapPayload(force) {
  ...
  var cameraCount = map
    ? (map.cameraBeats && map.cameraBeats.length) ||
      (map.beats && map.beats.length) ||
      (map.kicks && map.kicks.length) ||
      0
    : 0;
  var pulseCount = map
    ? (map.pulseBeats && map.pulseBeats.length) ||
      (map.kicks && map.kicks.length) ||
      0
    : 0;
  var duration = map && isFinite(map.duration) ? Number(map.duration) : 0;
  var partialUntil =
    map && isFinite(map.partialUntilSec) ? Number(map.partialUntilSec) : 0;
  var key = map
    ? [
        source,
        map.analyzedAt || 0,
        ...
        map.tempoSource || "local",
      ].join("|")
    : "none";
  var shouldSendMap =
    !!force || key !== desktopOverlayPushState.lastLyricsBeatKey;
...
function desktopLyricsPayload(forceBeatMap, includeCustomFontData) {
  ...
  var payload = {
    ...
    clickThrough: isDevelopmentLockedFx("desktopLyricsClickThrough")
      ? true
    ...
    feather: lyricsHasNativeKaraoke ? 0.03 : 0.055,
  ...
  if (Object.prototype.hasOwnProperty.call(beatPayload, "beatMap"))
    payload.beatMap = beatPayload.beatMap;
  return payload;
}
...
function updateDesktopWallpaperRuntimeControls(status) {
  ...
  if (toggle) {
    ...
    toggle.title = !supported
      ...
      : attaching
        ? "正在切换完整桌面模式"
        : "把完整 Mineradio 放到 Windows 桌面；右上角控制器可显示或隐藏桌面图标，Esc 退出";
  }
...
function applyDesktopWallpaperSafeArea(status, enabled) {
  ...
  var insets =
    status && status.safeInsets && typeof status.safeInsets === "object"
      ? status.safeInsets
      : null;
  ...
  for (var i = 0; i < names.length; i++) {
    var name = names[i];
    var value =
      enabled === true && insets ? Math.max(0, Number(insets[name]) || 0) : 0;
...
function syncDesktopWallpaperBodyClasses(status, enabled, interactive) {
  ...
  var explorerLayeredColorkey =
    enabled && desktopUsesLayeredExplorerColorkey(status);
  var iconsHidden = enabled && !desktopIconsAreVisible(status);
  document.documentElement.classList.toggle(
    "desktop-explorer-layered-colorkey-root",
    explorerLayeredColorkey,
  );
  ...
  document.body.classList.toggle(
    "desktop-explorer-layered-colorkey",
    explorerLayeredColorkey,
  );
  document.body.classList.toggle(
    "desktop-software-locked",
    interactive && status.softwareInteractionLocked === true,
  );
...
function releaseDesktopWallpaperStartupVisibilityGate() {
  var root = document.documentElement;
  if (!root || !root.classList.contains("startup-fast-skip-preload"))
    return false;
...
function ensureDesktopWallpaperFunctionalUi(reason) {
  ...
  if (
    typeof immersiveMode !== "undefined" &&
    immersiveMode &&
  ...
  if (homeActive) {
    if (typeof updateControlsChromeState === "function")
  ...
  try {
    shelfSuppressesConsole =
      typeof isBottomControlsSuppressedForShelf === "function" &&
  ...
  if (typeof controlsRevealHoldUntil !== "undefined") {
    controlsRevealHoldUntil = Math.max(
      controlsRevealHoldUntil || 0,
      performance.now() + 900,
    );
  }
...
function revealDesktopWallpaperUiOnActivation(enabled, interactive) {
  ...
  if (!nextActive) {
    ...
    if (document.body)
      document.body.classList.remove("desktop-wallpaper-hud-prime");
    if (typeof controlsRevealHoldUntil !== "undefined")
      controlsRevealHoldUntil = 0;
    setDesktopModeControlsOpen(false);
    setDesktopModeControlPeek(false);
  ...
  var activeSurface = document.getElementById(
    homeActive ? "empty-home" : "bottom-bar",
  );
  if (activeSurface) {
    void activeSurface.offsetWidth;
    try {
      window.getComputedStyle(activeSurface).opacity;
    } catch (_) {}
  }
  if (desktopWallpaperHudPrimeTimer)
    clearTimeout(desktopWallpaperHudPrimeTimer);
...
function applyDesktopWallpaperRuntimeStatus(payload) {
  ...
  if (isFinite(generation) && generation < desktopWallpaperStatusGeneration)
    return desktopWallpaperRuntimeState;
  if (isFinite(generation)) desktopWallpaperStatusGeneration = generation;
  desktopWallpaperRuntimeState = Object.assign(
    {},
    desktopWallpaperRuntimeState,
    status,
  );
  ...
  if (desktopWindowState && typeof desktopWindowState === "object") {
    ...
    desktopWindowState.isDesktopInteractive =
      nextEnabled && status.interactive === true;
  }
  ...
  revealDesktopWallpaperUiOnActivation(
    nextEnabled,
    status.interactive === true,
  );
  if (!nextEnabled) {
  ...
  scheduleDesktopIconShieldReport(
    !(nextEnabled && status.interactive === true),
  );
  scheduleDesktopPointerRouteReport(null, true);
  return desktopWallpaperRuntimeState;
...
function desktopWallpaperErrorLabel(error) {
  ...
  if (code.indexOf("WALLPAPER_PLATFORM_UNSUPPORTED") >= 0)
    return "当前系统不支持";
  if (code.indexOf("WALLPAPER_WORKERW_NOT_FOUND") >= 0)
    return "未找到桌面 WorkerW";
  if (code.indexOf("WALLPAPER_PROGMAN_NOT_FOUND") >= 0)
    return "未找到 Windows 桌面宿主";
  if (
    code.indexOf("WALLPAPER_NATIVE_ATTACH_ABORTED") >= 0 ||
    code.indexOf("WALLPAPER_START_SUPERSEDED") >= 0
  )
    return "启动已取消";
  if (code.indexOf("FULL_DESKTOP_RECOVERY_TRAY_UNAVAILABLE") >= 0)
    return "无法创建桌面模式恢复入口";
  if (
    code.indexOf("FULL_DESKTOP_WALLPAPER_ENGINE_SUSPEND_FAILED") >= 0 ||
    ...
    code.indexOf("WALLPAPER_ENGINE_SESSION_MISMATCH") >= 0
  )
    return "Wallpaper Engine 项目未能安全切换到桌面预览";
  if (
    code.indexOf("DESKTOP_MODE_DETACH") >= 0 ||
    code.indexOf("FULL_DESKTOP_DETACH") >= 0
  )
    return "主窗口恢复失败";
  return "无法进入完整桌面模式";
}
function initDesktopWallpaperRuntimeBridge(api) {
  if (!api) return;
  if (
    typeof api.onWallpaperModeState === "function" &&
    !desktopWallpaperStatusUnsubscribe
  ) {
    try {
      desktopWallpaperStatusUnsubscribe = api.onWallpaperModeState(
        applyDesktopWallpaperRuntimeStatus,
      );
    } catch (_) {
  ...
  if (typeof api.getWallpaperModeStatus === "function") {
    Promise.resolve()
      ...
      .then(applyDesktopWallpaperRuntimeStatus)
      .catch(function () {});
  }
}
function pushDesktopLyricsState(force) {
  ...
  if (
    !force &&
    now - desktopOverlayPushState.lyricsAt < desktopLyricsPushInterval()
  )
    return;
  var currentCustomFontId = desktopLyricsCurrentCustomFontId();
  var includeCustomFontData =
    !!currentCustomFontId &&
    currentCustomFontId !== desktopOverlayPushState.lastLyricsCustomFontId;
  ...
  var key =
    ...
    payload.text +
    "|" +
    Math.round(payload.progress * 1000) +
    "|" +
    Math.round((payload.progressSpan || 0) * 100) +
    "|" +
    payload.playing +
    "|" +
    payload.size +
    ...
    "|" +
    Math.round((motion.lyricGlowStrength || 0) * 100) +
    "|" +
    Math.round((motion.highBloom || 0) * 100) +
    "|" +
    Math.round((motion.beatGlow || 0) * 100) +
    "|" +
    Math.round((motion.beatPulse || 0) * 100) +
    "|" +
    Math.round((motion.bass || 0) * 100);
  if (
    !force &&
    key === desktopOverlayPushState.lastLyricsKey &&
    now - desktopOverlayPushState.lyricsAt < 900
  )
    return;
  ...
  api.updateDesktopLyrics(payload).catch(function (e) {
    console.warn("desktop lyrics update failed:", e);
  });
}
function applyDesktopLyricsState(force) {
  ...
  if (typeof api.setDesktopLyricsEnabled === "function") {
    desktopOverlayPushState.lastLyricsCustomFontId = payloadCustomFontId;
    api.setDesktopLyricsEnabled(!!payload.enabled, payload).catch(function (e) {
      console.warn("desktop lyrics state failed:", e);
    });
  } else {
...
function pushWallpaperState(force) {
  ...
  api
    .updateWallpaperMode(
      Object.assign(wallpaperPayload(), { reason: "renderer-status-refresh" }),
    )
    .then(function (result) {
      ...
      if (result && result.ok === false)
        console.warn(
          ...
          result.error || "WALLPAPER_UPDATE_FAILED",
        );
    })
    .catch(function (e) {
      console.warn("wallpaper update failed:", e);
    });
}
function applyWallpaperModeState(force) {
  ...
  if (!api) {
    ...
    return Promise.resolve({
      ok: false,
      enabled: false,
      error: "WALLPAPER_DESKTOP_API_UNAVAILABLE",
    });
  }
  normalizeDevelopmentLockedFxState();
  ...
  if (payload.enabled) {
    desktopWallpaperRuntimeState = Object.assign(
      {},
      desktopWallpaperRuntimeState,
      { attaching: true, enabled: true, lastError: "" },
    );
    updateDesktopWallpaperRuntimeControls(desktopWallpaperRuntimeState);
  }
  return Promise.resolve()
    ...
    .then(function (result) {
      result =
        result && typeof result === "object"
          ? result
          : { ok: false, enabled: false, error: "WALLPAPER_RESULT_INVALID" };
      if (operation !== desktopWallpaperRendererOperation)
        return Object.assign({}, result, { rendererStale: true });
      ...
      if (result.ok !== true) {
        ...
        fx.wallpaperMode =
          result.enabled === true ||
          failedStatus.enabled === true ||
          failedStatus.active === true;
        updateFxInputs();
      }
      return result;
    })
    .catch(function (error) {
      ...
      return {
        ok: false,
        enabled: false,
        error: desktopWallpaperRuntimeState.lastError,
      };
    });
}
...
setInterval(function () {
  ...
  if (fx && fx.wallpaperMode)
    ensureDesktopWallpaperFunctionalUi("health-watch");
}, 320);
...
(function initDesktopWindowShell() {
  ...
  function applyState(state) {
    var wasHidden =
      ...
      desktopWindowState.isVisible === false;
    ...
    if (isFullScreen !== wasFullScreen) {
      ...
      if (!isFullScreen) {
        ...
        setTimeout(function () {
          clearPlayerControlFocusState("desktop-fullscreen-exit");
        }, 80);
      }
    }
    syncCursorAutoHideMode();
