...
var PLAYLIST_PANEL_OPEN_DURATION_RANGE = {
  min: 0.08,
  max: 0.72,
  fallback: 0.28,
};
var PLAYLIST_PANEL_CLOSE_DURATION_RANGE = {
  min: 0.06,
  max: 0.48,
  fallback: 0.18,
};
function playlistPanelMotionRange(type) {
  return type === "close"
    ? PLAYLIST_PANEL_CLOSE_DURATION_RANGE
    : PLAYLIST_PANEL_OPEN_DURATION_RANGE;
}
function playlistPanelMotionMs(type) {
  ...
  var value =
    typeof fx !== "undefined" && fx && isFinite(fx[key])
      ? Number(fx[key])
      : range.fallback;
...
function markPlaylistPanelMotion(panel, duration) {
  if (!panel) return;
  panel.__playlistMotionUntil =
    performance.now() +
    (duration == null ? PLAYLIST_PANEL_MOTION_MS : duration);
}
function isPlaylistPanelInMotion(panel) {
  return !!(
    panel &&
    panel.__playlistMotionUntil &&
    performance.now() < panel.__playlistMotionUntil
  );
}
function isPlaylistPanelOpeningMotion(panel) {
  return !!(
    isPlaylistPanelInMotion(panel) &&
    !panel.classList.contains("playlist-panel-closing")
  );
}
function isSearchGlassReadyForReveal() {
  var root = document.documentElement;
  return (
    !root.classList.contains("control-glass-svg-ok") ||
...
function shouldAnimatePlaylistPanelOpen(panel) {
  ...
  if (
    panel.__lastOpenListAnimAt &&
    now - panel.__lastOpenListAnimAt < PLAYLIST_PANEL_OPEN_ANIM_COOLDOWN
  )
    return false;
...
function setPeek(el, on, key) {
  ...
  if (on) {
    ...
    if (
      key === "search" &&
      typeof prepareSearchGlassBeforePeek === "function" &&
      !wasPeek
    ) {
    ...
    var runPlaylistOpenAnimation =
      key === "pl" && !wasPeek ? shouldAnimatePlaylistPanelOpen(el) : false;
    ...
    peekTimers[key] = setTimeout(function () {
      ...
      if (key === "pl")
        setTimeout(
          function () {
            el.classList.remove("playlist-panel-closing");
          },
          playlistPanelMotionMs("close") + 80,
        );
      if (key === "fx") {
        var fabOff = document.getElementById("fx-fab");
        if (fabOff && !el.classList.contains("show"))
          fabOff.classList.remove("active");
      }
      peekTimers[key] = null;
...
function uploadTipWasSeen() {
  try {
    return localStorage.getItem(UPLOAD_TIP_STORE_KEY) === "1";
  } catch (e) {
    return true;
  }
}
function markUploadTipSeen() {
  try {
    localStorage.setItem(UPLOAD_TIP_STORE_KEY, "1");
  } catch (e) {}
}
    ...
    if (uploadBtn) {
      window.gsap.fromTo(
        ...
        { scale: 1, boxShadow: "0 10px 32px rgba(0,0,0,.22)" },
        {
          scale: 1.07,
          boxShadow:
            "0 0 0 8px rgba(244,210,138,0),0 16px 46px rgba(244,210,138,.14)",
          duration: 0.58,
          ease: "sine.inOut",
          yoyo: true,
          repeat: 3,
...
var secondaryPlaylistEdgeGuard = {
  enteredAt: 0,
  timer: null,
  x: 0,
  y: 0,
...
function isPlaylistFullscreenEdgeMode() {
  return !!(
    (typeof desktopRuntimeState !== "undefined" &&
      desktopRuntimeState &&
      desktopRuntimeState.fullscreen) ||
    (typeof desktopFullscreenActive !== "undefined" &&
...
function isSecondaryLeftDisplaySeamGuardActive() {
  var state =
    typeof desktopWindowState !== "undefined" && desktopWindowState
      ? desktopWindowState
      : {};
  return !!(
    !isPlaylistFullscreenEdgeMode() &&
    window.desktopWindow &&
    window.desktopWindow.isDesktop &&
    state.isPrimaryDisplay === false &&
    state.hasDisplayOnLeft
  );
}
...
function playlistPanelEdgeTopGutter(H) {
  return Math.max(
    ...
    Math.min(
      132,
      Math.round(H * (isPlaylistFullscreenEdgeMode() ? 0.1 : 0.14)),
    ),
  );
}
function playlistPanelEdgeBottomGutter(H) {
  return Math.max(
    ...
    Math.min(
      150,
      Math.round(H * (isPlaylistFullscreenEdgeMode() ? 0.13 : 0.16)),
    ),
  );
}
function isPlaylistPanelBottomControlsConflict(ex, ey, H) {
  ...
  var overHandle = !!(
    handleRect &&
    ex >= handleRect.left - 28 &&
    ex <= handleRect.right + 28 &&
    ey >= handleRect.top - 18 &&
    ey <= handleRect.bottom + 22
  );
  var barActive = !!(
    (bar &&
      bar.classList.contains("visible") &&
      !bar.classList.contains("soft-hidden")) ||
    document.body.classList.contains("controls-visible")
  );
  var overBarLeft = !!(
    barActive &&
    barRect &&
    ex >= barRect.left - 22 &&
    ex <= Math.min(barRect.left + 300, barRect.right + 22) &&
    ey >= barRect.top - 28 &&
    ey <= barRect.bottom + 22
  );
  return nearBottomLeft || overHandle || overBarLeft;
}
function isSecondaryPlaylistSafeBandPoint(ex, ey, H) {
  return (
    ey > playlistPanelEdgeTopGutter(H) &&
    ey < H - playlistPanelEdgeBottomGutter(H) &&
...
function armSecondaryPlaylistEdgeDwell() {
  ...
  secondaryPlaylistEdgeGuard.timer = setTimeout(function () {
    ...
    if (
      !isSecondaryPlaylistSafeBandPoint(
        secondaryPlaylistEdgeGuard.x,
        secondaryPlaylistEdgeGuard.y,
        secondaryPlaylistEdgeGuard.H,
      )
    )
      return;
...
function playlistPanelInitialEdgeTriggerX(defaultWidth, eventTarget) {
  if (
    ...
    !eventTarget ||
    !eventTarget.closest ||
    !eventTarget.closest("#empty-home")
  )
    return defaultWidth;
  var panel = document.getElementById("playlist-panel");
...
function isPlaylistEdgeTrigger(ex, ey, H, eventTarget) {
  ...
  if (isPlaylistFullscreenEdgeMode()) {
    ...
    return (
      ex >= 0 &&
      ex <
        playlistPanelInitialEdgeTriggerX(
          PLAYLIST_PANEL_FULLSCREEN_EDGE_TRIGGER_X,
          eventTarget,
        )
    );
  }
  ...
  var now = performance.now();
  if (!secondaryPlaylistEdgeGuard.enteredAt)
    secondaryPlaylistEdgeGuard.enteredAt = now;
  armSecondaryPlaylistEdgeDwell();
  return (
    now - secondaryPlaylistEdgeGuard.enteredAt >=
    SECONDARY_PLAYLIST_EDGE_DWELL_MS
  );
}
...
function isPlaylistPanelActiveState(panel) {
  return !!(
    ...
    (panel.classList.contains("peek") ||
      ...
      isPlaylistPanelOpeningMotion(panel))
  );
}
function isPlaylistFullscreenEdgeFocusHold(panel, ex, ey, H) {
  if (!isPlaylistFullscreenEdgeMode() || !isPlaylistPanelActiveState(panel))
    return false;
  if (isPlaylistPanelBottomControlsConflict(ex, ey, H)) return false;
  return (
    ex >= PLAYLIST_PANEL_FULLSCREEN_EDGE_LEAVE_TOLERANCE_X &&
    ex <= PLAYLIST_PANEL_FULLSCREEN_FOCUS_HOLD_X &&
    ey > playlistPanelEdgeTopGutter(H) &&
    ey < H - playlistPanelEdgeBottomGutter(H)
  );
}
function isFullscreenPlaylistQueueFocusLockedAtEdge(e) {
  ...
  var ey =
    e && isFinite(e.clientY)
      ? e.clientY
      : shelfHoverCue && isFinite(shelfHoverCue.y)
        ? shelfHoverCue.y
        : innerHeight * 0.5;
...
function isPlaylistPanelPanelHit(panel, ppRect, ex, ey) {
  ...
  return (
    ex >= targetRect.left - 26 &&
    ex <= targetRect.right + 28 &&
    ey >= targetRect.top - 30 &&
    ey <= targetRect.bottom + 30
  );
}
function isPlaylistPanelBridgeHit(panel, ppRect, ex, ey, H) {
  if (!isPlaylistPanelActiveState(panel)) return false;
  if (
    isSecondaryLeftDisplaySeamGuardActive() &&
    ex < SECONDARY_PLAYLIST_SEAM_CLOSE_X
  )
    return false;
  var targetRect = playlistPanelTargetRect(panel, ppRect);
...
function shouldClosePlaylistPanelFromPointer(ppOn, ex, ppRect, ey, H) {
  ...
  if (
    isPlaylistPanelOpeningMotion(panel) &&
    ex < targetRect.right + playlistPanelFocusPadding()
  )
    return false;
  if (
    isSecondaryLeftDisplaySeamGuardActive() &&
...
function isPlaylistPanelFocusActive(inTrigger, inPanel, pp, ex, ppRect, ey, H) {
  if (
    ...
    return false;
  return (
    inTrigger || inPanel || isPlaylistFullscreenEdgeFocusHold(pp, ex, ey, H)
  );
}
function shouldHidePlaylistPanelOnWindowLeave(e) {
  ...
  if (
    isPlaylistFullscreenEdgeMode() &&
    e.clientX <= 2 &&
    e.clientY > playlistPanelEdgeTopGutter(window.innerHeight) &&
    e.clientY <
      window.innerHeight - playlistPanelEdgeBottomGutter(window.innerHeight)
  )
    return false;
  return (
    e.clientX <= 12 ||
...
document.addEventListener(
  "mouseleave",
  function (e) {
    if (shouldHidePlaylistPanelOnWindowLeave(e))
      closePlaylistPanelSoft("window-leave");
  },
  true,
);
window.addEventListener("blur", function () {
  ...
  setTimeout(function () {
    closePlaylistPanelSoft("window-blur");
  }, 60);
});
window.addEventListener("mousemove", function (e) {
  ...
  if (immersiveMode) {
    ...
    if (inQueueTriggerImm || inQueuePanelImm || inQueueBridgeImm)
      setPeek(pp, true, "pl");
    else if (shouldClosePlaylistPanelFromPointer(ppOnImm, ex, ppRectImm, ey, H))
      setPeek(pp, false, "pl");
    var shelfCanFocusImm = !!(
      shelfManager &&
      shelfManager.canInteract &&
      shelfManager.canInteract()
    );
    var newFocusImm = null;
    var queueFocusImm = isPlaylistPanelFocusActive(
      inQueueTriggerImm,
      inQueuePanelImm || inQueueBridgeImm,
      pp,
      ex,
      ppRectImm,
      ey,
  ...
  var uploadImportOpen =
    typeof isUploadImportActive === "function" && isUploadImportActive();
  var inSearchPanel =
    saOn &&
    ex >= saRect.left - 24 &&
    ex <= saRect.right + 24 &&
  ...
  else if ((saOn || isSearchPeekRevealPending()) && !emptyHomeActive)
  ...
  var inFxFab =
    ...
    ex <= fabRect.right + 18 &&
    ey >= fabRect.top - 18 &&
    ey <= fabRect.bottom + 18;
  var inFxBridge =
    fpOn &&
    ex >= Math.min(fpRect.left, fabRect.left) - 18 &&
    ex <= W &&
    ey >= fpRect.bottom - 10 &&
    ey <= fabRect.bottom + 18;
  ...
  else if (shouldClosePlaylistPanelFromPointer(ppOn, ex, ppRect, ey, H))
    setPeek(pp, false, "pl");
  ...
  var newFocus = null;
  var queueFocusActive = isPlaylistPanelFocusActive(
    inQueueTrigger,
    inQueuePanel || inQueueBridge,
    pp,
    ex,
    ppRect,
    ey,
