...
var particleSpin = { vx: 0, vy: 0, damping: 0.9 };
  ...
  try {
    ...
    gestureCamera = new Camera(gestureVideo, {
      onFrame: async function () {
        if (gestureHands) await gestureHands.send({ image: gestureVideo });
      },
      width: 480,
      height: 360,
    });
    ...
    document.querySelectorAll("#cam-seg button").forEach(function (b) {
      b.classList.toggle("active", b.dataset.cam === "off");
    });
  }
}
...
function stopGestureControl() {
  if (!gestureActive) return;
  try {
    if (gestureCamera && gestureCamera.stop) gestureCamera.stop();
  } catch (e) {}
  try {
    if (gestureVideo && gestureVideo.srcObject)
      gestureVideo.srcObject.getTracks().forEach(function (t) {
        t.stop();
      });
  } catch (e) {}
  try {
...
function smoothLandmarks(lm) {
  if (!handLmSmooth) {
    handLmSmooth = lm.map(function (p) {
      return { x: 1 - p.x, y: p.y, z: p.z || 0 };
    });
    return handLmSmooth;
  }
...
function handOpenness(lm, palm) {
  ...
  for (var i = 0; i < tips.length; i++)
    avg += Math.hypot(lm[tips[i]].x - palm.x, lm[tips[i]].y - palm.y);
  avg /= tips.length;
...
function processHandFrame(rawLm) {
  ...
  if (isPinch && !pinchState.active) {
    ...
    var pinchDt = Math.max(
      1 / 120,
      Math.min(0.08, (nowPinch - pinchState.lastT) / 1000 || 1 / 60),
    );
    // v8: 方向修正 - 上下手与封面旋转同向
...
function drawHandSkeleton(lm, isPinch, openness, isFist) {
  ...
  aura.addColorStop(
    0,
    isFist ? "rgba(244,210,138,0.26)" : "rgba(255,255,255,0.22)",
  );
  ...
  for (var r = 0; r < 3; r++) {
    ...
    ctx.arc(
      px,
      py,
      ringR + r * 13 + Math.sin(uniforms.uTime.value * 1.5 + r) * 2,
      0,
      Math.PI * 2,
    );
    ctx.stroke();
  ...
  for (var i = 0; i < tips.length; i++) {
    ...
    var beamAlpha =
      clampRange(0.26 - dist / 720, 0.045, 0.18) * (0.55 + openness * 0.45);
    var grad = ctx.createLinearGradient(px, py, tx, ty);
    grad.addColorStop(
      0,
      "rgba(255,255,255," + (beamAlpha * 0.2).toFixed(3) + ")",
    );
    grad.addColorStop(
      0.65,
      "rgba(255,255,255," + (beamAlpha * 0.42).toFixed(3) + ")",
    );
    grad.addColorStop(
      1,
      primary.replace(
        /0\.\d+\)/,
        Math.min(0.72, beamAlpha + 0.14).toFixed(3) + ")",
      ),
    );
    ...
    ctx.stroke();
    var dotR =
      (tips[i] === 8 || tips[i] === 4 ? 4.2 : 3.0) + (isFist ? 0.8 : 0);
...
function tickGestureRotation(dt) {
  if (
    Math.abs(particleSpin.vx) > 0.0001 ||
    Math.abs(particleSpin.vy) > 0.0001
  ) {
  ...
  gestureGrip.value +=
    (gestureGrip.target - gestureGrip.value) *
    (gestureGrip.target > gestureGrip.value ? 0.18 : 0.1);
  gestureGrip.pulse *= Math.pow(0.84, dt * 60);
  if (uniforms.uGestureGrip)
    uniforms.uGestureGrip.value = clampRange(
      gestureGrip.value + gestureGrip.pulse * 0.16,
      0,
      1,
    );
  ...
  if (
    gestureActive &&
    handLmSmooth &&
    performance.now() - handLmLastSeen > 200
  ) {
...
function showGestureHUD(label, progress, detail) {
  ...
  document.getElementById("gesture-confirm").textContent =
    detail || "将手放进摄像头视野";
  var fill = document.getElementById("gesture-fill");
  if (fill)
    fill.style.width = Math.max(0, Math.min(100, (progress || 0) * 100)) + "%";
