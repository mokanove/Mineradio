...
function primeCinemaAfterTrackStart(reason) {
  ...
  beatCam.lastTriggerAt = Math.min(
    beatCam.lastTriggerAt || -10,
    currentT - 0.48,
  );
...
function alignBeatCameraCursorToTime(t) {
  ...
  var beats =
    ...
    currentBeatMap.beats ||
    currentBeatMap.kicks ||
    [];
  beatCam.nextIdx = 0;
  while (beatCam.nextIdx < beats.length) {
    var bt =
      typeof beats[beatCam.nextIdx] === "number"
        ? beats[beatCam.nextIdx]
        : beats[beatCam.nextIdx].time;
...
function updateCinemaDynamics(rawEnergy, rawLow) {
  ...
  if (isDj) {
    ...
    djMode.sectionEnergy +=
      (e - djMode.sectionEnergy) * (e > djMode.sectionEnergy ? 0.03 : 0.01);
    djMode.sectionLow +=
      (l - djMode.sectionLow) * (l > djMode.sectionLow ? 0.036 : 0.012);
    var change = Math.abs(e - prevEnergy) * 0.46 + Math.abs(l - prevLow) * 0.62;
    djMode.sectionChange +=
      (change - djMode.sectionChange) *
      (change > djMode.sectionChange ? 0.055 : 0.018);
    djMode.visualPulse *= Math.pow(0.3, 1 / 60);
  }
  cinemaDynamics.avg +=
    (composite - cinemaDynamics.avg) *
    (composite > cinemaDynamics.avg
      ? isDj
        ? 0.018
        : 0.01
      : isDj
        ? 0.006
        : 0.004);
  cinemaDynamics.lowAvg +=
    (l - cinemaDynamics.lowAvg) *
    (l > cinemaDynamics.lowAvg ? (isDj ? 0.022 : 0.012) : isDj ? 0.007 : 0.005);
  cinemaDynamics.peak = Math.max(
    isDj ? 0.36 : 0.3,
    cinemaDynamics.peak * (isDj ? 0.998 : 0.9988),
    composite,
  );
  var floor = Math.max(0.1, cinemaDynamics.avg * 0.82);
  ...
  var target = isDj
    ? 0.5 +
      lift * 0.66 +
      clamp01((l - cinemaDynamics.lowAvg) / 0.3) * 0.18 +
      clamp01(djMode.sectionChange * 2.4) * 0.08
  ...
  cinemaDynamics.scale +=
    (target - cinemaDynamics.scale) *
    (target > cinemaDynamics.scale
      ? isDj
        ? 0.07
        : 0.045
...
function cameraDynamicsScale(extra) {
  ...
  var djBoost = isDj
    ? 1.06 +
      clamp01(djMode.sectionLow) * 0.16 +
      clamp01(rtBeat.tempoConfidence) * 0.08
    : 1;
  return clampRange(
    (cinemaDynamics.scale || 0.82) *
      (cinemaTrackProfile.scale || 1) *
      (extra == null ? 1 : extra) *
      djBoost,
    isDj ? 0.24 : 0.18,
    isDj ? 1.42 : 1.18,
...
function resetCinemaTrackProfile(song) {
  ...
  cinemaTrackProfile.target = isDj ? 1.1 : 1.0;
  ...
  cinemaTrackProfile.punchPeak = 0.1;
...
function updateCinemaTrackProfile(sample) {
  ...
  var punchRaw = clamp01(
    (sample.lowOnset || 0) * 2.4 +
      (sample.energyOnset || 0) * 1.5 +
      sample.low * 0.16,
  );
  p.punchPeak = Math.max(0.1, p.punchPeak * 0.9975, punchRaw);
  var lowDrive = clamp01((p.lowAvg - 0.2) / 0.42);
  var loudDrive = clamp01((p.energyAvg - 0.18) / 0.4);
  var punchDrive = clamp01((p.punchPeak - 0.13) / 0.36);
  var vocalSoft = clamp01(
    (p.vocalAvg * 0.72 + p.melodyAvg * 0.42 - p.lowAvg * 0.34 - 0.08) / 0.42,
  );
  ...
  target = clampRange(
    target,
    djMode.active ? 0.68 : 0.28,
    djMode.active ? 1.26 : 1.12,
  );
  p.target = target;
  p.scale +=
    (target - p.scale) *
...
function readSonicRealtimeCameraSample() {
  if (typeof sonicAudioMonitorState === "undefined" || !sonicAudioMonitorState)
    return null;
  ...
  var kick = clamp01(
    Math.max(
      Number(frame.kickEnvelope) || 0,
      Number(frame.triggerPulse) || 0,
      Number(frame.beat) || 0,
    ),
  );
  var kickDominance = clamp01(Number(frame.kickLowDominance) || 0);
  var onset = clamp01(
    (Number(frame.kickFlux) || 0) * 1.55 +
  ...
  return {
    ...
    lowAttack: clampRange(
      onset * 0.055 + kick * 0.018 + kickDominance * 0.018,
      0,
      0.14,
    ),
    score: clamp01(
      (Number(frame.kickConfidence) || 0) * 0.52 +
        onset * 0.3 +
        kick * 0.16 +
        kickDominance * 0.12,
    ),
    strength: clamp01(0.34 + kick * 0.26 + low * 0.22 + onset * 0.24),
    lowDominance: Math.max(
      0.7,
      low / Math.max(0.001, competing),
      0.78 + kickDominance * 0.7,
    ),
    lowFluxDominance: Math.max(
      0.96,
      0.82 + onset * 1.28 + kickDominance * 0.24,
    ),
  };
}
...
function applyCinemaProfileFromBeatMap(map) {
  ...
  var events = (map.cameraBeats || map.beats || []).filter(function (b) {
    return b && typeof b !== "number" && b.camera !== false;
  });
  ...
  cinemaTrackProfile.scale +=
    (target - cinemaTrackProfile.scale) *
    (target < cinemaTrackProfile.scale ? 0.55 : 0.22);
}
...
function scheduleVisualApply(fn, delay, timeout) {
  ...
  setTimeout(function () {
    ...
    if (window.requestIdleCallback)
      requestIdleCallback(run, { timeout: timeout || 360 });
    else run();
  }, delay || 0);
}
...
function beatBandRms(data, sampleRate, fftSize, hz0, hz1) {
  ...
  var cacheKey =
    data.length + "|" + sampleRate + "|" + fftSize + "|" + hz0 + "|" + hz1;
  ...
  var stride =
    span > 72 && typeof runtimeAnalysisStride === "function"
...
function processRealtimeBeatEngine(dt) {
  ...
  var timeStride =
    typeof runtimeAnalysisStride === "function"
      ? runtimeAnalysisStride("time", beatTimeDomainData.length)
      : 1;
  var rmsCount = 0;
  ...
  rtBeat.subFast = follow(
    rtBeat.subFast,
    sub,
    0.018 * fastMul,
    0.064 * downMul,
  );
  rtBeat.subSlow = follow(rtBeat.subSlow, sub, 0.32 * slowMul, 0.52 * slowMul);
  rtBeat.lowFast = follow(rtBeat.lowFast, low, 0.016 * fastMul, 0.07 * downMul);
  rtBeat.lowSlow = follow(rtBeat.lowSlow, low, 0.3 * slowMul, 0.54 * slowMul);
  rtBeat.bodyFast = follow(
    rtBeat.bodyFast,
    body,
    0.02 * fastMul,
    0.082 * downMul,
  );
  rtBeat.bodySlow = follow(
    rtBeat.bodySlow,
    body,
    0.36 * slowMul,
    0.6 * slowMul,
  );
  rtBeat.vocalFast = follow(
    rtBeat.vocalFast,
    vocal,
    0.026 * fastMul,
    0.09 * downMul,
  );
  rtBeat.vocalSlow = follow(
    rtBeat.vocalSlow,
    vocal,
    0.34 * slowMul,
    0.58 * slowMul,
  );
  rtBeat.snapFast = follow(
    rtBeat.snapFast,
    snap,
    0.012 * fastMul,
    0.06 * downMul,
  );
  rtBeat.snapSlow = follow(
    rtBeat.snapSlow,
    snap,
    0.3 * slowMul,
    0.52 * slowMul,
  ...
  var peakDecay = dj ? 0.988 : 0.99;
  rtBeat.subPeak = Math.max(
    rtBeat.subPeak * Math.pow(peakDecay, dt * 60),
    sub,
    0.045,
  );
  rtBeat.lowPeak = Math.max(
    rtBeat.lowPeak * Math.pow(dj ? 0.987 : 0.989, dt * 60),
    low,
    0.06,
  );
  rtBeat.bodyPeak = Math.max(
    rtBeat.bodyPeak * Math.pow(peakDecay, dt * 60),
    body,
    0.04,
  );
  rtBeat.vocalPeak = Math.max(
    rtBeat.vocalPeak * Math.pow(peakDecay, dt * 60),
    vocal,
    0.04,
  );
  rtBeat.snapPeak = Math.max(
    rtBeat.snapPeak * Math.pow(peakDecay, dt * 60),
    snap,
    0.035,
  );
  ...
  var musicalOnset =
    ...
    rmsFlux * 0.2;
  var onset = dj
    ? drumOnset * 1.05 + musicalOnset * 0.07
    : drumOnset + musicalOnset * 0.16;

  var avgTau = onset > rtBeat.onsetAvg ? (dj ? 0.88 : 1.1) : dj ? 0.3 : 0.34;
  rtBeat.onsetAvg = follow(rtBeat.onsetAvg, onset, avgTau, avgTau);
  rtBeat.onsetPeak = Math.max(
    rtBeat.onsetPeak * Math.pow(dj ? 0.986 : 0.988, dt * 60),
    onset,
    0.032,
  );
  var floor = rtBeat.onsetAvg * (dj ? 0.88 : 0.84);
  var score = clamp01(
    (onset - floor) / Math.max(dj ? 0.013 : 0.014, rtBeat.onsetPeak - floor),
  );
  var subNorm = clamp01(
    sub / Math.max(0.045, rtBeat.subPeak * (dj ? 0.72 : 0.7)),
  );
  var lowNorm = clamp01(
    low / Math.max(0.06, rtBeat.lowPeak * (dj ? 0.74 : 0.72)),
  );
  var bodyNorm = clamp01(
    body / Math.max(0.045, rtBeat.bodyPeak * (dj ? 0.74 : 0.72)),
  );
  var vocalNorm = clamp01(vocal / Math.max(0.045, rtBeat.vocalPeak * 0.72));
  var snapNorm = clamp01(
    snap / Math.max(0.04, rtBeat.snapPeak * (dj ? 0.78 : 0.72)),
  );
  var nowT = audio.currentTime || 0;
  rtBeat.primedFrames++;
  var warmingUp =
    nowT < rtBeat.warmupUntil || rtBeat.primedFrames < (dj ? 5 : 10);
  ...
  var phaseWindow =
    expectedGap > 0
      ? Math.max(
          dj ? 0.055 : 0.055,
          Math.min(dj ? 0.105 : 0.105, expectedGap * (dj ? 0.16 : 0.16)),
        )
      : 0;
  var tempoDue =
    expectedGap > 0 &&
  ...
  var lowAttack = lowRise + lowFlux * 0.72 + subRise * 0.58 + subFlux * 0.4;
  var lowDominance =
    low / Math.max(0.001, vocal * 0.84 + body * 0.36 + snap * 0.1);
  var lowFluxDominance =
  ...
  if (sonicCamera) {
    ...
    lowAttack = Math.max(
      lowAttack,
      sonicCamera.lowAttack,
      lowAttack + sonicCamera.lowAttack * 0.35,
    );
    ...
    sonicKickAssist =
      sonicCamera.score > 0.44 &&
      sonicCamera.lowPresence > 0.42 &&
      sonicCamera.lowDominance > 0.96;
  }
  var voiceMask = dj
    ? vocalNorm > 0.64 &&
      lowDominance < 0.88 &&
      lowFluxDominance < 1.02 &&
      subNorm < 0.5 &&
      !sonicKickAssist
    : vocalNorm > 0.6 &&
      lowDominance < 0.8 &&
      lowFluxDominance < 1.04 &&
      !sonicKickAssist;
  var fastGroove = rtBeat.tempoGap > 0 && rtBeat.tempoGap < (dj ? 0.48 : 0.52);
  var drumGate =
    lowPresence > (dj ? 0.38 : 0.34) &&
    lowAttack >
      Math.max(dj ? 0.012 : 0.011, rtBeat.onsetAvg * (dj ? 0.3 : 0.26)) &&
    !voiceMask;
  drumGate =
    drumGate &&
    (lowDominance > (dj ? 0.78 : 0.64) ||
      lowFluxDominance > (dj ? 1.04 : 0.92) ||
      subNorm > (dj ? 0.54 : 0.48) ||
      sonicKickAssist);
  var strongTransient =
    drumGate &&
    score > (dj ? 0.48 : 0.46) &&
    (drumOnset > rtBeat.onsetAvg * (dj ? 0.72 : 0.66) || sonicKickAssist);
  var kickTransient =
    drumGate &&
    score > (dj ? 0.36 : 0.34) &&
    lowAttack >
      Math.max(dj ? 0.015 : 0.014, rtBeat.onsetAvg * (dj ? 0.4 : 0.34));
  var fastTransient =
    drumGate &&
    (fastGroove || sonicKickAssist) &&
    score > (dj ? 0.34 : 0.32) &&
    lowPresence > (dj ? 0.42 : 0.38);
  var tempoAssist =
    tempoDue &&
    rtBeat.tempoConfidence > (dj ? 0.34 : 0.34) &&
    drumGate &&
    lowPresence > (dj ? 0.42 : 0.36) &&
    score > (dj ? 0.24 : 0.18) &&
    lowAttack > Math.max(0.012, rtBeat.onsetAvg * (dj ? 0.34 : 0.26));
  var candidateHit =
    strongTransient || kickTransient || fastTransient || tempoAssist;
  if (warmingUp && !sonicKickAssist) candidateHit = false;
  var hasTempoLock =
    expectedGap >= (dj ? 0.32 : 0.42) &&
    expectedGap <= (dj ? 0.92 : 0.88) &&
    rtBeat.tempoConfidence > (dj ? 0.36 : 0.38);
  var lockedWindow = hasTempoLock
    ? Math.max(
        dj ? 0.062 : 0.07,
        Math.min(dj ? 0.118 : 0.11, expectedGap * (dj ? 0.17 : 0.16)),
      )
    : 0;
  ...
  if (candidateHit) {
    if (rtBeat.lastHitAt < 0) {
      rhythmAccept =
        (strongTransient || sonicKickAssist) &&
        score > (dj ? 0.5 : 0.5) &&
        lowPresence > (dj ? 0.44 : 0.4);
    } else if (hasTempoLock) {
      var oneBeatErr = Math.abs(gapRaw - expectedGap);
      var twoBeatErr = Math.abs(gapRaw - expectedGap * 2);
      rhythmAccept =
        oneBeatErr <= lockedWindow &&
        (kickTransient || strongTransient || fastTransient);
      rhythmAccept =
        rhythmAccept ||
        (twoBeatErr <= lockedWindow * 1.35 &&
          strongTransient &&
          score > (dj ? 0.54 : 0.58));
      rhythmAccept =
        rhythmAccept ||
        (gapRaw > expectedGap * 1.2 &&
          (strongTransient || fastTransient) &&
          lowPresence > (dj ? 0.44 : 0.38));
      if (dj) {
        rhythmAccept =
          rhythmAccept ||
          (gapRaw > expectedGap * 1.24 &&
            strongTransient &&
            score > 0.56 &&
            lowDominance > 0.92);
      }
    } else {
      var looseRealtimeMin = dj
        ? 0.285
        : Math.max(0.255, beatCam.realtimeMinInterval * 0.72);
      rhythmAccept =
        gapRaw >= looseRealtimeMin &&
        (strongTransient ||
          (kickTransient && score > (dj ? 0.42 : 0.4)) ||
          fastTransient) &&
        score > (dj ? 0.46 : 0.42) &&
        lowPresence > (dj ? 0.44 : 0.38);
    }
  ...
  if (
    !hit &&
    (candidateHit || score > 0.42 || vocalNorm > 0.62 || bodyNorm > 0.54)
  )
    rtBeat.stats.rejected++;
  var minGap = hasTempoLock
    ? Math.max(
        dj ? 0.255 : 0.255,
        Math.min(dj ? 0.43 : 0.46, expectedGap * (dj ? 0.54 : 0.56)),
      )
    : dj
      ? 0.285
      : Math.max(0.255, beatCam.realtimeMinInterval * 0.72);
  ...
  if (!hit) {
    ...
    return {
      ...
      vocal: vocalNorm,
      snap: snapNorm,
      tempoConfidence: rtBeat.tempoConfidence,
    };
  }
  ...
  if (rtBeat.lastHitAt > 0) {
    ...
    if (gap >= (dj ? 0.32 : 0.42) && gap <= (dj ? 0.96 : 0.88)) {
      gapShift = rtBeat.tempoGap
        ? Math.abs(gap - rtBeat.tempoGap) / Math.max(0.001, rtBeat.tempoGap)
        : 0;
      var tempoEase = hasTempoLock ? (dj ? 0.12 : 0.1) : dj ? 0.24 : 0.22;
      if (dj && gapShift > 0.16 && strongTransient && lowDominance > 0.95)
        tempoEase = Math.min(0.36, tempoEase + gapShift * 0.45);
      rtBeat.tempoGap = rtBeat.tempoGap
        ? rtBeat.tempoGap * (1 - tempoEase) + gap * tempoEase
        : gap;
      rtBeat.tempoConfidence = Math.min(
        1,
        rtBeat.tempoConfidence +
          (tempoAssist ? (dj ? 0.04 : 0.04) : dj ? 0.16 : 0.18),
      );
    }
  }
  ...
  var strength = dj
    ? clamp01(
        0.18 +
          score * 0.38 +
          lowPresence * 0.34 +
          Math.min(1.35, lowDominance) * 0.08 +
          rmsFlux * 0.72,
      )
    : clamp01(
        0.24 +
  ...
  if (tempoAssist)
    strength = Math.max(
      strength,
      (dj ? 0.46 : 0.48) +
        rtBeat.tempoConfidence * (dj ? 0.1 : 0.1) +
        lowPresence * (dj ? 0.14 : 0.14),
    );
  ...
  if (dj && strength > 0.78 && snapNorm > 0.56 && comboSlot !== 0)
    combo = "accent";
  if (dj && gapShift > 0.14 && strongTransient && lowPresence > 0.52)
    combo = "downbeat";
  rtBeat.pulse = Math.max(rtBeat.pulse, strength);
  if (dj) {
    djMode.tempoGap = rtBeat.tempoGap;
    djMode.tempoConfidence = rtBeat.tempoConfidence;
    djMode.sectionChange = Math.max(
      djMode.sectionChange,
      Math.min(1, gapShift * 1.4),
    );
  ...
  return {
    ...
    confidence: dj
      ? clamp01(
          score * 0.58 + lowPresence * 0.3 + rtBeat.tempoConfidence * 0.12,
        )
      : clamp01(
          score * 0.62 + lowPresence * 0.26 + rtBeat.tempoConfidence * 0.12,
        ),
    low: Math.max(0.05, lowPresence),
    body: Math.max(0.02, bodyNorm * (dj ? 0.5 : 0.62)),
    snap: Math.max(0.02, snapNorm * (dj ? 0.86 : 1)),
    mass: dj
      ? clamp01(lowPresence * 0.84 + bodyNorm * 0.1)
      : clamp01(lowPresence * 0.76 + bodyNorm * 0.2),
    sharpness: dj
      ? clamp01(snapNorm * 0.58 + bodyNorm * 0.1)
      : clamp01(snapNorm * 0.7 + bodyNorm * 0.12),
...
function scheduleBeatCamera(beat, source) {
  ...
  var strength =
    typeof beat === "number"
      ? 0.72
      : Math.max(0, Math.min(1, beat.strength || 0.72));
  var confidence =
    typeof beat === "number"
      ? 0.72
      : Math.max(0, Math.min(1, beat.confidence || 0.72));
  var isPrimary = typeof beat === "number" ? true : beat.primary !== false;
  var visualImpact =
    typeof beat === "number"
      ? strength
      : Math.max(0, Math.min(1, beat.impact == null ? strength : beat.impact));
  ...
  var dj =
    djMode.active && (isLiveSource || isDjMapSource || (beat && beat.dj));
  if (isMapSource && !isPrimary) return;
  if (isMapSource && visualImpact < 0.18 && strength < 0.56) return;
  if (isMapSource && confidence < 0.3 && strength < 0.68) return;
  ...
  if (
    trackScale < 0.5 &&
    isLiveSource &&
    strength < (dj ? 0.58 : 0.84) &&
    visualImpact < (dj ? 0.42 : 0.56)
  )
    return;
  var lowTone =
    typeof beat === "number"
      ? 0.62
      : Math.max(0, beat.low == null ? 0.62 : beat.low);
  var bodyTone =
    typeof beat === "number"
      ? 0.22
      : Math.max(0, beat.body == null ? 0.22 : beat.body);
  var snapTone =
    typeof beat === "number"
      ? 0.16
      : Math.max(0, beat.snap == null ? 0.16 : beat.snap);
  ...
  var sharpness =
    typeof beat === "number"
      ...
      : Math.max(
          0,
          Math.min(1, beat.sharpness == null ? snapTone : beat.sharpness),
        );
  var mass =
    typeof beat === "number"
      ? lowTone
      : Math.max(
          ...
          Math.min(
            1,
            beat.mass == null
              ? lowTone * 0.72 + bodyTone * 0.36 + strength * 0.2
              : beat.mass,
          ),
        );
  var nowT = audio ? audio.currentTime : uniforms.uTime.value;
  ...
  if (dj) {
    if (
      rawSnapTone > 0.58 &&
      rawSnapTone > rawLowTone * 0.86 &&
      rawSnapTone > rawBodyTone * 1.08
    )
      mode = "snap";
    else if (rawBodyTone > 0.36 && rawBodyTone > rawLowTone * 0.56)
      mode = "body";
  } else {
    if (
      snapTone > 0.42 &&
      snapTone > lowTone * 1.18 &&
      snapTone > bodyTone * 1.08
    )
      mode = "snap";
  ...
  if (dj) {
    ...
    var snapDrive = clamp01((rawSnapTone - 0.3) / 0.6);
    if (mode === "deep")
      amp = 0.16 + strength * 0.2 + lowDrive * 0.25 + confidence * 0.05;
    else if (mode === "body")
      amp = 0.12 + strength * 0.15 + bodyDrive * 0.18 + lowDrive * 0.06;
  ...
  var attack = dj
    ...
    : Math.max(
        0.014,
        Math.min(0.038, beatCam.attack * (1.18 - sharpness * 0.55)),
      );
  var hold = dj
    ? mode === "deep"
      ? 0.038 + lowTone * 0.014
    ...
    : Math.max(
        ...
        Math.min(
          0.052,
          beatCam.hold * (0.62 + lowTone * 0.55 + bodyTone * 0.25),
        ),
      );
  var release = dj
    ? mode === "deep"
    ...
    : Math.max(
        ...
        Math.min(
          0.255,
          beatCam.release *
            (0.76 + mass * 0.56 + bodyTone * 0.18 - sharpness * 0.18),
        ),
      );
  var idx =
    typeof beat === "number"
      ? Math.floor(time * 2.7)
      : beat.index || Math.floor(time * 2.7);
  ...
  var zoomAmp =
    0.07 + mass * 0.19 + (mode === "deep" ? 0.095 : 0.018) + strength * 0.045;
  var thetaAmp = 0.00035;
  var phiAmp =
    0.002 + (mode === "body" ? 0.012 : mode === "snap" ? 0.005 : 0.002);
  ...
  if (dj) {
    ...
    var snapDrive2 = clamp01((rawSnapTone - 0.3) / 0.6);
    if (mode === "deep") {
      zoomAmp = 0.115 + lowDrive2 * 0.17 + strength * 0.036;
      ...
      phiAmp = 0.0024 + bodyDrive2 * 0.004;
    ...
    if (isDjMapSource) {
      var offlineContrast = Math.pow(
        clamp01((visualImpact - 0.16) / 0.72),
        1.06,
      );
      var offlineDrive =
        0.72 + offlineContrast * 0.94 + Math.pow(strength, 1.22) * 0.14;
      var sectionLowGate = clamp01(((djMode.sectionLow || 0) - 0.03) / 0.32);
      var sectionEnergyGate = clamp01(
        ((djMode.sectionEnergy || 0) - 0.045) / 0.4,
      );
      var liveSectionGate = Math.max(
        sectionLowGate * 0.58 + sectionEnergyGate * 0.34,
        visualImpact * 0.82,
      );
      var weakSectionScale =
        0.54 + Math.pow(clamp01(liveSectionGate), 0.78) * 0.46;
      var comboDrive =
        combo === "downbeat"
          ? 0.96 + offlineContrast * 0.38
          : combo === "drop"
            ? 0.8 + offlineContrast * 0.26
            : combo === "accent"
              ? 0.74 + offlineContrast * 0.3
              : combo === "push"
                ? 0.68 + offlineContrast * 0.16
                : 0.52 + offlineContrast * 0.12;
      if (mode === "deep") {
        amp *= offlineDrive * comboDrive * 1.38;
        zoomAmp *= 1.14 + offlineContrast * 0.68 + lowDrive2 * 0.2;
        phiAmp *= 0.72 + offlineContrast * 0.22;
        thetaAmp *= 0.72 + offlineContrast * 0.2;
        release *= 0.98 + offlineContrast * 0.2;
      } else if (mode === "body") {
        amp *= offlineDrive * comboDrive * 1.24;
        zoomAmp *= 0.9 + offlineContrast * 0.32;
        phiAmp *= 1.0 + offlineContrast * 0.42 + bodyDrive2 * 0.18;
        ...
        thetaAmp *= 0.86 + offlineContrast * 0.3;
  ...
  if (dj && isDjMapSource && zoomAmp > 0.3)
    zoomAmp = 0.3 + (zoomAmp - 0.3) * 0.52;
  amp = Math.max(
    dj ? (isDjMapSource ? 0.018 : 0.04) : 0.08,
    Math.min(dj ? (isDjMapSource ? 0.92 : 0.34) : 0.68, amp),
  );
  if (isLiveSource) {
    var fastLiveGroove =
      rtBeat && rtBeat.tempoGap > 0 && rtBeat.tempoGap < (dj ? 0.48 : 0.52);
    var liveMinInterval = dj
      ? Math.max(
          0.255,
          Math.min(0.43, rtBeat.tempoGap ? rtBeat.tempoGap * 0.54 : 0.3),
        )
      : Math.max(
          0.255,
          beatCam.realtimeMinInterval * (fastLiveGroove ? 0.62 : 0.72),
        );
    if (
      time - beatCam.lastRealtimeAt < liveMinInterval &&
      strength < (dj ? 0.66 : 0.68)
    ) {
    ...
    for (var ei = beatCam.events.length - 1; ei >= 0; ei--) {
      ...
      if (
        pending.source === "map" &&
        pending.hit > time &&
        pending.hit - time < beatCam.realtimeMergeWindow
      ) {
  ...
  if (isDjMapSource) {
    ...
    var djMinGap = Math.max(
      0.255,
      Math.min(0.47, beat && beat.step ? beat.step * 0.52 : 0.32),
    );
  ...
  if (beatCam.events.length > maxEvents)
    beatCam.events.splice(0, beatCam.events.length - maxEvents);
}
...
function updateBeatCamera(dt) {
  ...
  if (
    beatCam.prevAudioTime >= 0 &&
    Math.abs(t - beatCam.prevAudioTime) > 0.55
  ) {
  ...
  if (leadEvent) {
    ...
    if (combo === "downbeat") {
      ...
      rollKick =
        sign * leadPunch * (leadEvent.rollAmp || 0) * (0.45 + snapFlick * 0.3);
    ...
    if (leadEvent.dj) {
      var djSide =
        sign *
        leadPunch *
        (leadEvent.thetaAmp || 0.0012) *
        (0.7 + (leadEvent.body || 0) * 0.65 + (leadEvent.snap || 0) * 0.35);
      thetaKick += djSide;
      if (leadEvent.mode === "snap" || combo === "accent") {
        rollKick +=
          sign *
          leadPunch *
          (leadEvent.rollAmp || 0.003) *
          (0.52 + snapFlick * 0.34);
      }
      if (combo === "downbeat") radiusKick *= 1.06;
      else if (combo === "drop") phiKick *= 1.18;
      punch = Math.min(0.9, punch * (1.04 + (leadEvent.mass || 0) * 0.1));
    }
  }
  var djEase = djMode.active;
  beatCam.punch +=
    (punch - beatCam.punch) *
    (punch > beatCam.punch ? (djEase ? 0.82 : 0.72) : djEase ? 0.44 : 0.38);
  beatCam.thetaKick +=
    (thetaKick - beatCam.thetaKick) *
    (Math.abs(thetaKick) > Math.abs(beatCam.thetaKick)
      ? djEase
        ? 0.8
        : 0.7
      : djEase
  ...
  beatCam.phiKick +=
    ...
    (Math.abs(phiKick) > Math.abs(beatCam.phiKick)
      ? djEase
        ? 0.8
        : 0.7
      : djEase
  ...
  beatCam.rollKick +=
    ...
    (Math.abs(rollKick) > Math.abs(beatCam.rollKick)
      ? djEase
        ? 0.82
        : 0.72
      : djEase
