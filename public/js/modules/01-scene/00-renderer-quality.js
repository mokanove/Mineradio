...
function sampleDisplayRefreshHz(now) {
  ...
  renderRefreshState.stableHz =
    Math.abs(stable - hz) > 18 ? hz : stable * 0.9 + hz * 0.1;
  return renderRefreshState.stableHz;
}
function estimatedDisplayRefreshHz() {
  return Math.max(
    48,
    Math.min(240, renderRefreshState.stableHz || renderRefreshState.hz || 60),
  );
}
...
function sampleAdaptiveFrameCost(costMs, targetFps) {
  ...
  if (adaptiveFrameLoadState.avgMs > budget) {
    adaptiveFrameLoadState.pressure = Math.min(
      8,
      adaptiveFrameLoadState.pressure + 0.7,
    );
  } else if (adaptiveFrameLoadState.avgMs < budget * 0.62) {
    adaptiveFrameLoadState.pressure = Math.max(
      0,
      adaptiveFrameLoadState.pressure - 0.3,
    );
  } else {
    adaptiveFrameLoadState.pressure = Math.max(
      0,
      adaptiveFrameLoadState.pressure - 0.1,
    );
  }
  adaptiveFrameLoadState.level =
    adaptiveFrameLoadState.pressure >= 4
      ? 2
      : adaptiveFrameLoadState.pressure >= 2
        ? 1
        : 0;
...
function selectAdaptiveRenderCadence(kind, tier) {
  ...
  var budgetLevel =
    typeof runtimePerfBudgetLevel === "function" ? runtimePerfBudgetLevel() : 2;
  var divisor = 1;
  kind = kind || "playback";
  ...
  if (kind !== "interaction") {
    if (budgetLevel <= 0 && pressure >= 2 && displayHz >= 118)
      divisor = Math.max(divisor, 2);
    else if (budgetLevel === 1 && pressure >= 2 && displayHz >= 144)
      divisor = Math.max(divisor, 2);
    if (pressure >= 3 && displayHz >= 180) divisor = Math.max(divisor, 3);
  }
  divisor = clampAdaptiveCadenceDivisor(
    displayHz,
    divisor,
    kind === "idle" ? 48 : 60,
  );
...
function renderQualityProfile() {
  ...
  var profile =
    typeof runtimeHardwareProfile !== "undefined" && runtimeHardwareProfile
      ? runtimeHardwareProfile
      : null;
  var lowSpec = profile && profile.lowSpec;
  if (quality === "eco")
    return {
      cap: lowSpec ? 0.88 : 0.95,
      min: 0.52,
      budget: lowSpec ? 1900000 : 2400000,
    };
  if (quality === "balanced")
    return {
      cap: lowSpec ? 0.98 : 1.12,
      min: 0.62,
      budget: lowSpec ? 2800000 : 3800000,
    };
  if (quality === "ultra") return { cap: 1.75, min: 0.85, budget: 7800000 };
  return {
    cap: lowSpec ? 1.12 : RENDER_DPR_CAP,
    min: lowSpec ? 0.66 : RENDER_MIN_DPR,
    budget: lowSpec ? 3600000 : RENDER_PIXEL_BUDGET,
  };
}
function getRenderPixelRatio() {
  var device = window.devicePixelRatio || 1;
  if (isDeepBackgroundMode()) return Math.min(device, 0.3);
...
function markRenderInteraction(reason, holdMs) {
  ...
  renderInteractionBoostUntil = Math.max(
    renderInteractionBoostUntil,
    now + (holdMs || RENDER_INTERACTION_HOLD_MS),
  );
...
function getRenderLoadTier() {
  var cssPixels = Math.max(1, innerWidth * innerHeight);
  var renderPixels =
    typeof getRenderPixelLoad === "function" ? getRenderPixelLoad() : cssPixels;
