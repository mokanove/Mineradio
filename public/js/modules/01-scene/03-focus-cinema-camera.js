...
function clearCenteredViewOffsets() {
  ...
  if (typeof particlePointerSpin !== "undefined")
    particlePointerSpin.active = false;
  if (typeof resetParticleRotationTarget === "function")
    resetParticleRotationTarget(false);
...
function updateCamera() {
  ...
  if (orbit.recentering) {
    var recenterThetaDelta = shortestAngleDelta(
      orbit.userTheta,
      orbit.baselineTheta,
    );
    ...
    var recenterEase = clampRange(0.05 + recenterDistance * 0.08, 0.052, 0.135);
    ...
    var visualPhiTarget = Math.max(
      orbit.minPhi,
      Math.min(orbit.maxPhi, orbit.baselinePhi + orbit.cinePhi),
    );
    var visualRadiusTarget = Math.max(
      orbit.minRadius,
      Math.min(orbit.maxRadius, orbit.baselineRadius + orbit.cineRadius),
    );
    var visualSettled =
      Math.abs(shortestAngleDelta(orbit.theta, visualThetaTarget)) < 0.0016 &&
      Math.abs(orbit.phi - visualPhiTarget) < 0.0016 &&
      Math.abs(orbit.radius - visualRadiusTarget) < 0.018;
    var recenterTimedOut =
      orbit.recenterStartedAt &&
      performance.now() - orbit.recenterStartedAt > 1800;
    if (
      Math.abs(recenterThetaDelta) < 0.0012 &&
  ...
  if (fa) {
    ...
    targetPhi = Math.max(
      orbit.minPhi,
      Math.min(orbit.maxPhi, orbit.userPhi + orbit.cinePhi),
    );
    targetRadius = Math.max(
      orbit.minRadius,
      Math.min(orbit.maxRadius, orbit.userRadius + orbit.cineRadius),
    );
    tLookAt = ZERO_VEC;
  }
  // 丝滑变速: 线性 lerp 自然给出 "快→慢" 缓出曲线
  var focusEase = fa ? 0.16 : 0.1;
  ...
  if (
    !fa &&
    !shelfFocusType &&
    typeof readSonicLyricLookAtTarget === "function" &&
    typeof SONIC_CAMERA_LYRIC_LOOK_AT !== "undefined" &&
    readSonicLyricLookAtTarget(SONIC_CAMERA_LYRIC_LOOK_AT)
  ) {
  ...
  if (shelfFocusType) {
    var shelfFocusWanted =
      typeof focusHover !== "undefined" && focusHover
        ? focusHover.wantType
        : null;
    ...
    if (shelfFocusMovingIn || shelfFocusMovingOut) {
      ...
      var shelfCameraSpeed = shelfFocusMovingIn
        ? shelfMotion.cameraEnterSpeed
        : shelfMotion.cameraExitSpeed;
  ...
  if (shelfFocusType && !fa) {
    ...
    if (
      Math.abs(shortestAngleDelta(orbit.theta, targetTheta)) < 0.003 &&
      Math.abs(orbit.phi - targetPhi) < 0.003 &&
      Math.abs(orbit.radius - targetRadius) < 0.03 &&
      shelfFocusLookDx * shelfFocusLookDx +
        shelfFocusLookDy * shelfFocusLookDy +
        shelfFocusLookDz * shelfFocusLookDz <
        0.0009
    ) {
  ...
  var cy = Math.cos(orbit.phi),
    sy = Math.sin(orbit.phi);
  var ct = Math.cos(orbit.theta),
    st = Math.sin(orbit.theta);
  camera.position.set(
...
function requestStageLyricCameraSnap(frames) {
  ...
  stageLyrics.snapCameraLockFrames = Math.max(
...
function shouldAvoidStageLyricsForShelf() {
  ...
  return !!(
    shelfVisibility > 0.24 ||
    (shelfHoverCue && shelfHoverCue.value > 0.28)
  );
}
function activateFocusZone(type) {
  ...
  if (type === "shelf-side") {
    if (shouldUseWallpaperSafeShelfCamera()) {
      ...
      orbit.focus.phi = shelfProfile.portrait ? 0.0 : 0.02;
      ...
      orbit.focus.radius = shelfProfile.portrait ? 5.28 : 4.2;
      orbit.focus.lookAt.set(
        shelfProfile.portrait ? 1.08 : 2.32,
        shelfProfile.portrait ? -0.18 : -0.1,
        0.72,
      );
    ...
    if (shouldUseWallpaperSafeShelfCamera()) {
      ...
      orbit.focus.lookAt.set(
        shelfProfile.portrait ? 0.72 : 2.28,
        shelfProfile.portrait ? -0.36 : -0.32,
        0.84,
      );
      camPunch = Math.max(camPunch, 0.3);
      requestStageLyricCameraSnap(10);
    } else {
      orbit.focus.theta = shelfProfile.portrait ? 0.16 : 0.34;
      orbit.focus.phi = shelfProfile.portrait ? -0.03 : -0.06;
      orbit.focus.radius = shelfProfile.portrait ? 5.9 : 4.86;
      orbit.focus.lookAt.set(
        shelfProfile.portrait ? 0.62 : 1.74,
        shelfProfile.portrait ? -0.08 : 0.02,
        0.82,
      );
...
function setFocusZone(type, immediate) {
  if (type && !shouldUseShelfDynamicCamera(type)) {
    if (/^shelf-/.test(String(orbit.focus.type || "")))
  ...
  if (focusHover.wantType === type) {
    if (type === "queue" && immediate) {
      ...
      if (!orbit.focus.active || orbit.focus.type !== type)
...
function updateCinema(dt) {
  ...
  var beatDamp =
    (orbit.focus.active ? (dj ? 0.66 : 0.55) : dj ? 1.12 : 1.0) * shake;
  var idleDamp = damp * (dj ? 0.72 : 1.0) * shake;
  orbit.cineTheta =
    Math.sin(cinemaT * 0.08) * 0.012 * idleDamp + beatCam.thetaKick * beatDamp;
  orbit.cinePhi =
    Math.sin(cinemaT * 0.06 + 1.0) * 0.01 * idleDamp +
    beatCam.phiKick * beatDamp;
  orbit.cineRadius =
    Math.sin(cinemaT * 0.04 + 2.0) * 0.08 * idleDamp -
    beatCam.radiusKick * beatDamp * (dj ? 1.22 : 1.18);
}
updateCamera();
...
function recenterCamera() {
  ...
  if (
    ...
    ((fx && fx.lyricCameraLock) || shouldUseWallpaperLyricCameraLock())
  )
