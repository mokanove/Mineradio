...
var SONIC_ORBIT_BASELINE = { theta: 0.0, phi: 0.18, radius: 8.4 };
...
function readFreeCameraState() {
  ...
  try {
    var raw =
      JSON.parse(localStorage.getItem(FREE_CAMERA_STORE_KEY) || "{}") || {};
    if (raw.position) {
      state.position.set(
...
function readSonicLyricLookAtTarget(out) {
  ...
  if (
    typeof shouldUseWallpaperLyricCameraLock === "function" &&
    shouldUseWallpaperLyricCameraLock()
  )
    return false;
  if (typeof stageLyrics === "undefined" || !stageLyrics || !stageLyrics.group)
    return false;
  ...
  out.z = clampRange(out.z + 0.1, -2.6, 1.55);
  return true;
}

function freeCameraPointerLockElement() {
  return typeof renderer !== "undefined" && renderer && renderer.domElement
    ? renderer.domElement
    : document.body;
}
...
function scheduleFreeCameraPointerLockRetry(delay) {
  ...
  if (!freeCamera || !freeCamera.active || !freeCameraPointerLock.desired)
    return;
  freeCameraPointerLock.retryTimer = setTimeout(
    function () {
      freeCameraPointerLock.retryTimer = 0;
...
function requestFreeCameraPointerLock(reason) {
  ...
  var el = freeCameraPointerLockElement();
  if (!el || !el.requestPointerLock || freeCameraPointerLockActive())
    return freeCameraPointerLockActive();
...
document.addEventListener("pointerlockerror", function () {
  freeCameraPointer.seen = false;
  if (freeCamera && freeCamera.active && freeCameraPointerLock.desired)
    scheduleFreeCameraPointerLockRetry(420);
});
  ...
  try {
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        position: {
          x: freeCamera.position.x,
          y: freeCamera.position.y,
          z: freeCamera.position.z,
        },
        yaw: freeCamera.yaw,
...
function defaultOrbitStateForPreset(p) {
  ...
  if (p === 6) return { theta: 0.18, phi: 0.1, radius: 7.4 };
...
function applyPresetOrbitBaseline(p, opts) {
  ...
  orbit.baselineRadius = clampRange(
    base.radius,
    orbit.minRadius,
    orbit.maxRadius,
  );
...
if (typeof fx !== "undefined" && fx)
  applyPresetOrbitBaseline(fx.preset, { syncCurrent: true, startup: true });
...
function captureCurrentOrbitAsBaseline() {
  var theta = Number.isFinite(orbit.theta)
    ? orbit.theta - (orbit.cineTheta || 0)
    : orbit.userTheta;
  var phi = Number.isFinite(orbit.phi)
    ? orbit.phi - (orbit.cinePhi || 0)
    : orbit.userPhi;
  var radius = Number.isFinite(orbit.radius)
    ? orbit.radius - (orbit.cineRadius || 0)
    : orbit.userRadius;
...
function getDefaultFreeCameraResetPose() {
  var baseTheta = Number.isFinite(orbit.baselineTheta)
    ? orbit.baselineTheta
    : 0;
  var basePhi = clampRange(
    Number.isFinite(orbit.baselinePhi) ? orbit.baselinePhi : 0.08,
    orbit.minPhi,
    orbit.maxPhi,
  );
  var baseRadius = clampRange(
    Number.isFinite(orbit.baselineRadius) ? orbit.baselineRadius : 6.6,
    orbit.minRadius,
    orbit.maxRadius,
  );
  ...
  if (
    ...
    typeof setSkullCameraTargetVectors === "function"
  ) {
    var look = new THREE.Vector3();
    var shelfComposition =
      typeof isSkullShelfCompositionActive === "function" &&
      isSkullShelfCompositionActive();
    setSkullCameraTargetVectors(
      pose.position,
      look,
      innerHeight > innerWidth * 1.08,
      shelfComposition,
      0,
    );
...
function syncOrbitAfterFreeCameraReset(pose) {
  ...
  if (typeof focusHover !== "undefined" && focusHover) {
    ...
    if (focusHover.pendingTimer) {
      clearTimeout(focusHover.pendingTimer);
      focusHover.pendingTimer = null;
    }
    if (focusHover.exitTimer) {
      clearTimeout(focusHover.exitTimer);
  ...
  if (fx && fx.preset === SKULL_PRESET_INDEX) {
    if (typeof resetSkullPresetView === "function")
  ...
  var radius = clampRange(
    Math.sqrt(px * px + py * py + pz * pz) || orbit.baselineRadius || 6.6,
    orbit.minRadius,
    orbit.maxRadius,
  );
  var theta = Math.atan2(px, pz);
  var phi = clampRange(
    Math.asin(clampRange(py / Math.max(0.001, radius), -1, 1)),
    orbit.minPhi,
    orbit.maxPhi,
  );
  orbit.baselineTheta = theta;
  ...
  if ((fx && fx.lyricCameraLock) || shouldUseWallpaperLyricCameraLock())
    requestStageLyricCameraSnap(14);
}
...
function applyFreeCameraToCamera() {
  ...
  if (cameraShake > 0 && Math.abs(beatCam.radiusKick) > 0.0001) {
    ...
    camera.position.addScaledVector(
      FREE_CAMERA_SHAKE_DIR,
      beatCam.radiusKick * cameraShake * 0.52,
    );
  }
  var cameraPunch =
    Math.max(
      camPunch * 0.55,
      beatCam.punch * 0.54 + beatCam.radiusKick * 0.16,
    ) * cameraShake;
  var targetFov =
    clampRange(freeCamera.fov || BASE_FOV, 26, 72) - cameraPunch * 1.75;
  camera.fov +=
    (targetFov - camera.fov) * (targetFov < camera.fov ? 0.24 : 0.12);
...
function resetFreeCameraToDefault() {
  ...
  var fromPos = freeCamera.position
    ? freeCamera.position.clone()
...
function updateFreeCamera(dt) {
  ...
  if (freeCamera.resetTween) {
    ...
    var t = easeOutCubic01(
      (performance.now() - tw.start) / Math.max(1, tw.duration || 620),
    );
    freeCamera.position.copy(tw.from.position).lerp(tw.to.position, t);
    freeCamera.yaw =
      tw.from.yaw + shortestAngleDelta(tw.from.yaw, tw.to.yaw) * t;
    freeCamera.pitch = tw.from.pitch + (tw.to.pitch - tw.from.pitch) * t;
    freeCamera.roll =
      tw.from.roll + shortestAngleDelta(tw.from.roll, tw.to.roll) * t;
  ...
  freeCamera.velocity.lerp(
    targetVel,
    clampRange(ease * Math.max(0.001, dt || 1 / 60), 0, 1),
  );
  if (freeCamera.velocity.lengthSq() < 0.0004) freeCamera.velocity.set(0, 0, 0);
  freeCamera.position.addScaledVector(
    freeCamera.velocity,
    Math.max(0.001, dt || 1 / 60),
  );
  var rollDir = (keys.KeyQ ? 1 : 0) - (keys.KeyE ? 1 : 0);
  if (rollDir)
    freeCamera.roll = clampRange(
      freeCamera.roll + rollDir * dt * 0.9,
      -Math.PI,
      Math.PI,
    );
