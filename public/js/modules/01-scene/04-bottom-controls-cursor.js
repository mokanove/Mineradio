...
function desktopWallpaperKeepsPlayerConsoleVisible() {
  ...
  if (
    body.classList.contains("empty-home-active") ||
    body.classList.contains("home-controls-locked")
  )
    return false;
  try {
...
function setControlsHidden(hidden) {
  ...
  bar.classList.toggle(
    "soft-hidden",
    !!hidden && controlsAutoHide && bar.classList.contains("visible"),
  );
  if (hidden && typeof closeLyricTimingPopover === "function")
...
function isBottomControlsSuppressedForShelf() {
  ...
  return !!(
    shelfPinnedOpen ||
    shelfContentOpen ||
    (controlsShelfSuppressUntil &&
      performance.now() < controlsShelfSuppressUntil)
  );
}

function suppressBottomControlsForShelf(duration) {
  controlsShelfSuppressUntil =
    performance.now() + (duration == null ? 900 : duration);
...
function scheduleControlsHide(delay) {
  ...
  if (revealHoldRemaining > 0)
    requestedDelay = Math.max(
      requestedDelay,
      Math.ceil(revealHoldRemaining) + 32,
    );
...
function holdBottomControlsVisible(duration) {
  ...
  controlsRevealHoldUntil = Math.max(
    controlsRevealHoldUntil,
    performance.now() + holdDuration,
  );
...
function showRestoredPlaybackControls(reason) {
  ...
  try {
    ...
    if (controlsAutoHide)
      scheduleControlsHide(reason === "startup-autoplay" ? 2200 : 3600);
  } catch (e) {
...
function updateControlsAutoHideFromPointer(x, y) {
  ...
  if (diyPlayerMode) {
    ...
    var overFxPanel =
      fxPanel &&
      (fxPanel.classList.contains("peek") ||
        fxPanel.classList.contains("show")) &&
      fr &&
      x >= fr.left - 18 &&
      x <= fr.right + 18 &&
      y >= fr.top - 18 &&
  ...
  var overMini =
    ...
    x <= miniRect.right + 16 &&
    y >= miniRect.top - 16 &&
    y <= miniRect.bottom + 16;
  if (overHandle) wakeBottomHandle();
  if (overBar || overMini || overHandle)
    revealBottomControls(overHandle ? 900 : 520);
...
["mousemove", "pointermove", "mousedown", "wheel", "touchstart"].forEach(
  function (type) {
    window.addEventListener(type, revealCursorForActivity, {
      passive: true,
      capture: true,
    });
  },
