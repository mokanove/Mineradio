...
function stageLyricRenderSignatureForCurrentState() {
  var lyricSignature =
    typeof currentAppliedLyricRenderSignature === "function"
      ? currentAppliedLyricRenderSignature()
      : "";
...
function stageLyricCanPreserveSameRender(signature) {
  if (!signature || !stageLyrics || stageLyrics.renderSignature !== signature)
    return false;
  if (
    stageLyrics.current ||
    stageLyrics.currentPayload ||
  ...
  return !!(
    ...
    (stageLyricPrewarm.mesh ||
      stageLyricPrewarm.timer ||
      stageLyricPrewarm.workTimer ||
      stageLyricPrewarm.workRaf ||
      stageLyricPrewarm.build)
  );
}

function stageLyricPrewarmPayload() {
  return stageLyricPrewarm &&
    stageLyricPrewarm.mesh &&
    stageLyricPrewarm.mesh.userData
...
function stageLyricLightPrewarmReason(reason) {
  return (
    /^(intro-first-line|renderLyrics|renderLyrics-title|toggleLyricsPanel|setParticleLyricsSilently|track-demand-light|startup-restore|startup-restore-lyrics|playback-resume|playback-started)$/.test(
      String(reason || ""),
...
function cancelStageLyricPrewarmBuildOnly() {
  ...
  if (
    stageLyricPrewarm.build &&
    stageLyricPrewarm.build.state &&
    typeof cancelCooperativeLyricMeshBuild === "function"
  ) {
...
function clearStageLyricSingleLinePrewarmItem(key) {
  if (
    ...
    !stageLyricSingleLinePrewarm.items ||
    !key
  )
    return;
...
function clearStageLyricSingleLinePrewarmCache() {
  if (!stageLyricSingleLinePrewarm || !stageLyricSingleLinePrewarm.items)
    return;
  Object.keys(stageLyricSingleLinePrewarm.items).forEach(
    clearStageLyricSingleLinePrewarmItem,
  );
  stageLyricSingleLinePrewarm.order = [];
}
...
function stageLyricTextLoadInfo() {
  ...
  var translationMode = normalizeLyricTranslationMode(
    fx && fx.lyricTranslationMode,
  );
  var key =
    stageLyricTrackKeyForMode(mode) +
    "|native=" +
    (lyricsHasNativeKaraoke ? 1 : 0);
  if (
    stageLyricTextLoadCache &&
    stageLyricTextLoadCache.key === key &&
    stageLyricTextLoadCache.info
  )
    return stageLyricTextLoadCache.info;
  ...
  for (var i = 0; i < total; i++) {
    ...
    if (translation) {
      ...
      cjkTotal += (translation.match(/[\u3400-\u9fff\uf900-\ufaff]/g) || [])
        .length;
    }
  ...
  var preferLightweight = !!(
    multiLineLoad &&
    denseCjk &&
    (total >= 12 || nativeWordLines >= 4 || layeredRows >= 20)
  );
  var info = {
    total: total,
    cjkTotal: cjkTotal,
...
function stageLyricRestoreWarmupSeconds() {
  var seconds =
    Number(stageLyricRestoreWarmup && stageLyricRestoreWarmup.time) || 0;
  if (seconds < 0.35) return null;
  if (
    stageLyricRestoreWarmup.token &&
    typeof trackSwitchToken !== "undefined" &&
  ...
  var actual =
    audio && isFinite(Number(audio.currentTime))
...
function stageLyricIndexForSeconds(seconds) {
  ...
  var lyricT =
    typeof getAdjustedLyricPlaybackTime === "function"
      ? getAdjustedLyricPlaybackTime(seconds)
      : seconds;
...
function requestStageLyricRestoreWarmup(seconds, token, reason) {
  ...
  if (typeof scheduleStageLyricFullTrackWarmup === "function")
    scheduleStageLyricFullTrackWarmup("track-ready-fast", 140);
  return true;
}

function requestStageLyricWarmup(reason, ms) {
  var duration = clampRange(Number(ms) || 120, 24, 900);
  stageLyricWarmup.until = Math.max(
    Number(stageLyricWarmup.until) || 0,
    stageLyricNowMs() + duration,
  );
...
function invalidateStageLyricPayloadForNewLyrics(reason) {
  ...
  if (typeof invalidateLyricQualityTextures === "function")
    invalidateLyricQualityTextures(reason || "new-lyrics");
...
function stageLyricWarmupPending() {
  ...
  if (normalizeLyricDisplayMode(fx && fx.lyricDisplayMode) === "single") {
    ...
    if (
      singleLineIndex >= 0 &&
      stageLyricSingleLineIndexPrewarmReady(singleLineIndex)
    )
      return false;
  }
...
function stageLyricPayloadIsSingleLine(payload) {
  ...
  return (
    normalizeLyricDisplayMode(payload.mode || (fx && fx.lyricDisplayMode)) ===
    "single"
  );
}
...
function stageLyricSingleLineNextIndex(index) {
  if (
    !lyricsLines ||
    !lyricsLines.length ||
    normalizeLyricDisplayMode(fx && fx.lyricDisplayMode) !== "single"
  )
    return -1;
...
function stageLyricSingleLineIndexPrewarmReady(index) {
  ...
  if (!stageLyricPrewarm || stageLyricPrewarm.targetIndex !== index)
    return false;
...
function stageLyricSingleLinePrewarmDelay(
  currentIndex,
  targetIndex,
  lyricTime,
  slot,
  ...
  var secondsUntil =
    targetLine && isFinite(Number(targetLine.t))
...
function stageLyricSingleLinePrewarmCanServePayload(payload) {
  ...
  var item =
    key && stageLyricSingleLinePrewarm && stageLyricSingleLinePrewarm.items
...
function scheduleStageLyricSingleLineNextPrewarm(
  currentIndex,
  lyricTime,
  reason,
) {
  ...
  for (var i = 0; i < upcoming.length; i++) {
    ...
    allReady = false;
    var delay = stageLyricSingleLinePrewarmDelay(
      currentIndex,
      targetIndex,
      lyricTime,
      i,
    ...
    var prewarmReason =
      i === 0
        ? reason || "single-line-next"
        : "single-line-lookahead-" + (i + 1);
...
function scheduleStageLyricSingleLineBootstrapPrewarm(reason, delay) {
  ...
  if (
    !currentCoveredBySharedPrewarm &&
    !stageLyricSingleLineIndexPrewarmReady(currentIndex)
  ) {
    scheduleStageLyricSingleLineCachePrewarm(
      currentIndex,
      (reason || "single-line-bootstrap") + "-current",
      Math.min(wait, 8),
    );
  }
  var upcoming = stageLyricSingleLineUpcomingIndexes(currentIndex, 6);
  for (var i = 0; i < upcoming.length; i++) {
    ...
    scheduleStageLyricSingleLineCachePrewarm(
      upcoming[i],
      (reason || "single-line-bootstrap") + "-lookahead-" + (i + 1),
      Math.max(0, Math.min(wait + 8 + i * 18, 96)),
    );
  }
  return true;
}
...
function resetPreparedStageLyricMesh(mesh, payload, lineStep) {
  ...
  mesh.position.set(singleLineStartX, 0.2, 1.46 - Math.abs(enterDir) * 0.055);
  ...
  var lineWorldStep = clampRange(Number(data.lineWorldStep) || 0.38, 0.2, 0.94);
...
function takeStageLyricPrewarmMesh(payload) {
  ...
  var data =
    stageLyricPrewarm.mesh.userData && stageLyricPrewarm.mesh.userData.lyric;
  var targetLineIndex =
    payload && payload.trackIndex != null ? Number(payload.trackIndex) : NaN;
  if (
    data &&
    data.usesTrack &&
    payload &&
    payload.trackKey &&
    data.trackKey === payload.trackKey &&
...
function stageLyricPreparedFullPayloadForIndex(index) {
  ...
  index = Math.max(
    0,
    Math.min(lyricsLines.length - 1, Math.round(Number(index) || 0)),
  );
  var payload = buildStageLyricDisplayPayload(index);
...
function shouldDeferStageLyricSyncBuild(payload, redrawOnly) {
  if (redrawOnly || !payload || !lyricsLines || !lyricsLines.length)
    return false;
  if (stageLyricPayloadIsSingleLine(payload)) return false;
  ...
  if (
    stageLyrics.current &&
    stageLyricMeshCanServePayload(stageLyrics.current, payload)
  )
    return false;
...
function ensureStageLyricPlaybackWarmup(reason, delay) {
  ...
  var idx =
    stageLyrics && stageLyrics.currentIdx >= 0
      ? stageLyrics.currentIdx
      : chooseStageLyricPrewarmIndex();
  if (idx < 0) return false;
  ...
  if (
    ...
    (stageLyricPrewarm.timer ||
      ...
      stageLyricPrewarm.build ||
      stageLyricPrewarm.mesh ||
      stageLyricFullTrackWarmupTimer)
  ) {
    return true;
  }
  ...
  scheduleStageLyricPrewarmForIndex(
    idx,
    reason || "playback-resume",
    delay == null ? 16 : delay,
  );
  if (typeof scheduleStageLyricFullTrackWarmup === "function")
    scheduleStageLyricFullTrackWarmup("track-ready-fast", 96);
  return true;
}
...
function requestStageLyricLightweightUpgrade(reason, delay) {
  ...
  if (typeof scheduleStageLyricFullTrackWarmup === "function") {
    scheduleStageLyricFullTrackWarmup(
      reason || "lightweight-upgrade",
      delay == null ? 132 : delay,
    );
    return true;
  }
  return ensureStageLyricPlaybackWarmup(
    reason || "lightweight-upgrade",
    delay == null ? 32 : delay,
  );
}
...
function stageLyricResidentRowKey(row) {
  ...
  return (
    Math.round(lineIndex) +
    "|" +
    (row.isTranslation ? "translation" : "primary")
  );
}
...
function cancelStageLyricResidentBuild() {
  ...
  if (stageLyricResidentBuild.timer)
    clearTimeout(stageLyricResidentBuild.timer);
  if (stageLyricResidentBuild.raf && typeof cancelAnimationFrame === "function")
    cancelAnimationFrame(stageLyricResidentBuild.raf);
  ...
  if (job && job.state) {
    if (typeof cancelLyricRowLayerGroupBuild === "function")
      cancelLyricRowLayerGroupBuild(job.state);
...
function cancelStageLyricResidentDemand() {
  if (stageLyricResidentDemand.timer)
    clearTimeout(stageLyricResidentDemand.timer);
...
function scheduleStageLyricResidentDemand(mesh, targetIndex, options) {
  ...
  if (stageLyricProgressPreviewActive())
    stageLyricResidentDemand.options.interactive = true;
  ...
  stageLyricResidentDemand.timer = setTimeout(function () {
    ...
    for (var optionKey in demandOptions)
      coalescedOptions[optionKey] = demandOptions[optionKey];
...
function buildStageLyricResidentPayload(index, start, end, options) {
  ...
  var translated = applyLyricTranslationModeToTrackEntries(
    entries,
    activeLine,
    entries.length * 2 + 2,
  );
...
function disposeStageLyricResidentRow(row) {
  if (!row) return;
  if (typeof releaseLyricRowQuality === "function")
...
function stageLyricResidentDisplayedScrollOffset(data, fallbackOffset) {
  if (!data || !Array.isArray(data.rowLayers) || !data.rowLayers.length)
    return fallbackOffset;
  var lineStepWorld = clampRange(Number(data.lineWorldStep) || 0.38, 0.2, 0.94);
  var samples = [];
  for (var i = 0; i < data.rowLayers.length; i++) {
    var row = data.rowLayers[i];
    if (
      ...
      !row.isPrimary ||
      !row.mesh ||
      !isFinite(Number(row.virtualIndex)) ||
      !isFinite(Number(row.mesh.position.y))
    )
      continue;
    var visualOffset =
      Number(row.virtualIndex) + Number(row.mesh.position.y) / lineStepWorld;
...
function stageLyricResidentTransformSnapshot(data) {
  data = data || {};
  var previewMotionLock =
    typeof stageLyricProgressPreviewActive === "function" &&
    stageLyricProgressPreviewActive();
  var pendingLineIndex =
    data.trackPendingPayload &&
    isFinite(Number(data.trackPendingPayload.trackIndex))
      ? Number(data.trackPendingPayload.trackIndex)
      : null;
  var targetLineIndex =
    previewMotionLock && pendingLineIndex != null
      ? pendingLineIndex
      : isFinite(Number(data.trackTargetLineIndex))
        ? Number(data.trackTargetLineIndex)
        : 0;
  var targetVirtualIndex = previewMotionLock
    ? lyricPrimaryVirtualIndex(targetLineIndex)
    : isFinite(Number(data.trackTargetVirtualIndex))
      ? Number(data.trackTargetVirtualIndex)
      : lyricPrimaryVirtualIndex(targetLineIndex);
  var sharedScrollOffset = isFinite(Number(data.trackScrollOffset))
    ? Number(data.trackScrollOffset)
    : targetVirtualIndex;
  var displayedScrollOffset = stageLyricResidentDisplayedScrollOffset(
    data,
    sharedScrollOffset,
  );
  return {
...
function primeStageLyricResidentRowTransform(data, row, transformSnapshot) {
  ...
  var targetVirtualIndex = isFinite(Number(snapshot.targetVirtualIndex))
    ? Number(snapshot.targetVirtualIndex)
    : lyricPrimaryVirtualIndex(targetLineIndex);
  var scrollOffset = isFinite(Number(snapshot.scrollOffset))
    ? Number(snapshot.scrollOffset)
    : targetVirtualIndex;
  var lineStepWorld = clampRange(Number(data.lineWorldStep) || 0.38, 0.2, 0.94);
  var translationLineStepWorld = clampRange(
    Number(data.translationLineStepWorld) || lineStepWorld,
    0.2,
    0.78,
  );
  var rowVirtualIndex = isFinite(Number(row.virtualIndex))
    ? Number(row.virtualIndex)
    : targetVirtualIndex;
  var rowLineIndex = isFinite(Number(row.lineIndex))
    ? Number(row.lineIndex)
    : null;
  var isActive = !!row.isPrimary && rowLineIndex === targetLineIndex;
  var parentIndex = row.isTranslation
    ? isFinite(Number(row.parentIndex))
      ? Number(row.parentIndex)
      : rowLineIndex
    : null;
  var currentTranslation = row.isTranslation && parentIndex === targetLineIndex;
  var visibilityAbs =
    row.isTranslation && parentIndex != null
  ...
  if (row.isTranslation) {
    yTarget = lyricTranslationAnchoredY(
      row,
      rowVirtualIndex,
      targetVirtualIndex,
      lineStepWorld,
      translationLineStepWorld,
      scrollOffset,
      0,
      currentTranslation,
      true,
  ...
  var zTarget =
    0.055 -
    Math.pow(Math.min(5.5, visibilityAbs), 1.06) * 0.145 +
    (currentTranslation ? 0.065 : 0);
  var scaleDistance = isActive || currentTranslation ? 0 : visibilityAbs;
  var scaleTarget = clampRange(
    1 - Math.min(5.5, scaleDistance) * 0.026,
    0.84,
    1.02,
  );
  if (row.isTranslation) {
  ...
  alignStageLyricResidentEffectToRow(row, row.glow, -0.03);
}
  ...
  for (var ri = 0; ri < bundle.rows.length; ri++) {
    ...
    if (existingRow) {
      if (!existingRow.readability && row.readability) {
        if (row.readability.parent)
          row.readability.parent.remove(row.readability);
        ...
        alignStageLyricResidentEffectToRow(
          existingRow,
          existingRow.readability,
          -0.012,
        );
...
function finishStageLyricResidentBuild(job) {
  ...
  if (
    ...
    stageLyrics.current !== job.mesh ||
    data.trackKey !== job.trackKey
  ) {
  ...
  var demandIndex =
    data.trackPendingPayload && data.trackPendingPayload.trackIndex != null
      ? Number(data.trackPendingPayload.trackIndex)
      : Number(data.trackTargetLineIndex);
  if (!data.trackPendingPayload)
...
function runStageLyricResidentBuild(job) {
  ...
  if (
    ...
    job.token !== stageLyricResidentBuild.token
  )
    return;
  var data = job.mesh && job.mesh.userData && job.mesh.userData.lyric;
  if (
  ...
  if (
    stageLyricShouldYieldToPendingInput() &&
    !job.interactive &&
    !job.textOnly
  ) {
...
function scheduleStageLyricResidentBuildWork(job, delay) {
  ...
  var queue = function () {
    ...
    if (
      ...
      stageLyricResidentBuild.timer = setTimeout(function () {
        runStageLyricResidentBuild(job);
      }, 12);
    }
  };
...
function startStageLyricResidentBuild(mesh, targetIndex, start, end, options) {
  ...
  cancelStageLyricResidentBuild();
  var payload = buildStageLyricResidentPayload(targetIndex, start, end, {
    textOnly: options.textOnly === true,
  });
  if (!payload) return false;
  var maskLayout = data.persistentMaskLayout || {
    fontSize: 128,
    lineHeight: 138,
  };
  var state = beginLyricRowLayerGroupBuild(
    payload,
    maskLayout,
    Number(data.worldW) || 6.1,
    Number(data.worldH) || 1.2,
    stageLyrics.palette,
    lyricMotionProfile(),
  );
  ...
  if (state && isFinite(Number(data.lineWorldStep)))
    state.lineStepWorld = Number(data.lineWorldStep);
  if (state && isFinite(Number(data.translationLineStepWorld)))
    state.translationLineStepWorld = Number(data.translationLineStepWorld);
  var job = {
    mesh: mesh,
    trackKey: data.trackKey,
...
function stageLyricPersistentPrimaryMap(data) {
  ...
  for (var i = 0; i < data.rowLayers.length; i++) {
    ...
    if (
      ...
      map[Math.round(Number(row.lineIndex))] = true;
  }
  return map;
}
...
function stageLyricPersistentLineEffectsResident(data, lineIndex, rowMap) {
  ...
  if (
    ...
    if (!translation || !translation.readability || !translation.glow)
      return false;
  }
  return true;
}
...
function stageLyricPersistentTargetRowsReady(mesh, targetIndex, options) {
  options = options || {};
  var data = mesh && mesh.userData && mesh.userData.lyric;
  if (!data || !data.trackPersistent || !lyricsLines || !lyricsLines.length)
    return false;
  targetIndex = Math.max(
    0,
    Math.min(lyricsLines.length - 1, Math.round(Number(targetIndex) || 0)),
  );
  var offsets = lyricDisplayOffsetsForMode(
    data.displayMode || (fx && fx.lyricDisplayMode),
  );
  var required = {};
  for (var oi = 0; oi < offsets.length; oi++) {
    ...
    if (
      normalizeLyricTranslationMode(fx && fx.lyricTranslationMode) !== "off" &&
      makeStageLyricTranslationEntry(entry, lineIndex === targetIndex)
    ) {
  ...
  for (var ki = 0; ki < requiredKeys.length; ki++) {
    ...
    if (!requiredRow || !requiredRow.mesh || !requiredRow.renderLineUploaded)
      return false;
    if (options.effects === true) {
      if (!requiredRow.readability || !requiredRow.renderReadabilityUploaded)
        return false;
...
function stageLyricPersistentTargetEffectsReady(mesh, targetIndex) {
  return stageLyricPersistentTargetRowsReady(mesh, targetIndex, {
    effects: true,
  });
}
...
function stageLyricPersistentNextTextRunwayRange(data, targetIndex, rowMap) {
  ...
  targetIndex = Math.max(
    0,
    Math.min(last, Math.round(Number(targetIndex) || 0)),
  );
  rowMap = rowMap || stageLyricPersistentResidentRowMap(data);
  var nextIndex = -1;
  var previousIndex = -1;
  for (var next = targetIndex; next <= last; next++) {
    if (
      lyricLineDisplayTextAt(next) &&
      !stageLyricPersistentLineRowsResident(data, next, rowMap)
    ) {
      nextIndex = next;
      break;
    }
  ...
  for (var previous = targetIndex - 1; previous >= 0; previous--) {
    if (
      lyricLineDisplayTextAt(previous) &&
      !stageLyricPersistentLineRowsResident(data, previous, rowMap)
    ) {
...
function commitStageLyricPersistentPendingTarget(mesh) {
  ...
  if (
    !payload ||
    payload.trackIndex == null ||
    !stageLyricPersistentTargetRowsReady(mesh, payload.trackIndex)
  )
    return false;
...
function ensureStageLyricPersistentTrackRows(mesh, targetIndex, options) {
  ...
  var interactivePreview = !!(
    options.interactive || stageLyricProgressPreviewActive()
  );
  ...
  var desiredStart = Math.max(
    0,
    targetIndex + minOffset - (interactivePreview ? 4 : 6),
  );
  var desiredEnd = Math.min(
    last,
    targetIndex + maxOffset + (interactivePreview ? 10 : 24),
  );
  var visibleStart = Math.max(0, targetIndex + minOffset);
  var visibleEnd = Math.min(last, targetIndex + maxOffset);
  var targetMissing = !stageLyricPersistentLineRowsResident(
    data,
    targetIndex,
    residentRowMap,
  );
  ...
  for (
    var visibleIndex = visibleStart;
    visibleIndex <= visibleEnd;
    visibleIndex++
  ) {
    if (
      lyricLineDisplayTextAt(visibleIndex) &&
      !stageLyricPersistentLineRowsResident(data, visibleIndex, residentRowMap)
    ) {
      firstVisibleMissing = visibleIndex;
      break;
    }
  }
  ...
  if (firstMissing < 0) {
    for (var i = desiredStart; i <= desiredEnd; i++) {
      if (
        lyricLineDisplayTextAt(i) &&
        !stageLyricPersistentLineRowsResident(data, i, residentRowMap)
      ) {
        firstMissing = i;
        break;
      }
  ...
  if (firstMissing < 0) {
    ...
    for (
      var effectIndex = visibleStart;
      effectIndex <= visibleEnd;
      effectIndex++
    ) {
      if (
        lyricLineDisplayTextAt(effectIndex) &&
        !stageLyricPersistentLineEffectsResident(
          data,
          effectIndex,
          residentRowMap,
        )
    ...
    if (firstEffectsMissing >= 0) {
      ...
      if (effectJob && effectJob.mesh === mesh) {
        if (
          !effectJob.textOnly &&
          effectJob.start <= visibleStart &&
          effectJob.end >= visibleEnd
        )
          return true;
        cancelStageLyricResidentBuild();
      }
      return startStageLyricResidentBuild(
        mesh,
        targetIndex,
        visibleStart,
        visibleEnd,
    ...
    if (!interactivePreview && !data.trackPendingPayload) {
      ...
      var runwayRange = stageLyricPersistentNextTextRunwayRange(
        data,
        targetIndex,
        residentRowMap,
      );
  ...
  var usable = 0;
  for (
    var initialIndex = buildStart;
    initialIndex <= buildEnd;
    initialIndex++
  ) {
    if (
      lyricLineDisplayTextAt(initialIndex) &&
      !stageLyricPersistentLineRowsResident(data, initialIndex, residentRowMap)
    )
      usable += 1;
  }
  var chunkLimit = interactivePreview
    ? Math.max(8, offsets.length)
    : Math.max(28, offsets.length);
  if (!interactivePreview || firstVisibleMissing < 0) {
    while (buildEnd < desiredEnd && usable < chunkLimit) {
      buildEnd += 1;
      if (
        lyricLineDisplayTextAt(buildEnd) &&
        !stageLyricPersistentLineRowsResident(data, buildEnd, residentRowMap)
      )
        usable += 1;
    }
  }
  ...
  if (activeJob && activeJob.mesh === mesh) {
    var activeCoversVisibleWindow =
      activeJob.start <= visibleStart && activeJob.end >= visibleEnd;
    if (
      firstVisibleMissing >= 0 &&
      activeCoversVisibleWindow &&
      (!interactivePreview || activeJob.textOnly)
    )
      return true;
  ...
  return startStageLyricResidentBuild(mesh, targetIndex, buildStart, buildEnd, {
    reason:
      options.reason ||
      (targetMissing ? "persistent-track-demand" : "persistent-track-ahead"),
    textOnly: true,
    interactive: interactivePreview,
  });
}
...
function trimStageLyricPersistentTrackRows(mesh, targetIndex, options) {
  ...
  if (
    !data ||
    !data.trackPersistent ||
    !Array.isArray(data.rowLayers) ||
    !data.rowLayers.length ||
    !lyricsLines ||
    !lyricsLines.length
  )
    return false;
  targetIndex = Math.max(
    0,
    Math.min(
      lyricsLines && lyricsLines.length ? lyricsLines.length - 1 : 0,
      Math.round(Number(targetIndex) || 0),
    ),
  );
  var offsets = lyricDisplayOffsetsForMode(
    data.displayMode || (fx && fx.lyricDisplayMode),
  ...
  var keepEnd = Math.min(
    lyricsLines.length - 1,
    targetIndex + maxOffset + (interactive ? 5 : 24),
  );
  ...
  if (interactive && isFinite(preserveTargetIndex)) {
    preserveTargetIndex = Math.max(
      0,
      Math.min(lyricsLines.length - 1, Math.round(preserveTargetIndex)),
    );
    preserveStart = Math.max(0, preserveTargetIndex + minOffset - 2);
    preserveEnd = Math.min(
      lyricsLines.length - 1,
      preserveTargetIndex + maxOffset + 2,
    );
  }
  ...
  for (var i = 0; i < data.rowLayers.length; i++) {
    ...
    var keepForTarget =
      roundedLineIndex != null &&
      roundedLineIndex >= keepStart &&
      roundedLineIndex <= keepEnd;
    var keepForCommittedWindow =
      roundedLineIndex != null &&
      roundedLineIndex >= preserveStart &&
      roundedLineIndex <= preserveEnd;
    var keepEffects =
      roundedLineIndex == null ||
      keepForTarget ||
      keepForCommittedWindow ||
      roundedLineIndex === targetIndex ||
      roundedLineIndex === preserveTargetIndex;
...
function initializeStageLyricPersistentTrack(mesh, payload) {
  var data = mesh && mesh.userData && mesh.userData.lyric;
  var mode = normalizeLyricDisplayMode(
  ...
  if (!data.trackPersistent) {
    ...
    data.trackScrollWindowKey =
      data.trackKey || (payload && payload.trackKey) || "";
    ...
    var stableLayout =
      typeof stableStageLyricRowMaskLayout === "function"
        ? stableStageLyricRowMaskLayout()
        : { fontSize: 128, lineHeight: 138 };
    data.persistentMaskLayout = {
      fontSize:
        Number(layoutMask.logicalFontSize) ||
        Number(stableLayout.fontSize) ||
        128,
      lineHeight:
        Number(layoutMask.logicalLineHeight) ||
        Number(stableLayout.lineHeight) ||
        (Number(stableLayout.fontSize) || 128) * 1.08,
    };
  ...
  ensureStageLyricPersistentTrackRows(
    mesh,
    payload && payload.trackIndex != null
      ? payload.trackIndex
      : data.trackTargetLineIndex,
    { reason: "persistent-track-bootstrap" },
  );
  return true;
}
...
function stageLyricShouldHoldOutgoingForReveal(current, incoming) {
  ...
  return !!(
    currentData &&
    incomingData &&
    currentData.usesTrack &&
    incomingData.usesTrack &&
    currentData.trackKey &&
    currentData.trackKey === incomingData.trackKey &&
...
function releaseStageLyricRevealHoldsForSuccessor(successor) {
  ...
  for (var i = 0; i < stageLyrics.outgoing.length; i++) {
    ...
    if (
      !outgoing ||
      !outgoing.userData ||
      outgoing.userData.lyricRevealSuccessor !== successor
    )
      continue;
...
function stageLyricLightweightPrewarmAwaitingTakeover() {
  ...
  if (stageLyricPrewarm.build && stageLyricPrewarm.build.lightweight)
    return true;
  if (
...
function upgradeCurrentStageLyricFromPreparedTrack(reason) {
  ...
  if (!payload) {
    if ((Number(stageLyricResumeUpgradeDeferUntil) || 0) > stageLyricNowMs())
      return false;
    requestStageLyricLightweightUpgrade(reason || "lightweight-upgrade", 132);
    return false;
  }
  var progress = stageLyrics.current.userData
  ...
  if (stageLyrics.current && stageLyrics.current.userData) {
    stageLyrics.current.userData.age = Math.max(
      Number(stageLyrics.current.userData.age) || 0,
      0.36,
    );
  }
  return true;
...
function stageLyricPlaybackSeconds() {
  var preview =
    typeof getProgressDragPreviewSeconds === "function"
      ? getProgressDragPreviewSeconds()
      : null;
  if (preview != null && isFinite(Number(preview)))
    return Math.max(0, Number(preview));
  var restoreSeconds = stageLyricRestoreWarmupSeconds();
  if (restoreSeconds != null) return restoreSeconds;
  return audio && isFinite(Number(audio.currentTime))
...
function stageLyricProgressSeekVisualReady(seconds) {
  ...
  var lyricT =
    typeof getAdjustedLyricPlaybackTime === "function"
      ? getAdjustedLyricPlaybackTime(t)
      : t;
...
function stageLyricSingleLineBootstrapIndex() {
  ...
  if (normalizeLyricDisplayMode(fx && fx.lyricDisplayMode) !== "single")
    return -1;
  if (stageLyrics && stageLyrics.currentIdx >= 0)
    return Math.max(
      0,
      Math.min(lyricsLines.length - 1, stageLyrics.currentIdx),
    );
...
function stageLyricShouldBuildCooperatively(payload) {
  return !!(
    payload &&
    normalizeLyricDisplayMode(payload.mode) !== "single" &&
...
function updateStageLyricBuildStats(job, active, chunkMs) {
  ...
  var phase =
    (job && job.lastPhase) || (buildState && buildState.lastPhase) || "";
  stats.active = !!active;
  stats.reason = (job && job.reason) || stats.reason || "";
  stats.lightweight = !!(job && job.lightweight);
  stats.totalRows = buildState
    ? Number(buildState.totalRows) || 0
    : Number(stats.totalRows) || 0;
  stats.builtRows = rowState
    ? Number(rowState.cursor) || 0
    : active
      ? Number(stats.builtRows) || 0
      : Number(stats.totalRows) || 0;
  stats.totalPhases = buildState
    ? Number(buildState.totalPhases) || 0
    : Number(stats.totalPhases) || 0;
  stats.builtPhases = buildState
    ? Number(buildState.completedPhases) || 0
    : Number(stats.builtPhases) || 0;
  stats.lastPhase = phase || stats.lastPhase || "";
  stats.lastChunkMs = Number(chunkMs) || 0;
  stats.maxChunkMs = Math.max(
    Number(stats.maxChunkMs) || 0,
    Number(chunkMs) || 0,
  );
  stats.phaseMaxMs = stats.phaseMaxMs || {};
  if (phase)
    stats.phaseMaxMs[phase] = Math.max(
      Number(stats.phaseMaxMs[phase]) || 0,
      Number(chunkMs) || 0,
    );
  if (job && job.startedAt)
    stats.elapsedMs = Math.max(0, stageLyricNowMs() - job.startedAt);
  if (!active && job && job.startedAt)
    stats.lastBuildMs = Math.max(0, stageLyricNowMs() - job.startedAt);
...
function finishStageLyricCooperativePrewarm(job) {
  ...
  if (
    !mesh ||
    job.token !== stageLyricPrewarm.token ||
    !fx ||
    !fx.particleLyrics ||
    !lyricsLines ||
    !lyricsLines.length ||
    stageLyricPrewarmBuildGuardKey() !== job.guardKey
  ) {
  ...
  if (stageLyricPrewarm.mesh && stageLyricPrewarm.mesh !== mesh)
    disposeLyricMesh(stageLyricPrewarm.mesh);
  primeLyricMeshOpacity(mesh, 0);
  mesh.visible = false;
  ...
  if (!job.lightweight && stageLyricCurrentUsesPersistentTrack()) {
    ...
    ensureStageLyricPersistentTrackRows(
      stageLyrics.current,
      stageLyrics.currentIdx,
      { reason: "discard-full-track-takeover" },
    );
  } else if (!job.lightweight && stageLyricCurrentUsesLightweightTrack()) {
    upgradeCurrentStageLyricFromPreparedTrack(
      job.reason || "full-prewarm-ready",
    );
  }
  updateStageLyricBuildStats(job, false, stageLyricNowMs() - startedAt);
}
...
function runStageLyricCooperativePrewarm(job) {
  ...
  if (done) {
    ...
    scheduleStageLyricCooperativeWork(
      job,
      stageLyricCooperativeNextDelay(job, chunkMs),
    );
    return;
  }
  scheduleStageLyricCooperativeWork(
...
function startStageLyricCooperativePrewarm(
  payload,
  key,
  token,
  lightweight,
...
function scheduleStageLyricPrewarmForIndex(targetIndex, reason, delay) {
  ...
  if (hasTarget && lyricsLines && lyricsLines.length)
    prewarmIndex = Math.max(0, Math.min(lyricsLines.length - 1, prewarmIndex));
  ...
  if (lightweight && stageLyricPrewarm.mesh) {
    var guardIndex =
      prewarmIndex != null ? prewarmIndex : chooseStageLyricPrewarmIndex();
    if (stageLyricPrewarmFullCanServeIndex(guardIndex)) {
      upgradeCurrentStageLyricFromPreparedTrack(
        reason || "full-prewarm-before-light",
      );
      return;
    }
  }
  if (!lightweight && stageLyricLightweightPrewarmAwaitingTakeover()) {
    queueStageLyricFullTrackWarmupRetry(
      reason || "track-ready",
      Math.max(96, wait),
    );
    return;
  ...
  stageLyricPrewarm.timer = setTimeout(function () {
    ...
    if (
      lightweight &&
      stageLyricPrewarm.mesh &&
      stageLyricPrewarmFullCanServeIndex(idx)
    ) {
      upgradeCurrentStageLyricFromPreparedTrack(
        reason || "full-prewarm-before-light",
      );
    ...
    var payload = buildStageLyricDisplayPayload(idx, {
      lightweightTrack: lightweight,
    });
    var key = stageLyricPreparedKey(payload);
    ...
    try {
      if (stageLyricShouldBuildCooperatively(payload)) {
        startStageLyricCooperativePrewarm(
          payload,
          key,
          token,
          lightweight,
...
function scheduleStageLyricPrewarm(reason, delay) {
  var singleLineIndex = stageLyricSingleLineBootstrapIndex();
  scheduleStageLyricPrewarmForIndex(
    singleLineIndex >= 0 ? singleLineIndex : null,
    reason,
    delay,
  );
}
...
function stageLyricFullTrackWarmupDelay(delay, reason) {
  ...
  if (
    /lightweight-upgrade|track-ready-fast|playback-resume|playback-started|startup-restore/i.test(
      reasonText,
    )
  )
  ...
  return Math.max(
    base,
    760 + extra + lineCount * 38 + (translationMode === "multi" ? 260 : 0),
  );
}
...
function queueStageLyricFullTrackWarmupRetry(reason, delay) {
  ...
  if (
    stageLyricFullTrackWarmupTimer &&
    stageLyricFullTrackWarmupTargetAt &&
...
function runStageLyricFullTrackWarmup(reason) {
  if (stageLyricCurrentUsesPersistentTrack()) {
    ensureStageLyricPersistentTrackRows(
      stageLyrics.current,
      stageLyrics.currentIdx,
      { reason: reason || "persistent-track-warmup" },
    );
    return true;
  }
  ...
  var run = function () {
    ...
    scheduleStageLyricPrewarm(
      reason || "track-ready",
      stageLyricMultiLineWarmupLoad() ? 96 : 24,
    );
  };
  if (stageLyricMultiLineWarmupLoad() && window.requestIdleCallback) {
    stageLyricFullTrackWarmupIdle = window.requestIdleCallback(
      function () {
        stageLyricFullTrackWarmupIdle = 0;
        run();
      },
...
function showStageLine(text, redrawOnly, options) {
  ...
  if (
    !redrawOnly &&
    stageLyrics.current &&
    setLyricTrackTarget(stageLyrics.current, payload)
  ) {
    stageLyrics.currentText = payload.text;
    stageLyrics.currentDisplayKey = payload.key;
    stageLyrics.currentPayload = payload;
    stageLyrics.current.userData.enterDirection =
      lineStep > 0 ? -1 : lineStep < 0 ? 1 : 0;
    stageLyrics.current.userData.lastTrackSwitchAt =
      uniforms && uniforms.uTime ? uniforms.uTime.value : 0;
    return true;
  }
  if (
    options.noSyncBuild &&
    shouldDeferStageLyricSyncBuild(payload, redrawOnly)
  ) {
  ...
  var mesh =
    takeStageLyricSingleLinePrewarmMesh(payload) ||
    takeStageLyricPrewarmMesh(payload);
  if (!mesh) {
    var singleLineBoundaryNoSyncBuild =
      options.noSyncBuild &&
      singleLinePayload &&
      !redrawOnly &&
      stageLyrics.current;
  ...
  if (
    !redrawOnly &&
    singleLinePayload &&
    typeof markRenderInteraction === "function"
  ) {
    markRenderInteraction("lyric-swap", 360);
  }
  var outgoingMesh = stageLyrics.current;
  var holdOutgoingForReveal =
    !redrawOnly && stageLyricShouldHoldOutgoingForReveal(outgoingMesh, mesh);
  ...
  if (redrawOnly && stageLyrics.current) {
    ...
    stageLyrics.current.userData.lyricRevealSuccessor = holdOutgoingForReveal
      ? mesh
      : null;
  ...
  if (!redrawOnly) {
    var primeAmount = singleLinePayload
      ? 0
      : Math.abs(lineStep) > 0
        ? 0.34
        : 0.24;
...
function refreshCurrentLyricStyle() {
  ...
  if (stageLyrics.currentIdx >= 0 && stageLyricMultiLineWarmupLoad()) {
    redrawPayload =
      buildStageLyricDisplayPayload(stageLyrics.currentIdx, {
...
function clearStageLyrics() {
  if (typeof invalidateLyricQualityTextures === "function")
    invalidateLyricQualityTextures("clear-stage-lyrics");
  cancelStageLyricResidentBuild();
  clearStageLyricFullTrackWarmup();
  disposeStageLyricPrewarmMesh();
  clearStageLyricSingleLinePrewarmCache();
  ...
  stageLyrics.renderSignature = "";
  stageLyrics.transitionLineStep = 0;
  while (stageLyrics.outgoing.length)
    disposeLyricMesh(stageLyrics.outgoing.pop());
}
...
function updateStageLyrics3D(dt) {
  ...
  var lyricGlowStrength = fx.lyricGlow
    ...
    : 0;
  var glowDrive = Math.min(1.7, Math.max(0, lyricGlowStrength / 0.5));
  var glowBreath = lyricGlowStrength > 0 ? 0.5 + 0.5 * Math.sin(t * 1.05) : 0;
  var musicBloom = Math.max(lyricSunEnergy, beatPulse * 0.1);
  var beatGlowRaw =
    fx.lyricGlowBeat && lyricGlowStrength > 0
      ? Math.max(
          beatPulse * 1.22,
          beatCam.punch * 0.86 + beatCam.radiusKick * 1.85,
        )
      : 0;
  stageLyrics.beatGlow +=
    (beatGlowRaw - stageLyrics.beatGlow) *
    (beatGlowRaw > stageLyrics.beatGlow ? 0.32 : 0.1);
  if (!isFinite(stageLyrics.beatGlow)) stageLyrics.beatGlow = 0;
  var skullLyricPreset = !!(fx && fx.preset === SKULL_PRESET_INDEX);
  var sonicLyricPreset = !!(
    typeof SONIC_PRESET_INDEX !== "undefined" &&
    fx &&
    fx.preset === SONIC_PRESET_INDEX
  );
  var solarBloom =
    lyricGlowStrength > 0
      ? (0.18 +
          glowBreath * 0.16 +
          musicBloom * 0.9 +
          stageLyrics.beatGlow * 1.18 +
          Math.sin(t * 0.37 + 1.2) * 0.035) *
        glowDrive
      : 0;
  if (skullLyricPreset && lyricGlowStrength > 0) {
    solarBloom =
      (0.035 +
        glowBreath * 0.03 +
        musicBloom * 0.11 +
        Math.pow(Math.max(0, stageLyrics.beatGlow), 1.26) * 1.45 +
        Math.pow(Math.max(0, skullBeatFlash || 0), 1.08) * 1.18) *
      glowDrive;
  }
  solarBloom = Math.max(0, Math.min(1.45, solarBloom));
  stageLyrics.highBloom +=
    (solarBloom - stageLyrics.highBloom) *
    (solarBloom > stageLyrics.highBloom
      ? skullLyricPreset
        ? 0.22
        : 0.075
  ...
  var followDrive =
    fx.lyricGlowBeat && lyricGlowStrength > 0
      ? Math.min(1.35, stageLyrics.beatGlow)
      : 0;
  var followXTarget =
    followDrive * (beatCam.thetaKick * 34 + beatCam.rollKick * 8);
  var followYTarget =
    followDrive * (beatCam.phiKick * 42 - beatCam.radiusKick * 0.48);
  var followRollTarget =
    followDrive * (beatCam.rollKick * 22 + beatCam.thetaKick * 10);
  ...
  stageLyrics.glowFollowRoll +=
    (followRollTarget - stageLyrics.glowFollowRoll) * 0.22;
  stageLyrics.glowFollowX *= 0.92;
  stageLyrics.glowFollowY *= 0.92;
  stageLyrics.glowFollowRoll *= 0.9;
  ...
  var skullMouthLyrics = !!(
    camera &&
    fx &&
    fx.preset === SKULL_PRESET_INDEX &&
    skullParticleGroup &&
    skullParticleGroup.visible
  );
  var shelfDetailOpen = !!(
    shelfManager &&
  ...
  var shelfDetailLyricProfile = shelfDetailOpen
    ? {
        opacity: skullShelfDetailOpen ? 0.3 : 0.38,
        readability: skullShelfDetailOpen ? 0.2 : 0.26,
        bloom: skullShelfDetailOpen ? 0.2 : 0.24,
        glowCap: skullShelfDetailOpen ? 0.05 : 0.07,
  ...
  if (wallpaperLyricLock) {
    layoutScale *= wallpaperShelfLyrics ? 0.6 : 0.84;
    layoutX = clampRange(
      layoutX + (wallpaperShelfLyrics ? -1.34 : 0),
      -4.0,
      4.0,
    );
    layoutY = clampRange(
      layoutY + (wallpaperShelfLyrics ? -0.04 : 0.08),
      -2.4,
      2.7,
    );
    layoutZ = clampRange(
      layoutZ + (wallpaperShelfLyrics ? 1.02 : 1.15),
      -3.2,
      3.2,
    );
    ...
    layoutScale *= normalShelfDetailOpen ? 0.56 : 0.7;
    layoutX = clampRange(
      layoutX - (normalShelfDetailOpen ? 1.78 : 1.58),
      -4.0,
      4.0,
    );
    layoutY = clampRange(
      layoutY + (normalShelfDetailOpen ? 0.18 : 0.08),
      -2.4,
      2.7,
    );
  ...
  var skullLyricEdgeGuard = !!(
    fx &&
    fx.preset === SKULL_PRESET_INDEX &&
    (orbit.centerLocked || orbit.recentering)
  );
  var lockFit =
    cameraLockedLyrics || skullLyricEdgeGuard || skullMouthLyrics
      ? lyricCameraLockFit(
          layoutScale,
          layoutX,
          layoutY,
          skullMouthLyrics ? Math.max(2.2, 4.4 + layoutZ) : lockDistance,
        )
      : 1;
  ...
  stageLyrics.lockFitScale +=
    (lockFit - stageLyrics.lockFitScale) *
    (lockFit < stageLyrics.lockFitScale ? 0.18 : 0.1);
  ...
  if (skullMouthLyrics) {
    ...
    skullLyricMouthTarget
      .copy(skullLyricMouthLocal)
      .applyMatrix4(skullParticleGroup.matrixWorld);
    ...
    skullLyricMouthTarget.addScaledVector(skullLyricMouthForward, 0.02);
    ...
    stageLyricTargetQuaternion(
      skullLyricReadableQuat,
      layoutTiltX,
      layoutTiltY,
    );
    ...
    if (!stageLyrics.group.userData.skullMouthLocked) {
      ...
      stageLyrics.group.quaternion.slerp(lyricTargetQuat, 0.3);
    }
  } else if (cameraLockedLyrics) {
    if (stageLyrics.group.userData)
      stageLyrics.group.userData.skullMouthLocked = false;
    setStageLyricViewBasisFromCameraOrQuaternion(null);
    lyricLayoutBase
      .copy(camera.position)
      .addScaledVector(lyricCameraDir, lockBaseDistance);
    ...
    if (stageLyrics.snapCameraLockFrames > 0) {
      ...
      var lockPosEase = wallpaperLyricLock
        ? wallpaperShelfLyrics
          ? 0.42
          : 0.34
        : 0.24;
      var lockQuatEase = wallpaperLyricLock
        ? wallpaperShelfLyrics
          ? 0.44
          : 0.36
        : 0.22;
  ...
  function tickMesh(mesh, isCurrent) {
    ...
    if (!isCurrent && mesh.userData && mesh.userData.lyricRevealSuccessor) {
      ...
      if (
        ...
        stageLyricTrackRevealReady(revealSuccessor)
      ) {
    ...
    var a = Math.min(
      1,
      mesh.userData.age / (isCurrent ? lyricMotion.enter : lyricMotion.exit),
    );
    a = a * a * (3 - 2 * a);
    var data = mesh.userData.lyric || {};
    if (previewMotionLock) {
      if (!mesh.userData.progressPreviewMotionLocked) {
        ...
        mesh.userData.progressPreviewHoldRotationZ =
      ...
      if (
        isFinite(Number(mesh.userData.progressPreviewMotionBlend)) &&
        Number(mesh.userData.progressPreviewMotionBlend) < 1
      ) {
        mesh.userData.progressPreviewMotionBlend = Math.min(
          1,
          Number(mesh.userData.progressPreviewMotionBlend) + dt / 0.32,
        );
      } else {
        mesh.userData.progressPreviewMotionBlend = 1;
      }
    }
    var previewMotionBlend = previewMotionLock
      ? 0
      : clampRange(Number(mesh.userData.progressPreviewMotionBlend), 0, 1);
    if (
      isCurrent &&
      data.trackPersistent &&
      data.trackPendingPayload &&
      stageLyricPersistentTargetRowsReady(
        mesh,
        data.trackPendingPayload.trackIndex,
      )
    ) {
      commitStageLyricPersistentPendingTarget(mesh);
    }
    var lineStepWorld = clampRange(
      Number(data.lineWorldStep) || lyricMotion.slide,
      0.2,
      0.94,
    );
    ...
    var glitchAmount =
      !previewMotionLock && style === "glitch"
        ? clampRange(Number(lyricMotion.glitch) || 0, 0, 1.5)
        : 0;
    var glitchSlice =
      style === "glitch"
        ? clampRange(Number(lyricMotion.glitchSlice) || 0, 0, 1.4)
        : 0;
    var glitchRate =
      style === "glitch"
        ? clampRange(Number(lyricMotion.glitchRate) || 1, 0.45, 2.2)
        : 1;
    var glitchJitter =
      style === "glitch"
        ? clampRange(Number(lyricMotion.glitchJitter) || 0, 0, 1.8)
        : 0;
    var glitchCameraDrive = 0;
    var glitchPulse = 0;
    if (glitchAmount > 0) {
      var beatShake = clampRange(
        Math.abs(beatCam.rollKick) * 1.5 +
          Math.abs(beatCam.thetaKick) * 0.8 +
          Math.abs(beatCam.radiusKick) * 1.25 +
          beatCam.punch * 1.05,
        0,
        1.35,
      );
      var rawBeatDrive = clampRange(
        Math.max(beatPulse * 1.12, stageLyrics.beatGlow * 0.86, beatShake) *
          (0.86 + clampRange(Number(fx && fx.cinemaShake) || 0, 0, 1.8) * 0.1),
        0,
        1.55,
      );
      var beatFollowDrive = lyricMotion.glitchCameraBind ? rawBeatDrive : 0;
      var rhythmGate = lyricMotion.glitchCameraBind
        ? clampRange((beatFollowDrive - 0.14) / 0.58, 0, 1)
        : 1;
      glitchCameraDrive = beatFollowDrive;
      if (
        !isFinite(mesh.userData.glitchNextAt) ||
        mesh.userData.glitchNextAt <= 0
      ) {
        mesh.userData.glitchNextAt =
          t + (0.09 + Math.random() * 0.34) / (0.62 + glitchRate * 0.42);
      }
      if (
        lyricMotion.glitchCameraBind &&
        beatFollowDrive > 0.22 &&
        t - (mesh.userData.glitchLastBeatAt || -10) > 0.055
      ) {
        mesh.userData.glitchBurst = Math.max(
          mesh.userData.glitchBurst || 0,
          beatFollowDrive *
            rhythmGate *
            (0.52 + glitchJitter * 0.18 + Math.random() * 0.58),
        );
        mesh.userData.glitchHold = Math.max(
          mesh.userData.glitchHold || 0,
          (0.014 + Math.random() * 0.06) * (0.75 + glitchJitter * 0.2),
        );
        mesh.userData.glitchSeed = Math.random() * 997;
        mesh.userData.glitchLastBeatAt = t;
        mesh.userData.glitchNextAt = Math.min(
          mesh.userData.glitchNextAt,
          t + (0.055 + Math.random() * 0.16),
        );
      }
      if (t >= mesh.userData.glitchNextAt) {
        if (!lyricMotion.glitchCameraBind || beatFollowDrive > 0.3) {
          var randomBurst =
            0.12 +
            Math.pow(Math.random(), 0.52) *
              (lyricMotion.glitchCameraBind ? 0.48 : 0.66);
          ...
          mesh.userData.glitchHold = Math.max(
            mesh.userData.glitchHold || 0,
            (0.01 + Math.random() * 0.076) * (0.8 + glitchJitter * 0.18),
          );
          mesh.userData.glitchSeed = Math.random() * 997;
        }
        mesh.userData.glitchNextAt =
          t +
          (lyricMotion.glitchCameraBind
            ? 0.14 + Math.random() * 0.55
            : 0.075 + Math.random() * 0.52) /
            (0.58 + glitchRate * 0.46);
      }
      mesh.userData.glitchHold = Math.max(
        0,
        (mesh.userData.glitchHold || 0) - dt,
      );
      var decay =
        mesh.userData.glitchHold > 0 ? Math.pow(0.22, dt) : Math.pow(0.018, dt);
      mesh.userData.glitchBurst = Math.max(
        0,
        (mesh.userData.glitchBurst || 0) * decay,
      );
      glitchPulse = clampRange(
        ((mesh.userData.glitchBurst || 0) * rhythmGate +
          beatFollowDrive * (0.16 + glitchJitter * 0.045)) *
          glitchAmount,
        0,
        1.95,
      );
    ...
    if (data.textMat && data.textMat.uniforms) {
      if (data.textMat.uniforms.uGlitchBurst)
        data.textMat.uniforms.uGlitchBurst.value = glitchPulse;
      if (data.textMat.uniforms.uGlitchSeed)
        data.textMat.uniforms.uGlitchSeed.value =
          mesh.userData.glitchSeed || seed || 0;
    }
    var glitchJitterSeed = mesh.userData.glitchSeed || seed;
    var jitterScale = 0.5 + glitchJitter * 0.62;
    var textJitterX = glitchPulse
      ? Math.sin(t * (61.0 + glitchRate * 26.0) + glitchJitterSeed) *
        (0.011 + glitchSlice * 0.023) *
        (0.65 + glitchPulse * 0.55) *
        jitterScale
      : 0;
    var textJitterY = glitchPulse
      ? Math.cos(t * (37.0 + glitchRate * 15.0) + glitchJitterSeed * 1.7) *
        (0.003 + glitchSlice * 0.005) *
        (0.55 + glitchPulse * 0.45) *
        jitterScale
      : 0;
    ...
    if (
      isCurrent &&
      data.textMat &&
      data.textMat.uniforms &&
      data.textMat.uniforms.uProgress
    ) {
      var targetProgress = clampRange(
        Number(mesh.userData.targetLyricProgress) || 0,
        0,
        1,
      );
      shownProgress = clampRange(
        Number(data.textMat.uniforms.uProgress.value) || 0,
        0,
        1,
      );
      if (mesh.userData.nativeKaraokeProgress) {
        ...
        var progressEase =
          Math.abs(progressDiff) > 0.42
            ? Math.max(0.52, lyricMotion.progressEase * 1.55)
            : lyricMotion.progressEase;
        if (mesh.userData.age < 0.16)
          progressEase = Math.max(progressEase, 0.28);
        shownProgress = clampRange(
          shownProgress + progressDiff * progressEase,
          0,
          1,
        );
      }
    ...
    if (data.textMesh) {
      data.textMesh.position.x +=
        (textJitterX - data.textMesh.position.x) * (glitchPulse ? 0.74 : 0.22);
      data.textMesh.position.y +=
        (textJitterY - data.textMesh.position.y) * (glitchPulse ? 0.74 : 0.22);
    }
    if (data.readability) {
      data.readability.position.x +=
        (textJitterX * 0.46 - data.readability.position.x) *
        (glitchPulse ? 0.58 : 0.18);
      data.readability.position.y +=
        (textJitterY * 0.4 - data.readability.position.y) *
        (glitchPulse ? 0.58 : 0.18);
    }
    if (data.context) {
      data.context.position.x +=
        (textJitterX * 0.28 - data.context.position.x) *
        (glitchPulse ? 0.44 : 0.12);
    }
    if (data.glow) {
      var glowFollowX = data.glowFrameLocked ? 0.045 : 0.14;
      var glowFollowY = data.glowFrameLocked ? 0.04 : 0.12;
      data.glow.visible = !data.suppressStaticGlow;
      if (!data.suppressStaticGlow) {
        data.glow.position.set(
          glowX * glowFollowX + textJitterX * 0.32,
          glowY * glowFollowY + textJitterY * 0.36,
          -0.006,
        );
    ...
    if (isCurrent) {
      ...
      if (!isFinite(Number(data.globalOpacity)))
        data.globalOpacity =
          data.textMat &&
          data.textMat.uniforms &&
          data.textMat.uniforms.uOpacity
            ? Number(data.textMat.uniforms.uOpacity.value) || 0
            : 0;
      var currentOpacity = data.globalOpacity;
      var opacityEase =
        shelfDetailOpen && currentOpacity > lyricOpacityTarget
          ? shelfDetailLyricProfile.easeDown
          : 0.16;
      opacity = clampRange(
        currentOpacity + (lyricOpacityTarget - currentOpacity) * opacityEase,
        0,
        1,
      );
      ...
      if (data.readabilityMat) {
        ...
        var readabilityEase =
          shelfDetailOpen && data.readabilityMat.opacity > readabilityTarget
            ? 0.28
            : 0.16;
        data.readabilityMat.opacity +=
          (readabilityTarget - data.readabilityMat.opacity) * readabilityEase;
      }
      ...
      if (data.contextMat) {
        var contextTarget =
          opacity *
          shelfDetailLyricProfile.readability *
          clampRange(0.74 + lyricContextOpacityValue() * 0.24, 0.78, 0.98) *
          contextIntro;
        data.contextMat.opacity +=
          (contextTarget - data.contextMat.opacity) *
          (contextTarget > data.contextMat.opacity ? 0.11 : 0.075);
        if (data.context) {
          data.context.userData = data.context.userData || {};
          if (previewMotionLock) {
            if (!data.context.userData.progressPreviewMotionLocked) {
              data.context.userData.progressPreviewHoldY =
                Number(data.context.position.y) || 0;
              data.context.userData.progressPreviewHoldScale =
                Number(data.context.scale.x) || 1;
              data.context.userData.progressPreviewMotionLocked = true;
            }
            data.context.position.y =
              data.context.userData.progressPreviewHoldY;
            data.context.scale.setScalar(
              data.context.userData.progressPreviewHoldScale,
            );
          } else {
            data.context.userData.progressPreviewMotionLocked = false;
            var contextY =
              ((0.5 - shownProgress) * lyricMotion.contextDrift +
                (style === "float" && verticalFloatOn
                  ? Math.sin(t * 0.84 + seed) * 0.016
                  : 0)) *
                previewMotionBlend +
              (mesh.userData.enterDirection || 0) *
                (1 - a) *
                lineStepWorld *
                0.12;
            var contextScale =
              ...
              (verticalFloatOn
                ...
                : 0);
            data.context.position.y +=
              (contextY - data.context.position.y) * 0.11;
            data.context.scale.setScalar(
              previewMotionBlend < 1
                ? data.context.scale.x +
                    (contextScale - data.context.scale.x) * 0.06
                : contextScale,
            );
          }
        }
      ...
      var currentLineGlow =
        lyricGlowStrength > 0
          ? Math.min(
              shelfDetailLyricProfile.glowCap * 1.05,
              (0.1 +
                solar * 0.4 +
                lyricBeatGlow * 0.24 * shelfDetailLyricDim +
                beatPulse * 0.08) *
                Math.min(2.4, glowDrive) *
                (lyricMotion.glowLift || 1),
            )
          : 0;
      updateLyricRowLayers(data, {
        ...
        contextDrift: lyricMotion.contextDrift + Math.abs(lineStepWorld) * 0.03,
        style: style,
        time: t,
        seed: seed,
        jitterX: textJitterX,
      ...
      if (data.textMat && data.textMat.uniforms.uSolar) {
        ...
        var solarEase =
          shelfDetailOpen && data.textMat.uniforms.uSolar.value > solarTarget
            ? 0.26
            : 0.12;
        data.textMat.uniforms.uSolar.value +=
          (solarTarget - data.textMat.uniforms.uSolar.value) * solarEase;
      }
      solar = stageLyrics.highBloom * shelfDetailLyricDim;
      var warmth = Math.max(0, Math.min(1, solar * 1.1));
      if (data.glowMat) {
        var glowTarget =
          lyricGlowStrength > 0
            ? Math.min(
                shelfDetailLyricProfile.glowCap,
                (0.075 +
                  solar * 0.34 +
                  stageLyrics.beatGlow * 0.16 * shelfDetailLyricDim) *
                  Math.min(3.0, glowDrive) *
                  (lyricMotion.glowLift || 1),
              )
            : 0;
        ...
        data.glowMat.opacity +=
          ...
          (glowTarget > data.glowMat.opacity
            ...
            : shelfDetailOpen
              ...
              : 0.055);
        data.glowMat.color
          .copy(lyricStageGlowThreeColor(stageLyrics.palette, "#9cffdf", 0.36))
          .lerp(lyricSunHotColor, warmth);
      }
      if (data.sparkMat) {
        var sparkTarget =
          lyricGlowStrength > 0 && fx.lyricGlowParticles && !shelfDetailOpen
            ? Math.min(
                0.42,
                (0.1 + solar * 0.14 + stageLyrics.beatGlow * 0.1) *
                  Math.min(1.6, glowDrive),
              )
            : 0;
        var sparkOpacity = getLyricSparkOpacity(data);
        sparkOpacity +=
          (sparkTarget - sparkOpacity) *
          (sparkTarget > sparkOpacity ? 0.13 : shelfDetailOpen ? 0.22 : 0.075);
        setLyricSparkOpacity(data, sparkOpacity);
        var sparkSizeTarget =
          fx.lyricGlowParticles && !shelfDetailOpen
            ? 0.05 + solar * 0.016 + stageLyrics.beatGlow * 0.026 + bass * 0.008
            : 0.035;
        setLyricSparkSize(
          data,
          getLyricSparkSize(data) +
            (sparkSizeTarget - getLyricSparkSize(data)) * 0.12,
        );
        var sparkColor = lyricSunHotColor
          .clone()
          .lerp(lyricSunColor, 0.22 + solar * 0.18);
        setLyricSparkColor(data, sparkColor);
      }
      if (data.sunMat) {
        var sunTarget =
          lyricGlowStrength > 0 && !shelfDetailOpen
            ? Math.min(
                0.88,
                (Math.pow(Math.min(1.35, solar), 1.08) * 0.28 +
                  stageLyrics.beatGlow * 0.2) *
                  Math.min(2.4, glowDrive) *
                  (style === "shine" ? 1.18 : 1),
              )
            : 0;
        ...
        data.sunMat.opacity +=
          (sunTarget - data.sunMat.opacity) * (shelfDetailOpen ? 0.18 : 0.055);
        data.sunMat.color
          .copy(lyricSunColor)
          .lerp(lyricSunHotColor, solar * 0.55);
      }
      if (data.sun) {
        var sunPulse = solar;
        ...
        data.sun.scale.set(
          0.82 +
            sunPulse * 0.36 +
            beatScale +
            Math.sin(t * 1.6) * sunPulse * 0.018,
          0.6 +
            sunPulse * 0.34 +
            beatScale * 0.72 +
            Math.cos(t * 1.25) * sunPulse * 0.02,
          1,
        );
        data.sun.rotation.z += Math.sin(t * 0.32 + seed) * 0.01 * sunPulse;
      }
      var breathe =
        (Math.sin(t * 0.92 + seed) * 0.05 +
          Math.sin(t * 0.41 + seed * 0.7) * 0.028) *
        lyricFloatAmp *
        previewMotionBlend;
      if (previewMotionLock) {
        mesh.position.set(
        ...
        mesh.scale.setScalar(
          Number(mesh.userData.progressPreviewHoldScale) || 1,
        );
        mesh.rotation.z =
          Number(mesh.userData.progressPreviewHoldRotationZ) || 0;
      } else if (skullMouthLyrics) {
        var mouthMeshY =
          -0.07 +
          (Math.sin(t * 0.5 + seed) * 0.018 +
            Math.sin(t * 1.12 + seed) * 0.006) *
            previewMotionBlend;
        var mouthMeshZ =
          0.018 + Math.cos(t * 0.46 + seed) * 0.007 * previewMotionBlend;
        var mouthMeshScale =
          1.08 + a * 0.04 + breathe * 0.12 + bass * 0.024 + beatPulse * 0.014;
        ...
        mesh.scale.setScalar(
          previewMotionBlend < 1
            ? mesh.scale.x + (mouthMeshScale - mesh.scale.x) * 0.06
            : mouthMeshScale,
        );
        var mouthRotationTarget =
          (Math.sin(t * 0.3 + seed) * 0.01 +
            textJitterX * 0.2 +
            glitchCameraDrive * glitchAmount * 0.012) *
          previewMotionBlend;
        mesh.rotation.z =
          previewMotionBlend < 1
            ? mesh.rotation.z + (mouthRotationTarget - mesh.rotation.z) * 0.18
            : mouthRotationTarget;
      } else {
        mesh.userData.skullMouthMeshLocked = false;
        var rootScaleTarget =
          0.96 + a * 0.055 + breathe + bass * 0.038 + beatPulse * 0.014;
        mesh.scale.setScalar(
          previewMotionBlend < 1
            ? mesh.scale.x + (rootScaleTarget - mesh.scale.x) * 0.06
            : rootScaleTarget,
        );
        if (singleLineSwap) {
          mesh.position.y +=
            (0.18 +
              (verticalFloatOn
                ? (Math.sin(t * 0.55 + seed) * 0.055 +
                    Math.sin(t * 1.35 + seed) * 0.014) *
                  previewMotionBlend
                : 0) -
              mesh.position.y) *
            0.075;
          mesh.position.z +=
            (1.48 +
              (verticalFloatOn
                ? Math.cos(t * 0.48 + seed) * 0.08 * previewMotionBlend
                : 0) -
              mesh.position.z) *
            0.08;
        } else {
          ...
          mesh.position.y +=
            (0.2 +
              ...
              progressLift +
              (verticalFloatOn
                ? (Math.sin(t * 0.55 + seed) * 0.046 +
                    Math.sin(t * 1.35 + seed) * 0.012) *
                  previewMotionBlend
                : 0) -
              mesh.position.y) *
            (enterDir ? 0.115 : 0.08);
          mesh.position.z +=
            (1.48 -
              Math.abs(enterDir) * 0.045 * (1 - a) +
              (verticalFloatOn
                ? Math.cos(t * 0.48 + seed) * 0.07 * previewMotionBlend
                : 0) -
              mesh.position.z) *
            0.09;
        }
        var rootRotationTarget =
          (Math.sin(t * 0.34 + seed) *
            (style === "smooth" ? 0.006 : style === "float" ? 0.026 : 0.018) +
            textJitterX * 0.18 +
            glitchCameraDrive * glitchAmount * 0.014) *
          previewMotionBlend;
        mesh.rotation.z =
          previewMotionBlend < 1
            ? mesh.rotation.z + (rootRotationTarget - mesh.rotation.z) * 0.18
            : rootRotationTarget;
      }
      if (data.sparks && data.sparkMat)
        data.sparks.visible =
          fx.lyricGlowParticles || getLyricSparkOpacity(data) > 0.015;
      if (data.sparks && data.basePositions) {
        var pos = data.sparks.geometry.attributes.position;
        var arr = pos.array,
          base = data.basePositions;
        data.sparks.rotation.z +=
          ((fx.lyricGlowParticles ? 0.0009 : 0.00025) +
            stageLyrics.beatGlow * 0.0007) *
          (dt * 60);
        ...
        for (var si = 0; si < arr.length / 3; si++) {
          ...
          var dustBreath = fx.lyricGlowParticles
            ? 0.62 + 0.38 * Math.sin(t * (0.32 + (si % 7) * 0.025) + s)
            : 0.18;
          var drift = fx.lyricGlowParticles ? 1 : 0.3;
          arr[si * 3] =
            base[si * 3] +
            Math.sin(t * (0.18 + (si % 5) * 0.025) + s) *
              (0.045 + bass * 0.03 + particleBeat * 0.052) *
              drift +
            Math.cos(t * 0.11 + s) * 0.018 * dustBreath;
          arr[si * 3 + 1] =
            base[si * 3 + 1] +
            Math.cos(t * (0.16 + (si % 6) * 0.024) + s) *
              (0.042 + mid * 0.026 + particleBeat * 0.046) *
              drift +
            Math.sin(t * 0.13 + s) * 0.016 * dustBreath;
          arr[si * 3 + 2] =
            base[si * 3 + 2] +
            Math.sin(t * (0.24 + (si % 4) * 0.035) + s) *
              (0.036 + particleBeat * 0.028) *
              drift;
        }
    ...
    opacity = holdingForLyricReveal
      ? clampRange(
          Number(data.globalOpacity) || shelfDetailLyricProfile.opacity,
          0,
          1,
        )
    ...
    if (data.readabilityMat)
      data.readabilityMat.opacity =
        opacity *
        (shelfDetailOpen ? shelfDetailLyricProfile.readability : 0.58);
    if (data.contextMat)
      data.contextMat.opacity =
        opacity *
        (shelfDetailOpen
          ? shelfDetailLyricProfile.readability * 0.72
          : clampRange(0.46 + lyricContextOpacityValue() * 0.26, 0.54, 0.76));
    updateLyricRowLayers(data, {
      opacity: opacity,
    ...
    if (data.textMat && data.textMat.uniforms.uSolar)
      data.textMat.uniforms.uSolar.value *= shelfDetailOpen ? 0.72 : 0.86;
    if (data.glowMat)
      data.glowMat.opacity =
        !data.suppressStaticGlow && lyricGlowStrength > 0
          ? shelfDetailOpen
            ? Math.min(
                shelfDetailLyricProfile.glowCap * 0.4,
                opacity * 0.05 * lyricGlowStrength,
              )
            : opacity * 0.08 * lyricGlowStrength
          : 0;
    if (data.sparkMat) {
      var outgoingSpark =
        lyricGlowStrength > 0 && fx.lyricGlowParticles && !shelfDetailOpen
          ? Math.max(
              opacity * 0.24 * lyricGlowStrength,
              (1 - a) * 0.18 * lyricGlowStrength,
            )
          : 0;
      setLyricSparkOpacity(data, outgoingSpark);
      setLyricSparkSize(data, 0.046 + (1 - a) * 0.02);
    }
    if (data.sunMat)
      data.sunMat.opacity =
        !data.suppressStaticGlow && lyricGlowStrength > 0 && !shelfDetailOpen
          ? opacity * 0.08 * lyricGlowStrength
          : 0;
    if (previewMotionLock) {
      mesh.position.set(
    ...
    if (!isFinite(mesh.userData.exitStartY))
      mesh.userData.exitStartY = mesh.position.y;
    if (!isFinite(mesh.userData.exitStartZ))
      mesh.userData.exitStartZ = mesh.position.z;
    var exitDir = mesh.userData.exitDirection || 0;
    mesh.position.y +=
      (mesh.userData.exitStartY +
        exitDir * lineStepWorld * 1.02 * a +
        0.05 * a -
        mesh.position.y) *
      (lyricMotion.style === "quick" ? 0.24 : 0.18);
    mesh.position.z +=
      (mesh.userData.exitStartZ - 0.24 * a - mesh.position.z) * 0.16;
    mesh.scale.setScalar(0.98 - a * 0.06);
    return a < 1;
  }
  ...
  if (typeof finalizeLyricQualitySelectionFrame === "function")
    finalizeLyricQualitySelectionFrame();
}

var lyricKaraokeMeasureCanvas = null;
function lyricKaraokeMeasureContext() {
  if (!lyricKaraokeMeasureCanvas)
    lyricKaraokeMeasureCanvas = document.createElement("canvas");
...
function lyricKaraokeWordRanges(line) {
  ...
  if (line._karaokeMetricKey === key && Array.isArray(line._karaokeWordRanges))
    return line._karaokeWordRanges;
  var ctx = lyricKaraokeMeasureContext();
  var text = String(line.text || "");
  var fontSize = 128;
  var fullWidth = Math.max(
    1,
    lyricMeasureTextAtSize(ctx, text, fontSize, lyricFontWeightValue()),
  );
...
function getLyricLineProgress(line, nextLine, now) {
  ...
  var nextT =
    nextLine && nextLine.t > line.t
      ? nextLine.t
      : Math.min(
          (audio && audio.duration) || now + 4,
          line.t + (line.duration || 4.8),
        );
...
function stageLyricContextEntry(index, currentIndex) {
  ...
  var farAlpha =
    mode === "cinema" ? contextOpacity * 0.64 : contextOpacity * 0.52;
  var nearScale = mode === "cinema" ? 0.9 : 0.88;
...
function makeStageLyricTranslationEntry(entry, isCurrent) {
  ...
  return {
    ...
    alpha: isCurrent
      ? clampRange(lyricTranslationOpacityValue() + 0.08, 0.48, 1)
      : clampRange(baseAlpha * 0.62, 0.24, 0.6),
    scale: isCurrent
      ? clampRange(scale * 1.08, 0.7, 1.12)
      : clampRange(scale * 0.92, 0.5, 0.96),
    weight: 650,
    lineOffset: 0,
    parentIndex: parentIndex,
    lineIndex:
      entry && entry.lineIndex != null ? Number(entry.lineIndex) : undefined,
    virtualIndex: lyricTranslationVirtualIndex(parentIndex),
  };
}
function applyLyricTranslationModeToEntries(
  entries,
  activeLine,
  maxRowsOverride,
) {
  ...
  if (mode === "dual") {
    if (entries.length <= 1)
      return { entries: [activeEntry, activeTranslation], activeLine: 0 };
    var dualOut = [];
...
function applyLyricTranslationModeToTrackEntries(
  entries,
  activeLine,
  maxRowsOverride,
) {
...
function stageLyricTrackKeyForMode(mode) {
  ...
  var last =
    lyricsLines && lyricsLines.length
      ? lyricsLines[lyricsLines.length - 1]
      : null;
  var song =
    Array.isArray(playQueue) && currentIdx >= 0 && playQueue[currentIdx]
      ? playQueue[currentIdx]
      : null;
  var songKey = song
    ? typeof songProviderKey === "function"
      ? songProviderKey(song)
      : String(song.id || song.mid || song.name || "")
    : "";
  return [
    "track",
    songKey,
...
function lyricMeshTrackWindow(index, mode, options) {
  ...
  if (lightweightTrack) {
    ...
    var lightFullTrackLimit = hasTranslations
      ...
      : denseMultiLine
        ? 10
        : 14;
    if (total <= lightFullTrackLimit)
      return { start: 0, end: last, lightweight: true };
    var lightPageSize =
      Math.ceil(
        lineCount *
          (hasTranslations
            ...
            : denseMultiLine
              ? 0.96
              : 1.12),
      ) + (hasTranslations ? (denseMultiLine ? 4 : 5) : denseMultiLine ? 5 : 6);
    if (mode === "cinema" || mode === "custom")
      lightPageSize += Math.ceil(lineCount * 0.3);
    var lightMin = denseMultiLine
      ? Math.max(lineCount + 2, hasTranslations ? 8 : 9)
      : hasTranslations
        ? 9
        : 10;
    var lightMax = denseMultiLine
      ? Math.max(lightMin, hasTranslations ? lineCount + 4 : lineCount + 6)
      : hasTranslations
        ? 18
        : 24;
    lightPageSize = Math.max(lightMin, Math.min(total, lightPageSize));
    lightPageSize = Math.min(lightPageSize, lightMax);
    var lightOverlap = Math.max(
      2,
      Math.ceil(
        lineCount *
          (hasTranslations
    ...
    lightOverlap = Math.min(Math.floor(lightPageSize * 0.3), lightOverlap);
    ...
    if (lightEnd - lightStart + 1 < lightPageSize && lightStart > 0)
      lightStart = Math.max(0, lightEnd - lightPageSize + 1);
    return { start: lightStart, end: lightEnd, lightweight: true };
  }
  var denseFullMultiLine =
    mode !== "single" ||
    translationMode === "multi" ||
    translationMode === "dual";
  ...
  var pageSize = hasTranslations
    ...
    : denseFullMultiLine
      ...
      : 216;
  if (mode === "cinema" || mode === "custom")
    pageSize += Math.ceil(lineCount * 2.2);
  pageSize = Math.max(
    pageSize,
    Math.ceil(lineCount * (hasTranslations ? 7.2 : 6.2)) +
      (hasTranslations ? 42 : 52),
  );
  pageSize = Math.max(denseFullMultiLine ? 160 : 96, Math.min(total, pageSize));
  var overlap = Math.max(
    denseFullMultiLine ? 64 : 36,
    Math.ceil(lineCount * (hasTranslations ? 3.4 : 2.8)) + 22,
  );
  ...
  if (end - start + 1 < pageSize && start > 0)
    start = Math.max(0, end - pageSize + 1);
  return { start: start, end: end };
}
function buildStageLyricMeshTrackEntries(index, mode, options) {
  mode = normalizeLyricDisplayMode(mode);
  if (!lyricsLines || !lyricsLines.length || index < 0)
    return { entries: [], activeLine: 0, start: 0, end: -1 };
  ...
  if (!entries.length)
    return { entries: [], activeLine: 0, start: start, end: end };
  var translated = applyLyricTranslationModeToTrackEntries(
    entries,
    activeLine,
    entries.length * 2 + 2,
  ...
  for (var ri = 0; ri < translated.entries.length; ri++) {
    ...
    if (
      ...
      lineMap[Number(row.lineIndex)] = ri;
  }
  return {
    entries: translated.entries,
    activeLine: isFinite(
      Number(lineMap[Math.max(0, Math.round(Number(index) || 0))]),
    )
      ? Number(lineMap[Math.max(0, Math.round(Number(index) || 0))])
      : translated.activeLine,
    start: start,
    end: end,
...
function buildStageLyricTrackEntries(index, mode) {
  ...
  if (
    stageLyricTrackCache &&
    stageLyricTrackCache.key === cacheKey &&
    Array.isArray(stageLyricTrackCache.entries)
  ) {
    var cachedLine =
      stageLyricTrackCache.lineMap &&
      stageLyricTrackCache.lineMap[Math.max(0, Math.round(Number(index) || 0))];
    return {
...
function buildStageLyricDisplayPayload(index, options) {
  ...
  var translated = applyLyricTranslationModeToEntries(
    entries,
    activeLine,
    entries.length * 2 + 2,
  );
...
function buildStageLyricPlaybackPayload(index) {
  ...
  if (multiLineWarmup) {
    ...
    var lightweightPayload = buildStageLyricDisplayPayload(index, {
      lightweightTrack: true,
    });
    if (
      needsFirstLineTakeover &&
      (shouldStartLightweight ||
        stageLyricPrewarmCanServePayload(lightweightPayload))
    )
      return lightweightPayload;
    if (
      currentIsLightweight &&
      lightweightPayload &&
      stageLyricMeshCanServePayload(stageLyrics.current, lightweightPayload)
    )
      return lightweightPayload;
    ...
    if (
      currentIsLightweight &&
      fullPayload &&
      !stageLyricPrewarmCanServePayload(fullPayload) &&
      lightweightPayload
    )
      return lightweightPayload;
    return fullPayload;
...
function resetStageLyricResumeFrameGates() {
  var now = stageLyricNowMs();
  if (
    typeof resetFrameGate === "function" &&
    typeof mainFrameGates !== "undefined" &&
    mainFrameGates
  ) {
...
function markStageLyricsPlaybackResume(reason) {
  ...
  var repeatedResume =
    idx >= 0 &&
    idx === stageLyricResumeWarmupLastIndex &&
    now - stageLyricResumeWarmupLastAt < 760;
  ...
  if (currentIsLightweight) {
    stageLyricResumeUpgradeDeferUntil = Math.max(
      Number(stageLyricResumeUpgradeDeferUntil) || 0,
      now + 520,
    );
...
function tickLyricsParticles() {
  ...
  if (newIdx < 0) {
    ...
    if (
      ...
      stageLyrics.currentText !== introText
    ) {
      stageLyrics.currentIdx = -2;
      stageLyrics.transitionLineStep = 0;
      ...
      if (lyricsLines && lyricsLines.length) {
        ...
        scheduleStageLyricPrewarmForIndex(0, "intro-first-line", 24);
        if (typeof scheduleStageLyricFullTrackWarmup === "function")
          scheduleStageLyricFullTrackWarmup("track-ready", 180);
      }
    }
    if (stageLyrics.current) {
      ...
      var introEnd =
        firstLine && firstLine.t > 0
          ? firstLine.t
          : Math.min((audio && audio.duration) || 4.8, 4.8);
      var introLine = {
        t: 0,
        text: introText,
      ...
      updateLyricMeshProgress(
        stageLyrics.current,
        getLyricLineProgress(introLine, null, lyricT),
      );
    }
    return;
  }
  if (
    (stageLyrics.currentIdx < 0 || !stageLyrics.current) &&
    stageLyricWarmupPending()
  ) {
    return;
  }
  var displayPayload = null;
  if (
    newIdx === stageLyrics.currentIdx &&
    stageLyrics.current &&
    stageLyrics.currentPayload
  ) {
  ...
  if (
    newIdx !== stageLyrics.currentIdx ||
    stageLyrics.currentDisplayKey !== displayPayload.key
  ) {
    stageLyrics.transitionLineStep =
      stageLyrics.currentIdx >= 0 && newIdx >= 0
    ...
    if (
      !displayed &&
      displayPayload &&
      !displayPayload.trackLightweight &&
      stageLyricMultiLineWarmupLoad()
    ) {
      requestStageLyricDemandPrewarm(displayPayload);
      var lightweightFallback = buildStageLyricDisplayPayload(newIdx, {
        lightweightTrack: true,
      });
  ...
  if (stageLyrics.current) {
    ...
    if (
      stageLyricPayloadIsSingleLine(displayPayload) &&
      (displayedNewLine || progress > 0.35)
    ) {
      scheduleStageLyricSingleLineNextPrewarm(
        newIdx,
        lyricT,
        displayedNewLine ? "single-line-next" : "single-line-tail",
      );
    }
  }
}
...
function disposeLyricsParticles() {
  ...
  if (stageLyrics.starRiver) {
    if (stageLyrics.starRiver.parent)
      stageLyrics.starRiver.parent.remove(stageLyrics.starRiver);
    if (stageLyrics.starRiver.geometry)
      stageLyrics.starRiver.geometry.dispose();
    if (stageLyrics.starRiver.material)
      stageLyrics.starRiver.material.dispose();
