...
var LYRIC_CAMERA_LOCK_MAX_SCALE = 0.8;
...
function stageLyricTargetQuaternion(baseQuat, tiltX, tiltY) {
  ...
  return lyricTargetQuat
    .copy(baseQuat || lyricBaseQuat)
    .multiply(lyricTiltQuat);
}
function getStageLyricLockBounds() {
  ...
  function take(mesh) {
    ...
    var meshScale = Math.max(
      mesh.scale && isFinite(mesh.scale.x) ? mesh.scale.x : 1,
      mesh.scale && isFinite(mesh.scale.y) ? mesh.scale.y : 1,
    );
...
function lyricCameraLockFit(layoutScale, layoutX, layoutY, distance) {
  ...
  var visibleW =
    visibleH * (camera.aspect || innerWidth / Math.max(1, innerHeight) || 1.78);
  var bounds = getStageLyricLockBounds();
  var skullSafe = !!(fx && fx.preset === SKULL_PRESET_INDEX);
  var safeW = Math.max(
    visibleW * (skullSafe ? 0.36 : 0.42),
    visibleW * (skullSafe ? 0.7 : 0.84) -
      Math.abs(layoutX || 0) * (skullSafe ? 1.36 : 1.22),
  );
  var safeH = Math.max(
    visibleH * (skullSafe ? 0.16 : 0.18),
    visibleH * (skullSafe ? 0.34 : 0.44) -
      Math.abs(layoutY || 0) * (skullSafe ? 0.98 : 0.82),
  );
  ...
  var lockScaleCap = Math.min(
    1,
    (skullSafe ? 0.94 : LYRIC_CAMERA_LOCK_MAX_SCALE) / layoutScale,
  );
  return clampRange(
    Math.min(viewportFit, lockScaleCap),
    skullSafe ? 0.36 : 0.42,
    1,
  );
}
