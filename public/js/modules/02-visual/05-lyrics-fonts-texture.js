...
function customLyricFontRecordForKey(key) {
  ...
  for (var i = 0; i < customLyricFonts.length; i++) {
    if (customLyricFonts[i] && customLyricFonts[i].id === id)
      return customLyricFonts[i];
  }
  return null;
}
...
function normalizeCustomLyricFontRecord(raw) {
  ...
  if (
    !id ||
    !/^data:(font\/(ttf|otf|woff2?|sfnt)|application\/(font-woff|x-font-ttf|x-font-otf|octet-stream)|application\/vnd\.ms-fontobject);base64,/i.test(
      dataUrl,
    )
  )
    return null;
  return {
    ...
    family:
      String(raw.family || "MineradioCustomLyricFont-" + id)
        .replace(/["\\]/g, "")
        .slice(0, 72) || "MineradioCustomLyricFont-" + id,
    dataUrl: dataUrl,
...
function readCustomLyricFonts() {
  try {
    var raw = JSON.parse(
      localStorage.getItem(CUSTOM_LYRIC_FONT_STORE_KEY) || "[]",
    );
    if (!Array.isArray(raw)) return [];
    return raw
      .map(normalizeCustomLyricFontRecord)
      .filter(Boolean)
...
function saveCustomLyricFonts() {
  try {
    localStorage.setItem(
      ...
      JSON.stringify(customLyricFonts || []),
    );
    return true;
  } catch (e) {
...
function registerCustomLyricFont(record) {
  ...
  if (!record || typeof FontFace !== "function" || !document.fonts)
    return Promise.resolve(false);
  if (source && source.loaded && source.id === record.id)
...
function registerSavedCustomLyricFonts() {
  ...
  customLyricFonts.forEach(function (record) {
    registerCustomLyricFont(record);
  });
}
...
function lyricFontStackForKey(key) {
  ...
  if (customFont)
    return (
      quotedCssFontFamily(customFont.family) +
  ...
  if (key === "round")
    return '"HarmonyOS Sans SC","Microsoft YaHei UI","PingFang SC","Noto Sans SC",sans-serif';
  if (key === "mono")
...
function lyricFontWeightValue() {
  ...
  return (
    Math.round(clampRange(Number(fx && fx.lyricWeight) || 900, 500, 900) / 50) *
    50
  );
}
function lyricFontCss(fontSize, weight) {
  var w =
    weight == null
      ...
      : Math.round(
          clampRange(Number(weight) || lyricFontWeightValue(), 500, 900) / 50,
        ) * 50;
  return w + " " + fontSize + "px " + lyricFontStackForKey(fx && fx.lyricFont);
}
function lyricLetterSpacingPx(fontSize) {
  return (
    clampRange(Number(fx && fx.lyricLetterSpacing) || 0, -0.04, 0.18) *
    Math.max(1, fontSize || 1)
  );
}
function lyricLineHeightFactor() {
  return clampRange(Number(fx && fx.lyricLineHeight) || 1, 0.72, 1.8);
}
var lyricTextMeasureCache = {
  fonts: {},
  order: [],
  maxFonts: 64,
  maxCharsPerFont: 512,
...
function lyricMeasuredCharacterWidth(ctx, character) {
  ...
  if (Object.prototype.hasOwnProperty.call(cache.values, character))
    return cache.values[character];
  while (cache.order.length >= lyricTextMeasureCache.maxCharsPerFont)
    delete cache.values[cache.order.shift()];
...
function warmLyricTextMeasureCache() {
  ...
  measureTextWithLetterSpacing(
    ctx,
    "歌词 Lyrics 0123456789",
    lyricLetterSpacingPx(128),
  );
}
...
function measureTextWithLetterSpacing(ctx, text, spacing) {
  ...
  if ("fontKerning" in ctx && "letterSpacing" in ctx) {
    ...
    try {
      ...
      var glyphWidth =
        ctx.measureText(text).width - probeSpacing * chars.length;
      if (isFinite(glyphWidth))
        return Math.max(1, glyphWidth + spacing * (chars.length - 1));
    } finally {
...
function lyricMeasureText(ctx, text, fontSize) {
  return measureTextWithLetterSpacing(
    ctx,
    text,
...
function drawTextWithLetterSpacing(ctx, text, x, y, spacing, stroke) {
  ...
  for (var i = 0; i < chars.length; i++) {
    ...
    cursor +=
      lyricMeasuredCharacterWidth(ctx, chars[i]) +
      (i < chars.length - 1 ? spacing : 0);
  }
  ctx.textAlign = align;
}
...
function applyStonePrintTexture(ctx, W, H, fontSize, randomFn) {
  ...
  for (var p = 0; p < noiseW * noiseH; p++) {
    ...
    var vein =
      Math.sin(x0 * 0.19 + y0 * 0.043) * 0.1 + Math.sin(y0 * 0.31) * 0.06;
    var r = random() + vein;
    var a = 0;
  ...
  for (var c = 0; c < 26; c++) {
    ...
    var radius = 1.8 + random() * (size * 0.06);
