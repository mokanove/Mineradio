function lyricLineCenterWorldY(mask, entry, lineIndex, worldH) {
  ...
  var y0 = isFinite(Number(mask.lineY0))
    ? Number(mask.lineY0)
    : h / 2 + fontSize * 0.36;
  var baseline =
    y0 + lineIndex * lineHeight + lyricEntryLineOffset(entry) * lineHeight;
...
function lyricRowVirtualIndex(entry, fallbackIndex) {
  entry = entry || {};
  if (
    entry.translationLine &&
    entry.parentIndex != null &&
    isFinite(Number(entry.parentIndex))
  )
    return lyricTranslationVirtualIndex(entry.parentIndex);
  if (entry.lineIndex != null && isFinite(Number(entry.lineIndex)))
    return lyricPrimaryVirtualIndex(entry.lineIndex);
  if (entry.virtualIndex != null && isFinite(Number(entry.virtualIndex)))
    return Number(entry.virtualIndex);
...
function lyricLayerVirtualIndex(entry, fallbackIndex, activeLine, usesTrack) {
  ...
  if (!usesTrack) {
    var localActive =
      activeLine != null && isFinite(Number(activeLine))
        ? Number(activeLine)
        : 0;
    if (entry.translationLine && entry.parentRole === "current")
      return localActive + lyricTranslationVisualGapValue();
    return Number(fallbackIndex) || 0;
  }
...
function lyricTranslationLineStepWorld(mask, worldH) {
  ...
  return clampRange(step, 0.2, 0.78);
}

function lyricTranslationAnchoredY(
  entry,
  fallbackIndex,
  activeLine,
  lineStepWorld,
  ...
  if (!usesTrack) {
    var rowVirtualLocal =
      entry.virtualIndex != null && isFinite(Number(entry.virtualIndex))
        ? Number(entry.virtualIndex)
        : lyricLayerVirtualIndex(entry, fallbackIndex, activeLine, false);
    var localOffset =
      scrollOffset == null || !isFinite(Number(scrollOffset))
        ? activeLine
        : Number(scrollOffset);
    var localDelta = rowVirtualLocal - localOffset;
    var localSign = localDelta >= 0 ? 1 : -1;
    var parentDeltaLocal =
      localDelta - localSign * lyricTranslationVisualGapValue();
    var parentAbsLocal = Math.abs(parentDeltaLocal);
    var parentDriftLocal = currentTranslation
      ? 0
      : (Number(rowDrift) || 0) *
        clampRange(0.7 + parentAbsLocal * 0.1, 0.65, 1.2);
    return (
      -parentDeltaLocal * lineStepWorld +
      parentDriftLocal -
      localSign * lyricTranslationVisualGapValue() * translationLineStepWorld
    );
  }
  var parentIndex =
    entry.parentIndex != null && isFinite(Number(entry.parentIndex))
      ? Number(entry.parentIndex)
      : entry.lineIndex != null && isFinite(Number(entry.lineIndex))
        ? Number(entry.lineIndex)
        : null;
  var parentVirtual =
    parentIndex != null ? lyricPrimaryVirtualIndex(parentIndex) : activeLine;
  var rowVirtual =
    entry.virtualIndex != null && isFinite(Number(entry.virtualIndex))
      ? Number(entry.virtualIndex)
  ...
  var parentDrift = currentTranslation
    ? 0
    : (Number(rowDrift) || 0) * clampRange(0.7 + parentAbs * 0.1, 0.65, 1.2);
  var sign = rowVirtual >= parentVirtual ? 1 : -1;
  return (
    -parentDelta * lineStepWorld +
    parentDrift -
    sign * lyricTranslationVisualGapValue() * translationLineStepWorld
  );
}
...
function lyricNearestPrimaryLineIndexForVirtual(
  virtualIndex,
  fallbackLineIndex,
) {
  var count =
    typeof lyricsLines !== "undefined" && lyricsLines ? lyricsLines.length : 0;
  if (!count || !isFinite(Number(virtualIndex)))
    return Math.max(0, Math.round(Number(fallbackLineIndex) || 0));
  ...
  return Math.abs(lyricPrimaryVirtualIndex(lower) - value) <=
    Math.abs(lyricPrimaryVirtualIndex(upper) - value)
    ? lower
    : upper;
}
...
function lyricRowDepthScale(entry, index, activeLine) {
  var abs = Math.min(
    5.5,
    Math.abs(lyricRowVisualDelta(entry, index, activeLine)),
  );
...
function makeLyricBackfaceReadableMaterial(opts) {
  opts = opts || {};
  var color =
    opts.color && opts.color.isColor
      ? opts.color.clone()
      : new THREE.Color(opts.color == null ? 0xffffff : opts.color);
    ...
    uniforms: {
      ...
      uOpacity: {
        value:
          opts.opacity == null
            ? 0
            : clampRange(Number(opts.opacity) || 0, 0, 1),
      },
    },
    vertexShader: [
      "varying vec2 vUv;",
...
function getLyricTextureMaterialOpacity(mat) {
  if (mat && mat.uniforms && mat.uniforms.uOpacity)
    return Number(mat.uniforms.uOpacity.value) || 0;
...
function setLyricTextureMaterialColor(mat, color) {
  ...
  if (
    ...
    mat.uniforms.uColor.value.copy
  )
...
function lyricBackgroundAdaptStrengthValue() {
  var fallback =
    fxDefaults && isFinite(Number(fxDefaults.lyricBackgroundAdapt))
      ? Number(fxDefaults.lyricBackgroundAdapt)
      : 0;
  var value =
    fx && fx.lyricBackgroundAdapt != null
      ? Number(fx.lyricBackgroundAdapt)
      : fallback;
...
function lyricReadabilityColorForBrightBackdrop(strength) {
  ...
  if (!lyricReadabilityLightColor)
    lyricReadabilityLightColor = new THREE.Color(0xffffff);
  if (!lyricReadabilityDarkColor)
    lyricReadabilityDarkColor = new THREE.Color(0x04070c);
  if (!lyricReadabilityMixColor)
    lyricReadabilityMixColor = new THREE.Color(0xffffff);
  return lyricReadabilityMixColor
    .copy(lyricReadabilityLightColor)
    .lerp(lyricReadabilityDarkColor, clampRange(strength * 0.92, 0, 0.92));
}
...
function makeLyricLineMask(entry, baseMask, asActive) {
  ...
  return compactLyricLineMaskTexture(
    makeLyricMask(
      {
        mode: "single",
        key:
          ...
          (drawEntry.role || "") +
          "|" +
          Math.round((drawEntry.scale || 1) * 1000) +
          "|" +
          drawEntry.text,
        activeLine: 0,
        entries: [drawEntry],
      },
      {
        ...
        lineHeight:
          baseMask && (baseMask.logicalLineHeight || baseMask.lineHeight),
      },
    ),
  );
}
...
function lyricTranslationMeshScale(entry) {
  ...
  var scale = isFinite(Number(entry.scale))
    ? Number(entry.scale)
    : lyricTranslationScaleValue();
  var defaultScale =
    fxDefaults && isFinite(Number(fxDefaults.lyricTranslationScale))
      ? Number(fxDefaults.lyricTranslationScale)
      : 0.78;
  var roleBoost = entry.parentRole === "current" ? 1.08 : 0.92;
  var defaultEntryScale = clampRange(
    defaultScale * roleBoost,
    entry.parentRole === "current" ? 0.7 : 0.5,
    entry.parentRole === "current" ? 1.12 : 0.96,
  );
...
function lyricRowLogicalWorldWidth(mask, baseWorldW) {
  mask = mask || {};
  baseWorldW = Math.max(0.1, Number(baseWorldW) || 6.1);
  var logicalWidth = Math.max(
    1,
    Number(mask.logicalWidth) || Number(mask.width) || 2048,
  );
...
function makeLyricRowGlowMesh(row, pal, worldW, preparedGlowTexture) {
  ...
  pal = pal || {};
  worldW = Math.max(0.1, Number(worldW) || 6.1);
  var lineMask = row.lineMask;
  var lineWorldH = Math.max(
    0.05,
    Number(row.lineWorldH) ||
      worldW * ((lineMask.height || 1) / Math.max(1, lineMask.width || 1)),
  );
  var lineTextWorldW =
    worldW *
    ((lineMask.activeTextWidth || lineMask.textWidth || lineMask.width) /
      Math.max(1, lineMask.width || 1));
  lineTextWorldW = clampRange(lineTextWorldW, worldW * 0.1, worldW * 1.0);
  ...
  var rowGlowTex =
    ...
    makeLyricGlowTexture(
      row.text || "",
      glowRaster.fontSize,
      glowRaster.textWidth,
      lineMask.lines,
      glowRaster.lineHeight,
      lineMask.fitScaleX,
      lineMask.entries,
      lineMask.activeLine,
      null,
  ...
  var rowGlowTextPx = Math.max(
    1,
    rowGlowMeta.textWidth ||
      lineMask.activeTextWidth ||
      lineMask.textWidth ||
      lineMask.width ||
      1,
  );
  var rowGlowTextureRatio = Math.max(
    1,
    (rowGlowMeta.width || rowGlowTextPx) / rowGlowTextPx,
  );
  ...
  var rowGlowWorldW = clampRange(
    Math.max(
      ...
      lineTextWorldW *
        Math.max(rowGlowTextureRatio, row.isTranslation ? 1.04 : 1.08),
    ),
    lineTextWorldW + rowGlowPad * 0.62,
    worldW * (row.isTranslation ? 1.0 : 1.08),
  );
  ...
  var rowGlowAspect = Math.max(
    0.001,
    (rowGlowMeta.height || lineMask.height || 1) /
      Math.max(1, rowGlowMeta.width || lineMask.width || 1),
  );
  var rowGlowWorldH = clampRange(
    rowGlowWorldW * rowGlowAspect,
    lineWorldH * (row.isTranslation ? 0.56 : 0.66),
    lineWorldH * (row.isTranslation ? 1.12 : 1.36),
  );
  ...
  glow.position.set(row.baseX || 0, row.baseY || 0, (row.baseZ || 0) - 0.03);
  glow.scale.setScalar(row.baseScale || 1);
  return { glow: glow, glowMat: glowMat };
}

function beginLyricRowLayerGroupBuild(
  payload,
  mask,
  worldW,
  worldH,
  ...
  var usesTrack = !!(
    ...
    isFinite(Number(payload.trackIndex))
  );
  var activeLineIndex = usesTrack
    ? Number(payload.trackIndex)
    : payload
      ? payload.activeLine
      : 0;
  var activeLine = usesTrack
    ? lyricPrimaryVirtualIndex(activeLineIndex)
    : activeLineIndex;
  ...
  var visibleRadius = Math.max(
    0.85,
    displayLineCount * 0.5 * lyricPrimarySlotStepValue(),
  );
  ...
  var readabilityPhaseCount =
    typeof LYRIC_READABILITY_BUILD_PHASES === "number"
      ? LYRIC_READABILITY_BUILD_PHASES
      : 3;
  var glowPhaseCount =
    typeof LYRIC_GLOW_BUILD_PHASES === "number" ? LYRIC_GLOW_BUILD_PHASES : 6;
  var totalPhases = 0;
  for (var pi = 0; pi < entries.length; pi++) {
    var phaseEntry = entries[pi] || {};
    var phaseHasGlow =
      !phaseEntry.translationLine ||
      phaseEntry.parentRole === "current" ||
      usesTrack;
    totalPhases += textOnly
      ? 1
      : 1 + readabilityPhaseCount + (phaseHasGlow ? glowPhaseCount : 1);
  }
...
function beginLyricRowLayerBuildEntry(state) {
  ...
  var virtualIndex = lyricLayerVirtualIndex(
    entry,
    i,
    state.activeLine,
    state.usesTrack,
  ...
  var isActive =
    !entry.translationLine &&
    (state.usesTrack
      ? entryLineIndex === state.activeLineIndex
  ...
  if (entry.translationLine) {
    var translationLayoutEntry = !state.usesTrack
      ...
      : entry;
    lineY = lyricTranslationAnchoredY(
      translationLayoutEntry,
      i,
      state.activeLine,
      state.lineStepWorld,
      state.translationLineStepWorld,
      state.activeLine,
      0,
      Math.abs(delta) < 0.001,
      state.usesTrack,
  ...
  var material;
  if (!entry.translationLine) {
    material = makeLyricShaderMaterial(
      lineMask,
      state.pal,
      state.motionProfile,
    );
    material.uniforms.uOpacity.value = 0;
    if (material.uniforms.uActiveMix)
      material.uniforms.uActiveMix.value = isActive ? 1 : 0;
  } else {
    material = makeLyricBackfaceReadableMaterial({
      ...
      color: entry.translationLine
        ? lyricThreeColor(
            state.pal.highlight || state.pal.primary,
            "#eaf6ff",
            0.42,
          )
        : lyricThreeColor(
            state.pal.primary || state.pal.secondary,
            "#d6f8ff",
            0.34,
          ),
    });
  ...
  var targetAlpha =
    entry.alpha == null ? 1 : clampRange(Number(entry.alpha), 0, 1);
...
function appendLyricRowReadabilityLayer(state) {
  ...
  if (!row.readabilityBuild)
    row.readabilityBuild = beginLyricReadabilityTextureBuild(row.lineMask);
  var readabilityDone = stepLyricReadabilityTextureBuild(row.readabilityBuild);
  state.lastPhase =
    "row-readability-" + (row.readabilityBuild.lastPhase || "step");
  state.completedPhases += 1;
  ...
  var readability = new THREE.Mesh(
    new THREE.PlaneGeometry(
      row.lineWorldW || state.worldW,
      row.lineWorldH,
      1,
      1,
    ),
...
function finishLyricRowLayerBuildEntry(state) {
  ...
  var glow = null;
  var glowMat = null;
  var shouldCreateRowGlow =
    !row.isTranslation || row.parentRole === "current" || state.usesTrack;
  if (shouldCreateRowGlow) {
    if (!row.glowBuild) {
      var glowRaster = lyricGlowRasterMetrics(row.lineMask);
      row.glowBuild = beginLyricGlowTextureBuild(
        row.text || "",
        glowRaster.fontSize,
        glowRaster.textWidth,
        row.lineMask.lines,
        glowRaster.lineHeight,
        row.lineMask.fitScaleX,
        row.lineMask.entries,
        row.lineMask.activeLine,
        null,
        glowRaster.scale,
      );
    }
...
function appendLyricRowLayerBuildPhase(state) {
  ...
  if (state.pendingPhase === "readability")
    return appendLyricRowReadabilityLayer(state);
...
function appendLyricRowLayerBuildEntry(state) {
  ...
  while (!state.done && state.cursor === startCursor)
...
function makeLyricRowLayerGroup(
  payload,
  mask,
  worldW,
  worldH,
  ...
  while (!stepLyricRowLayerGroupBuild(state, 1024, Infinity)) {
    /* synchronous compatibility path */
  }
  return finishLyricRowLayerGroupBuild(state);
}

var lyricRenderUploadFrameBudget = {
  frame: 0,
  remaining: 1,
  consumed: 0,
  maxConsumed: 0,
...
function lyricQualityDisposeTexture(texture) {
  if (!texture) return;
  if (typeof disposeOwnedLyricTexture === "function")
    disposeOwnedLyricTexture(texture);
...
function lyricQualityRememberRow(row) {
  if (row && lyricQualityResidentIndex(row) < 0)
    lyricQualityState.residents.push(row);
}
...
function lyricQualityOwnerActive(data, row) {
  return !!(
    ...
    (!row || data.rowLayers.indexOf(row) >= 0)
  );
}
...
function lyricQualityHasPendingTexture() {
  for (var i = 0; i < lyricQualityState.residents.length; i++) {
    if (
      lyricQualityState.residents[i] &&
      lyricQualityState.residents[i].qualityPendingTexture
    )
      return true;
  }
  return false;
...
function setLyricRowTextureMap(row, texture) {
  ...
  if (row.mat.uniforms && row.mat.uniforms.uMap)
    row.mat.uniforms.uMap.value = texture;
...
function lyricQualityRecount() {
  ...
  for (var i = lyricQualityState.residents.length - 1; i >= 0; i--) {
    ...
    bytes +=
      Math.max(0, Number(row.qualityBytes) || 0) +
      Math.max(0, Number(row.qualityPendingBytes) || 0);
  }
...
function updateLyricQualityStats(tier) {
  ...
  for (var i = 0; i < lyricQualityState.residents.length; i++) {
    ...
    if (texture && texture.image)
      targetWidth = Math.max(targetWidth, Number(texture.image.width) || 0);
  }
  window.__mineradioLyricQualityStats = {
    tier: clampRange(
      Math.round(Number(tier) || lyricTextureClarityScale()),
      1,
      4,
    ),
...
function releaseLyricRowQuality(row, restoreBase) {
  ...
  if (
    restoreBase !== false &&
    row.baseLineTexture &&
    lyricQualityCurrentMap(row) !== row.baseLineTexture
  ) {
  ...
  if (qualityTexture && qualityTexture !== row.baseLineTexture)
    lyricQualityDisposeTexture(qualityTexture);
  if (
    pendingTexture &&
    pendingTexture !== row.baseLineTexture &&
    pendingTexture !== qualityTexture
  )
    lyricQualityDisposeTexture(pendingTexture);
  lyricQualityRecount();
}
...
function discardLyricRowPendingQuality(row) {
  ...
  if (
    pendingTexture &&
    pendingTexture !== row.baseLineTexture &&
    pendingTexture !== row.qualityTexture
  )
    lyricQualityDisposeTexture(pendingTexture);
}
...
function invalidateLyricQualityTextures(reason, options) {
  ...
  var releaseCommitted =
    ...
    /clear-stage-lyrics|hard-release/i.test(String(reason || ""));
  ...
  if (lyricQualityState.idle) {
    if (typeof cancelIdleCallback === "function")
      cancelIdleCallback(lyricQualityState.idle);
    lyricQualityState.idle = 0;
  }
  for (
    var queuedIndex = 0;
    queuedIndex < lyricQualityState.queue.length;
    queuedIndex++
  ) {
    var queuedRow =
      lyricQualityState.queue[queuedIndex] &&
      lyricQualityState.queue[queuedIndex].row;
    if (!queuedRow) continue;
    queuedRow.qualityGeneration =
      (Number(queuedRow.qualityGeneration) || 0) + 1;
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    row.qualityHotUntil = Math.max(
      Number(row.qualityHotUntil) || 0,
      fallbackHotUntil,
    );
    row.qualityFallbackUntil = Math.max(
      Number(row.qualityFallbackUntil) || 0,
      fallbackHotUntil,
    );
...
function lyricQualityEffectiveBudgetBytes(tier, oneReplacementBytes) {
  ...
  if (Number(lyricQualityState.transitionBudgetUntil) > now) {
    budget = Math.max(
      budget,
      Math.max(0, Number(lyricQualityState.transitionBaseBytes) || 0) +
        Math.max(0, Number(oneReplacementBytes) || 0),
    );
...
function lyricQualityEffectiveMaxRows() {
  return (
    lyricQualityMaxResidentRows() +
    (Number(lyricQualityState.transitionBudgetUntil) > lyricQualityNowMs()
      ? 1
      : 0)
  );
}
...
function lyricQualityEnsureCapacity(extraBytes, pinnedRow, tier) {
  ...
  var atomicReplacement = !!(
    pinnedResident &&
    pinnedRow &&
    pinnedRow.qualityTexture &&
    Number(pinnedRow.qualitySelectionFrame) ===
      lyricQualityState.selectionFrame &&
  ...
  var candidates = lyricQualityState.residents.filter(function (row) {
    return (
      ...
      Number(row.qualityFallbackUntil) <= now
    );
  });
  candidates.sort(function (a, b) {
    return (
      (Number(a.qualityLastUsedAt) || 0) - (Number(b.qualityLastUsedAt) || 0)
    );
  });
  while (
    (lyricQualityState.bytes + extraBytes > budget ||
      (!pinnedResident && lyricQualityState.residents.length >= maxRows)) &&
    candidates.length
  ) {
  ...
  if (atomicReplacement) {
    return (
      pinnedResident && Number(pinnedRow.qualityProjectedPoolBytes) <= budget
    );
  }
  return (
    lyricQualityState.bytes + extraBytes <= budget &&
...
function scheduleLyricQualityBuild(delay) {
  if (
    lyricQualityState.timer ||
    lyricQualityState.idle ||
    !lyricQualityState.queue.length
  )
    return;
  if (delay > 0) {
  ...
  var run = function (deadline) {
    ...
    lyricQualityState.queue.sort(function (a, b) {
    ...
    if (
      !row ||
      job.globalGeneration !== lyricQualityState.generation ||
      job.rowGeneration !== row.qualityGeneration ||
      row.qualityQueuedKey !== job.key ||
      row.qualityWanted !== true ||
      Number(row.qualityHotUntil) <= now ||
      !lyricQualityOwnerActive(job.data, row)
    ) {
      if (row && row.qualityQueuedKey === (job && job.key))
        row.qualityQueuedKey = "";
      scheduleLyricQualityBuild(0);
      return;
    }
    ...
    if (
      ...
      job.rowGeneration !== row.qualityGeneration ||
      row.qualityWanted !== true ||
      Number(row.qualityHotUntil) <= lyricQualityNowMs() ||
      !lyricQualityOwnerActive(job.data, row)
    ) {
    ...
    if (
      row.qualityPendingTexture &&
      row.qualityPendingTexture !== row.qualityTexture
    )
      lyricQualityDisposeTexture(row.qualityPendingTexture);
...
function queueLyricRowQuality(data, row, tier, priority) {
  if (
    !lyricQualityOwnerActive(data, row) ||
    !row.lineMask ||
    tier <= 1 ||
    row.qualityWanted !== true ||
    Number(row.qualityHotUntil) <= lyricQualityNowMs()
  ...
  var key =
    ...
    target.height +
    "|" +
    (Number(row.lineMask.stoneSeed) || 0);
  if (
    (row.qualityTexture &&
      row.qualityTier === tier &&
      row.qualityRasterKey === key) ||
    (row.qualityPendingTexture &&
      row.qualityPendingTier === tier &&
      row.qualityPendingKey === key) ||
...
function pruneLyricQualityQueue(now) {
  ...
  lyricQualityState.queue = lyricQualityState.queue.filter(function (job) {
    ...
    if (!valid && row && row.qualityQueuedKey === (job && job.key))
      row.qualityQueuedKey = "";
    return valid;
  });
}
...
function disposeLyricQualityOwner(data) {
  ...
  lyricQualityState.frameCandidates = lyricQualityState.frameCandidates.filter(
    function (candidate) {
      return (
        candidate && candidate.data !== data && rows.indexOf(candidate.row) < 0
      );
    },
  );
  lyricQualityState.frameCommits = lyricQualityState.frameCommits.filter(
    function (candidate) {
      return (
        candidate && candidate.data !== data && rows.indexOf(candidate.row) < 0
      );
  ...
  lyricQualityState.queue = lyricQualityState.queue.filter(function (job) {
    ...
    if (remove && job.row && job.row.qualityQueuedKey === job.key)
      job.row.qualityQueuedKey = "";
    return !remove;
  });
...
function commitLyricRowQuality(row) {
  if (!row || lyricQualityState.deferFinalize || row.qualityWanted !== true)
    return false;
  if (row.qualityPendingTexture) {
    ...
    if (
      previous &&
      previous !== row.qualityTexture &&
      previous !== row.baseLineTexture
    )
      lyricQualityDisposeTexture(previous);
...
function beginLyricQualitySelectionFrame(deferFinalize) {
  ...
  for (
    var residentIndex = 0;
    residentIndex < lyricQualityState.residents.length;
    residentIndex++
  ) {
  ...
  for (
    var queueIndex = 0;
    queueIndex < lyricQualityState.queue.length;
    queueIndex++
  ) {
    var queuedRow =
      lyricQualityState.queue[queueIndex] &&
      lyricQualityState.queue[queueIndex].row;
    if (queuedRow) queuedRow.qualityWanted = false;
  }
}

function registerLyricQualityCandidates(
  data,
  candidates,
  tier,
  rootPriority,
...
function registerLyricQualityCommitCandidate(data, row, priority) {
  if (
    ...
    (!row.qualityPendingTexture &&
      (!row.qualityTexture ||
        lyricQualityCurrentMap(row) === row.qualityTexture))
  )
    return;
...
function commitDeferredLyricQualityRows() {
  ...
  var commits = lyricQualityState.frameCommits.slice().sort(function (a, b) {
    return a.priority - b.priority;
  });
  var seen = [];
  for (var i = 0; i < commits.length; i++) {
    ...
    if (
      !row ||
      seen.indexOf(row) >= 0 ||
      !lyricQualityOwnerActive(candidate.data, row)
    )
      continue;
    seen.push(row);
...
function lyricQualityCandidateTarget(candidate) {
  ...
  var target = lyricQualityTargetMetrics(
    candidate.row.lineMask,
    candidate.tier,
  );
  ...
  return {
    ...
    key:
      ...
      target.height +
      "|" +
      (Number(candidate.row.lineMask.stoneSeed) || 0),
  };
}
...
function finalizeLyricQualitySelectionFrame() {
  ...
  if (tier <= 1) {
    ...
    for (var baseIndex = 0; baseIndex < baseRows.length; baseIndex++)
      releaseLyricRowQuality(baseRows[baseIndex], true);
    pruneLyricQualityQueue(now);
    updateLyricQualityStats(tier);
    return;
  }
  var sorted = lyricQualityState.frameCandidates.slice().sort(function (a, b) {
    return a.priority - b.priority;
  });
  var candidates = [];
  var candidateRows = [];
  for (var sortedIndex = 0; sortedIndex < sorted.length; sortedIndex++) {
    ...
    if (
      !sortedCandidate ||
      !lyricQualityOwnerActive(sortedCandidate.data, sortedCandidate.row) ||
      candidateRows.indexOf(sortedCandidate.row) >= 0
    )
      continue;
  ...
  function rememberDesired(row, projectedBytes) {
    if (!row || desiredRows.indexOf(row) >= 0 || desiredRows.length >= maxRows)
      return false;
  ...
  var fallbackRows = lyricQualityState.residents
    .filter(function (row) {
      return (
        row &&
        (row.qualityTexture || row.qualityPendingTexture) &&
    ...
    .sort(function (a, b) {
      return (
        (Number(b.qualityLastUsedAt) || 0) - (Number(a.qualityLastUsedAt) || 0)
      );
    });
  for (
    var fallbackIndex = 0;
    fallbackIndex < fallbackRows.length;
    fallbackIndex++
  ) {
    var fallbackRow = fallbackRows[fallbackIndex];
    rememberDesired(
      fallbackRow,
      Math.max(0, Number(fallbackRow.qualityBytes) || 0) +
        Math.max(0, Number(fallbackRow.qualityPendingBytes) || 0),
    );
  }
  ...
  for (
    ...
    selectionIndex < candidates.length;
    selectionIndex++
  ) {
    ...
    var nextLargestSelectionItem = Math.max(
      largestSelectionItem,
      selectionBytes,
    );
    if (
      desiredProjectedBytes + selectionBytes >
      lyricQualityEffectiveBudgetBytes(tier, nextLargestSelectionItem)
    )
      continue;
    if (rememberDesired(selectionCandidate.row, selectionBytes))
      largestSelectionItem = nextLargestSelectionItem;
  }
  var residentRows = lyricQualityState.residents.slice();
  for (
    var releaseIndex = 0;
    releaseIndex < residentRows.length;
    releaseIndex++
  ) {
    var releaseRow = residentRows[releaseIndex];
    if (desiredRows.indexOf(releaseRow) < 0)
      releaseLyricRowQuality(releaseRow, true);
  }
  lyricQualityRecount();
  var buildCandidates = [];
  for (
    ...
    var selectedProjectedBytes =
      desiredRowIndex >= 0
        ? desiredRowProjectedBytes[desiredRowIndex]
        : targetBytes;
    var replacementProjectedPoolBytes = Math.max(
      0,
      desiredProjectedBytes - selectedProjectedBytes + targetBytes,
    );
    ...
    if (
      replacementProjectedPoolBytes >
      lyricQualityEffectiveBudgetBytes(tier, targetBytes)
    )
      continue;
  ...
  for (
    var desiredIndex = 0;
    desiredIndex < desiredRows.length;
    desiredIndex++
  ) {
    var desiredRow = desiredRows[desiredIndex];
    if (!desiredRow) continue;
    desiredRow.qualityWanted = true;
    if (Number(desiredRow.qualityFallbackUntil) > now)
      desiredRow.qualityHotUntil = Math.max(
        Number(desiredRow.qualityHotUntil) || 0,
        Number(desiredRow.qualityFallbackUntil),
      );
  }
  for (var buildIndex = 0; buildIndex < buildCandidates.length; buildIndex++) {
    var buildCandidate = buildCandidates[buildIndex];
    queueLyricRowQuality(
      buildCandidate.data,
      buildCandidate.row,
      buildCandidate.tier,
      buildCandidate.priority,
    );
  }
  pruneLyricQualityQueue(now);
...
function consumeLyricRenderUploadFrameBudget() {
  ...
  lyricRenderUploadFrameBudget.maxConsumed = Math.max(
    lyricRenderUploadFrameBudget.maxConsumed,
    lyricRenderUploadFrameBudget.consumed,
  );
  if (typeof window !== "undefined") {
    window.__mineradioLyricUploadBudgetStats = {
      frame: lyricRenderUploadFrameBudget.frame,
...
function updateLyricRowLayers(data, opts) {
  ...
  var contextIntro =
    opts.contextIntro == null
      ? 1
      : clampRange(Number(opts.contextIntro) || 0, 0, 1);
  ...
  var displayMode = normalizeLyricDisplayMode(
  ...
  var motionBlend = previewMotionLock
    ? 0
    : clampRange(
        opts.motionBlend == null ? 1 : Number(opts.motionBlend) || 0,
        0,
        1,
      );
  ...
  var verticalFloatOn =
    !previewMotionLock &&
    motionBlend > 0.001 &&
    (typeof lyricVerticalFloatEnabled === "function"
      ? lyricVerticalFloatEnabled()
      : true);
  var baseEase =
    opts.ease == null ? 0.16 : clampRange(Number(opts.ease) || 0.16, 0.04, 1);
  var frameDelta = Number(opts.deltaTime);
  var frameScale =
    isFinite(frameDelta) && frameDelta > 0
      ? clampRange(frameDelta * 60, 0.25, 3)
      : 1;
  ...
  var pendingPayload =
    data.trackPersistent && data.trackPendingPayload
      ? data.trackPendingPayload
      : null;
  var pendingTargetLineIndex =
    pendingPayload &&
    pendingPayload.trackIndex != null &&
    isFinite(Number(pendingPayload.trackIndex))
  ...
  var targetIndex =
    pendingTargetLineIndex != null
      ...
      : opts.targetVirtualIndex != null &&
          isFinite(Number(opts.targetVirtualIndex))
  ...
  var baseTrackEase =
    opts.trackEase == null
      ? clampRange(baseEase * 1.16, 0.08, 0.34)
      : clampRange(Number(opts.trackEase) || 0.18, 0.04, 0.6);
  var trackEase = 1 - Math.pow(1 - baseTrackEase, frameScale);
  var nowMs =
    typeof performance !== "undefined" && performance.now
      ? performance.now()
      : Date.now();
  var trackTargetJustCommitted = !!(
    data.trackPersistent &&
    isFinite(Number(data.trackTargetCommittedAt)) &&
    nowMs - Number(data.trackTargetCommittedAt) <= 34
  );
  var initialTextRevealPending = !!(
    data.usesTrack && data.renderInitialTextReady !== true
  );
  var textureEffectsAllowed = !initialTextRevealPending;
  var persistentTrackTransparentPrewarm = !!(
    data.usesTrack &&
    data.trackPersistent &&
    !initialTextRevealPending
  );
  var visibleRadiusForSnap = Math.max(
    1.2,
    Number(data.trackVisibleRadius) || 3,
  );
  ...
  var persistentPrimedTrack = !!(
    data.trackPersistent && data.trackScrollPrimed
  );
  var continuousTrackSlotStep = 1;
  if (data.usesTrack && persistentPrimedTrack) {
    var trackLineCount =
      typeof lyricsLines !== "undefined" && lyricsLines
        ? lyricsLines.length
        : 0;
    var neighborLineIndex =
      targetLineIndex < Math.max(0, trackLineCount - 1)
        ? targetLineIndex + 1
        : Math.max(0, targetLineIndex - 1);
    continuousTrackSlotStep = Math.abs(
      lyricPrimaryVirtualIndex(neighborLineIndex) -
        lyricPrimaryVirtualIndex(targetLineIndex),
    );
    if (!isFinite(continuousTrackSlotStep) || continuousTrackSlotStep < 0.25)
      continuousTrackSlotStep = 1;
  }
  var needsScrollSnap =
    !isFinite(currentScrollOffset) ||
    (!persistentPrimedTrack &&
      !previewMotionLock &&
      ((isFinite(Number(data.trackScrollSnapUntil)) &&
        nowMs <= Number(data.trackScrollSnapUntil)) ||
        Math.abs(targetIndex - currentScrollOffset) >
          Math.max(3.2, visibleRadiusForSnap * 1.85)));
  if (needsScrollSnap) {
    ...
    if (data.usesTrack && persistentPrimedTrack) {
      ...
      var continuousTrackMaxStep =
        continuousTrackSlotStep * continuousTrackMaxRowsPerFrame;
      trackStep = clampRange(
        trackStep,
        -continuousTrackMaxStep,
        continuousTrackMaxStep,
      );
    }
  ...
  var displayedTrackOffset =
    typeof stageLyricResidentDisplayedScrollOffset === "function"
  ...
  var previewTrackCorridor = !!(
    ...
    data.usesTrack &&
    data.trackPersistent &&
    isFinite(Number(displayedTrackOffset)) &&
    Math.abs(targetIndex - Number(displayedTrackOffset)) > 0.2
  );
  var presentationLineIndex = previewTrackCorridor
    ? lyricNearestPrimaryLineIndexForVirtual(
        displayedTrackOffset,
        targetLineIndex,
      )
    : targetLineIndex;
  var presentationIndex = previewTrackCorridor
    ? lyricPrimaryVirtualIndex(presentationLineIndex)
    : targetIndex;
  var visibilityScrollOffset = previewTrackCorridor
    ? Number(displayedTrackOffset)
    : scrollOffset;
  data.trackPreviewCorridorActive = previewTrackCorridor;
  data.trackPresentationLineIndex = presentationLineIndex;
  var rowDrift = previewMotionLock
    ? 0
    : (0.5 - shownProgress) * contextDrift * motionBlend;
  var rowGlow = clampRange(Number(opts.rowGlow) || 0, 0, 1);
  var rowGlowBeat = clampRange(Number(opts.rowGlowBeat) || 0, 0, 1.5);
  var backdropAdapt = lyricSonicBackdropAdaptActive()
    ? lyricBackgroundAdaptStrengthValue()
    : 0;
  var readabilityBackdropColor =
    backdropAdapt > 0.001
      ? lyricReadabilityColorForBrightBackdrop(backdropAdapt)
      : null;
  var activeRow = null;
  var renderRevealCandidates = [];
  var lyricQualityTier = lyricTextureClarityScale();
  var qualityBuildDeferred =
    typeof isProgressDragPreviewActive === "function" &&
    isProgressDragPreviewActive();
  ...
  for (var i = 0; i < data.rowLayers.length; i++) {
    ...
    var liveDelta =
      (row.virtualIndex != null && isFinite(Number(row.virtualIndex))
    ...
    var visibilityAbs = Math.abs(
      (row.virtualIndex != null && isFinite(Number(row.virtualIndex))
        ? Number(row.virtualIndex)
        : i) - visibilityScrollOffset,
    );
    var rowLineIndex =
      row.lineIndex != null && isFinite(Number(row.lineIndex))
        ? Number(row.lineIndex)
        : null;
    var isActive =
      !!row.isPrimary &&
      (rowLineIndex != null
        ? rowLineIndex === presentationLineIndex
        : targetAbs < 0.015);
    row.delta = liveDelta;
    ...
    var contextAlpha = row.isTranslation
      ? clampRange(
          row.targetAlpha * (1 - Math.max(0, targetAbs - 0.65) * 0.12),
          0.08,
          0.62,
        )
      : clampRange(
          row.targetAlpha * (1 - Math.max(0, targetAbs - 0.25) * 0.07),
          0.16,
          0.92,
        );
    ...
    if (row.isTranslation) {
      parentIndex =
        row.parentIndex != null && isFinite(Number(row.parentIndex))
          ? Number(row.parentIndex)
          : row.virtualIndex != null
            ? Number(row.virtualIndex) - lyricTranslationVisualGapValue()
      ...
      if (translationMode === "dual") {
        var currentParent =
          Math.abs(parentIndex - presentationLineIndex) < 0.001;
        var nextParent =
          Math.abs(parentIndex - (presentationLineIndex + 1)) < 0.001;
        parentFade = currentParent ? 1 : nextParent ? 0.56 : 0;
      }
      if (translationMode !== "multi") {
        translationFocus = parentFade;
        contextAlpha = clampRange(
          currentTranslationOpacity * parentFade,
          0,
        ...
        var contextTranslationAlpha = clampRange(
          row.targetAlpha * (1 - Math.max(0, parentDistance - 0.35) * 0.16),
          0.08,
          0.58,
        );
        contextAlpha = clampRange(
          contextTranslationAlpha * (1 - parentFade) +
            currentTranslationOpacity * parentFade,
          0.08,
      ...
      if (
        data.usesTrack &&
        parentIndex != null &&
        isFinite(Number(parentIndex))
      ) {
        var parentVirtualForVisibility = lyricPrimaryVirtualIndex(parentIndex);
        visibilityAbs = Math.abs(
          parentVirtualForVisibility - visibilityScrollOffset,
        );
      } else if (currentTranslation || row.parentRole === "current") {
        visibilityAbs = 0;
      }
      singleLineTranslationSwap =
        singleLineStaticSwap &&
        (currentTranslation || row.parentRole === "current");
    }
    var lineWindowAllowed = lyricLineAllowedForDisplayMode(
      rowWindowLineIndex,
      presentationLineIndex,
      displayMode,
    );
    ...
    var lineUploadPrewarm = !!(
      ...
      !initialTextRevealPending &&
      !lineWindowAllowed &&
      rowWindowLineIndex != null &&
      isFinite(Number(rowWindowLineIndex)) &&
      rowLineOffset >= revealPrewarmMinOffset - 1 &&
      rowLineOffset <= revealPrewarmMaxOffset + 1
    );
    var pendingWindowAllowed =
      pendingTargetLineIndex != null &&
      lyricLineAllowedForDisplayMode(
        rowWindowLineIndex,
        pendingTargetLineIndex,
        displayMode,
      );
    ...
    var visibleFade = lineWindowAllowed
      ? motionAnchor
        ? 1
        : clampRange((visibleRadius + 1.1 - visibilityAbs) / 1.1, 0, 1)
      : 0;
    visibleFade = visibleFade * visibleFade * (3 - 2 * visibleFade);
    var renderWindowActive = !!(
      lineWindowAllowed &&
      (motionAnchor || visibleFade > 0.002)
    );
    if (renderWindowActive && !row.renderWindowActive) {
      ...
      row.renderRevealAt =
        nowMs +
        (motionAnchor || row.renderLineUploaded ? 0 : 10 + revealLane * 14) +
        (row.renderLineUploaded ? 0 : row.isTranslation ? 8 : 0);
    ...
    if (lyricQualityTier <= 1) {
      ...
      !initialTextRevealPending &&
      (renderWindowActive || lineUploadPrewarm || pendingWindowAllowed)
    ) {
      var qualityPriority = isActive
        ? 10
        : currentTranslation
          ? 12
          : lineUploadPrewarm
            ? 34
            : 24 + Math.min(8, visibilityAbs);
    ...
    var lineLayerVisible =
      renderWindowActive && (initialTextRevealPending || nowMs >= rowRevealAt);
    var readabilityLayerVisible =
      textureEffectsAllowed &&
      lineLayerVisible &&
      readability > 0.001 &&
      nowMs >= rowRevealAt + 18;
    var existingGlowOpacity = row.glowMat
      ? getLyricTextureMaterialOpacity(row.glowMat)
      : 0;
    var glowLayerWanted =
      rowGlow > 0.001 || rowGlowBeat > 0.001 || existingGlowOpacity > 0.004;
    var glowLayerVisible =
      textureEffectsAllowed &&
      lineLayerVisible &&
      glowLayerWanted &&
      nowMs >= rowRevealAt + 40;
    var anchorRevealPriority = isActive ? 0 : currentTranslation ? 1 : null;
    var contextRevealPriority =
      20 + Math.min(8, visibilityAbs) * 6 + (row.isTranslation ? 1 : 0);
    if (row.mesh) {
      ...
      if ((lineLayerVisible || lineUploadPrewarm) && !row.renderLineUploaded) {
        var lineRevealPriority = lineUploadPrewarm
          ...
          : anchorRevealPriority == null
            ? contextRevealPriority
            : anchorRevealPriority;
        renderRevealCandidates.push({
          row: row,
          mesh: row.mesh,
          flag: "renderLineUploaded",
          priority: lineRevealPriority,
        });
      }
      if (
        row.renderLineUploaded &&
        renderWindowActive &&
        (row.qualityPendingTexture ||
          (row.qualityTexture &&
            lyricQualityCurrentMap(row) !== row.qualityTexture))
        ...
        renderRevealCandidates.push({
          ...
          priority:
            ...
            (isActive
              ...
              : currentTranslation
                ? 1
                : Math.min(8, visibilityAbs)),
        });
      }
    }
    if (row.readability) {
      row.readability.visible =
        readabilityLayerVisible && !!row.renderReadabilityUploaded;
      if (readabilityLayerVisible && !row.renderReadabilityUploaded) {
        renderRevealCandidates.push({
          row: row,
          mesh: row.readability,
          flag: "renderReadabilityUploaded",
          priority:
            100 +
            (anchorRevealPriority == null
              ? contextRevealPriority
    ...
    if (pendingWindowAllowed) {
      var pendingDistance = Math.abs(
        Number(rowWindowLineIndex) - pendingTargetLineIndex,
      );
      ...
      if (row.mesh && !row.renderLineUploaded) {
        renderRevealCandidates.push({
          ...
          mesh: row.mesh,
          flag: "renderLineUploaded",
          priority: 4 + pendingPriority,
          transparentPrewarm: true,
        });
        ...
        renderRevealCandidates.push({
          ...
          mesh: row.glow,
          flag: "renderGlowUploaded",
          priority: 204 + pendingPriority,
          transparentPrewarm: true,
        });
      }
    }
    if (persistentTrackTransparentPrewarm) {
      var trackPrewarmOrder =
        Math.max(0, rowLineIndex == null ? i : Math.round(rowLineIndex)) * 2 +
        (row.isTranslation ? 1 : 0);
      if (row.mesh && !row.renderLineUploaded) {
        renderRevealCandidates.push({
          row: row,
          mesh: row.mesh,
          flag: "renderLineUploaded",
          priority: 300 + trackPrewarmOrder * 0.01,
          transparentPrewarm: true,
        });
      } else if (row.readability && !row.renderReadabilityUploaded) {
        renderRevealCandidates.push({
          ...
          mesh: row.readability,
          flag: "renderReadabilityUploaded",
          priority: 400 + trackPrewarmOrder * 0.01,
          transparentPrewarm: true,
        });
      } else if (row.glow && !row.renderGlowUploaded) {
        renderRevealCandidates.push({
          ...
          mesh: row.glow,
          flag: "renderGlowUploaded",
          priority: 500 + trackPrewarmOrder * 0.01,
          transparentPrewarm: true,
        });
      }
    }
    ...
    var depthFade = motionAnchor
      ? 1
      : clampRange(1 - visibilityAbs * 0.055, 0.54, 1) * visibleFade;
    var yTarget =
      -liveDelta * lineStepWorld +
      (motionAnchor ? 0 : rowDrift * clampRange(0.7 + abs * 0.1, 0.65, 1.2));
    if (row.isTranslation) {
      yTarget =
        singleLineTranslationSwap && isFinite(Number(row.baseY))
          ? Number(row.baseY)
          : lyricTranslationAnchoredY(
              row,
              i,
              presentationIndex,
              lineStepWorld,
    ...
    var zTarget =
      zBase -
      (motionAnchor ? 0 : Math.abs(rowDrift) * 0.18) +
      (row.isTranslation ? translationFocus * 0.065 : 0);
    ...
    var baseScale = clampRange(
      1 - Math.min(5.5, scaleDistance) * 0.026,
      0.84,
      1.02,
    );
    if (row.isTranslation)
      baseScale *= clampRange(Number(row.fontScale) || 1, 0.72, 1.34);
    if (singleLineTranslationSwap) {
      ...
      baseScale *= 1.0 + translationFocus * 0.16;
    }
    var stableMotionIndex =
      row.virtualIndex != null && isFinite(Number(row.virtualIndex))
        ? Number(row.virtualIndex)
        : rowLineIndex != null
          ? rowLineIndex
    ...
    var scaleTarget =
      baseScale *
      (motionAnchor || !verticalFloatOn
        ? 1
        : 1 +
          Math.sin(t * 0.68 + seed + stableMotionIndex * 0.71) *
            (style === "float" ? 0.012 : 0.004) *
            motionBlend);
    ...
    if (row.mesh) {
      ...
      if (data.usesTrack && persistentPrimedTrack) {
        ...
        var continuousRowMaxStepWorld =
          continuousTrackSlotStep *
          lineStepWorld *
          continuousRowMaxRowsPerFrame;
        rowYStep = clampRange(
          rowYStep,
          -continuousRowMaxStepWorld,
          continuousRowMaxStepWorld,
        );
      }
      row.mesh.position.y += rowYStep;
      row.mesh.position.z += (zTarget - row.mesh.position.z) * ease;
      row.mesh.scale.setScalar(
        row.mesh.scale.x + (scaleTarget - row.mesh.scale.x) * ease,
      );
      row.mesh.renderOrder = isActive
        ? renderBase + 0.4
        : row.isTranslation
          ? renderBase +
            0.05 +
            (currentTranslation ? 0.34 : translationFocus * 0.3)
          : renderBase - 0.4 - Math.min(5.5, abs) * 0.015;
    }
    if (row.mat && row.mat.uniforms) {
      if (row.mat.uniforms.uOpacity) {
        var lineOpacityTarget =
          data.usesTrack &&
          (initialTextRevealPending ||
            !lineLayerVisible ||
            !row.renderLineUploaded)
        ...
        if (initialTextRevealPending) row.mat.uniforms.uOpacity.value = 0;
          ...
          row.mat.uniforms.uOpacity.value +=
            (lineOpacityTarget - row.mat.uniforms.uOpacity.value) * ease;
          if (
            trackTargetJustCommitted &&
            lineOpacityTarget > 0.001 &&
            row.renderLineUploaded &&
            row.mat.uniforms.uOpacity.value < 0.004
          ) {
            row.mat.uniforms.uOpacity.value = Math.min(
              lineOpacityTarget,
              0.004,
            );
          }
      ...
      if (row.mat.uniforms.uProgress)
        row.mat.uniforms.uProgress.value = isActive ? shownProgress : 0;
      if (row.mat.uniforms.uActiveMix) {
        var activeMixTarget = isActive ? 1 : 0;
        row.mat.uniforms.uActiveMix.value +=
          (activeMixTarget - row.mat.uniforms.uActiveMix.value) *
          (isActive ? 0.34 : 0.62);
        if (!isActive && row.mat.uniforms.uActiveMix.value < 0.015)
          row.mat.uniforms.uActiveMix.value = 0;
      }
      if (row.mat.uniforms.uSolar && !isActive) {
        row.mat.uniforms.uSolar.value +=
          (0 - row.mat.uniforms.uSolar.value) * 0.48;
        if (row.mat.uniforms.uSolar.value < 0.003)
          row.mat.uniforms.uSolar.value = 0;
      }
      if (row.mat.uniforms.uGlitchBurst && opts.glitchPulse)
        row.mat.uniforms.uGlitchBurst.value = isActive
          ? opts.glitchPulse
          : opts.glitchPulse * 0.35;
    } else if (row.mat) {
      var fallbackLineOpacityTarget =
        data.usesTrack &&
        (initialTextRevealPending ||
          !lineLayerVisible ||
          !row.renderLineUploaded)
      ...
      if (initialTextRevealPending) row.mat.opacity = 0;
        ...
        if (
          trackTargetJustCommitted &&
          fallbackLineOpacityTarget > 0.001 &&
          row.renderLineUploaded &&
          row.mat.opacity < 0.004
        ) {
      ...
      row.readability.position.x +=
        ((isActive
          ...
          : currentTranslation
            ...
            : jitterX * 0.16) -
          row.readability.position.x) *
        (opts.glitchPulse ? 0.42 : 0.12);
      if (data.usesTrack && persistentPrimedTrack && row.mesh)
        row.readability.position.y = row.mesh.position.y;
      else
        row.readability.position.y +=
          (yTarget +
            (verticalFloatOn
              ? isActive
                ? jitterY * 0.4
            ...
            row.readability.position.y) *
          ease;
      row.readability.position.z +=
        (zTarget - 0.012 - row.readability.position.z) * ease;
      row.readability.scale.setScalar(
        row.readability.scale.x +
          (scaleTarget - row.readability.scale.x) * ease,
      );
      row.readability.renderOrder = row.mesh
        ? row.mesh.renderOrder - 0.04
        : row.isTranslation
          ? renderBase
          : renderBase - 0.45;
    }
    if (row.readabilityMat) {
      var readabilityMix = row.isTranslation
        ? 0.46 + translationFocus * 0.18
        : isActive
          ? 0.74
          : 0.52;
      if (readabilityBackdropColor)
        setLyricTextureMaterialColor(
          row.readabilityMat,
          readabilityBackdropColor,
        );
      else if (lyricReadabilityLightColor)
        setLyricTextureMaterialColor(
          row.readabilityMat,
          lyricReadabilityLightColor,
        );
      var readabilityBoost =
        1 +
        backdropAdapt *
          (motionAnchor ? (row.isTranslation ? 0.62 : 0.86) : 0.46);
      var readabilityOpacity = getLyricTextureMaterialOpacity(
        row.readabilityMat,
      );
      var readabilityOpacityTarget =
        !data.usesTrack ||
        (readabilityLayerVisible && row.renderReadabilityUploaded)
      ...
      setLyricTextureMaterialOpacity(
        ...
        initialTextRevealPending
          ? 0
          : readabilityOpacity +
              (readabilityOpacityTarget - readabilityOpacity) * ease,
      );
    }
    if (row.glow) {
      var glowJitterX = isActive
        ? jitterX
        : currentTranslation
          ? jitterX * 0.72
          : translationGlowFocus > 0.001
            ? jitterX * 0.34 * translationGlowFocus
            : 0;
      var glowJitterY = verticalFloatOn
        ? isActive
          ? jitterY * 0.92
          : currentTranslation
            ? jitterY * 0.64
            : translationGlowFocus > 0.001
              ? jitterY * 0.3 * translationGlowFocus
              : 0
        : 0;
      ...
      var glowTargetZ = row.mesh ? row.mesh.position.z - 0.03 : zTarget - 0.03;
      var glowTargetScale = row.mesh ? row.mesh.scale.x : scaleTarget;
      var glowLockedToText =
        !!row.mesh &&
        (isActive || currentTranslation || translationGlowFocus > 0.001);
      if (
        (data.usesTrack && persistentPrimedTrack) ||
        previewMotionLock ||
        glowLockedToText
      ) {
        ...
        row.glow.position.x +=
          (glowTargetX - row.glow.position.x) *
          (opts.glitchPulse ? 0.52 : Math.max(glowEase, 0.26));
        row.glow.position.y += (glowTargetY - row.glow.position.y) * glowEase;
        row.glow.position.z += (glowTargetZ - row.glow.position.z) * glowEase;
        row.glow.scale.setScalar(
          row.glow.scale.x + (glowTargetScale - row.glow.scale.x) * glowEase,
        );
      }
      row.glow.renderOrder = row.isTranslation
        ? renderBase - 0.02
        : renderBase - 0.52;
    }
    if (row.glowMat) {
      var glowOpacityTarget = isActive
        ...
        : row.isTranslation
          ? target *
            rowGlow *
            (currentTranslation
              ? 0.46 + rowGlowBeat * 0.08
              : (0.3 + rowGlowBeat * 0.06) * translationGlowFocus) *
            depthFade
          : 0;
      if (backdropAdapt > 0.001) glowOpacityTarget *= 1 - backdropAdapt * 0.3;
      if (data.usesTrack && (!glowLayerVisible || !row.renderGlowUploaded))
        glowOpacityTarget = 0;
      var glowOpacity = getLyricTextureMaterialOpacity(row.glowMat);
      var nextGlowOpacity =
        glowOpacity +
        (glowOpacityTarget - glowOpacity) *
          (glowOpacityTarget > glowOpacity ? 0.2 : 0.34);
      if (!isActive && nextGlowOpacity < 0.004) nextGlowOpacity = 0;
      setLyricTextureMaterialOpacity(
        row.glowMat,
        initialTextRevealPending ? 0 : nextGlowOpacity,
      );
    }
  }
  registerLyricQualityCandidates(
    data,
    lyricQualityCandidates,
    lyricQualityTier,
    opts.qualityRootPriority,
    qualityBuildDeferred,
  );
  var deferQualityCommit = lyricQualityState.deferFinalize;
  if (deferQualityCommit && renderRevealCandidates.length) {
    for (
      var deferredRevealIndex = 0;
      deferredRevealIndex < renderRevealCandidates.length;
      deferredRevealIndex++
    ) {
      var deferredReveal = renderRevealCandidates[deferredRevealIndex];
      if (!deferredReveal || !deferredReveal.quality) continue;
      registerLyricQualityCommitCandidate(
        data,
        deferredReveal.row,
        (Number(opts.qualityRootPriority) || 0) + deferredReveal.priority,
      );
    }
  }
  if (deferQualityCommit && lyricQualityCandidates.length) {
    for (
      var deferredPrewarmIndex = 0;
      deferredPrewarmIndex < lyricQualityCandidates.length;
      deferredPrewarmIndex++
    ) {
      var deferredPrewarm = lyricQualityCandidates[deferredPrewarmIndex];
      if (
        !deferredPrewarm ||
        !deferredPrewarm.row ||
        !deferredPrewarm.row.renderLineUploaded ||
        !deferredPrewarm.row.qualityPendingTexture
      )
        continue;
      registerLyricQualityCommitCandidate(
        data,
        deferredPrewarm.row,
        (Number(opts.qualityRootPriority) || 0) +
          240 +
          deferredPrewarm.priority,
      );
    }
  }
  if (!deferQualityCommit) finalizeLyricQualitySelectionFrame();
  if (renderRevealCandidates.length) {
    renderRevealCandidates.sort(function (a, b) {
      return a.priority - b.priority;
    });
    for (
      var revealIndex = 0;
      revealIndex < renderRevealCandidates.length;
      revealIndex++
    ) {
  ...
  if (initialTextRevealPending) {
    ...
    if (initialTextRows > 0 && initialTextRowsReady) {
      ...
      if (!opts.initialRevealReflow) {
        var revealOpts = {};
        for (var revealOptKey in opts)
          revealOpts[revealOptKey] = opts[revealOptKey];
