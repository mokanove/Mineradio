function applyLyricVerticalEdgeFade(
  ctx,
  W,
  H,
  strength,
  ...
  var midBoost =
    activeLine > 0 && activeLine < lineCount - 1 ? 0.018 * strength : 0;
  ctx.save();
  ...
  fade.addColorStop(
    topBand * 0.45,
    "rgba(255,255,255," + (0.34 + strength * 0.2).toFixed(3) + ")",
  );
...
function beginLyricMaskLayoutBuild(input, layoutOverride) {
  ...
  if (!payload)
    payload = {
      entries: [{ text: "", role: "current", alpha: 1, scale: 1 }],
      activeLine: 0,
      text: "",
      combinedText: "",
    };
  var baseCanvasW = 2048;
  var rendererMaxTexture =
    renderer && renderer.capabilities && renderer.capabilities.maxTextureSize
      ? renderer.capabilities.maxTextureSize
      : 4096;
  var maxCanvasW = Math.max(
    baseCanvasW,
    Math.min(6144, rendererMaxTexture || 4096),
  );
  var entries =
    payload && payload.entries && payload.entries.length
      ? payload.entries
      : [{ text: "", role: "current", alpha: 1, scale: 1 }];
  ...
  var activeLine = Math.max(
    0,
    Math.min(lines.length - 1, payload.activeLine || 0),
  );
  var fitMeasureIndexes = [];
  for (var fi = 0; fi < entries.length; fi++) {
    var fitAlpha =
      entries[fi] && entries[fi].alpha == null
        ? 1
        : Number(entries[fi] && entries[fi].alpha);
...
function measureLyricMaskLayoutBaseEntry(state, index) {
  ...
  if (!isFinite(baseWidth)) {
    baseWidth = lyricMeasureTextAtSize(
      state.ctx,
      text,
      state.layoutMeasureBaseSize,
      weight,
    );
...
function scaledLyricMaskLayoutWidth(state, index, size) {
  var entry = state.entries[index] || {};
  var cacheKey = lyricMaskLayoutCacheKey(
    state.lines[index] || "",
    lyricEntryWeight(entry),
  );
  var baseWidth = state.layoutBaseWidthCache[cacheKey];
  if (!isFinite(baseWidth))
    baseWidth = measureLyricMaskLayoutBaseEntry(state, index);
  return (
    baseWidth *
    ((Math.max(0.01, Number(size) || state.layoutMeasureBaseSize) *
      (entry.scale || 1)) /
      state.layoutMeasureBaseSize)
  );
}
...
function finalizeLyricMaskLayoutBuild(state) {
  ...
  var fontSize =
    isFinite(state.lockedFontSize) && state.lockedFontSize > 0
      ? clampRange(state.lockedFontSize, 42, 160)
      : 128;
  var widest = 1;
  function measureWidestAtSize(size) {
    ...
    for (var li = 0; li < fitMeasureIndexes.length; li++) {
      measured = Math.max(
        measured,
        scaledLyricMaskLayoutWidth(state, fitMeasureIndexes[li], size),
      );
    }
    return measured;
  }
  function lyricMaskLayoutFits(size, measuredWidth) {
    var testLineHeight =
      size * (lines.length > 1 ? 0.98 : 1.0) * lyricLineHeightFactor();
  ...
  if (!isFinite(state.lockedFontSize) || state.lockedFontSize <= 0) {
    ...
    var baseLineHeight =
      fitBaseSize * (lines.length > 1 ? 0.98 : 1.0) * lyricLineHeightFactor();
    var baseBlockH = fitBaseSize + (lines.length - 1) * baseLineHeight;
    var fitRatio = Math.min(
      1,
      maxWidth / Math.max(1, baseWidest),
      (H - 76) / Math.max(1, baseBlockH),
    );
    var estimatedFont =
      Math.floor((fitBaseSize * Math.max(0.01, fitRatio)) / 4) * 4;
    fontSize = Math.max(
      minimumStepFont,
      Math.min(fitBaseSize, estimatedFont || minimumStepFont),
    );
    ...
    while (
      fontSize > minimumStepFont &&
      !lyricMaskLayoutFits(fontSize, widest)
    ) {
      fontSize -= 4;
      widest = measureWidestAtSize(fontSize);
    }
  ...
  for (var mi = 0; mi < fitMeasureIndexes.length; mi++) {
    ...
    var measuredLineWidth = scaledLyricMaskLayoutWidth(
      state,
      measureIndex,
      fontSize,
    );
  ...
  widest = Math.max(
    ...
    lyricMeasureTextAtSize(
      ctx,
      lines[widestMeasureIndex] || "",
      fontSize * (widestEntry.scale || 1),
      lyricEntryWeight(widestEntry),
    ),
  );
  var canvasWidthPad = Math.max(220, fontSize * 2.2);
  var neededCanvasW = Math.ceil(
    Math.min(maxCanvasW, Math.max(baseCanvasW, widest + canvasWidthPad)),
  );
  var W = neededCanvasW;
    ...
    activeLine === widestMeasureIndex
      ...
      : Math.max(
          ...
          lyricMeasureTextAtSize(
            ctx,
            lines[activeLine] || "",
            fontSize * (activeEntry.scale || 1),
            lyricEntryWeight(activeEntry),
          ),
        );
...
function stepLyricMaskLayoutBuild(state, maxEntries) {
  ...
  var measured = 0;
  while (
    state.measureCursor < state.fitMeasureIndexes.length &&
    measured < limit
  ) {
    measureLyricMaskLayoutBaseEntry(
      state,
      state.fitMeasureIndexes[state.measureCursor],
    );
...
function measureLyricMaskLayout(input, layoutOverride) {
  ...
  while (!stepLyricMaskLayoutBuild(state, 1024)) {
...
function makeLyricMaskLayoutMetrics(input, layoutOverride) {
  ...
  while (!stepLyricMaskLayoutMetricsBuild(state, 1024)) {
    /* synchronous compatibility path */
  }
...
function lyricMaskStoneSeed(lines, entries, fontSize) {
  ...
  for (var i = 0; i < (entries || []).length; i++) {
    var entry = entries[i] || {};
    signature +=
      (entry.translationLine ? "t" : "p") +
      ":" +
      lyricEntryWeight(entry) +
      ":" +
      Math.round((Number(entry.scale) || 1) * 1000) +
      "|";
  }
...
function makeLyricMask(input, layoutOverride) {
  ...
  applyLyricVerticalEdgeFade(
    ctx,
    W,
    H,
    lyricEdgeFadeValue() * (payload.contextLayer ? 1.15 : 0.74),
    activeLine,
    lines.length,
  );
...
function lyricTextureClarityScale() {
  if (typeof normalizeLyricTextureClarity === "function")
    return normalizeLyricTextureClarity(fx && fx.lyricTextureClarity);
...
function lyricRowTextureWidthBudget() {
  var canvasWidth =
    renderer && renderer.domElement ? Number(renderer.domElement.width) : 0;
  if (!isFinite(canvasWidth) || canvasWidth <= 0) {
    ...
    canvasWidth =
      (typeof window !== "undefined"
        ? Math.max(1, Number(window.innerWidth) || 1280)
        : 1280) * dpr;
  }
  var budget = Math.ceil(clampRange(canvasWidth, 1024, 3072) / 64) * 64;
  var profile =
    typeof runtimeHardwareProfile !== "undefined"
  ...
  var rendererMaxTexture =
    renderer && renderer.capabilities
      ...
      : 0;
  if (isFinite(rendererMaxTexture) && rendererMaxTexture > 0)
    budget = Math.min(budget, rendererMaxTexture);
...
function lyricQualityPoolBudgetBytes(tier) {
  ...
  else if (profile && profile.balancedSpec)
    mib = tier === 2 ? 48 : tier === 3 ? 96 : 144;
...
function lyricQualityMaxResidentRows() {
  var profile =
    typeof runtimeHardwareProfile !== "undefined"
      ? runtimeHardwareProfile
      : null;
  return profile && profile.lowSpec
    ? 4
    : profile && profile.balancedSpec
      ? 6
      : 8;
}
...
function lyricQualityTargetMetrics(mask, tier) {
  ...
  var logicalW = Math.max(
    1,
    Number(mask.logicalWidth) || Number(mask.width) || 1,
  );
  var logicalH = Math.max(
    1,
    Number(mask.logicalHeight) || Number(mask.height) || 1,
  );
  ...
  var rendererMax =
    renderer && renderer.capabilities
      ? Number(renderer.capabilities.maxTextureSize)
      : 0;
  var maxDimension = Math.min(
    6144,
    isFinite(rendererMax) && rendererMax > 0 ? rendererMax : 4096,
  );
  ...
  if (
    width <= baseW + 8 ||
    height <= Math.max(1, Number(mask.height) || logicalH) + 4
  )
    return null;
  return {
    tier: tier,
...
function makeLyricQualityTexture(mask, tier) {
  ...
  var lines =
    Array.isArray(mask.lines) && mask.lines.length ? mask.lines : [""];
  ...
  var fontSize = Math.max(
    1,
    Number(mask.logicalFontSize) ||
      (Number(mask.fontSize) || 128) /
        Math.max(0.01, Number(mask.rasterScale) || 1),
  );
  var lineHeight = Math.max(
    1,
    Number(mask.logicalLineHeight) ||
      (Number(mask.lineHeight) || fontSize) /
        Math.max(0.01, Number(mask.rasterScale) || 1),
  );
  var lineY0 = isFinite(Number(mask.logicalLineY0))
    ? Number(mask.logicalLineY0)
    : (Number(mask.lineY0) || logicalH / 2 + fontSize * 0.36) /
      Math.max(0.01, Number(mask.rasterScale) || 1);
  var fitScaleX = Number(mask.fitScaleX) || 1;
  var activeLine = Math.max(
    0,
    Math.min(lines.length - 1, Number(mask.activeLine) || 0),
  );
  var canvas = document.createElement("canvas");
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    var lineFontSize = fontSize * (entry.scale || 1);
    var lineY =
      lineY0 + i * lineHeight + lyricEntryLineOffset(entry) * lineHeight;
    ctx.globalAlpha = entry.alpha == null ? 1 : entry.alpha;
    ctx.font = lyricFontCss(lineFontSize, lyricEntryWeight(entry));
    if (fitScaleX < 1) {
      ctx.save();
  ...
  ctx.globalAlpha = 1;
  var stoneSeed =
    Number(mask.stoneSeed) || lyricMaskStoneSeed(lines, entries, fontSize);
  applyStonePrintTexture(
    ctx,
    logicalW,
    logicalH,
    fontSize,
    lyricSeededRandom(stoneSeed),
  );
  applyLyricVerticalEdgeFade(
    ctx,
    logicalW,
    logicalH,
    lyricEdgeFadeValue() * (mask.contextLayer ? 1.15 : 0.74),
    activeLine,
    lines.length,
  );
  ...
  return {
    ...
    bytes: target.bytes,
    key:
      target.tier + "x|" + target.width + "x" + target.height + "|" + stoneSeed,
  };
}
  ...
  var maximum =
    profile && profile.lowSpec
      ...
      : profile && profile.balancedSpec
        ...
        : 3072;
  var budget =
    Math.ceil(clampRange(canvasWidth * qualityScale, minimum, maximum) / 64) *
    64;
  var rendererMaxTexture =
    renderer && renderer.capabilities
      ? Number(renderer.capabilities.maxTextureSize)
...
function lyricGlowRasterMetrics(mask) {
  ...
  var logicalFontSize = Math.max(
    1,
    Number(mask.logicalFontSize) ||
      (Number(mask.fontSize) || 128) / sourceScale,
  );
  var logicalTextWidth = Math.max(
    1,
    Number(mask.logicalActiveTextWidth) ||
      Number(mask.logicalTextWidth) ||
      (Number(mask.activeTextWidth) || Number(mask.textWidth) || 1) /
        sourceScale,
  );
  var logicalLineHeight = Math.max(
    1,
    Number(mask.logicalLineHeight) ||
      (Number(mask.lineHeight) || logicalFontSize * lyricLineHeightFactor()) /
        sourceScale,
  );
  ...
  var minimumRasterFont =
    profile && profile.lowSpec ? 64 : profile && profile.balancedSpec ? 72 : 80;
  var budgetScale = widthBudget / Math.max(1, logicalGlowWidth);
  var fontScale = minimumRasterFont / logicalFontSize;
  var scale = clampRange(
    Math.max(sourceScale, budgetScale, fontScale),
    0.01,
    1,
  );
  return {
...
function compactLyricLineMaskTexture(mask) {
  ...
  var sourceWidth = Math.max(
    1,
    Number(sourceCanvas.width) || Number(mask.width) || 1,
  );
  var sourceHeight = Math.max(
    1,
    Number(sourceCanvas.height) || Number(mask.height) || 1,
  );
  ...
  if (!isFinite(Number(mask.logicalWidth)) || Number(mask.logicalWidth) <= 0)
    mask.logicalWidth = sourceWidth;
  if (!isFinite(Number(mask.logicalHeight)) || Number(mask.logicalHeight) <= 0)
    mask.logicalHeight = sourceHeight;
  if (
    !isFinite(Number(mask.logicalFontSize)) ||
    Number(mask.logicalFontSize) <= 0
  )
    mask.logicalFontSize = Number(mask.fontSize) || 128;
  if (
    !isFinite(Number(mask.logicalLineHeight)) ||
    Number(mask.logicalLineHeight) <= 0
  )
    mask.logicalLineHeight =
      Number(mask.lineHeight) || Number(mask.logicalFontSize) * 1.08;
  if (!isFinite(Number(mask.logicalLineY0)))
    mask.logicalLineY0 = Number(mask.lineY0) || 0;
  if (
    !isFinite(Number(mask.logicalTextWidth)) ||
    Number(mask.logicalTextWidth) <= 0
  )
    mask.logicalTextWidth = Number(mask.textWidth) || sourceWidth;
  if (
    !isFinite(Number(mask.logicalActiveTextWidth)) ||
    Number(mask.logicalActiveTextWidth) <= 0
  )
    mask.logicalActiveTextWidth =
      Number(mask.activeTextWidth) || Number(mask.logicalTextWidth);
  if (
    !isFinite(Number(mask.logicalTextHeight)) ||
    Number(mask.logicalTextHeight) <= 0
  )
    mask.logicalTextHeight =
      Number(mask.textHeight) || Number(mask.logicalFontSize);
  ...
  compactTexture.anisotropy = Math.min(
    8,
    renderer.capabilities.getMaxAnisotropy
      ? renderer.capabilities.getMaxAnisotropy()
      : 1,
...
function beginLyricReadabilityTextureBuild(mask) {
  ...
  var lines =
    mask && Array.isArray(mask.lines) && mask.lines.length ? mask.lines : [""];
  var entries = mask && Array.isArray(mask.entries) ? mask.entries : [];
  var lineHeight =
    (mask && mask.lineHeight) || fontSize * lyricLineHeightFactor();
  ...
  var activeLine = Math.max(
    0,
    Math.min(lines.length - 1, Number(mask && mask.activeLine) || 0),
  );
  var hasLineY0 = mask && isFinite(Number(mask.lineY0));
  var y0 = hasLineY0
    ? Number(mask.lineY0)
    : H / 2 + fontSize * 0.36 - activeLine * lineHeight;
  return {
    mask: mask,
    canvas: canvas,
    ctx: ctx,
    ...
    activeLine: activeLine,
    y0: y0,
    pixelScale: clampRange(Number(mask && mask.rasterScale) || 1, 0.2, 1),
    phase: 0,
    lastPhase: "",
    texture: null,
    done: false,
...
function drawLyricReadabilityStrokeLines(state, dx, dy) {
  ...
  for (var i = 0; i < state.lines.length; i++) {
    ...
    var y =
      state.y0 +
      i * state.lineHeight +
      lyricEntryLineOffset(entry) * state.lineHeight +
      (dy || 0);
    ...
    var alpha =
      entry.alpha == null
        ...
        : clampRange(
            Number(entry.alpha),
            entry.translationLine ? 0.1 : 0.22,
            1,
          );
    ctx.font = lyricFontCss(lineFontSize, lyricEntryWeight(entry));
    ctx.globalAlpha = prevAlpha * alpha * (entry.translationLine ? 0.62 : 1);
    if (entry.translationLine)
      ctx.lineWidth = Math.max(1.8 * state.pixelScale, prevLineWidth * 0.52);
...
function stepLyricReadabilityTextureBuild(state) {
  ...
  if (state.phase === 2) {
    ...
    ctx.lineWidth = Math.max(9 * pixelScale, fontSize * 0.07);
  ...
  ctx.lineWidth = Math.max(3.2 * pixelScale, fontSize * 0.03);
  ctx.strokeStyle = "rgba(255,255,255,1)";
  drawLyricReadabilityStrokeLines(state, 0, 0);
  ctx.restore();
  applyLyricVerticalEdgeFade(
    ...
    state.H,
    lyricEdgeFadeValue() *
      (state.mask && state.mask.contextLayer ? 1.08 : 0.62),
    state.activeLine,
    state.lines.length,
  );
...
function finishLyricReadabilityTextureBuild(state) {
  ...
  while (!stepLyricReadabilityTextureBuild(state)) {
    /* synchronous compatibility path */
  }
...
function makeLyricReadabilityTexture(mask) {
  return finishLyricReadabilityTextureBuild(
    beginLyricReadabilityTextureBuild(mask),
  );
}

function beginLyricGlowTextureBuild(
  text,
  fontSize,
  textWidth,
  lines,
  ...
  var useMaskFrame = !!(
    sourceMask &&
    isFinite(Number(sourceMask.width)) &&
    isFinite(Number(sourceMask.height))
  );
  var canvas = document.createElement("canvas");
  var measureCanvas = document.createElement("canvas");
  ...
  var measuredWidth = Math.max(
    1,
    textWidth || lyricMeasureTextAtSize(measureCtx, text, fontSize) * fitScaleX,
  );
  for (var li = 0; li < drawLines.length; li++) {
    ...
    measuredWidth = Math.max(
      ...
      lyricMeasureTextAtSize(
        ...
        drawLines[li],
        lineFontSize,
        lyricEntryWeight(entries[li]),
      ) * fitScaleX,
    );
  }
  var pixelScale = clampRange(
    Number(sourceMask && sourceMask.rasterScale) || Number(rasterScale) || 1,
    0.2,
    1,
  );
  ...
  activeLine = Math.max(
    0,
    Math.min(drawLines.length - 1, Number(activeLine) || 0),
  );
  var W = useMaskFrame
    ? Math.round(sourceMask.width)
    : Math.ceil(measuredWidth + padX * 2);
  var H = useMaskFrame
    ? Math.round(sourceMask.height)
  ...
  ctx.font = lyricFontCss(fontSize);
  var y0 =
    useMaskFrame && isFinite(Number(sourceMask.lineY0))
      ? Number(sourceMask.lineY0)
      : H / 2 + fontSize * 0.36 - activeLine * lh;
  return {
    text: text,
    fontSize: fontSize,
    drawLines: drawLines,
  ...
  for (var i = 0; i < state.drawLines.length; i++) {
    ...
    var alpha =
      entry.alpha == null
        ...
        : clampRange(
            Number(entry.alpha),
            entry.translationLine ? 0.08 : 0.22,
            1,
          );
    var y =
      state.y0 +
    ...
    if (entry.translationLine)
      ctx.lineWidth = Math.max(1.8 * state.pixelScale, prevLineWidth * 0.48);
    if (state.fitScaleX < 1) {
      ctx.save();
      ctx.translate(state.W / 2 + (dx || 0), 0);
      ctx.scale(state.fitScaleX, 1);
      if (ctx.lineWidth > 0)
        lyricStrokeText(ctx, state.drawLines[i], 0, y, lineFontSize);
...
function drawLyricGlowRadialPass(state, start, end) {
  ...
  for (var ri = start; ri < end; ri++) {
    ...
    drawLyricGlowText(
      state,
      Math.cos(ang) * 7 * pixelScale,
      Math.sin(ang) * 4 * pixelScale,
    );
  }
  ctx.restore();
}
...
function finishLyricGlowTexturePixels(state) {
  ...
  Object.assign(tex.userData, {
    width: state.W,
    height: state.H,
    textWidth: state.useMaskFrame
      ? state.sourceMask.activeTextWidth ||
        state.sourceMask.textWidth ||
        state.measuredWidth
      : state.measuredWidth,
...
function stepLyricGlowTextureBuild(state) {
  ...
  var pixelScale = state.pixelScale;
  if (state.phase === 0) {
    drawLyricGlowBlurPass(
      state,
      "blur(" + Math.max(1, 14 * pixelScale).toFixed(2) + "px)",
      0.46,
      Math.max(10 * pixelScale, fontSize * 0.1),
    );
    state.lastPhase = "blur-14";
  } else if (state.phase === 1) {
    drawLyricGlowBlurPass(
      state,
      "blur(" + Math.max(1.5, 34 * pixelScale).toFixed(2) + "px)",
      0.34,
      Math.max(18 * pixelScale, fontSize * 0.18),
    );
    state.lastPhase = "blur-34";
  } else if (state.phase === 2) {
    drawLyricGlowBlurPass(
      state,
      "blur(" + Math.max(2, 78 * pixelScale).toFixed(2) + "px)",
      0.22,
      Math.max(28 * pixelScale, fontSize * 0.26),
    );
    state.lastPhase = "blur-78";
  } else if (state.phase === 3) {
    drawLyricGlowBlurPass(
      state,
      "blur(" + Math.max(3, 116 * pixelScale).toFixed(2) + "px)",
      0.13,
      Math.max(42 * pixelScale, fontSize * 0.4),
    );
    ...
    if (state.phase === LYRIC_GLOW_BUILD_PHASES - 1)
      finishLyricGlowTexturePixels(state);
  }
  state.phase += 1;
...
function finishLyricGlowTextureBuild(state) {
  if (!state) return null;
  while (!stepLyricGlowTextureBuild(state)) {
    /* synchronous compatibility path */
  }
  return state.texture;
}

function makeLyricGlowTexture(
  text,
  fontSize,
  textWidth,
  lines,
  ...
  return finishLyricGlowTextureBuild(
    beginLyricGlowTextureBuild(
      text,
      fontSize,
      textWidth,
      lines,
