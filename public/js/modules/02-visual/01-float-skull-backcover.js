...
function createFloatLayer() {
  ...
  fgeo.setAttribute(
    "position",
    new THREE.BufferAttribute(floatPositionsArr, 3),
  );
...
var SKULL_MODEL_BASE_ROTATION_Y = 0.0;
var SKULL_MODEL_SCALE = 2.34;
var SKULL_MODEL_BASE_POSITION = { x: 0, y: 0.22, z: 0.1 };
var skullAmpPulse = 0;
...
function effectiveSkullVisualTint() {
  ...
  var color = custom
    ...
    : pal.secondary ||
      pal.primary ||
      fx.visualTintColor ||
      fxDefaults.visualTintColor ||
      "#9db8cf";
  color = normalizeHexColor(color || "#9db8cf", "#9db8cf");
  var strength = custom
    ? 0.98
    : pal && (pal.secondary || pal.primary)
      ? 0.3
      : 0.14;
...
function syncSkullParticleColors() {
  if (
    !skullParticleGroup ||
    !skullParticleGroup.material ||
    !skullParticleGroup.material.uniforms
  )
    return;
  var u = skullParticleGroup.material.uniforms;
  var tint = effectiveSkullVisualTint();
  var custom = !!tint.custom;
  var strength = clampRange(
    Number(tint.strength) || 0,
    0,
    custom ? 0.99 : 0.78,
  );
  skullTintScratch.tint.set(tint.color);
  skullTintScratch.soft
    .copy(skullTintScratch.tint)
    .lerp(new THREE.Color("#e8f5ff"), custom ? 0.05 : 0.28);
  skullTintScratch.bright
    .copy(skullTintScratch.tint)
    .lerp(
      new THREE.Color(custom ? "#f6fbff" : "#fff7d6"),
      custom ? 0.14 : 0.46,
    );
  skullTintScratch.dark
    .copy(skullTintScratch.tint)
    .lerp(new THREE.Color("#05070c"), custom ? 0.74 : 0.72);
  skullTintScratch.boneA
    .copy(custom ? skullBaseColors.neutralBoneA : skullBaseColors.boneA)
    .lerp(skullTintScratch.soft, strength * (custom ? 0.99 : 0.64));
  skullTintScratch.boneB
    .copy(custom ? skullBaseColors.neutralBoneB : skullBaseColors.boneB)
    .lerp(skullTintScratch.bright, strength * (custom ? 0.94 : 0.46));
  skullTintScratch.shadow
    .copy(custom ? skullBaseColors.neutralShadow : skullBaseColors.shadow)
    .lerp(skullTintScratch.dark, strength * (custom ? 0.72 : 0.42));
  skullTintScratch.light
    .copy(custom ? skullBaseColors.neutralLight : skullBaseColors.light)
    .lerp(skullTintScratch.bright, strength * (custom ? 0.98 : 0.76));
...
function loadSkullParticleAsset() {
  if (
    skullParticleAsset.data ||
    skullParticleAsset.promise ||
    skullParticleAsset.failed
  )
    return (
      skullParticleAsset.promise || Promise.resolve(skullParticleAsset.data)
    );
  if (typeof fetch !== "function") {
    skullParticleAsset.failed = true;
    return Promise.resolve(null);
  }
  skullParticleAsset.promise = fetch(
    "assets/skull-decimation-points.bin?v=regular-surface-teeth-soften-20260621",
    { cache: "reload" },
  )
    .then(function (res) {
...
function createSkullParticleLayer() {
  ...
  if (!asset) {
    ...
    function eyeCut(x, y, z, side) {
      ...
      var p = rotate2(x - side * 0.38, y - 0.02, side * 0.1);
      var almond =
    ...
    function noseCut(x, y, z) {
      ...
      var t = clampRange((-0.12 - y) / 0.5, 0, 1);
      var half = 0.05 + t * 0.185;
      return Math.abs(x) < half && z > 0.38 + t * 0.18;
    }
    function mouthGap(x, y, z) {
      return z > 0.18 && y < -0.66 && y > -1.03 && Math.abs(x) < 0.3;
    }
    ...
    for (var j = 0; j < 1450; j++) {
      ...
      skullPushPoint(pos, seed, kind, x, y, z, 0.15 + Math.random() * 0.1);
    }
    ...
    [-1, 1].forEach(function (side) {
      ...
      skullPushCurve(
        ...
        function (t) {
          ...
          var py = Math.sin(a) * (0.205 + Math.cos(a * 2.0) * 0.01);
          var r = rotate2(px, py, -side * 0.1);
          return {
            x: cx + r.x,
    ...
    skullPushCurve(
      ...
      function (t) {
        var x = -0.25 + t * 0.5;
        return { x: x, y: -0.62 + Math.sin(t * Math.PI) * 0.03, z: 0.7 };
      },
      0.86,
      0.008,
    );
    ...
    skullPushCurve(
      ...
      function (t) {
        var x = -0.39 + t * 0.78;
        return { x: x, y: -0.7 + Math.sin(t * Math.PI) * 0.018, z: 0.73 };
      },
      0.96,
      0.006,
    );
    skullPushCurve(
      ...
      function (t) {
        var x = -0.36 + t * 0.72;
        return { x: x, y: -1.005 - Math.sin(t * Math.PI) * 0.018, z: 0.7 };
      },
      0.78,
      0.008,
    );
    for (var tooth = -4; tooth <= 4; tooth++) {
      var tx = tooth * 0.082;
      var height = tooth === 0 ? 0.3 : 0.25 + (4 - Math.abs(tooth)) * 0.012;
      skullPushCurve(
        pos,
        seed,
        kind,
  ...
  var geo = asset
    ? buildSkullParticleGeometryFromAsset(asset)
  ...
  if (!asset) {
    ...
    geo.setAttribute(
      "seed",
      new THREE.BufferAttribute(new Float32Array(seed), 1),
    );
    geo.setAttribute(
      "kind",
      new THREE.BufferAttribute(new Float32Array(kind), 1),
    );
  }
...
function clearSkullPresetResidue() {
  ...
  if (skullParticleGroup.material && skullParticleGroup.material.uniforms) {
    if (skullParticleGroup.material.uniforms.uOpacity)
      skullParticleGroup.material.uniforms.uOpacity.value = 0;
    if (skullParticleGroup.material.uniforms.uJawOpen)
      skullParticleGroup.material.uniforms.uJawOpen.value = 0;
    if (skullParticleGroup.material.uniforms.uSkullFlash)
      skullParticleGroup.material.uniforms.uSkullFlash.value = 0;
  }
}
function resetSkullPresetView(immediate, opts) {
  ...
  if (
    !opts.keepLyricLock &&
    typeof stageLyrics !== "undefined" &&
    stageLyrics &&
    stageLyrics.group &&
    stageLyrics.group.userData
  )
    stageLyrics.group.userData.skullMouthLocked = false;
  if (!opts.keepLyricLock && typeof requestStageLyricCameraSnap === "function")
  ...
  skullParticleGroup.position.set(
    shelfComposition ? -1.18 : SKULL_MODEL_BASE_POSITION.x,
    shelfComposition ? 0.32 : SKULL_MODEL_BASE_POSITION.y,
    SKULL_MODEL_BASE_POSITION.z,
  );
  skullParticleGroup.scale.setScalar(
    shelfComposition ? 3.02 : SKULL_MODEL_SCALE,
  );
  skullParticleGroup.rotation.set(
  ...
  if (camera && typeof setSkullCameraTargetVectors === "function") {
    ...
    setSkullCameraTargetVectors(
      skullCameraTargetPos,
      skullCameraTargetLook,
      portrait,
      shelfComposition,
      0,
    );
...
function skullBreathOffset(t, shelfComposition) {
  var strength = shelfComposition ? 0.7 : 1.0;
  return {
    x:
      strength *
      (Math.sin(t * 0.33 + 1.7) * 0.028 + Math.sin(t * 0.61 + 0.4) * 0.01),
    y:
      strength *
      (Math.sin(t * 0.38 + 0.2) * 0.036 + Math.sin(t * 0.83 + 2.1) * 0.012),
...
function setSkullCameraTargetVectors(
  ...
  if (shelfComposition) {
    pos.set(
      portrait ? -0.06 : 0.0,
      portrait ? -2.36 : -2.5,
      (portrait ? 4.88 : 4.96) + zoom * 0.78,
    );
    look.set(portrait ? -0.04 : 0.0, portrait ? -0.26 : -0.2, 0.03);
    return;
  }
  pos.set(0.0, portrait ? -2.38 : -2.52, (portrait ? 4.92 : 4.98) + zoom);
  look.set(0.0, portrait ? -0.28 : -0.2, 0.02);
}
function applySkullCameraCinemaMotion(portrait, shelfComposition) {
  ...
  var lateral =
    Math.sin(cinemaT * 0.08) * 0.03 * idleDamp +
    beatCam.thetaKick * 0.78 * beatDamp;
  var vertical =
    Math.sin(cinemaT * 0.06 + 1.0) * 0.024 * idleDamp +
    beatCam.phiKick * 0.68 * beatDamp;
  var depth =
    Math.sin(cinemaT * 0.04 + 2.0) * 0.075 * idleDamp -
    beatCam.radiusKick * 0.58 * beatDamp;
  if (portrait) {
    lateral *= 0.72;
    vertical *= 0.88;
  }
  ...
  camera.lookAt(skullCameraMixedLook);
  camera.rotation.z +=
    beatCam.rollKick * beatDamp * (shelfComposition ? 0.42 : 0.7);
}
function applySkullCameraPose(dt) {
  ...
  skullCameraBlend +=
    ((active ? 1 : 0) - skullCameraBlend) *
    Math.min(1, dt * (active ? 4.8 : 7.2));
  if (skullCameraBlend < 0.002) return;
  skullWheelZoom +=
    (skullWheelZoomTarget - skullWheelZoom) * Math.min(1, dt * 8.0);
  var portrait = innerHeight > innerWidth * 1.08;
  var shelfComposition = isSkullShelfCompositionActive();
  var shelfMixTarget = shelfComposition ? 1 : 0;
  skullShelfCameraMix +=
    (shelfMixTarget - skullShelfCameraMix) *
    Math.min(1, dt * (shelfMixTarget > skullShelfCameraMix ? 4.6 : 5.8));
  if (Math.abs(skullShelfCameraMix - shelfMixTarget) < 0.002)
    skullShelfCameraMix = shelfMixTarget;
  setSkullCameraTargetVectors(
    skullCameraBasePos,
    skullCameraBaseLook,
    portrait,
  ...
  setSkullCameraTargetVectors(
    skullCameraShelfPos,
    skullCameraShelfLook,
    portrait,
    true,
    skullWheelZoom,
  );
  skullCameraTargetPos
    .copy(skullCameraBasePos)
    .lerp(skullCameraShelfPos, skullShelfCameraMix);
  skullCameraTargetLook
    .copy(skullCameraBaseLook)
    .lerp(skullCameraShelfLook, skullShelfCameraMix);
  camera.position.lerp(skullCameraTargetPos, skullCameraBlend);
  skullCameraMixedLook
    .set(orbit.lookAt.x, orbit.lookAt.y, orbit.lookAt.z)
    .lerp(skullCameraTargetLook, skullCameraBlend);
  camera.lookAt(skullCameraMixedLook);
...
function updateSkullParticleLayer(dt) {
  ...
  skullParticleOpacity +=
    (target - skullParticleOpacity) * Math.min(1, dt * (active ? 3.2 : 2.4));
  if (skullParticleOpacity < 0.006 && !active) {
    skullParticleGroup.visible = false;
    return;
  }
  skullParticleGroup.visible = true;
  skullParticleGroup.material.uniforms.uOpacity.value =
    skullParticleOpacity *
    clampRange(0.78 + (fx.intensity || 0.85) * 0.18, 0.56, 1.0);
  var beatTransient = clampRange(Math.max(0, beatPulse - 0.16) / 0.84, 0, 1.35);
  var flashTarget = clampRange(
    Math.pow(beatTransient, 1.34) * 1.08 +
      Math.max(0, bass - 0.6) * 0.18 * beatTransient,
    0,
    1,
  );
  skullBeatFlash +=
    (flashTarget - skullBeatFlash) *
    Math.min(1, dt * (flashTarget > skullBeatFlash ? 24.0 : 6.2));
  if (skullParticleGroup.material.uniforms.uSkullFlash)
    skullParticleGroup.material.uniforms.uSkullFlash.value = skullBeatFlash;
  var jawTarget = clampRange(
    0.6 +
      (0.5 + 0.5 * Math.sin(uniforms.uTime.value * 0.5)) * 0.05 +
      bass * 0.06 +
      skullBeatFlash * 0.09,
    0.52,
    0.88,
  ...
  skullJawOpen +=
    (jawTarget - skullJawOpen) *
    Math.min(1, dt * (jawTarget > skullJawOpen ? 7.8 : 3.4));
  if (skullParticleGroup.material.uniforms.uJawOpen)
    skullParticleGroup.material.uniforms.uJawOpen.value = skullJawOpen;
  var shelfComposition = isSkullShelfCompositionActive();
  var shelfMix = clampRange(
    skullShelfCameraMix || (shelfComposition ? 1 : 0),
    0,
    1,
  );
  var drift = skullBreathOffset(uniforms.uTime.value, shelfComposition);
  var ampTarget = clampRange(
    bass * 0.006 + mid * 0.004 + skullBeatFlash * 0.07,
    0,
    0.09,
  );
  skullAmpPulse +=
    (ampTarget - skullAmpPulse) *
    Math.min(1, dt * (ampTarget > skullAmpPulse ? 11.0 : 4.0));
  var shelfScale = 3.02;
  var targetScale =
    (SKULL_MODEL_SCALE + (shelfScale - SKULL_MODEL_SCALE) * shelfMix) *
    (1 + skullAmpPulse) *
    clampRange(1 - skullWheelZoom * 0.055, 0.92, 1.08);
  var shelfX = -1.18;
  var shelfY = 0.32;
  var targetX =
    SKULL_MODEL_BASE_POSITION.x +
    (shelfX - SKULL_MODEL_BASE_POSITION.x) * shelfMix +
    drift.x;
  var targetY =
    SKULL_MODEL_BASE_POSITION.y +
    (shelfY - SKULL_MODEL_BASE_POSITION.y) * shelfMix +
    drift.y;
  var targetZ = SKULL_MODEL_BASE_POSITION.z + drift.z;
  skullParticleGroup.position.x +=
    (targetX - skullParticleGroup.position.x) * Math.min(1, dt * 4.2);
  skullParticleGroup.position.y +=
    (targetY - skullParticleGroup.position.y) * Math.min(1, dt * 4.8);
  skullParticleGroup.position.z +=
    (targetZ - skullParticleGroup.position.z) * Math.min(1, dt * 4.2);
  skullParticleGroup.scale.x +=
    (targetScale - skullParticleGroup.scale.x) * Math.min(1, dt * 4.6);
  ...
  skullParticleGroup.rotation.y +=
    (targetRotY - skullParticleGroup.rotation.y) * rotEase;
  skullParticleGroup.rotation.x +=
    (targetRotX - skullParticleGroup.rotation.x) * rotEase;
  skullParticleGroup.rotation.z +=
    (0 - skullParticleGroup.rotation.z) * Math.min(1, dt * 6.0);
}
