...
test("native layered guard puts the complete main HWND below Explorer icons and restores all snapshots", () => {
  ...
  assert.match(
    script,
    /SetLayeredWindowAttributes\(_listView, DESKTOP_LAYER_COLOR_KEY, 255, LWA_COLORKEY\)/,
  );
  assert.match(
    script,
  ...
  assert.match(
    script,
    /SendMessageTimeout\(_listView, message, wParam, lParam/,
  );
  assert.match(script, /SMTO_ABORTIFHUNG, 500/);
  assert.match(
    script,
    /DESKTOP_ICON_LAYER_BACKGROUND_(?:QUERY|APPLY|RESTORE)_TIMEOUT/,
  );
  ...
  assert.match(
    script,
    /finally \{[\s\S]*try \{ RestoreCurrentListViewBackground\(\); \}[\s\S]*if \(backgroundRestoreFailure == null\) \{[\s\S]*try \{ RestoreCurrentListViewTransparency\(\); \}[\s\S]*try \{ RestoreOriginalListViewVisibility\(\); \}/,
  );
  assert.match(
    script,
    /private static void RestoreCurrentListViewTransparency\(\) \{[\s\S]*if \(!BoundTargetIdentityMatches\(\)\)[\s\S]*SetWindowLongPtr\(_listView/,
  );
  assert.match(
    script,
    /private static void RestoreOriginalListViewVisibility\(\) \{[\s\S]*if \(!BoundTargetIdentityMatches\(\)\)[\s\S]*ShowWindow\(_listView/,
  );
  assert.match(
    script,
  ...
  assert.match(
    script,
    /topLevelHost == _topLevelHost && iconHost == _iconHost && listView == _listView/,
  );
  assert.match(
    script,
...
test("external guard keeps WMI crash recovery but runs PowerShell under headless System32 conhost", () => {
  ...
  assert.match(
    source,
    /--headless .* -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass/,
  );
  assert.match(
    source,
...
test("native layered ACK rejects an unconfirmed Explorer background key", () => {
  ...
  assert.throws(
    () => parseNativeIconLayerLayout(missing),
  ...
  assert.throws(
    () =>
      parseNativeIconLayerLayout({
        ...layout(),
        nativeBackgroundKeyApplied: false,
      }),
    /DESKTOP_ICON_LAYER_ACK_INVALID/,
  );
});
...
test("native layered wrapper exposes explicit icon visibility and terminal diagnostics", async () => {
  ...
  child.stdin.on("data", (chunk) => {
    ...
    if (control) {
      ...
      setImmediate(() =>
        child.stdout.write(
          `${JSON.stringify(layout(sequence, locked, desktopIconsVisible))}\n`,
        ),
      );
    }
    if (/^Q$/m.test(text))
  ...
  assert.equal(
    (await watcher.updateShields([{ x: 10, y: 20, width: 30, height: 40 }]))
      .controlSequence,
    0,
  );
  ...
  assert.equal(
    (await watcher.setIconsVisible(false)).desktopIconsVisible,
    false,
  );
  ...
  assert.equal(
    exitDetails && exitDetails.terminalError,
    "DESKTOP_ICON_TEST_TERMINAL",
  );
});
