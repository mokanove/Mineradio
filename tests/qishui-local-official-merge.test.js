...
async function testPcSearchAndPublicFallback() {
  ...
  await withHttpsMock(
    ({ url, options }) => {
      ...
      if (
        parsed.hostname === "api.qishui.com" &&
        parsed.pathname === "/luna/pc/search/track"
      ) {
        ...
        assert(
          /sessionid=fixture-session/.test(
            String((options.headers && options.headers.Cookie) || ""),
          ),
        );
        return {
          body: {
            data: {
              result_groups: [
                { data: [pcSearchTrack("pc-search-1", "本地搜索测试")] },
              ],
              has_more: false,
            },
          },
      ...
      throw new Error(
        "Unexpected request: " + parsed.hostname + parsed.pathname,
      );
    },
    async () => {
      const result = await qishui.handleQishuiSearch(
        "本地搜索测试",
        8,
        cookie,
        0,
  ...
  await withHttpsMock(
    ({ url }) => {
      ...
      if (
        parsed.hostname === "api-vehicle.volcengine.com" &&
        parsed.pathname === "/v2/search/type"
      ) {
        return {
          body: {
            data: {
    ...
    async () => {
      ...
      assert(
        result.pcSearchError,
        "PC search failure must be retained as diagnostic context",
      );
    },
  );
}
...
async function testTrackV2GetFallbackAndBitratePriority() {
  ...
  await withHttpsMock(
    ({ url, options }) => {
      ...
      if (
        parsed.hostname !== "api.qishui.com" ||
        parsed.pathname !== "/luna/pc/track_v2"
      ) {
        throw new Error(
          "Unexpected request: " + parsed.hostname + parsed.pathname,
        );
      ...
      if (method === "POST")
        return { statusCode: 503, body: { message: "fixture POST failure" } };
      assert.strictEqual(
    ...
    async () => {
      ...
      assert.strictEqual(
        result.url,
        "https://media.example/audio.m4a?br=128000",
      );
  ...
  await withHttpsMock(
            ...
            track: {
              ...
              audio_info: {
                play_info_list: [
                  {
                    main_play_url:
                      "https://media.example/primary.m4a?br=128000",
                    duration: 180,
                    format: "m4a",
                  },
                ],
    ...
    async () => {
      ...
      assert.strictEqual(
        result.url,
        "https://media.example/primary.m4a?br=128000",
        "bit_rates playable_url must remain a last-resort source",
      );
    },
  );
}
...
async function testLyricFallbackAndConversion() {
  ...
  assert.strictEqual(
    converted.yrc,
    "[1000,2000](1000,500,0)世(1500,500,0)界\n[4000,1000](4000,1000,0)和平",
  );
  ...
  await withHttpsMock(
    ...
    async () => {
      const result = await qishui.handleQishuiLyric(
        "lyric-track-fixture",
...
async function testPcAccountWritesAndComments() {
  ...
  await withHttpsMock(
    ({ url }) => {
      ...
      if (parsed.pathname === "/luna/pc/me/collection/mixed") {
        return {
          body: {
            data: {
              media_resources: [pcSearchTrack("liked-fixture", "已喜欢歌曲")],
            },
          },
        };
      }
      if (
        parsed.pathname === "/luna/pc/user/playlist" ||
        parsed.pathname === "/luna/pc/me/recently-played-media"
      ) {
    ...
    async () => {
      const checked = await qishui.handleQishuiCheckTracksLiked(
        ["liked-fixture", "unknown-fixture"],
        cookie,
      );
      ...
      assert.strictEqual(
        checked.complete,
        false,
        "a bounded library page must not pretend to be a complete like index",
      );
    },
  );
  ...
  await withHttpsMock(
    ...
    async () => {
      assert.strictEqual(
        (await qishui.handleQishuiSetTrackLiked("track-like", true, cookie))
          .liked,
        true,
      );
      assert.strictEqual(
        (await qishui.handleQishuiSetTrackLiked("track-like", false, cookie))
          .liked,
        false,
      );
      assert.strictEqual(
        (
          await qishui.handleQishuiSetPlaylistCollected(
            "playlist-one",
            true,
            cookie,
          )
      ...
      assert.strictEqual(
        (
          await qishui.handleQishuiPlaylistAddSong(
            "playlist-one",
            { providerSongId: "playlist-track" },
            cookie,
          )
        ).success,
        true,
      ...
      assert.strictEqual(
        (await qishui.handleQishuiSetAlbumCollected("album-one", true, cookie))
          .collected,
        true,
      );
      assert.strictEqual(
        (await qishui.handleQishuiSetAlbumCollected("album-one", false, cookie))
          .collected,
        false,
      );
      assert.strictEqual(
        (await qishui.handleQishuiReportRecentlyPlayed("recent-track", cookie))
          .reported,
        true,
      );
      const comments = await qishui.handleQishuiComments(
        "comment-track",
        { count: 12, cursor: "" },
        cookie,
      );
      ...
      const created = await qishui.handleQishuiCreateComment(
        "comment-track",
        "新评论",
        cookie,
      );
  ...
  assert.deepStrictEqual(
    writeByPath.get("/luna/pc/me/collection/media/delete"),
    {
      media: [{ type: "track", id: "track-like" }],
      scene: "",
    },
  );
  assert.deepStrictEqual(writeByPath.get("/luna/pc/me/collection/playlist"), {
    playlist_ids: ["playlist-one"],
  });
  assert.deepStrictEqual(
    writeByPath.get("/luna/pc/me/collection/playlist/delete"),
    { playlist_ids: ["playlist-one"] },
  );
  ...
  assert.deepStrictEqual(writeByPath.get("/luna/pc/me/collection/album"), {
    album_ids: ["album-one"],
  });
  assert.deepStrictEqual(
    writeByPath.get("/luna/pc/me/collection/album/delete"),
    { album_ids: ["album-one"] },
  );
...
async function run() {
  const source = fs.readFileSync(
    path.join(__dirname, "..", "qishui-api.js"),
    "utf8",
  );
  ...
  console.log(
    "[OK] Qishui local PC search, playback fallback, lyrics, collections, recent-play, and comments verified.",
  );
}
