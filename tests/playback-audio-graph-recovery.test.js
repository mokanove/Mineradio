...
function makeHarness(options = {}) {
  ...
  const audioContext = {
    ...
    createMediaStreamSource(stream) {
      ...
      assert.ok(
        stream && stream.id,
        "capture stream was not forwarded to AudioContext",
      );
  ...
  const context = vm.createContext({
    ...
    Audio: function ReplacementAudio() {
      ...
      if (!options.allowReplacement) {
        throw new Error(
          "Audio element replacement is forbidden during this graph recovery test",
        );
      }
      return makeAudio();
    },
  ...
  context.replaceAudioElementForGraphRecovery = function observedReplacement(
    reason,
    opts,
  ) {
    calls.replace += 1;
...
function testCaptureTrackSwitchCreatesFreshAudioLifetime() {
  ...
  context.source = context.audioCtx.createMediaStreamSource(
    audio.captureStream(),
  );
  ...
  assert.equal(
    context.source.__mineradioUsesCapture,
    true,
    "test did not establish the legacy capture-backed state",
  );
  assert.equal(calls.createMediaStreamSource, 1);
  assert.strictEqual(
    context.audio,
    originalAudio,
    "capture recovery replaced the Audio element",
  );
  assert.equal(
    audio.__mineradioMediaSourceBound,
    true,
    "lifetime MediaElementSource marker was cleared",
  );
  ...
  assert.notStrictEqual(
    freshAudio,
    originalAudio,
    "capture-backed track switch reused the frozen Audio lifetime",
  );
  assert.equal(
    calls.replace,
    1,
    "capture-backed track switch did not perform one controlled replacement",
  );
  ...
  assert.equal(
    calls.pause,
    1,
    "old capture-backed media must be paused exactly once",
  );
  assert.equal(originalAudio.paused, true);
  assert.equal(
    originalAudio.src,
    oldSrc,
    "replacement unexpectedly rewrote the old media URL",
  );
  ...
  assert.equal(
    freshAudio.__mineradioQueueItemKey,
    "queue-alpha",
    "fresh Audio lost queue identity",
  );
  assert.equal(freshAudio.src, "", "fresh Audio inherited the old track URL");
  assert.equal(
    freshAudio.currentSrc,
    "",
    "fresh Audio inherited the old currentSrc",
  );
  assert.equal(
    freshAudio.onended,
    null,
    "fresh Audio inherited the old ended handler",
  );
  assert.equal(
    freshAudio.onloadedmetadata,
    null,
    "fresh Audio inherited the old metadata handler",
  );
  ...
  assert.equal(
    context.source.__mineradioUsesCapture,
    false,
    "fresh Audio did not establish MediaElementSource",
  );
  assert.equal(
    calls.createMediaElementSource,
    2,
    "fresh Audio did not get its own lifetime MediaElementSource",
  );
  assert.equal(
    calls.createMediaStreamSource,
    1,
    "fresh Audio unexpectedly retained the old capture graph",
  );
  assert.equal(
    calls.replace,
    1,
    "fresh Audio init fell into media-source-rebind replacement",
  );
...
function testFrozenClockDoesNotTriggerCaptureRebuild() {
  ...
  assert.equal(
    calls.createMediaStreamSource,
    0,
    "one advance followed by a frozen media clock rebuilt capture",
  );
  assert.equal(context.source.__mineradioUsesCapture, false);
}
...
function testTwoAdvancingSilentSamplesTriggerCaptureRebuild() {
  ...
  assert.strictEqual(
    context.source,
    lifetimeMediaSource,
    "one silent sample replaced the lifetime media source",
  );
  runNextTimer(0.82);
  ...
  assert.strictEqual(
    context.source,
    lifetimeMediaSource,
    "silent analyser recovery replaced the lifetime MediaElementSource",
  );
  assert.equal(
    context.source.__mineradioUsesCapture,
    false,
    "bound media was incorrectly moved to captureStream",
  );
  assert.equal(
    calls.createMediaElementSource,
    1,
    "silent analyser recovery rebound MediaElementSource",
  );
  assert.equal(
    calls.createMediaStreamSource,
    0,
    "silent analyser recovery created a capture source for bound media",
  );
  assert.equal(
    calls.captureStream,
    0,
    "silent analyser recovery called captureStream for bound media",
  );
  assert.equal(
    calls.replace,
    0,
    "silent analyser recovery replaced the media element",
  );
  assert.equal(calls.pause, 0, "silent analyser recovery paused playback");
  assert.equal(
    audio.currentTime,
    0.82,
    "silent analyser recovery rewound or froze the media clock",
  );
  assert.equal(
    audio.paused,
    pausedBeforeRecovery,
    "silent analyser recovery changed paused state",
  );
}
...
function testTransientCaptureSourceFailureRetainsForcedCaptureRetry() {
  ...
  assert.equal(
    audio.__mineradioForceCaptureSource,
    true,
    "failed capture attempt consumed the forced-capture retry",
  );
  assert.equal(
    calls.createMediaElementSource,
    0,
    "failed fresh capture attempt unexpectedly bound MediaElementSource",
  );
  ...
  assert.equal(
    context.initAudio(),
    true,
    "second capture attempt did not recover after the track became ready",
  );
  ...
  assert.equal(
    calls.createMediaElementSource,
    0,
    "fresh capture retry fell back to MediaElementSource",
  );
...
function testForcedCaptureWaitsForMediaReadyState() {
  ...
  assert.equal(
    context.initAudio(),
    false,
    "force-capture init must wait while media has no decoded data",
  );
  assert.equal(
    calls.captureStream,
    0,
    "captureStream was called before media reached HAVE_CURRENT_DATA",
  );
  assert.equal(
    calls.createMediaStreamSource,
    0,
    "MediaStreamSource was created before media became ready",
  );
  assert.equal(
    calls.createMediaElementSource,
    0,
    "ready-state wait fell back to a forbidden MediaElementSource rebind",
  );
  ...
  assert.equal(
    audio.__mineradioForceCaptureSource,
    true,
    "ready-state wait consumed the forced-capture marker",
  );
  assert.strictEqual(context.audio, originalAudio);
  ...
  assert.equal(
    context.initAudio(),
    true,
    "force-capture init did not recover after media became ready",
  );
