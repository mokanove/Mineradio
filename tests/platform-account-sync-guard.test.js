...
assert(
  /snapshot\.provider\s*\|\|\s*snapshot\.sourceKey\s*\|\|\s*snapshot\.resolvedPlaybackProvider/.test(
    listenStats,
  ),
  "listen reports must follow the song account provider before a matched playback source",
);
assert(
  /body\.provider\s*\|\|\s*song\.provider\s*\|\|\s*song\.source\s*\|\|\s*song\.sourceKey\s*\|\|\s*song\.type\s*\|\|\s*song\.resolvedPlaybackProvider/.test(
    server,
  ),
...
console.log(
  "[OK] Platform account actions, provider-affine listen reports, and truthful duration-sync boundaries are guarded.",
);
