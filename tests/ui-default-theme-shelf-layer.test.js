...
function objectPropertyCall(source, propertyName, functionName) {
  ...
  assert.notEqual(
    propertyStart,
    -1,
    `expected ${propertyName} ${functionName}() assignment`,
  );
...
test("first-launch UI accent defaults and missing-field fallback stay white", () => {
  ...
  const persistence = read(
    "public/js/modules/02-visual/04-visual-settings-persistence.js",
  );
...
test("persistent shelf stays behind lyrics until selection or detail raises it", () => {
  ...
  assert.ok(
    sideLayer,
    "expected side shelf to raise only for detail, pin, or lifted selection",
  );
  assert.ok(
    stageLayer,
    "expected stage shelf to raise only for detail or selection",
  );
  ...
  assert.ok(
    sidePassive < normalLyrics,
    "passive side shelf must render behind lyrics",
  );
  assert.ok(
    stagePassive < normalLyrics,
    "unselected stage shelf must render behind lyrics",
  );
  assert.ok(
    sideRaised > normalLyrics,
    "selected/pinned/detail side shelf must render above lyrics",
  );
  assert.ok(
    stageRaised > normalLyrics,
    "selected/detail stage shelf must render above lyrics",
  );
  assert.ok(
    detailRaised > normalLyrics,
    "opened shelf detail must render above lyrics",
  );
});
...
test("the removed stage floor reflection cannot regress", () => {
  ...
  assert.doesNotMatch(
    shelf,
    /new\s+THREE\.PlaneGeometry\(\s*10\s*,\s*1\.8\s*\)/,
  );
});
