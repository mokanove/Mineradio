...
test("daily recommendation mapper preserves every valid upstream song in order", () => {
  const mapperSource = namedFunctionSource(
    source,
...
test("discover home returns the complete mapped daily list without a fixed song cap", () => {
  ...
  assert.match(
    discoverSource,
    /dailySongs\s*=\s*mapDailyRecommendationSongs\(raw\)/,
  );
  assert.doesNotMatch(
    discoverSource,
    /dailySongs[\s\S]{0,300}\.slice\s*\(\s*0\s*,\s*(?:8|12)\s*\)/,
  );
