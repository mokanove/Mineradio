...
class FakeBrowserWindow extends EventEmitter {
  constructor(options = {}) {
    ...
    this.bounds = {
      ...(options.bounds || { x: 120, y: 90, width: 1180, height: 720 }),
    };
  ...
  setResizable(value) {
    ...
    this.calls.push(["setResizable", !!value]);
  }
  setMovable(value) {
    this.movable = !!value;
    this.calls.push(["setMovable", !!value]);
  }
  setFocusable(value) {
  ...
  setHasShadow(value) {
    ...
    this.calls.push(["setHasShadow", !!value]);
  }
...
function makeRuntime(options = {}) {
  ...
  const runtime = new FullDesktopModeRuntime({
    ...
    attachNative: async (input) => {
      ...
      if (typeof options.attachNative === "function")
        return options.attachNative(input, calls.attach.length);
      return {
        ok: true,
    ...
    detachNative: async (input) => {
      ...
      if (typeof options.detachNative === "function")
        return options.detachNative(input, calls.detach.length);
      return {
        ok: true,
        targetWindowId: String((input && input.hwnd) || "424242"),
      };
    ...
    probeDesktopIcons: async (input) => {
      ...
      if (typeof options.probeDesktopIcons === "function")
        return options.probeDesktopIcons(input, calls.probe.length);
      return {
        ok: true,
    ...
    startDesktopIconWatcher: (input) => {
      ...
      if (typeof options.startDesktopIconWatcher === "function") {
        return options.startDesktopIconWatcher(
          input,
          calls.watcherStart.length,
          watcher,
        );
      }
      return watcher;
...
test("enable defaults to a fully interactive Mineradio desktop", async () => {
  ...
  assert.deepEqual(status.pointerRoute, {
  ...
  assert.deepEqual(status.safeInsets, {
    top: 0,
    right: 0,
    bottom: 40,
    left: 0,
  ...
  assert.equal(
    win.shadowEnabled,
    false,
    "desktop child must not retain a top-level native shadow",
  );
  assert.equal(
    calls.attach.length,
    0,
    "interactive enable must not enter the passive wallpaper WorkerW",
  );
  assert.equal(
    calls.coexist.length,
    1,
    "interactive enable must attach the complete Mineradio surface below the real desktop icons",
  );
  assert.equal(
    calls.detach.length,
    0,
    "initial top-level window must not make an unnecessary detach round trip",
  );
  assert.equal(
    win.calls.some((call) => call[0] === "hide"),
    true,
    "coexistence attach must stay hidden until native color-key ACK",
  );
  assert.equal(
    win.calls.some((call) => call[0] === "focus"),
    false,
    "interactive desktop must not steal focus",
  );
  const routeCall = win.calls
    .filter((call) => call[0] === "setIgnoreMouseEvents")
    .at(-1);
  assert.deepEqual(routeCall, ["setIgnoreMouseEvents", false, undefined]);
  assert.deepEqual(
    win.shape,
    [],
    "continuous Mineradio surface must not contain desktop icon holes",
  );
});
...
test("toggling to passive mode attaches the same main window to WorkerW", async () => {
  ...
  assert.equal(win.webContents.backgroundThrottling, false);
  const ignoreCall = win.calls
    .filter((call) => call[0] === "setIgnoreMouseEvents" && call[1] === true)
    .at(-1);
  assert.equal(
    ignoreCall && ignoreCall[2],
    undefined,
    "passive mode must not enable forwarded synthetic pointer events",
  );
...
test("passive preparation is serialized before hiding or attaching the main window", async () => {
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "prepare-order-enable",
  });
  ...
  assert.equal(
    calls.attach.length,
    0,
    "WorkerW attach ran before preview/session preparation settled",
  );
  assert.equal(
    win.visible,
    true,
    "the interactive window was hidden before preparation settled",
  );
  assert.equal(
    win.calls.filter((call) => call[0] === "hide").length,
    hideCountBefore,
  );
...
test("passive preparation failure leaves the full Mineradio window interactive", async () => {
  ...
  const { runtime, calls } = makeRuntime({
    beforePassive: async () => ({
      ok: false,
      error: "SYNTHETIC_PREVIEW_PREPARE_FAILURE",
    }),
  });
  await runtime.enable(win, {
    interactive: true,
    reason: "prepare-failure-enable",
  });
  const hideCountBefore = win.calls.filter((call) => call[0] === "hide").length;
  ...
  assert.equal(
    calls.attach.length,
    0,
    "failed preparation must not enter WorkerW",
  );
  assert.equal(
    win.calls.filter((call) => call[0] === "hide").length,
    hideCountBefore,
...
test("dispose during passive preparation prevents a late WorkerW attach", async () => {
  ...
  prepared.resolve({ ok: true, preview: true, stopped: true });
  const [transitionResult, disposeResult] = await Promise.all([
    transition,
    disposing,
  ]);
  ...
  assert.match(
    String(transitionResult.error),
    /FULL_DESKTOP_PASSIVE_PREPARE_SUPERSEDED/,
  );
  ...
  assert.equal(
    calls.attach.length,
    0,
    "dispose allowed a prepared transition to attach after cancellation",
  );
  assert.equal(win.visible, true);
  assert.equal(win.ignoreMouse, false);
});
...
test("disable detaches and restores the exact pre-desktop window state", async () => {
  ...
  assert.equal(status.interactive, false);
  assert.ok(
    calls.detach.length > detachCountBefore,
    "disable must detach a passive WorkerW child before restoring",
  );
  ...
  assert.equal(
    win.shadowEnabled,
    true,
    "normal-window native shadow was not restored",
  );
});
...
test("software lock passes normal areas through while the controller remains an unlock island", async () => {
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "software-lock-enable",
  });

  const before = runtime.getStatus("software-lock-before");
  assert.deepEqual(before.iconRevealRects, [
    { x: 8, y: 8, width: 86, height: 92 },
  ]);
  ...
  const locked = await runtime.setSoftwareInteractionLocked(
    true,
    "software-lock-on",
  );
  ...
  assert.equal(
    win.ignoreMouse,
    true,
    "locked software areas must pass input through to Explorer",
  );
  const lockedRouteCall = win.calls
    .filter((call) => call[0] === "setIgnoreMouseEvents")
    .at(-1);
  assert.deepEqual(
    lockedRouteCall,
  ...
  assert.equal(
    win.ignoreMouse,
    false,
    "the same top-right controller must stay clickable while locked",
  );
  assert.equal(runtime.interactiveBindingHealthy(), true);

  runtime.updatePointerRoute({
    overSoftwareUi: false,
    overDesktopControls: false,
  });
  assert.equal(
    win.ignoreMouse,
    true,
    "leaving the controller must restore locked desktop pass-through",
  );
  assert.equal(runtime.interactiveBindingHealthy(), true);
  ...
  assert.equal(
    win.ignoreMouse,
    false,
    "entering the recovery corridor must make unlock reachable again",
  );

  const unlocked = await runtime.setIconInteractionLocked(
    false,
    "legacy-icon-lock-off",
  );
  ...
  assert.equal(
    win.ignoreMouse,
    false,
    "unlock from the controller must immediately restore natural Mineradio input",
  );
  runtime.updatePointerRoute({
    overSoftwareUi: true,
    overDesktopControls: false,
  ...
  assert.equal(
    win.ignoreMouse,
    false,
    "unlocked software UI must receive natural pointer input",
  );
  assert.deepEqual(win.shape, []);
});
...
test("trusted desktop pointer focus restores web contents without raising the DefView child window", async () => {
  ...
  assert.equal(
    win.focused,
    false,
    "web-content focus must not activate or raise the DefView child HWND",
  );
  assert.equal(
    win.calls.some((call) => call[0] === "focus"),
    false,
...
test("software lock blurs keyboard focus and rejects focus requests until unlocked", async () => {
  ...
  assert.equal(win.focusable, false);
  assert.equal(
    win.calls.some((call) => call[0] === "blur"),
    true,
  );
...
test("keyboard focus requests are rejected outside interactive coexistence mode", async () => {
  ...
  await runtime.enable(win, {
    interactive: false,
    reason: "keyboard-passive-enable",
  });
...
test("software lock is rejected while inactive and disable always restores normal window input", async () => {
  ...
  const inactive = await runtime.setSoftwareInteractionLocked(
    true,
    "software-lock-inactive",
  );
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "software-lock-cleanup-enable",
  });
  runtime.updatePointerRoute({
    overSoftwareUi: false,
    overDesktopControls: false,
  ...
  assert.equal(win.ignoreMouse, false);
  assert.equal(
    runtime.getStatus("software-lock-cleanup-status").softwareInteractionLocked,
    false,
  );
});
...
test("desktop icon visibility uses the watcher and confirmed stop restores visible state", async () => {
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "icons-visible-enable",
  });

  assert.deepEqual(
    calls.iconVisibility,
    [],
    "entering must preserve Explorer's original visible state",
  );
  const hidden = await runtime.setDesktopIconsVisible(false, "icons-hidden");
  assert.equal(hidden.ok, true);
  assert.equal(hidden.desktopIconsVisible, false);
  assert.equal(
    runtime.getStatus("icons-hidden-status").desktopIconsVisible,
    false,
  );
  ...
  assert.equal(
    runtime.getStatus("icons-restored-after-stop").desktopIconsVisible,
    true,
  );
});
...
test("unchanged desktop icon visibility is reasserted through the native watcher", async () => {
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "icons-reassert-enable",
  });

  const shown = await runtime.setDesktopIconsVisible(
    true,
    "icons-reassert-visible",
  );
...
test("desktop icon visibility failure rolls software state back to the acknowledged value", async () => {
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "icons-rollback-enable",
  });
  failNextHide = true;

  const failed = await runtime.setDesktopIconsVisible(
    false,
    "icons-rollback-failed",
  );
  assert.equal(failed.ok, false);
  assert.match(failed.error, /SYNTHETIC_ICON_VISIBILITY_FAILURE/);
  assert.equal(failed.desktopIconsVisible, true);
  assert.equal(
    runtime.getStatus("icons-rollback-status").desktopIconsVisible,
    true,
  );
  assert.deepEqual(
    calls.iconVisibility,
    [true],
    "watcher was not restored to the previous visibility",
  );
});
...
test("desktop mode adopts an initially hidden Explorer icon state without forcing it visible", async () => {
  ...
  const enabled = await runtime.enable(win, {
    interactive: true,
    reason: "icons-initially-hidden-enable",
  });
  ...
  assert.deepEqual(
    calls.iconVisibility,
    [],
    "entering forced a hidden Explorer desktop visible",
  );
...
test("rapid interaction changes are serialized and finish disabled", async () => {
  ...
  const enabling = runtime.enable(win, {
    interactive: false,
    reason: "rapid-enable",
  });
  ...
  assert.equal(
    maxNativeInFlight,
    1,
    "native attach/detach operations overlapped",
  );
  assert.equal(win.ignoreMouse, false);
  assert.equal(win.focusable, true);
});
...
test("native attach failure fails closed and restores the normal window", async () => {
  ...
  let result;
  try {
    result = await runtime.enable(win, {
      interactive: false,
  ...
  assert.equal(status.interactive, false);
  assert.match(
    String(result.error || status.lastError),
    /SYNTHETIC_ATTACH_FAILURE/,
  );
  assert.deepEqual(win.getBounds(), originalBounds);
  assert.equal(win.ignoreMouse, false);
  assert.equal(win.focusable, true);
...
test("coexist attach places the complete Mineradio surface below SysListView without mouse synthesis", () => {
  ...
  assert.match(
    script,
    /SetWindowPos\(\$target, \[IntPtr\]::new\(1\)[\s\S]*0x0030/,
  );
  assert.doesNotMatch(
    script,
    /SetWindowPos\(\$target, \[IntPtr\]::Zero[\s\S]*0x0030/,
  );
  assert.doesNotMatch(
    script,
    /GetCursorPos|SetCursorPos|SendInput|SetWindowsHookEx|WM_MOUSEMOVE/,
  );
});
...
test("native detach validates parent only after converting WS_CHILD to WS_POPUP", () => {
  ...
  assert.ok(
    styleWrite >= 0,
    "detach script must convert the native window style",
  );
  assert.ok(
    parentValidation > styleWrite,
    "parent validation ran before WS_CHILD was cleared",
  );
});
...
test("partial detach failure is strictly recovered to a known WorkerW passive state", async () => {
  ...
  await runtime.enable(win, {
    interactive: false,
    reason: "partial-detach-enable",
  });
  const result = await runtime.setInteractive(
    true,
    "partial-detach-interactive",
  );
  ...
  assert.equal(
    calls.attach.length,
    2,
    "detach failure did not perform a strict WorkerW reattach",
  );
  assert.equal(win.visible, true);
  assert.equal(win.ignoreMouse, true);
  assert.equal(win.focusable, false);
...
test("failed detach recovery hides the window and never reports a stale passive attachment", async () => {
  ...
  await runtime.enable(win, {
    interactive: false,
    reason: "unknown-state-enable",
  });
  const result = await runtime.setInteractive(
    true,
    "unknown-state-interactive",
  );
  ...
  assert.equal(
    status.enabled,
    true,
    "runtime must retain cleanup ownership of an uncertain HWND",
  );
  assert.equal(status.interactive, false);
  assert.equal(
    status.embedded,
    false,
    "stale WorkerW acknowledgement was reported after recovery failed",
  );
  ...
  assert.equal(
    win.visible,
    false,
    "unknown native state must remain hidden instead of covering the desktop",
  );
  assert.equal(win.ignoreMouse, true);
  ...
  assert.equal(
    calls.detach.length,
    2,
    "fail-closed path did not attempt a final strict top-level restore",
  );
});
...
test("failed WorkerW recovery falls back to a verified disabled top-level window when possible", async () => {
  ...
  await runtime.enable(win, {
    interactive: false,
    reason: "verified-fallback-enable",
  });
  const result = await runtime.setInteractive(
    true,
    "verified-fallback-interactive",
  );
...
test("removing the saved display rebases normal-window restore bounds onto an available display", async () => {
  ...
  const screen = {
    ...
    getDisplayMatching: (bounds) => {
      const centerX =
        Number((bounds && bounds.x) || 0) +
        Number((bounds && bounds.width) || 1) / 2;
      return (
        displays.find(
          (display) =>
  ...
  assert.notDeepEqual(
    restored,
    originalBounds,
    "window was restored to the disconnected display",
  );
});
...
test("dispose invalidates queued interaction work before final cleanup", async () => {
  ...
  const enabling = runtime.enable(win, {
    interactive: false,
    reason: "dispose-enable",
  });
  const queuedInteractive = runtime.setInteractive(
    true,
    "dispose-queued-interactive",
  );
  ...
  assert.equal(
    calls.attach.length,
    0,
    "dispose did not invalidate queued native attach work",
  );
});
...
test("unconfirmed native-layer stop keeps the same watcher and cannot be bypassed by a second stop", async () => {
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "restore-gate-enable",
  });
  await assert.rejects(
    runtime.stopIconShapeWatcher(),
  ...
  assert.equal(
    stopCalls,
    2,
    "second stop silently bypassed the unconfirmed guard",
  );
...
test("restore-unconfirmed watcher exit taints native state and blocks a clean disable report", async () => {
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "unconfirmed-exit-enable",
  });
  assert.equal(
    (await runtime.setDesktopIconsVisible(false, "unconfirmed-icons-hidden"))
      .ok,
    true,
  );
  ...
  assert.equal(
    tainted.desktopIconsVisible,
    false,
    "unconfirmed exit falsely claimed Explorer icons were restored",
  );
  ...
  assert.equal(
    calls.detach.length,
    0,
    "disable detached after native restoration became unknown",
  );
});
...
test("host-change rebind waits for a restored terminal ACK and restore failure cancels it", async () => {
  ...
  await failed.runtime.enable(new FakeBrowserWindow(), {
    interactive: true,
    reason: "host-failed-enable",
  });
  ...
  assert.deepEqual(
    failedReasons,
    [],
    "host error rebound before terminal restoration ACK",
  );
  ...
  assert.deepEqual(
    failedReasons,
    [],
    "restore-unconfirmed host exit still rebound",
  );
  ...
  await restored.runtime.enable(new FakeBrowserWindow(), {
    interactive: true,
    reason: "host-restored-enable",
  });
  ...
  assert.equal(
    restored.runtime.getStatus("host-restored").nativeStateKnown,
    true,
  );
});
...
test("unexpected watcher exit with confirmed restoration queues recovery without hiding Mineradio", async () => {
  ...
  await runtime.enable(win, {
    interactive: true,
    reason: "unexpected-exit-enable",
  });
  assert.equal(
    (
      await runtime.setDesktopIconsVisible(
        false,
        "unexpected-exit-icons-hidden",
      )
    ).ok,
    true,
  );
...
test("setInteractive true repairs a stale enabled interactive state instead of returning unchanged", async () => {
  ...
  runtime.clearExitedIconLayerState({
  ...
  assert.equal(win.visible, true);
  assert.ok(
    calls.coexist.length > coexistBefore,
    "stale interactive state did not rebind the native icon host",
  );
});
