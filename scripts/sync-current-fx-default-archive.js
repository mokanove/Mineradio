...
function readFxDefaults() {
  ...
  if (start < 0 || end < 0)
    throw new Error("Unable to locate fxDefaults object.");
  return vm.runInNewContext(
    `(${text.slice(start + marker.length, end + 2)})`,
    Object.create(null),
  );
}
...
const mismatched = keys.filter(
  (key) => JSON.stringify(defaults[key]) !== JSON.stringify(current[key]),
);
if (missing.length || mismatched.length) {
  throw new Error(
    `Runtime defaults are not synchronized with the captured settings. Missing: ${missing.join(", ") || "none"}; mismatched: ${mismatched.join(", ") || "none"}.`,
  );
}

const snapshot = {
  visualPresetSchema: current.visualPresetSchema || "skull-preset-v2",
};
...
if (!shouldWrite) {
  console.log(
    `[DRY RUN] ${keys.length} settings match ${sourcePath}. Add --write to refresh ${archivePath}.`,
  );
  process.exit(0);
}
