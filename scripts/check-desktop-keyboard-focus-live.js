...
async function run() {
  ...
  try {
    ...
    assert.equal(
      prepared.status && prepared.status.softwareInteractionLocked,
      false,
    );
    ...
    assert.equal(
      locked.result && locked.result.softwareInteractionLocked,
      true,
    );
    assert.equal(locked.rendererFocusRequestRejected, true);
    assert.equal(
      locked.documentFocused,
      false,
      "software lock did not blur renderer keyboard focus",
    );
    ...
    assert.equal(
      unlocked.result && unlocked.result.softwareInteractionLocked,
      false,
    );
    assert.equal(
      unlocked.documentFocused,
      false,
    ...
    assert.equal(
      restored.inserted,
      true,
      "focused search input did not accept text",
    );
    assert.equal(
      restored.f13 > 0,
      true,
      "renderer did not receive a keydown after focus restoration",
    );
