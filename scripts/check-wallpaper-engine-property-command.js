...
const absolutePackageReference = process.argv.includes(
...
function prepareSilentStage() {
  ...
  const project = JSON.parse(
    fs.readFileSync(sourceProjectFile, "utf8").replace(/^\uFEFF/, ""),
  );
  const properties = project && project.general && project.general.properties;
  if (!properties || !properties.newproperty)
    throw new Error("Expected Scene audio property was not found");
  ...
  if (!absolutePackageReference)
    fs.linkSync(sourcePackage, path.join(stageRoot, "scene.pkg"));
}
...
function runVerbatimFromExecutableDirectory(
  args,
  ...
  const quote = (value) => {
    ...
    return (
      '"' + text.replace(/(\\*)"/g, '$1$1\\"').replace(/(\\+)$/g, "$1$1") + '"'
    );
  };
  const verbatimArgs = args.map((value, index) =>
    index === rawIndex ? String(value) : quote(value),
  );
  const result = childProcess.spawnSync(
    path.basename(targetExecutable),
    verbatimArgs,
    {
...
function runViaCommandProcessor(args, targetExecutable = executable) {
  ...
  const argumentLine = args
    .map((value, index) => (index === rawIndex ? String(value) : quote(value)))
    .join(" ");
  const commandLine =
    quote(targetExecutable) + (argumentLine ? " " + argumentLine : "");
  const result = childProcess.spawnSync(
...
async function main() {
  if (locationMute) {
    throw new Error(
      "--location-mute is disabled: this QA script must never change Wallpaper Engine global audio state; use location-scoped applyProperties checks instead.",
    );
  }
  const evidence = {};
  prepareSilentStage();
  try {
    if (!skipPreOpenProperty) {
      evidence.preOpenProperty = runVerbatimFromExecutableDirectory([
        ...
        "applyProperties",
        "-properties",
        'RAW~({"newproperty":0,"volume":0})~END',
        "-location",
        location,
      ]);
    }
    evidence.spacedExecutable = runViaCommandProcessor(
      ["--version"],
      process.execPath,
    );
    evidence.spacedExecutableVerbatimCmd = runViaVerbatimCommandProcessor(
      ["--version"],
      process.execPath,
    );
    evidence.spacedExecutableFromCwd = runVerbatimFromExecutableDirectory(
      ["--version"],
      process.execPath,
    );
    evidence.open = run([
      ...
      "openWallpaper",
      "-file",
      openPackageDirectly ? sourcePackage : projectFile,
      "-playInWindow",
      location,
      "-width",
      onScreen ? "1440" : "960",
    ...
    evidence.combined = run([
      ...
      "applyProperties",
      "-properties",
      'RAW~({"volume":0,"newproperty":0})~END',
      "-location",
      location,
    ]);
