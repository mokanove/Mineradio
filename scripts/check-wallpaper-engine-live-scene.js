...
async function main() {
  ...
  const screenshotPath = path.join(
    outputDir,
    "wallpaper-engine-scene-live-frame.png",
  );
  const frameData = String(result.applied.frameDataUrl || "").replace(
  ...
  if (
    result.applied.kind !== "engine" ||
    !result.applied.active ||
    !result.applied.hasStream ||
    result.applied.frames < 2
  ) {
    console.error(
      JSON.stringify({ wallpaperEngineLiveSceneDiagnostic: result }, null, 2),
    );
  }
  ...
  assert(
    result.runtime && result.runtime.available === true,
    (result.runtime && result.runtime.reason) ||
      "Wallpaper Engine runtime unavailable",
  );
  ...
  assert.strictEqual(
    result.applied.active,
    true,
    result.applied.runtimeError || "native Scene layer did not become active",
  );
  assert.strictEqual(result.applied.hasStream, true);
  assert(
    result.applied.videoWidth > 0 && result.applied.videoHeight > 0,
    "native Scene stream has no video dimensions",
  );
  assert(
    result.applied.frames >= 2,
    `native Scene stream did not advance frames (${result.applied.frames})`,
  );
  assert(
    result.applied.pixels.nonBlack > 200,
    "native Scene capture is black or empty",
  );
  assert(
    /^[a-f0-9]{24}$/.test(result.applied.sessionId),
    "native Scene session id is missing",
  );
  assert.strictEqual(
    result.applied.captureMode,
    "main-prepared",
    "native Scene did not use the main-process prepared capture path",
  );
  if (requestedWorkshopId)
    assert.strictEqual(
      String(result.applied.workshopId || ""),
      requestedWorkshopId,
      "requested workshop Scene was not selected",
    );
  assert.strictEqual(
    result.applied.audioTrackCount,
    0,
    "native Scene capture must not contain an audio track",
  );
  assert(
    result.applied.runtimeStatus &&
      result.applied.runtimeStatus.sourceWindowEmbedded === true,
    "Wallpaper Engine source window was not docked directly behind Mineradio",
  );
  assert.strictEqual(
    result.applied.runtimeStatus.sourceWindowAligned,
    true,
    "Wallpaper Engine source window is not pixel-aligned with Mineradio",
  );
  const sourceRect = result.applied.runtimeStatus.sourceWindowRect;
  const hostRect = result.applied.runtimeStatus.hostWindowRect;
  assert(
    sourceRect && hostRect,
    "Wallpaper Engine source/host bounds are unavailable",
  );
  for (const edge of ["left", "top", "right", "bottom"]) {
    assert(
      Math.abs(Number(sourceRect[edge]) - Number(hostRect[edge])) <= 2,
      `Wallpaper Engine ${edge} edge is not pixel-aligned`,
    );
  }
  assert(
    Math.abs(result.applied.videoWidth - result.applied.runtimeStatus.width) <=
      2,
    "native Scene capture width does not match the Mineradio compositor",
  );
  assert(
    Math.abs(
      result.applied.videoHeight - result.applied.runtimeStatus.height,
    ) <= 2,
    "native Scene capture height does not match the Mineradio compositor",
  );
  if (
    result.applied.cursorCapture &&
    result.applied.cursorCapture !== "never"
  ) {
    assert.strictEqual(
      result.applied.runtimeStatus.sourceWindowAligned,
      true,
      "cursor-capturing fallback requires exact source/host alignment",
    );
  }
  assert.strictEqual(
    result.applied.runtimeStatus.audioMuted,
    true,
    "Wallpaper Engine Scene audio mute was not applied",
  );
